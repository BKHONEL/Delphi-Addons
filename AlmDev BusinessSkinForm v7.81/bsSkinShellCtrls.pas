...
begin
  ...
  FDlgFrm := TbsSelDirDlgForm.CreateEx(Application, CtrlSkinData, FShowToolBar, FToolButtonsTransparent);
  with FDlgFrm do
  try
    Caption := Self.Title;
    BSF.BorderIcons := [];
...
begin
  ...
  FDlgFrm := TbsOpenDlgForm.CreateEx(Application, FSaveMode,
    CtrlSkinData, FToolButtonsTransparent);
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TbsOpenPreviewDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData, FToolButtonsTransparent);
  FDlgFrm.OnFolderChange := FOnFolderChange;
  FDlgFrm.CheckFileExists := FCheckFileExists;
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TbsOpenSoundDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData, FToolButtonsTransparent);
  FDlgFrm.OnFolderChange := FOnFolderChange;
  FDlgFrm.CheckFileExists := FCheckFileExists;
  FDlgFrm.OverwritePromt := OverwritePromt;
  with FDlgFrm do
