...
type
  ...
  protected
    ...
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer); override;
    procedure MouseMove(Shift: TShiftState; X, Y: Integer); override;
    function GetHitMode: TspFormLayerBorderHit;
...
begin
  inherited;
  if Parent.SupportNCArea and (Parent.QuickButtons.Count <> 0) and (Parent.QuickImageList <> nil)
  then
    TestActive(X, Y);
end;
...
function TspDynamicSkinForm.CheckStayOnTopWindows;
var
  i: Integer;
begin
  Result := 0;
...
begin
  ...
  then
    begin
      ...
      CreateHSkinImage(LO, RO, Buffer, Picture, R1,
  ...
  case BorderType of
    spflTop:
      begin
        ...
        if PtInRect(R1, P) then Result := spflhTopLeft else
        if PtInRect(R2, P) then Result := spflhTop else
        if PtInRect(R3, P) then Result := spflhTopRight
        else
          begin
            if P.Y > FForm.Top
            then
    ...
    spflBottom:
      begin
        ...
        if PtInRect(R3, P) then Result := spflhBottomRight
        else
          begin
...
var
  Offset1, Offset2, Y, YCnt, YO, h: Integer;
...
begin
  ...
  then
    begin
      SendMessage(DSF.FForm.Handle, WM_NCPAINT, 0, 0);
    end;
end;
