...
uses Windows, Messages, Forms, Sysutils, Classes, Graphics, Controls, ImgList,
  ...
  spUtils {$IFDEF DEVEX_SUPPORT}, cxControls, cxGrid, cxLookAndFeelPainters {$ENDIF};
...
begin
  ...
  then
    begin
      ...
      StatusBarSubClass.Add('ttnttstatusbar');
      //
      LabelSubClass.Clear;
...
begin
  for i := FHandleList.Count - 1 downto 0 do
  begin
    FHookCollection.RemoveControl(TdsaSubclass(FHandleList[i]).Handle, true);
  end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Frame3D(Cnvs, R, SP_XP_BTNFRAMECOLOR, SP_XP_BTNFRAMECOLOR, 1);
...
begin
  ...
  begin
    ...
    begin
      with FManager.Adapter.SkinData do
        PanelData := TspDataSkinPanelControl(CtrlList.Items[GetControlIndex('panel')]);
...
procedure TdsaDateTimeSubclass.PaintBorder;
var
  Cnvs: TCanvas;
  BR, R, SR: TRect;
  C: TColor;
...
procedure TdsaComboBoxSubclass.PaintBorder;
var
  BR, R, SR: TRect;
  C: TColor;
  Canvas: TCanvas;
...
begin
  if (TdsaSubclassHackCustomComboBox(Control).Style = csDropDown) or
     (TdsaSubclassHackCustomComboBox(Control).Style = csDropDownList)
  then
    begin
      CallDefaultProc(TMessage(Message));
      Handled := true;
...
begin
  ...
  then
    begin
      ...
      with SpeedButton do
       DrawGlyphAndText(Buffer.Canvas, R, Margin, Spacing, TspButtonLayout(Layout),
        Text, Glyph, NumGlyphs, GetGlyphNum(NumGlyphs), ((TdsaSubclassHackSpeedButton(SpeedButton).FState = bsDown) or
       (TdsaSubclassHackSpeedButton(SpeedButton).FState = bsExclusive)));
      Canvas.Draw(0, 0, Buffer);
      Buffer.Free;
    end
...
function FindBSFComponent(AForm: TCustomForm): TspDynamicSkinForm;
var
  i: Integer;
begin
  Result := nil;
...
begin
  inherited;
  if DSF = nil then DSF := FindBSFComponent(GetParentForm(FControl));
end;
