...
begin
  ...
  if Parent.CanDrawAppMenuCaption and (ObjectRect.Left < Parent.GetAppMenuCaptionOffset)
  then
    Exit;
...
begin
  ...
  then
    begin
      ...
      if PopupMenu <> nil
      then
        DrawQuickButtonArrow(ItemRect, Active, Down, Enabled);
      // image
      ...
      then
        begin
          X := ItemRect.Left + 3;
          DrawQuickButtonText(Caption, ItemRect, Active, Down, Enabled);
        end;
  ...
  with Parent.FQuickButtons[i] do
   ...
   then
     begin
       ...
       Inc(X, RectWidth(ItemRect) + 4);
       Inc(R.Left, RectWidth(ItemRect) + 4);
     end;
end;
...
procedure TbsBusinessSkinForm.DrawAppMenuCaption(Cnvs: TCanvas; R: TRect);
begin
  if FSkinRibbon = nil then Exit;
  if FSkinRibbon.AppMenu = nil then Exit;
  FSkinRibbon.AppMenu.DrawAppMenuCaption(Cnvs, R);
end;
...
begin
  ...
  begin
    ...
    DrawAppMenuCaption(CaptionBitmap.Canvas, TempRect);
  end;  
  //
      ...
      with TbsSkinCaptionObject(ObjectList.Items[i]) do
       ...
       then
         begin
           ...
           then
             begin
               LeftO := Parent.QuickButtons[Parent.QuickButtons.Count - 1].ItemRect.Right;
               R := ObjectRect;
               Inc(R.Left, LeftO);
               Result := PtInRect(R, P);
             end
