...
{$IFDEF VER200}
 ...
 type
   TsfPngImage = class(TPngImage);

{$ELSE}

uses
 Windows, Classes, Graphics, SysUtils, sf_zlib;
...
type
  ...
  TsfPngTransparencyMode = (bsptmNone, bsptmBit, bsptmPngLayerial);
  pCardinal = ^Cardinal;
  pRGBPixel = ^TRGBPixel;
  ...
  protected
    ...
    property Owner: TsfPngImage read fOwner;
  public
    constructor Create(AOwner: TsfPngImage);
    destructor Destroy; override;
    property Count: Cardinal read fCount write SetSize;
  end;

  TsfPngLayer = class;
  TsfPngLayerClass = class of TsfPngLayer;

  TsfPngList = class(TsfPngPointerList)
  private
    function GetItem(Index: Cardinal): TsfPngLayer;
  public
    function FindPngLayer(PngLayerClass: TsfPngLayerClass): TsfPngLayer;
    procedure RemovePngLayer(PngLayer: TsfPngLayer); overload;
    function Add(PngLayerClass: TsfPngLayerClass): TsfPngLayer;
    function ItemFromClass(PngLayerClass: TsfPngLayerClass): TsfPngLayer;
    property Item[Index: Cardinal]: TsfPngLayer read GetItem;
  end;

  TsfPngLayerIHDR = class;
  TsfPngLayerpHYs = class;
  TsfInterlaceMethod = (bsimNone, bsimAdam7);
  TsfCompressionLevel = 0..9;
  TsfPngFilter = (bspfNone, bspfSub, bspfUp, bspfAverage, bspfPaeth);
  TsfPngFilters = set of TsfPngFilter;

  TsfPngImage = class(TGraphic)
  private
    fMaxIdatSize: Integer;
    fInterlaceMethod: TsfInterlaceMethod;
    fPngLayerList: TsfPngList;
    fCanvas: TCanvas;
    fFilters: TsfPngFilters;
    fCompressionLevel: TsfCompressionLevel;
    ...
    function GetTransparencyMode: TsfPngTransparencyMode;
  ...
  protected
    ...
    procedure AssignPNG(Source: TsfPngImage);
    function GetEmpty: Boolean; override; 
    function GetHeader: TsfPngLayerIHDR;
  ...
  public
    ...
    property Header: TsfPngLayerIHDR read GetHeader;
    property TransparencyMode: TsfPngTransparencyMode read GetTransparencyMode;

    property Width: Integer read GetWidth;
    property Height: Integer read GetHeight;
    property InterlaceMethod: TsfInterlaceMethod read fInterlaceMethod
      write fInterlaceMethod;
    property Filters: TsfPngFilters read fFilters write fFilters;
    property MaxIdatSize: Integer read fMaxIdatSize write SetMaxIdatSize;
    property Empty: Boolean read GetEmpty;
    property CompressionLevel: TsfCompressionLevel read fCompressionLevel
      write fCompressionLevel;
    property PngLayers: TsfPngList read fPngLayerList;
    property Pixels[const X, Y: Integer]: TColor read GetPixels write SetPixels;
  end;

  TsfPngLayerName = Array[0..3] of Char;
  ...
  public
    ...
    procedure Assign(Source: TsfPngLayer); virtual;
    constructor Create(Owner: TsfPngImage); virtual;
    destructor Destroy; override;
    function LoadFromStream(Stream: TStream; const PngLayerName: TsfPngLayerName;
  ...
  TsfPngLayerIEND = class(TsfPngLayer);
  ...
  TsfPngLayerIHDR = class(TsfPngLayer)
  private
    ImageHandle: HBitmap;
    ImageDC: HDC;
  ...
  public
    ...
    constructor Create(Owner: TsfPngImage); override;
    destructor Destroy; override;
    procedure Assign(Source: TsfPngLayer); override;
  ...
  TsfPngLayerpHYs = class(TsfPngLayer)
  private
  ...
  TsfPngLayergAMA = class(TsfPngLayer)
  private
  ...
  TsfPngLayerPLTE = class(TsfPngLayer)
  protected
    fCount: Integer;
  private
  ...
  TsfPngLayertRNS = class(TsfPngLayer)
  private
  ...
  TsfPngLayerIDAT = class(TsfPngLayer)
  private
    Header: TsfPngLayerIHDR;
  ...
  TsfPngLayertIME = class(TsfPngLayer)
  private
    fYear: Word;
  ...
  TsfPngLayertEXt = class(TsfPngLayer)
  private
  ...
  TsfPngLayerzTXt = class(TsfPngLayertEXt)
    function LoadFromStream(Stream: TStream; const PngLayerName: TsfPngLayerName;
      Size: Integer): Boolean; override;
    function SaveToStream(Stream: TStream): Boolean; override;
  end;

procedure RegisterPngLayer(PngLayerClass: TsfPngLayerClass);
...
var
  PngLayerClasses: TsfPngPointerList;
...
type
  pPngLayerClassInfo = ^TsfPngLayerClassInfo;
  TsfPngLayerClassInfo = record
    ClassName: TsfPngLayerClass;
  end;
...
begin
  if PngLayerClasses = nil then PngLayerClasses := TsfPngPointerList.Create(nil);
  new(NewClass);
...
begin
  RegisterPngLayer(TsfPngLayerIEND);
  RegisterPngLayer(TsfPngLayerIHDR);
  RegisterPngLayer(TsfPngLayerIDAT);
  RegisterPngLayer(TsfPngLayerPLTE);
  RegisterPngLayer(TsfPngLayergAMA);
  RegisterPngLayer(TsfPngLayertRNS);
  RegisterPngLayer(TsfPngLayerpHYs);
  RegisterPngLayer(TsfPngLayertIME);
  RegisterPngLayer(TsfPngLayertEXt);
  RegisterPngLayer(TsfPngLayerzTXt);
end;

function CreateClassPngLayer(Owner: TsfPngImage; Name: TsfPngLayerName): TsfPngLayer;
var
  i       : Integer;
  NewPngLayer: TsfPngLayerClass;
begin
  NewPngLayer := TsfPngLayer;
...
function ZLIBInitDeflate(Stream: TStream;
  Level: TsfCompressionlevel; Size: Cardinal): TZStreamRec2;
begin
  Fillchar(Result, SIZEOF(TZStreamRec2), #0);
  with Result do
  begin
...
constructor TsfPngPointerList.Create(AOwner: TsfPngImage);
begin
  inherited Create;
  fOwner := AOwner;
  fMemory := nil;
...
function TsfPngPointerList.Remove(Value: Pointer): Pointer;
var
  I, Position: Integer;
begin
  Position := -1;
...
procedure TsfPngPointerList.Add(Value: Pointer);
begin
  Count := Count + 1;
  Item[Count - 1] := Value;
end;

destructor TsfPngPointerList.Destroy;
begin
  if fMemory <> nil then
...
function TsfPngPointerList.GetItem(Index: Cardinal): Pointer;
begin
...
procedure TsfPngPointerList.Insert(Value: Pointer; Position: Cardinal);
begin
...
procedure TsfPngPointerList.SetItem(Index: Cardinal; const Value: Pointer);
begin
  if (Index <= Count - 1) then
    fMemory[Index] := Value
end;

procedure TsfPngPointerList.SetSize(const Size: Cardinal);
begin
...
function TsfPngList.FindPngLayer(PngLayerClass: TsfPngLayerClass): TsfPngLayer;
var
  i: Integer;
begin
  Result := nil;
...
procedure TsfPngList.RemovePngLayer(PngLayer: TsfPngLayer);
begin
  Remove(PngLayer);
  PngLayer.Free
end;

function TsfPngList.Add(PngLayerClass: TsfPngLayerClass): TsfPngLayer;
var
  IHDR: TsfPngLayerIHDR;
  IEND: TsfPngLayerIEND;

  IDAT: TsfPngLayerIDAT;
  PLTE: TsfPngLayerPLTE;
begin
  Result := nil; 
  if ((PngLayerClass = TsfPngLayerIHDR) or (PngLayerClass = TsfPngLayerIDAT) or
    (PngLayerClass = TsfPngLayerPLTE) or (PngLayerClass = TsfPngLayerIEND)) and not
    (Owner.BeingCreated)
    then
      begin
      end
  else if ((PngLayerClass = TsfPngLayergAMA) and (ItemFromClass(TsfPngLayergAMA) <> nil)) or
     ((PngLayerClass = TsfPngLayertRNS) and (ItemFromClass(TsfPngLayertRNS) <> nil)) or
     ((PngLayerClass = TsfPngLayerpHYs) and (ItemFromClass(TsfPngLayerpHYs) <> nil)) then
     begin
     end
  else if ((ItemFromClass(TsfPngLayerIEND) = nil) or
    (ItemFromClass(TsfPngLayerIHDR) = nil)) and not Owner.BeingCreated then
    begin
    end
  else
  begin
    IHDR := ItemFromClass(TsfPngLayerIHDR) as TsfPngLayerIHDR;
    IEND := ItemFromClass(TsfPngLayerIEND) as TsfPngLayerIEND;
    Result := PngLayerClass.Create(Owner);
    if (PngLayerClass = TsfPngLayergAMA) or (PngLayerClass = TsfPngLayerpHYs) or
      (PngLayerClass = TsfPngLayerPLTE) then
      Insert(Result, IHDR.Index + 1)
    else if (PngLayerClass = TsfPngLayerIEND) then
      Insert(Result, Count)
    else if (PngLayerClass = TsfPngLayerIHDR) then
      Insert(Result, 0)
    else if (PngLayerClass = TsfPngLayertRNS) then
    begin
      IDAT := ItemFromClass(TsfPngLayerIDAT) as TsfPngLayerIDAT;
      PLTE := ItemFromClass(TsfPngLayerPLTE) as TsfPngLayerPLTE;
...
function TsfPngList.GetItem(Index: Cardinal): TsfPngLayer;
begin
  Result := inherited GetItem(Index);
end;

function TsfPngList.ItemFromClass(PngLayerClass: TsfPngLayerClass): TsfPngLayer;
var
  i: Integer;
begin
  Result := nil;
...
procedure TsfPngLayer.ResizeData(const NewSize: Cardinal);
begin
  fDataSize := NewSize;
  ReallocMem(fData, NewSize + 1);
end;

function TsfPngLayer.GetIndex: Integer;
var
  i: Integer;
begin
  Result := -1; 
...
function TsfPngLayer.GetHeader: TsfPngLayerIHDR;
begin
  Result := Owner.PngLayers.Item[0] as TsfPngLayerIHDR;
end;

procedure TsfPngLayer.Assign(Source: TsfPngLayer);
begin
  fName := Source.fName;
...
constructor TsfPngLayer.Create(Owner: TsfPngImage);
var
  PngLayerName: String;
begin
  inherited Create;
  PngLayerName := System.Copy(ClassName, Length('TsfPngLayer') + 1, Length(ClassName));
...
destructor TsfPngLayer.Destroy;
begin
  FreeMem(fData, fDataSize + 1);
  inherited Destroy;
end;

function TsfPngLayer.GetPngLayerName: String;
begin
  Result := fName
end;

class function TsfPngLayer.GetName: String;
begin
  Result := System.Copy(ClassName, Length('TsfPngLayer') + 1, Length(ClassName));
end;

function TsfPngLayer.SaveData(Stream: TStream): Boolean;
var
...
function TsfPngLayer.SaveToStream(Stream: TStream): Boolean;
begin
  Result := SaveData(Stream)
end;

function TsfPngLayer.LoadFromStream(Stream: TStream; const PngLayerName: TsfPngLayerName;
...
function TsfPngLayertIME.LoadFromStream(Stream: TStream;
  const PngLayerName: TsfPngLayerName; Size: Integer): Boolean;
begin
...
procedure TsfPngLayertIME.Assign(Source: TsfPngLayer);
begin
  fYear := TsfPngLayertIME(Source).fYear;
  fMonth := TsfPngLayertIME(Source).fMonth;
  fDay := TsfPngLayertIME(Source).fDay;
  fHour := TsfPngLayertIME(Source).fHour;
  fMinute := TsfPngLayertIME(Source).fMinute;
  fSecond := TsfPngLayertIME(Source).fSecond;
end;

function TsfPngLayertIME.SaveToStream(Stream: TStream): Boolean;
begin
  ResizeData(7);  
...
function TsfPngLayerzTXt.LoadFromStream(Stream: TStream;
  const PngLayerName: TsfPngLayerName; Size: Integer): Boolean;
var
  ErrorOutput: String;
...
function TsfPngLayerztXt.SaveToStream(Stream: TStream): Boolean;
var
  Output: Pointer;
  OutputSize: Integer;
  ErrorOutput: String;
...
procedure TsfPngLayertEXt.Assign(Source: TsfPngLayer);
begin
  fKeyword := TsfPngLayertEXt(Source).fKeyword;
  fText := TsfPngLayertEXt(Source).fText;
end;

function TsfPngLayertEXt.LoadFromStream(Stream: TStream;
  const PngLayerName: TsfPngLayerName; Size: Integer): Boolean;
begin
  Result := inherited LoadFromStream(Stream, PngLayerName, Size);
...
function TsfPngLayertEXt.SaveToStream(Stream: TStream): Boolean;
begin
...
constructor TsfPngLayerIHDR.Create(Owner: TsfPngImage);
begin
  ImageHandle := 0;
  ImagePalette := 0;
  ImageDC := 0;
...
destructor TsfPngLayerIHDR.Destroy;
begin
  FreeImageData();
  inherited Destroy;
end;
...
procedure TsfPngLayerIHDR.Assign(Source: TsfPngLayer);
begin
  if Source is TsfPngLayerIHDR then
  begin
    IHDRData := TsfPngLayerIHDR(Source).IHDRData;
    PrepareImageData();
    CopyMemory(ImageData, TsfPngLayerIHDR(Source).ImageData,
      BytesPerRow * Integer(Height));
    CopyMemory(ImageAlpha, TsfPngLayerIHDR(Source).ImageAlpha,
      Integer(Width) * Integer(Height));
    BitmapInfo.bmiColors := TsfPngLayerIHDR(Source).BitmapInfo.bmiColors;
    CopyPalette(TsfPngLayerIHDR(Source).ImagePalette, ImagePalette);
  end;
end;

procedure TsfPngLayerIHDR.FreeImageData;
begin
  {Free old image data}
...
function TsfPngLayerIHDR.LoadFromStream(Stream: TStream; const PngLayerName: TsfPngLayerName;
  Size: Integer): Boolean;
begin
  Result := inherited LoadFromStream(Stream, PngLayerName, Size);
  ...
  Owner.InterlaceMethod := TsfInterlaceMethod(IHDRData.InterlaceMethod);
  PrepareImageData();
end;

function TsfPngLayerIHDR.SaveToStream(Stream: TStream): Boolean;
begin
...
function TsfPngLayerIHDR.CreateGrayscalePalette(Bitdepth: Integer): HPalette;
var
  j: Integer;
...
procedure TsfPngLayerIHDR.PaletteToDIB(Palette: HPalette);
var
  j: Integer;
  palEntries: TMaxLogPalette;
begin
...
procedure TsfPngLayerIHDR.PrepareImageData();
...
procedure TsfPngLayertRNS.SetTransparentColor(const Value: ColorRef);
var
  i: Byte;
  LookColor: TRGBQuad;
begin
...
function TsfPngLayertRNS.GetTransparentColor: ColorRef;
var
  PalettePngLayer: TsfPngLayerPLTE;
  i: Integer;
  Value: Byte;
begin
  Result := 0;
  with Header do
    case ColorType of
      ...
      COLOR_PALETTE:
      begin
        PalettePngLayer := Owner.PngLayers.ItemFromClass(TsfPngLayerPLTE) as TsfPngLayerPLTE;
...
function TsfPngLayertRNS.SaveToStream(Stream: TStream): Boolean;
begin
...
procedure TsfPngLayertRNS.Assign(Source: TsfPngLayer);
begin
  CopyMemory(@PaletteValues[0], @TsfPngLayerTrns(Source).PaletteValues[0], 256);
  fBitTransparency := TsfPngLayerTrns(Source).fBitTransparency;
  inherited Assign(Source);
end;

function TsfPngLayertRNS.LoadFromStream(Stream: TStream; const PngLayerName: TsfPngLayerName;
  Size: Integer): Boolean;
var
  i, Differ255: Integer;
begin
...
procedure TsfPngLayerIDAT.PreparePalette;
var
  Entries: Word;
  j      : Integer;
  palEntries: TMaxLogPalette;
...
function TsfPngLayerIDAT.IDATZlibRead(var ZLIBStream: TZStreamRec2;
...
procedure TsfPngLayerIDAT.CopyInterlacedRGB8(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyInterlacedRGB16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyInterlacedPalette148(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
const
...
procedure TsfPngLayerIDAT.CopyInterlacedPalette2(const Pass: Byte; Src, Dest,
  Trans, Extra: pChar);
var
  CurBit, Col: Integer;
  Dest2: PChar;
...
procedure TsfPngLayerIDAT.CopyInterlacedGray2(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  CurBit, Col: Integer;
  Dest2: PChar;
...
procedure TsfPngLayerIDAT.CopyInterlacedGrayscale16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyInterlacedRGBAlpha8(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyInterlacedRGBAlpha16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyInterlacedGrayscaleAlpha8(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyInterlacedGrayscaleAlpha16(const Pass: Byte;
  Src, Dest, Trans, Extra: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.DecodeInterlacedAdam7(Stream: TStream;
  var ZLIBStream: TZStreamRec2; const Size: Integer; var crcfile: Cardinal);
var
  CurrentPass: Byte;
...
procedure TsfPngLayerIDAT.CopyNonInterlacedRGB8(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedRGB16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedPalette148(
  Src, Dest, Trans, Extra: pChar);
begin
  CopyMemory(Dest, Src, Row_Bytes);
end;

procedure TsfPngLayerIDAT.CopyNonInterlacedGray2(
  Src, Dest, Trans, Extra: pChar);
var
  i: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedPalette2(
  Src, Dest, Trans, Extra: pChar);
var
  i: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedGrayscale16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedRGBAlpha8(
  Src, Dest, Trans, Extra: pChar);
var
  i: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedRGBAlpha16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedGrayscaleAlpha8(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsfPngLayerIDAT.CopyNonInterlacedGrayscaleAlpha16(
  Src, Dest, Trans, Extra: pChar);
var
  I: Integer;
begin
...
procedure TsfPngLayerIDAT.DecodeNonInterlaced(Stream: TStream;
  var ZLIBStream: TZStreamRec2; const Size: Integer; var crcfile: Cardinal);
var
  j: Cardinal;
  Trans, Data, Extra: pChar;
...
procedure TsfPngLayerIDAT.FilterRow;
var
  pp: Byte;
...
function TsfPngLayerIDAT.LoadFromStream(Stream: TStream; const PngLayerName: TsfPngLayerName;
  Size: Integer): Boolean;
var
  ...
  CRCFile  : Cardinal;
begin
  Header := Owner.PngLayers.Item[0] as TsfPngLayerIHDR;
...
function TsfPngLayerIDAT.SaveToStream(Stream: TStream): Boolean;
var
  ZLIBStream : TZStreamRec2;
begin
  Header := Owner.PngLayers.Item[0] as TsfPngLayerIHDR;
...
procedure TsfPngLayerIDAT.IDATZlibWrite(var ZLIBStream: TZStreamRec2;
...
procedure TsfPngLayerIDAT.FinishIDATZlib(var ZLIBStream: TZStreamRec2);
begin
  with ZLIBStream, ZLIBStream.ZLIB do
  begin
    next_in := nil;
...
procedure TsfPngLayerIDAT.EncodeNonInterlacedRGB8(Src, Dest, Trans: pChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TsfPngLayerIDAT.EncodeNonInterlacedRGB16(Src, Dest, Trans: pChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TsfPngLayerIDAT.EncodeNonInterlacedPalette148(Src, Dest, Trans: pChar);
begin
  CopyMemory(Dest, Src, Row_Bytes);
end;

procedure TsfPngLayerIDAT.EncodeNonInterlacedGrayscale16(Src, Dest, Trans: pChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TsfPngLayerIDAT.EncodeNonInterlacedRGBAlpha8(Src, Dest, Trans: pChar);
var
  i: Integer;
begin
  FOR i := 1 TO ImageWidth do
...
procedure TsfPngLayerIDAT.EncodeNonInterlacedRGBAlpha16(Src, Dest, Trans: pChar);
var
  i: Integer;
begin
  FOR i := 1 TO ImageWidth do
...
procedure TsfPngLayerIDAT.EncodeNonInterlacedGrayscaleAlpha8(
  Src, Dest, Trans: pChar);
var
  i: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeNonInterlacedGrayscaleAlpha16(
  Src, Dest, Trans: pChar);
var
  i: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeNonInterlaced(Stream: TStream;
  var ZLIBStream: TZStreamRec2);
var
  j: Cardinal;
  Data, Trans: PChar;
...
procedure TsfPngLayerIDAT.EncodeInterlacedRGB8(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeInterlacedRGB16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeInterlacedPalette148(const Pass: Byte;
  Src, Dest, Trans: pChar);
const
  BitTable: Array[1..8] of Integer = ($1, $3, 0, $F, 0, 0, 0, $FF);
  StartBit: Array[1..8] of Integer = (7 , 0 , 0, 4,  0, 0, 0, 0);
...
procedure TsfPngLayerIDAT.EncodeInterlacedGrayscale16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeInterlacedRGBAlpha8(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeInterlacedRGBAlpha16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeInterlacedGrayscaleAlpha8(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeInterlacedGrayscaleAlpha16(const Pass: Byte;
  Src, Dest, Trans: pChar);
var
  Col: Integer;
begin
...
procedure TsfPngLayerIDAT.EncodeInterlacedAdam7(Stream: TStream;
  var ZLIBStream: TZStreamRec2);
var
...
function TsfPngLayerIDAT.FilterToEncode: Byte;
var
...
begin
  ...
  for ii := FILTER_NONE TO FILTER_PAETH do
    if TsfPngFilter(ii) in Owner.Filters then
    begin
      Run := 0;
      if Owner.Filters = [TsfPngFilter(ii)] then
      begin
        Result := ii;
        exit;
      end;
...
function TsfPngLayerPLTE.GetPaletteItem(Index: Byte): TRGBQuad;
begin
  if Index > Count - 1 then
    begin
    end
...
function TsfPngLayerPLTE.LoadFromStream(Stream: TStream;
  const PngLayerName: TsfPngLayerName; Size: Integer): Boolean;
type
  pPalEntry = ^PalEntry;
  PalEntry = record
...
function TsfPngLayerPLTE.SaveToStream(Stream: TStream): Boolean;
var
  J: Integer;
  DataPtr: pByte;
  BitmapInfo: TMAXBITMAPINFO;
...
procedure TsfPngLayerPLTE.Assign(Source: TsfPngLayer);
begin
  if Source is TsfPngLayerPLTE then
    fCount := TsfPngLayerPLTE(Source).fCount;
end;

{TsfPngLayergAMA}

procedure TsfPngLayergAMA.Assign(Source: TsfPngLayer);
begin
  if Source is TsfPngLayergAMA then
    Gamma := TsfPngLayergAMA(Source).Gamma;
end;

constructor TsfPngLayergAMA.Create(Owner: TsfPngImage);
begin
  inherited Create(Owner);
  Gamma := 1;
end;

function TsfPngLayergAMA.GetValue: Cardinal;
begin
  if DataSize <> 4 then
  begin
    ResizeData(4);
...
function TsfPngLayergAMA.LoadFromStream(Stream: TStream;
  const PngLayerName: TsfPngLayerName; Size: Integer): Boolean;
var
  i: Integer;
  Value: Cardinal;
...
procedure TsfPngLayergAMA.SetValue(const Value: Cardinal);
begin
...
procedure TsfPngImage.Assign(Source: TPersistent);
begin
  if Source = nil then
    ClearPngLayers
  else if Source is TsfPngImage then
    AssignPNG(Source as TsfPngImage)
...
procedure TsfPngImage.CreateAlpha;
var
  TRNS: TsfPngLayerTRNS;
begin
  with Header do
    case ColorType of
      ...
      COLOR_PALETTE:
      begin
        if PngLayers.ItemFromClass(TsfPngLayerTRNS) = nil then
          TRNS := PngLayers.Add(TsfPngLayerTRNS) as TsfPngLayerTRNS
        else
          TRNS := PngLayers.ItemFromClass(TsfPngLayerTRNS) as TsfPngLayerTRNS;
          with TRNS do
          begin
            ResizeData(256);
            Fillchar(PaletteValues[0], 256, 255);
...
procedure TsfPngImage.ClearPngLayers;
var
  i: Integer;
begin
  InitializeGamma();
  for i := 0 TO Integer(PngLayers.Count) - 1 do
    TsfPngLayer(PngLayers.Item[i]).Free;
  PngLayers.Count := 0;
end;

constructor TsfPngImage.CreateBlank(ColorType, BitDepth: Cardinal;
  cx, cy: Integer);
var NewIHDR: TsfPngLayerIHDR;
begin
  Create;
  ...
  InitializeGamma;
  BeingCreated := True;
  PngLayers.Add(TsfPngLayerIEND);
  NewIHDR := PngLayers.Add(TsfPngLayerIHDR) as TsfPngLayerIHDR;
  ...
  if NewIHDR.HasPalette then
    TsfPngLayerPLTE(PngLayers.Add(TsfPngLayerPLTE)).fCount := 1 shl BitDepth;
  PngLayers.Add(TsfPngLayerIDAT);
  BeingCreated := False;
end;

constructor TsfPngImage.Create;
begin
  inherited Create;
  ...
  fPngLayerList := TsfPngList.Create(Self);

end;

destructor TsfPngImage.Destroy;
begin
  if fCanvas <> nil
  then
    begin
...
procedure TsfPngImage.GetPixelInfo(var LineSize, Offset: Cardinal);
begin
...
function TsfPngImage.GetHeight: Integer;
begin
  if HeaderPresent then
    Result := TsfPngLayerIHDR(PngLayers.Item[0]).Height
  else Result := 0;
end;

function TsfPngImage.GetWidth: Integer;
begin
  if HeaderPresent then
    Result := Header.Width
  else Result := 0;
...
function TsfPngImage.GetEmpty: Boolean;
begin
  Result := (PngLayers.Count = 0);
end;

procedure TsfPngImage.SetMaxIdatSize(const Value: Integer);
begin
  if Value < High(Word) then
    fMaxIdatSize := High(Word) else fMaxIdatSize := Value;
end;

function TsfPngImage.GetHeader: TsfPngLayerIHDR;
begin
  if (PngLayers.Count <> 0) and (PngLayers.Item[0] is TsfPngLayerIHDR) then
    Result := PngLayers.Item[0] as TsfPngLayerIHDR
  else
  begin
    Result := nil;
  end;
...
procedure TsfPngImage.DrawPngLayerialTrans(DC: HDC; Rect: TRect);
...
var
  ...
  TransparencyPngLayer: TsfPngLayertRNS;
  PalettePngLayer: TsfPngLayerPLTE;
...
begin
  ...
  case Header.BitmapInfo.bmiHeader.biBitCount of
    ...
    begin
      TransparencyPngLayer := TsfPngLayertRNS(PngLayers.ItemFromClass(TsfPngLayertRNS));
      PalettePngLayer := TsfPngLayerPLTE(PngLayers.ItemFromClass(TsfPngLayerPLTE));
...
procedure TsfPngImage.Draw(ACanvas: TCanvas; const Rect: TRect);
var
  Header: TsfPngLayerIHDR;
begin
  if Empty then Exit;
  Header := PngLayers.GetItem(0) as TsfPngLayerIHDR;
...
procedure TsfPngImage.LoadFromStream(Stream: TStream);
var
  Header    : Array[0..7] of Char;
  HasIDAT   : Boolean;
  PngLayerCount : Cardinal;
  PngLayerLength: Cardinal;
  PngLayerName  : TsfPngLayerName;
begin
  PngLayerCount := 0;
  ClearPngLayers();
  ...
  repeat
    ...
    try if not TsfPngLayer(PngLayers.Item[PngLayerCount - 1]).LoadFromStream(Stream,
...
procedure TsfPngImage.SetHeight(Value: Integer);
begin
  Resize(Width, Value)
end;

procedure TsfPngImage.SetWidth(Value: Integer);
begin
  Resize(Value, Height)
end;

function TsfPngImage.GetTransparent: Boolean;
begin
  Result := (TransparencyMode <> bsptmNone);
end;

procedure TsfPngImage.SaveToStream(Stream: TStream);
var
  j: Integer;
begin
...
procedure BuildHeader(Header: TsfPngLayerIHDR; Handle: HBitmap; Info: pBitmap);
var
  DC: HDC;
begin
...
procedure TsfPngImage.LoadFromResourceName(Instance: HInst;
  const Name: String);
var
...
procedure TsfPngImage.LoadFromResourceID(Instance: HInst; ResID: Integer);
begin
  LoadFromResourceName(Instance, String(ResID));
end;

procedure TsfPngImage.AssignTo(Dest: TPersistent);
...
var
  TRNS: TsfPngLayerTRNS;
begin
  if Dest is TsfPngImage then
    TsfPngImage(Dest).AssignPNG(Self)
...
procedure TsfPngImage.AssignHandle(Handle: HBitmap; Transparent: Boolean;
  TransparentColor: ColorRef);
var
...
begin
  ...
  ClearPngLayers();
  Header := TsfPngLayerIHDR.Create(Self);
  BuildHeader(Header, Handle, @BitmapInfo);
  if Header.HasPalette then PLTE := TsfPngLayerPLTE.Create(Self) else PLTE := nil;
  if Transparent then TRNS := TsfPngLayerTRNS.Create(Self) else TRNS := nil;
  IDAT := TsfPngLayerIDAT.Create(Self);
  IEND := TsfPngLayerIEND.Create(Self);
  TsfPngPointerList(PngLayers).Add(Header);
  if Header.HasPalette then TsfPngPointerList(PngLayers).Add(PLTE);
  if Transparent then TsfPngPointerList(PngLayers).Add(TRNS);
  TsfPngPointerList(PngLayers).Add(IDAT);
  TsfPngPointerList(PngLayers).Add(IEND);
...
procedure TsfPngImage.AssignPNG(Source: TsfPngImage);
var
  J: Integer;
begin
  ...
  FOR J := 0 TO PngLayers.Count - 1 DO
    ...
    begin
      PngLayers.SetItem(J, TsfPngLayerClass(TsfPngLayer(Item[J]).ClassType).Create(Self));
      TsfPngLayer(PngLayers.Item[J]).Assign(TsfPngLayer(Item[J]));
    end;
end;

function TsfPngImage.GetAlphaScanline(const LineIndex: Integer): pByteArray;
begin
  with Header do
    if (ColorType = COLOR_RGBALPHA) or (ColorType = COLOR_GRAYSCALEALPHA) then
...
function TsfPngImage.GetExtraScanline(const LineIndex: Integer): Pointer;
begin
  with Header do
...
function TsfPngImage.GetScanline(const LineIndex: Integer): Pointer;
begin
  with Header do
...
procedure TsfPngImage.InitializeGamma;
var
  i: Integer;
begin
  FOR i := 0 to 255 do
...
function TsfPngImage.GetTransparencyMode: TsfPngTransparencyMode;
var
  TRNS: TsfPngLayerTRNS;
begin
  with Header do
...
procedure TsfPngImage.RemoveTransparency;
var
  TRNS: TsfPngLayerTRNS;
begin
  with Header do
...
function TsfPngImage.GetTransparentColor: TColor;
var
  TRNS: TsfPngLayerTRNS;
begin
  TRNS := PngLayers.ItemFromClass(TsfPngLayerTRNS) as TsfPngLayerTRNS;
...
procedure TsfPngImage.SetTransparentColor(const Value: TColor);
var
  TRNS: TsfPngLayerTRNS;
begin
  if HeaderPresent then
    ...
    COLOR_PALETTE, COLOR_RGB, COLOR_GRAYSCALE:
      begin
        TRNS := PngLayers.ItemFromClass(TsfPngLayerTRNS) as TsfPngLayerTRNS;
        if not Assigned(TRNS) then TRNS := PngLayers.Add(TsfPngLayerTRNS) as TsfPngLayerTRNS;
        TRNS.TransparentColor := ColorToRGB(Value);
      end;
    end;
end;

function TsfPngImage.HeaderPresent: Boolean;
begin
  Result := ((PngLayers.Count <> 0) and (PngLayers.Item[0] is TsfPngLayerIHDR));
end;

function GetByteArrayPixel(const png: TsfPngImage; const X, Y: Integer): TColor;
var
  ByteData: Byte;
  DataDepth: Byte;
begin
  ...
  begin
    ...
    case ColorType of
      COLOR_PALETTE:
        with TsfPngLayerPLTE(png.PngLayers.ItemFromClass(TsfPngLayerPLTE)).Item[ByteData] do
...
procedure SetByteArrayPixel(const png: TsfPngImage; const X, Y: Integer;
  const Value: TColor);
const
...
function GetRGBLinePixel(const png: TsfPngImage;
...
procedure SetRGBLinePixel(const png: TsfPngImage;
...
function GetGrayLinePixel(const png: TsfPngImage;
  const X, Y: Integer): TColor;
var
  B: Byte;
begin
...
procedure SetGrayLinePixel(const png: TsfPngImage;
 const X, Y: Integer; Value: TColor);
begin
  PByteArray(png.Scanline[Y])^[X] := GetRValue(Value);
end;

procedure TsfPngImage.Resize(const CX, CY: Integer);
...
procedure TsfPngImage.SetPixels(const X, Y: Integer; const Value: TColor);
begin
...
function TsfPngImage.GetPixels(const X, Y: Integer): TColor;
begin
  if ((X >= 0) and (X <= Width - 1)) and
        ((Y >= 0) and (Y <= Height - 1)) then
    with Header do
...
function TsfPngImage.GetPalette: HPALETTE;
begin
  Result := Header.ImagePalette;
end;

procedure TsfPngLayerpHYs.Assign(Source: TsfPngLayer);
begin
  fPPUnitY := TsfPngLayerpHYs(Source).fPPUnitY;
  fPPUnitX := TsfPngLayerpHYs(Source).fPPUnitX;
  fUnit := TsfPngLayerpHYs(Source).fUnit;
end;

function TsfPngLayerpHYs.LoadFromStream(Stream: TStream; const PngLayerName: TsfPngLayerName;
  Size: Integer): Boolean;
begin
  Result := inherited LoadFromStream(Stream, PngLayerName, Size);
...
function TsfPngLayerpHYs.SaveToStream(Stream: TStream): Boolean;
begin
  ResizeData(9);
...
procedure TsfPngImage.DoSetPalette(Value: HPALETTE; const UpdateColors: boolean);
begin
  if (Header.HasPalette)  then
  begin
...
procedure TsfPngImage.SetPalette(Value: HPALETTE);
begin
...
initialization
  ...
  {$IFDEF RegisterPNG}
  TPicture.RegisterFileFormat('PNG', 'PNG for SmartFlash', TsfPngImage);
  {$ENDIF}
finalization
  {$IFDEF RegisterPNG}
  TPicture.UnregisterGraphicClass(TsfPngImage);
  {$ENDIF}
