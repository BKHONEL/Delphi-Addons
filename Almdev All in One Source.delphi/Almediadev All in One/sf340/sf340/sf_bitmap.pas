...
procedure TsfBitmap.FillRoundRect(R: TRect; Radius: integer;
  Color: TsfColor);
begin

end;

procedure TsfBitmap.LineTo(X, Y: integer; Color: TsfColor);
begin

end;

procedure TsfBitmap.MoveTo(X, Y: integer);
begin

end;

function TsfBitmap.TextHeight(AText: WideString): integer;
begin
  Result := 0;
end;

function TsfBitmap.TextWidth(AText: WideString; Flags: Integer): integer;
begin
  Result := 0;
end;

procedure TsfBitmap.FlipHorz;
var
 J, J2: Integer;
 Buffer: PsfColorArray;
 P1, P2: PsfColor;
begin
   J2 := Height - 1;
   ...
   begin
     P1 := PixelPtr[0, J];
     P2 := PixelPtr[0, J2];
     MoveLongwordFunc(P1, PsfColor(Buffer), Width);
     MoveLongwordFunc(P2, P1, Width);
     MoveLongwordFunc(PsfColor(Buffer), P2, Width);
     Dec(J2);
   end;
   FreeMem(Buffer);
end;
...
constructor TsfBitmapLink.Create;
begin
  inherited Create;
end;

destructor TsfBitmapLink.Destroy;
begin
  inherited Destroy;
end;

procedure TsfBitmapLink.Assign(Source: TPersistent);
begin
  if Source is TsfBitmapLink then
  begin
    FImage := (Source as TsfBitmapLink).FImage;
    FRect := (Source as TsfBitmapLink).FRect;
    FName := (Source as TsfBitmapLink).FName;
    FMasked := (Source as TsfBitmapLink).FMasked;
    FMaskedBorder := (Source as TsfBitmapLink).FMaskedBorder;
    FMaskedAngles := (Source as TsfBitmapLink).FMaskedAngles;
  end
  else
    inherited;
end;

procedure TsfBitmapLink.LoadFromStream(Stream: TStream);
begin
  FName := ReadString(Stream);
  Stream.Read(FRect, SizeOf(FRect));
end;

procedure TsfBitmapLink.SaveToStream(Stream: TStream);
begin
  WriteString(Stream, FName);
  Stream.Write(FRect, SizeOf(FRect));
end;

procedure TsfBitmapLink.CheckingMasked(Margin: TRect);
var
  i, j: integer;
  P: TsfColor;
  Pt: TPoint;
    ...
    for i := Left to Right - 1 do
      ...
      begin
        if (FImage.Bits <> nil) and (i >= 0) and (j >= 0) and (i < FImage.Width) and (j < FImage.Height) then
          P := PsfColor(@FImage.Bits[i + j * FImage.Width])^
        else
          P := 0;

        if P <> sfNone then
        begin
          if P = sfTransparent then
          begin
            FMasked := true;
            Break;
          end;
          if TsfColorRec(P).A < $FF then
          begin
            FMasked := true;
            Break;
          end;
  ...
  begin
    for i := Left to Right - 1 do
      ...
      begin
        ...
        if P <> sfNone then
        begin
          if (P = sfTransparent) or (TsfColorRec(P).A < $FF) then
          begin
...
procedure TsfBitmapLink.CheckingMasked;
begin
  CheckingMasked(Classes.Rect(0, 0, 0, 0));
end;

procedure TsfBitmapLink.Draw(Bitmap: TsfBitmap; X, Y: integer);
begin
...
procedure TsfBitmapLink.Draw(Canvas: TCanvas; X, Y: integer);
begin
  if FImage = nil then Exit;
  if FImage.Empty then Exit;
  if FRect.Right - FRect.Left <= 0 then Exit;
...
function TsfBitmapLink.GetAssigned: boolean;
begin
  Result := (FImage <> nil) and ((FRect.Right - FRect.Left) * (FRect.Bottom - FRect.Top) > 0);
end;

function TsfBitmapLink.GetBottom: integer;
begin
  Result := FRect.Bottom;
end;

function TsfBitmapLink.GetLeft: integer;
begin
  Result := FRect.Left;
end;

function TsfBitmapLink.GetRight: integer;
begin
  Result := FRect.Right;
end;

function TsfBitmapLink.GetTop: integer;
begin
  Result := FRect.Top;
end;

procedure TsfBitmapLink.SetBottom(const Value: integer);
begin
  FRect.Bottom := Value;
end;

procedure TsfBitmapLink.SetLeft(const Value: integer);
begin
  FRect.Left := Value;
end;

procedure TsfBitmapLink.SetRight(const Value: integer);
begin
  FRect.Right := Value;
end;

procedure TsfBitmapLink.SetTop(const Value: integer);
begin
  FRect.Top := Value;
end;
...
constructor TsfBitmapList.Create;
begin
  inherited Create;
end;

destructor TsfBitmapList.Destroy;
begin
  Clear;
  inherited Destroy;
end;

procedure TsfBitmapList.Clear;
var
  i: integer;
begin
  {$WARNINGS OFF}
  for i := 0 to Count-1 do
    TsfBitmap(Items[i]).Free;
  {$WARNINGS ON}

  inherited Clear;
end;

function TsfBitmapList.GetImage(index: integer): TsfBitmap;
begin
  {$WARNINGS OFF}
  if (index >= 0) and (index < Count) then
    Result := TsfBitmap(Items[index])
  else
    Result := nil;
  {$WARNINGS ON}
end;

function TsfBitmapList.GetBitmapLink(Image: TsfBitmap; Rect: TRect): TsfBitmapLink;
var
  i: integer;
begin
  Result := nil;
...
function TsfBitmapList.GetBitmapLink(Name: string; Rect: TRect): TsfBitmapLink;
var
  i: integer;
begin
  Result := nil;
...
function TsfBitmapList.GetBitmapLink(Name, Rect: string): TsfBitmapLink;
var
  i: integer;
begin
  Result := nil;
...
function TsfBitmapList.GetBitmapByName(index: string): TsfBitmap;
var
  i: integer;
begin
  Result := nil;
...
function TsfBitmap.GetCanvas: TCanvas;
begin
  if FCanvas = nil then
  begin
...
procedure TsfBitmap.LoadFromFile(AFileName: String);
var
  F: TFileStream;
begin
...
procedure TsfBitmap.SaveToFile(AFileName: String);
var
  F: TFileStream;
begin
...
function _PixelAlphaBlend(F, B: TsfColor): TsfColor;
asm
  { Test Fa = 255  }
...
procedure _LineAlphaBlend(Src, Dst: PsfColor; Count: Integer);
asm
  TEST    ECX,ECX
  JS      @4
...
procedure _LineTransparent(Src, Dst: PsfColor; Count: Integer);
asm
  TEST    ECX,ECX
  JS      @4
...
function Mmx_PixelAlphaBlend(F, B: TsfColor): TsfColor;
asm
...
procedure Mmx_LineAlphaBlend(Src, Dst: PsfColor; Count: Integer);
asm
  TEST      ECX,ECX
  JS        @4
...
procedure _BltToDibOpaque(Bits: Pointer; DstX, DstY, BitsW, BitsH: integer;
  Src: TsfBitmap; SrcRect: TRect); overload;
var
  SrcP, DstP: PsfColor;
  W, H, Y: Integer;
begin
  ...
  { 32-bit -DIB }
  DstP := @PsfColorArray(Bits)[DstX + (BitsH - DstY - 1) * BitsW];
  for Y := 0 to H - 1 do
  begin
...
procedure _StretchToDibOpaque(Bits: Pointer; DstRect, DstClip: TRect;
  BitsW, BitsH: integer; Src: TsfBitmap; SrcRect: TRect); overload;
var
  R: TRect;
...
begin
  ...
  try
    ...
    begin
      ...
      DstLine := @PsfColorArray(Bits)[DstClip.Left + (BitsH - DstClip.Top - 1) * BitsW];
      OldSrcY := -1;
...
procedure _BltOpaque(Dst: TsfBitmap; DstRect: TRect;
  Src: TsfBitmap; SrcX, SrcY: Integer); overload;
var
  SrcP, DstP: PsfColor;
  W, DstY: Integer;
begin
...
procedure _BltTransparent(Dst: TsfBitmap; DstRect: TRect;
  Src: TsfBitmap; SrcX, SrcY: Integer); overload;
var
  SrcP, DstP: PsfColor;
  W, DstY: Integer;
...
procedure _BltAlphaBlend(Dst: TsfBitmap; DstRect: TRect;
  Src: TsfBitmap; SrcX, SrcY: Integer); overload;
var
  SrcP, DstP: PsfColor;
  W, DstY: Integer;
...
procedure _StretchOpaque(Dst: TsfBitmap; DstRect, DstClip: TRect;
  Src: TsfBitmap; SrcRect: TRect);
var
  R: TRect;
  SrcW, SrcH, DstW, DstH, DstClipW, DstClipH: Integer;
  SrcY, OldSrcY: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      DstLine := PsfColorArray(Dst.PixelPtr[DstClip.Left, DstClip.Top]);
      OldSrcY := -1;
      for J := 0 to DstClipH - 1 do
      begin
        if DstH > 1 then
...
procedure _StretchTransparent(Dst: TsfBitmap; DstRect, DstClip: TRect;
  Src: TsfBitmap; SrcRect: TRect);
var
  R: TRect;
  SrcW, SrcH, DstW, DstH, DstClipW, DstClipH: Integer;
...
procedure _StretchAlphaBlend(Dst: TsfBitmap; DstRect, DstClip: TRect;
  Src: TsfBitmap; SrcRect: TRect);
var
  R: TRect;
  SrcW, SrcH, DstW, DstH, DstClipW, DstClipH: Integer;
