...
begin
  ...
  if (Img.PixelFormat = pf32bit) and Fast32.Attach(Img) then begin
    ...
    for x := 0 to Img.Width - 1 do
      for y := 0 to Img.Height - 1 do with Fast32.Pixels[x, y] do
        if (R = $FF) and (G = $FF) and (B = $FF) and (A = 0) then Fast32.Pixels[x, y] := BlackColor;
  end;
  FreeAndNil(Fast32);
end;
...
begin
  ...
  if (Img.PixelFormat = pf32bit) and Fast32.Attach(Img) then begin
    ...
    for x := 0 to Img.Width - 1 do
      for y := 0 to Img.Height - 1 do begin
        C := Fast32.Pixels[x, y];
        C.R := C.R * C.A div MaxByte;
        C.G := C.G * C.A div MaxByte;
        C.B := C.B * C.A div MaxByte;
        Fast32.Pixels[x, y] := C;
      end;
  end;
  FreeAndNil(Fast32);
