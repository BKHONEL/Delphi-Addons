...
function Ac_NCPaint(sw : TacScrollWnd; Handle : hwnd; wParam : WPARAM; lParam : LPARAM; ThumbPos : integer = -1; aDC : hdc = 0) : LRESULT;
  ...
  if Assigned(SkinData) and SkinData.Skinned then begin
    case Message.Msg of
      ...
      WM_PRINT : begin
        ...
        i := 0;
        CallPrevWndProc(CtrlHandle, WM_PRINTCLIENT, WPARAM(DC), LPARAM(i));
        RestoreDC(DC, SavedDC);
      ...
      WM_NCPAINT : if IsWindowVisible(CtrlHandle) then begin
        ...
        try
          BitBltBorder(DC, 0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, cxLeftEdge);
          i := 0;
          CallPrevWndProc(CtrlHandle, WM_PRINT, WPARAM(DC), LPARAM(i));
          Message.Result := Ac_NCPaint(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, DC);
        finally
          RestoreDC(DC, SavedDC);
          ReleaseDC(CtrlHandle, DC);
...
begin
  ...
  if (uxthemeLib <> 0) then begin
    ...
    else if not (Self is TacMnuWnd) and (SkinSection <> s_Dialog) then Ac_SetWindowTheme(AHandle, ' ', ' ');
  end;
  NewWndProcInstance := nil;
  SkinManager := ASkinManager;
  Destroyed := False;
...
begin
  if Caption <> '' then begin
    ...
    NewFont := ac_GetSysFont(FONT_MESSAGE);
    if NewFont <> 0 then OldFont := SelectObject(SkinData.FCacheBmp.Canvas.Handle, NewFont) else OldFont := 0;
    ...
    if OldFont <> 0 then SelectObject(SkinData.FCacheBMP.Canvas.Handle, OldFont);
    Result := True;
  end
  else Result := True;
end;
...
begin
  ...
  if SkinData.FOwnerControl = nil then begin
    NewFont := ac_GetSysFont(FONT_MESSAGE);
    if NewFont <> 0 then cf := SelectObject(SkinData.FCacheBMP.Canvas.Handle, NewFont);
  end
  ...
  if cf <> 0 then SelectObject(SkinData.FCacheBMP.Canvas.Handle, cf);
end;
