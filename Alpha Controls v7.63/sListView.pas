    ...
    if (vStyle = vsReport) and RowSelect then begin
      ...
      if bSelected then begin
        ...
        if sNdx < 0 then begin
          ...
          PaintItem(sNdx, s_Selection, CI, True, integer(Focused), aRect, Point(nRect.Left, nRect.Top), Bmp, SkinData.SkinManager);
          if bHot then begin
            fText := nRect;
      ...
      if not IsRectEmpty(fText) then begin
        ...
        if (sNdx = -1) or (SkinData.CustomFont) then begin
          ...
          if SkinData.CustomFont then begin
            DrawText(ACanvas.Handle, PChar(Item.Caption), Length(Item.Caption), fText, DrawStyle);
          end
          else begin
...
//      if IsRectEmpty(nRect) then Exit;
      ACanvas := Canvas;
      aRect := nRect;
      if Color <> Canvas.Brush.Color then Canvas.FillRect(iRect); // Fill if other color was defined
...
begin
  ...
  if SkinData.Skinned then if (Stage in [cdPrePaint, cdPostPaint]) then begin
    ...
    if bSelected then begin
      ...
      ACanvas := Canvas;
      FillDC(ACanvas.Handle, Rect(aRect.Left, fText.Top, fText.Right, fText.Bottom), Color);
    end;
    ...
    if sNdx = -1 then begin
      ...
      if Bmp <> nil then begin
        if not SkinData.CustomFont then begin
          if bSelected and Item.Selected then Bmp.Canvas.Font.Color := SkinData.SkinManager.GetHighLightFontColor(Focused{(cdsFocused in State) and RowSelected} and Item.Selected) else Bmp.Canvas.Font.Color := Font.Color;
        end
