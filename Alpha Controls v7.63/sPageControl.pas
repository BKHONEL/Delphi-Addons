...
uses
  ...
  ComCtrls, sCommonData, sConst, sFade, sUpDown, extctrls, sSpeedButton, acSBUtils, menus
...
begin
  ...
  if (State = 1) and (rTab.Left < 0) then Exit;
...
//      acDrawText(TempBmp.Canvas.Handle, lCaption, dContent.TextRect, Flags);
...
{$ENDIF}
  ...
  Data.TextSize := acTextExtent(Canvas, lCaption);
...
//        else if WidthOf(TabRect) - Data.TextSize.cy > ArrowSize then ArrowUnderText := True;
    ...
    if FShowCloseBtns and not IsTabMenu and TsTabSheet(Page).UseCloseBtn then begin
      ...
      if (State = 2) then begin
        ...
        if TabPosition = tpTop then OffsetRect(Data.BtnRect, 1, 1);
      end;
    end
    ...
    if FShowCloseBtns and not IsTabMenu and TsTabSheet(Page).UseCloseBtn then begin
      ...
      if (State = 2) then begin
        OffsetRect(TabRect, 3, 0);
        OffsetRect(Data.BtnRect, 1 - integer(IsVertical), 2);
      end;
    end
    else Data.BtnRect := Rect(0, 0, 0, 0);
    // Image rect
    if (Images <> nil) and (Page.ImageIndex > -1) and (Page.ImageIndex <= Images.Count - 1) then begin
      ...
      Data.GlyphRect.Left := TabRect.Left + (WidthOf(TabRect) - Images.Width) div 2;
      Data.GlyphRect.Right := Data.GlyphRect.Left + Images.Width;
      if (State = 2) then begin
        OffsetRect(Data.GlyphRect, -3, -1);
