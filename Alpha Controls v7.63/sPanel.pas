...
//        FCommonData.FCacheBmp.PixelFormat := pf1bit; // FCacheBmp is not required
      end;
...
{
      if SkinData.BGChanged and not SkinData.Updating then begin
...
//          if IsWindowVisible(Handle) then
  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned then begin
    ...
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      AC_PREPARING : begin
        Message.Result := integer(FCommonData.Updating or ((FCommonData.FCacheBmp.Width <> Width) or (FCommonData.FCacheBmp.Height <> Height)) and IsCached(FCommonData) and not SkinData.CustomColor);
        Exit;
      end
