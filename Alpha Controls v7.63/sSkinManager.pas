  ...
  begin
    ...
    while (Y < h) and (Result = 0) do begin
      ...
      while (X < w) and (Result = 0) do begin
        if Fast32[X, Y].C = clFuchsia then Result := 1;
        inc(X);
      end;
      inc(Y);
    end;
...
begin
  if (mask.ClassName = s_ButtonHuge) and (mask.PropertyName = s_BordersMask) then Result := 2 {Region changed} else begin
    ...
    if Fast32.Attach(Bmp) then begin
      // Left top
      l := mask.R.Left;
      t := mask.R.Top;
...
{ For 8.0
        FormShadowSize.Top := SkinData.ExContentOffs;
        FormShadowSize.Left := SkinData.ExContentOffs;
        FormShadowSize.Right := SkinData.ExContentOffs;
        FormShadowSize.Bottom := SkinData.ExContentOffs;
}
...
begin
  if Value <> FActiveGraphControl then begin
    ...
    if (FActiveGraphControl <> nil) and (FActiveControl <> 0) then ActiveControl := 0;
