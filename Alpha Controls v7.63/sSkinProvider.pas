...
begin
  if not (csDestroying in SkinProvider.ComponentState) and SkinProvider.Form.HandleAllocated and TForm(SkinProvider.Form).AutoScroll and (SkinProvider.FormState and FS_SCROLLSUPDATING <> FS_SCROLLSUPDATING) then begin
...
//            SendMessage(TsSkinProvider(MDISkinProvider).Form.Handle, WM_NCPAINT, 0, 0);
            TsSkinProvider(MDISkinProvider).RepaintNC;
          end;
        end;
...
begin
{$IFDEF LOGGED}
//  if (Form <> nil) and (Form.FormStyle = fsMDIChild) then
    AddToLog(Message);
{$ENDIF}
...
//        if SkinData.SkinManager.ActiveControl <> 0 then SkinData.SkinManager.ActiveControl := 0;
  ...
  {$ENDIF}
            ...
            if AeroIsEnabled then SendMessage(Form.Handle, WM_SETREDRAW, 1, 0);
            OldWndProc(Message);
  ...
  {$ENDIF}
            ...
            if BorderForm = nil then begin
              SendMessage(Form.Handle, WM_SETREDRAW, 1, 0);
              FCommonData.BGChanged := True;
              RedrawWindow(Form.Handle, nil, 0, RDW_FRAME or RDW_INVALIDATE or RDW_ALLCHILDREN); // Update of controls which may be changed
            end;
            UpdateSkinCaption(Self);
          end;
              ...
              if FormChanged and (not (IsIconic(Form.Handle) and InAero)) then begin
                ...
                if FDrawNonClientArea then begin
                  ...
                  if (BorderForm <> nil) then begin // Update extended borders
                    ...
                    UpdateRgn(Self, True);
  //                  if InAero then SetWindowRgn(BorderForm.AForm.Handle, BorderForm.MakeRgn, False); // Update borders region
                    FCommonData.FUpdating := False;
                  end
                  else begin
                    RepaintNC;
  ...
  //      WM_DRAWITEM : if not FDrawNonClientArea then OldWndProc(Message) else AC_WMDrawItem(TWMDrawItem(Message));
        WM_NCHITTEST : begin
          ...
          if (CaptionHeight = 0) or not FDrawNonClientArea then OldWndProc(Message) else begin
            ...
            if (Message.Result in [Windows.HTCAPTION, Windows.HTNOWHERE, HTMENU, HTCLIENT, HTLEFT..HTBOTTOM]) then begin
              OldWndProc(Message);
              Exit;
            end;
            ...
            HTMINBUTTON : if biClicked then begin
              ...
              if PtInRect(ButtonMin.Rect, p) then begin
                ...
                if (Application.MainForm = Form) then begin
                  ...
                  SendMessage(Form.Handle, WM_SYSCOMMAND, SC_MINIMIZE, 0);
                  FormState := FormState and not FS_ANIMMINIMIZING;// and not FS_CHANGING;
                end;
                FCommonData.BGChanged := True;
                RegionChanged := True;
              end
...
{            if (ListSW = nil) and DrawNonClientArea then begin // Initialize skinned scrollbars if not initialized yet
  ...
  //          if SystemMenu = nil then PrepareForm; // If not intitialized (occurs when Scaled = False)
          ...
          if (GetWindowLong(Form.Handle, GWL_EXSTYLE) and WS_EX_LAYERED = WS_EX_LAYERED) and (FormTimer = nil) then RepaintNC;
          ...
          if (FormTimer = nil) or not FormTimer.Enabled {and DrawNonClientArea} then begin
            ...
            if ac_NoExtBordersIfMax and (Form.WindowState = wsMaximized) and (BorderForm <> nil) then begin
              FreeAndNil(BorderForm);
              UpdateRgn(Self, False);
            end
            else if BorderForm <> nil then BorderForm.UpdateExBordersPos;
          end;
        ...
        WM_WINDOWPOSCHANGING : begin
          if not RgnChanging and not (csLoading in Form.ComponentState) then begin
            if not IsZoomed(Form.Handle) and (TWMWindowPosChanged(Message).WindowPos.Flags and SWP_NOMOVE <> SWP_NOMOVE) and
              ...
              if sConst.ac_NoExtBordersIfMax and (Form.WindowState = wsNormal) and (BorderForm = nil) and ExtBordersNeeded(Self) then begin
                Self.InitExBorders(True);
              end;
...
//  addY := MaxBtnOffset(Self);
  if Assigned(SystemMenu) and HaveSysMenu then begin // Accommodation of buttons in a special order...
    if FCommonData.SkinManager.IsValidImgIndex(ButtonClose.ImageIndex) then begin
      ButtonClose.Rect.Right := Offset;
      PaintButton(ButtonClose, b, ButtonClose.ImageIndex, SystemMenu.VisibleClose);
    ...
    if SystemMenu.VisibleMin then begin
      if (Form.WindowState = wsMinimized) and IsIconic(Form.Handle) and (Application.MainForm <> Form) then begin // If form is minimized then changing to Normalize
    ...
    if biHelp in Form.BorderIcons then begin
      if FCommonData.SkinManager.IsValidImgIndex(ButtonHelp.ImageIndex) then begin
        ButtonHelp.Rect.Right := Offset - 6;
  ...
  if FCommonData.BGChanged then begin
    ...
    if sMan.IsValidSkinIndex(FCommonData.SkinIndex) then begin
      ...
      if IsMenuVisible(Self) and MenuPresent and (MenuHeight > 0) and DrawNonClientArea and sMan.SkinnedPopups then begin
        ...
        if not MenusInitialized then begin
          if Form.Menu <> nil then sMan.SkinableMenus.InitMenuLine(Form.Menu, FDrawNonClientArea);
          MenusInitialized := UpdateMenu;
        end;
{        if AeroIsEnabled
          then}
//        sMan.SkinableMenus.InitMenuLine(Form.Menu, FDrawNonCLientArea);
...
//            sbw := min(GetSystemMetrics(SM_CXSIZEFRAME), 6 {15 (Half of Png) - 9 (Shadow size)} );
            if sMan.ConstData.ExBorder >= 0 then begin
              PaintControlByTemplate(FCommonData.FCacheBmp, BorderForm.ShadowTemplate, Rect(0, 0, fWidth, fHeight),
                Rect(0, 0, BorderForm.ShadowTemplate.Width, BorderForm.ShadowTemplate.Height),
                Rect(ma[sMan.ConstData.ExBorder].WL, ma[sMan.ConstData.ExBorder].WT, ma[sMan.ConstData.ExBorder].WR, ma[sMan.ConstData.ExBorder].WB),
            ...
            end;
{
            PaintControlByTemplate(FCommonData.FCacheBmp, BorderForm.ShadowTemplate, Rect(0, 0, fWidth, fHeight),
              Rect(0, 0, BorderForm.ShadowTemplate.Width, BorderForm.ShadowTemplate.Height),
              Rect(ShadowSize.Left + sbw, ShadowSize.Top + sbw, ShadowSize.Right + sbw, ShadowSize.Bottom + sbw),
              ShadowSize, Rect(1, 1, 1, 1), False, False); // For internal shadows - stretch only allowed
}
...
begin
  ...
  case Message.HitTest of
    HTCLOSE, HTMINBUTTON, HTMAXBUTTON, HTHELP, HTUDBTN..HTUDBTN + 100 : SetHotHT(Message.HitTest); // Title icon hovered
...
procedure TsSkinProvider.RepaintNC;
var
  acM : TMessage;
begin
  acM := MakeMessage(WM_NCPAINT, 0, 0, 0);
  NewWndProc(acM);
end;
  ...
  case Message.Msg of
    WM_NCMOUSEMOVE : begin
      ...
      case Message.Result of
        HTCLOSE, HTMINBUTTON, HTMAXBUTTON, HTHELP, HTUDBTN..HTUDBTN + 100 : SetHotHT(Message.Result); // Title icon hovered
        else SetHotHT(0);
      end;
    end;
    WM_MOUSEMOVE, WM_LBUTTONDOWN..WM_MBUTTONDBLCLK : if MouseAboveTheShadow(TWMMouse(Message)) then begin
      ...
      ResetRgn := True;
{
      // Some anvirs and protection systems doesn't allow this code
      ...
      if (Wnd <> AForm.Handle) and (Wnd <> OwnerHandle) then begin
        if Message.Msg = WM_MOUSEMOVE then begin // Correcting coords
...
var
  ...
  var
    ...
    var
      X, Y : integer;
    begin
      for Y := R.Top to R.Bottom do for X := R.Left to R.Right do if (FastBmp.Pixels[X, Y].I = 0) and (FastBmp.Pixels[X, Y].G = 0) and (FastBmp.Pixels[X, Y].B = 0) then begin // Black color is not blurred
  ...
  begin
    ...
    FastBmp := TacFast32.Create;
    if FastBmp.Attach(SkinData.FCacheBmp) then begin
...
begin
  with FOwner do begin
    ...
    if (FormState and FS_FULLPAINTING <> 0) or aSkinChanging then begin
      ...
      Redraw := True;
      if sp <> nil then sp.SkinData.FCacheBmp := GetFormImage(sp, True) else begin
        DC := GetWindowDC(oWnd);
        SavedDC := SaveDC(DC);
        try
          R := ACClientRect(oWnd);
