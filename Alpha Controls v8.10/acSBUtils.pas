...
//      SkinData.CtrlSkinState := SkinData.CtrlSkinState or ACS_PRINTING;
      ...
      SendMessage(CtrlHandle, WM_ERASEBKGND, WPARAM(DC), 0);
  ...
  if not bPreventStyleChange then begin
    ...
    if Assigned(SkinData) and Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) then begin
      if Assigned(SkinData.FOwnerControl) then begin // If VCL used
        ...
        FrameColor := SkinData.SkinManager.SkinData.BorderColor;
      end;
...
{$ENDIF}
  ...
  if HasProperty(SkinData.FOwnerControl, acLineColors) then begin
    l := GetObjProp(SkinData.FOwnerControl, acLineColors);
    TryChangeIntProp(l, acFixedColor, SkinData.SkinManager.SkinData.BorderColor);
...
begin
  ...
  if EvenRowColor <> OddRowColor then begin
    TryChangeIntProp(SkinData.FOwnerControl, 'EvenRowColor', MixColors(SkinData.SkinManager.GetActiveEditFontColor, SkinData.SkinManager.GetActiveEditColor, 0.1));
    TryChangeIntProp(SkinData.FOwnerControl, 'OddRowColor', SkinData.SkinManager.GetActiveEditColor);
  end;
...
//        ReleaseCapture;
    ...
    WM_NCPAINT : if not DontRepaint then begin
      ...
      SavedDC := SaveDC(DC);
      try
        BitBltBorder(DC, 0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, 1);
        Message.Result := Ac_NCDraw(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, DC);
        if SimplyBox then begin
          BG.PleaseDraw := False;
  ...
  if (SkinData <> nil) then begin
    if not Destroyed and (SkinManager <> nil) then begin
      ...
      if not bRemoving or SkinManager.SkinRemoving then begin
        ...
        RestoreStdParams;
        if (uxthemeLib <> 0) then begin
          if (SkinData <> nil) and (SkinData.FOwnerObject is TsSkinProvider) and not TsSkinProvider(SkinData.FOwnerObject).DrawNonClientArea then begin
          end
...
begin
  if Caption <> '' then begin
    ...
    LoadSysFont(SkinData.FCacheBmp.Canvas, FONT_MESSAGE);
...
begin
  ...
  cRect := CaptionRect;
  if SkinData.FOwnerControl <> nil
    then SkinData.FCacheBMP.Canvas.Font.Assign(TsHackedControl(SkinData.FOwnerControl).Font)
