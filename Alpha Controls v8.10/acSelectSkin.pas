...
uses acntUtils, sStoreUtils, sConst, acSkinPreview{$IFDEF TNTUNICODE}{$IFNDEF D2006}, TntWideStrings{$ELSE}, WideStrings{$ENDIF}{$ENDIF};
...
begin
  ...
  if not (csDesigning in SkinManager.ComponentState) or (s = '') {$IFDEF DELPHI6UP}or not DirectoryExists(s){$ENDIF}
    then FormSkinSelect.sDirectoryEdit1.Text := SkinManager.SkinDirectory
    else FormSkinSelect.sDirectoryEdit1.Text := s;
...
begin
  ...
  s := ReadRegString(HKEY_CURRENT_USER, 'SOFTWARE\' + s_RegName, s_IntSkinsPath);
  if not IsIDERunning or (s = '') {$IFDEF DELPHI6UP}or not DirectoryExists(s){$ENDIF} then FormSkinSelect.sDirectoryEdit1.Text := SkinDir else FormSkinSelect.sDirectoryEdit1.Text := s;
