...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, {$IFDEF DELPHI_XE3}UITypes, {$ENDIF}{$IFNDEF DELPHI5}Types,{$ENDIF}
...
var
  ...
  Bmp : TBitmap;
begin
  DC := Form.Canvas.Handle;// GetDC(Form.Handle);
  with Form do if not sSkinProvider1.SkinData.BGChanged then begin
    ...
    if (TmpBmp = nil) then begin // Save BG
      ...
      if CI.Ready
        then BitBlt(TmpBmp.Canvas.Handle, 0, 0, TmpBmp.Width, TmpBmp.Height, CI.Bmp.Canvas.Handle, x, y, SRCCOPY)
        else FillDC(TmpBmp.Canvas.Handle, Rect(0, 0, TmpBmp.Width, TmpBmp.Height), CI.FillColor);
    end;
    Bmp.Assign(TmpBmp);

    if not InAnimationProcess then Marker(Bmp.Canvas.Handle, Point(0, GradY + ArrowSize));
...
begin
  ...
  while y < (ImgHeight - 1) div 2 do begin
    ...
    for x := 0 to ImgWidth - 1 do S[X] := C;
    inc(y)
  end;
...
begin
  if Key in ['0'..'9', 'A'..'F', 'a'..'f', Chr(3) {Ctrl-C}, Chr(22) {Ctrl-V}, Chr(27), Chr(13)] then inherited else Key := #0;
end;
...
begin
  ...
  RStep := (MaxByte - GetRValue(InternalColor)) / (ImgHeight div 2);
  GStep := (MaxByte - GetGValue(InternalColor)) / (ImgHeight div 2);
  BStep := (MaxByte - GetBValue(InternalColor)) / (ImgHeight div 2);
  y := 0;
  Result := 0;
  while y < (ImgHeight - 1) div 2 do begin
    R := R - RStep; G := G - GStep; B := B - BStep;
  ...
  RStep := GetRValue(InternalColor) / (ImgHeight div 2);
  GStep := GetGValue(InternalColor) / (ImgHeight div 2);
  BStep := GetBValue(InternalColor) / (ImgHeight div 2);
  while y < ImgHeight do begin
    R := R - RStep; G := G - GStep; B := B - BStep;
    if (abs(c.R - R) < 2) and (abs(c.G - G) < 2) and (abs(c.B - B) < 2) then begin
      Result := y;
...
begin
  if sSkinProvider1.SkinData.Skinned and not sSkinProvider1.SkinData.FCacheBmp.Empty then begin
    SkinMarker(Self);
  end
  else begin
