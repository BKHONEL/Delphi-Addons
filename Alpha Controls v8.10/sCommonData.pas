...
function IsCached(SkinData : TsCommonData) : boolean;
begin
  Result := {InAnimationProcess or v7.42 }(SkinData.CtrlSkinState and ACS_FAST <> ACS_FAST) or ControlIsActive(SkinData);
end;
  ...
  if (SkinData.SkinManager <> nil) and (SkinData.SkinIndex > -1) then begin
    with SkinData.SkinManager.gd[SkinData.SkinIndex] do begin
      if (SkinData.BorderIndex > -1) then begin // If border exists
        ...
        // If Border Is Too Big
        if ((SkinData.SkinManager.ma[SkinData.BorderIndex].WL + SkinData.SkinManager.ma[SkinData.BorderIndex].WR > 30) or
      ...
      if SkinData.SkinManager.IsValidImgIndex(ImgTL) then Exit;
      if SkinData.SkinManager.IsValidImgIndex(ImgTR) then Exit;
      if SkinData.SkinManager.IsValidImgIndex(ImgBL) then Exit;
      if SkinData.SkinManager.IsValidImgIndex(ImgBR) then Exit;
...
begin
  ...
  if (cd <> nil) and (cd is TsCommonData) and (TsCommonData(cd).SkinIndex >= 0) and (TsCommonData(cd).SkinIndex < Length(SkinManager.gd)) then begin
    if (SkinManager.gd[TsCommonData(cd).SkinIndex].Props[min(integer(State > 0), SkinManager.gd[TsCommonData(cd).SkinIndex].States - 1)].Transparency > 50) and (Ctrl.Parent.Parent <> nil)
...
begin
  with SkinData do if SkinSection = '' then case COC of
    ...
    COC_TsRadioButton : SkinSection := s_CheckBox;
