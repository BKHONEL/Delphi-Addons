...
begin
  ...
  if BorderStyle = bsSingle
    then PaintItem(FCommonData, GetParentCache(SkinData), True, integer(ControlIsActive(FCommonData)), Rect(0, 0, Width, Height), Point(Left, top), FCommonData.FCacheBmp, False)
    else PaintItemBG(FCommonData, GetParentCache(SkinData), integer(ControlIsActive(FCommonData)), Rect(0, 0, Width, Height), Point(Left, top), FCommonData.FCacheBmp);
  PaintText;
...
{$IFDEF D2009}
    ...
    FCommonData.FCacheBMP.Canvas.Font.Color := MixColors(Font.Color, Color, 0.65);
