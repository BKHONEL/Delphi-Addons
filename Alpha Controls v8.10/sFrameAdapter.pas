    ...
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      AC_GETCONTROLCOLOR : begin
        if SkinData.Skinned then begin
          ...
          else Message.LParam := LRESULT(clFuchsia);
        end
    ...
    if IsCached(FCommonData) then begin
      ...
      if not FCommonData.FUpdating then begin
        if (FCommonData.BorderIndex < 0) and (FCommonData.SkinManager.gd[FCommonData.SkinIndex].Props[0].Transparency = 100) then begin // Parent BG is taken
          ...
          if (BGInfo.BgType = btCache) and (BGInfo.Bmp <> nil) then BitBlt(DC, 0, 0, Frame.Width, Frame.Height, BGInfo.Bmp.Canvas.Handle, Frame.Left + BGInfo.Offset.X, Frame.Top + BGInfo.Offset.Y, SRCCOPY) else begin
            SavedDC := SaveDC(DC);
