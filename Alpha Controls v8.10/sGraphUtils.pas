...
uses
  ...
  StdCtrls, ComCtrls, sConst{$IFNDEF DELPHI5}, Types{$ENDIF}, ExtCtrls, Jpeg, acntUtils, math, {$IFDEF DELPHI_XE3}UITypes, {$ENDIF}
        ...
        0 : begin
          ...
          if Transparency <> 100 then begin
            ...
            if (ImagePercent > 0) then begin
              if TextureIndex < 0 then TextureIndex := gd[SkinIndex].TextureNormal;
    ...
    if ci.Ready or (ci.FillColor <> clFuchsia) then case Transparency of
      100 : begin
        if ci.Ready then begin
          if (ItemBmp <> ci.Bmp) then BitBlt(ItemBmp.Canvas.Handle, aRect.Left, aRect.Top, WidthOf(aRect, True), HeightOf(aRect, True), ci.Bmp.Canvas.Handle, ci.X + pP.X, ci.Y + pP.Y, SRCCOPY)
        end
