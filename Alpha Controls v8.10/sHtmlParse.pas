...
begin
     ...
     begin
       ...
       if (WrapWidth > 0) and (CurX + Bitmap.Canvas.TextWidth(SpecialChar) > WrapWidth) then
        begin
          CurX := Area.Left;
          NewRow;
        end;
...
var
  CutString : string; //changed variable name from c for clarity;
...
begin
  ...
  if CutString <> '' then
  repeat
{KJS ADDED TO WRAP AT DEFINED WRAP WIDTH WIDTH}
    shortenedString:=CutString;
    if (WrapWidth > 0) and (CurX + Bitmap.Canvas.TextWidth(Trim(shortenedString)) > WrapWidth) then
     begin
       HasNoSpace:=Pos(' ',Trim(shortenedString))=0;
       LastSpacePos:=0;
       //use area.left because we want to know if it's even possible
       if HasNoSpace and (Area.Left+Bitmap.Canvas.TextWidth(Trim(shortenedString)) >= WrapWidth) then //we just need to break it in the middle of a word
        begin
            //use CurLeft because we want to konw if it will fit here.
            while (CurX+Bitmap.Canvas.TextWidth(Trim(shortenedString)) > WrapWidth) do
            begin
               LastSpacePos:=Pred(Length(shortenedString));
               if (LastSpacePos = 0) then
                  break;
               shortenedString:=Copy(shortenedString,1,LastSpacePos);
            end;
            if ShortenedString <> '' then
               ShortenedString:=ShortenedString+'-';
        end
       else while (Pos(' ',shortenedString) > 0) and (CurX + Bitmap.Canvas.TextWidth(Trim(shortenedString)) > WrapWidth) do
        begin
           LastSpacePos:=Length(trim(shortenedString));
           while (LastSpacePos > 0) and (shortenedString[LastSpacePos] <> ' ') do
             dec(LastSpacePos);
           if (LastSpacePos = 0) then
              break;
           shortenedString:=Copy(shortenedString,1,LastSpacePos);
...
{KJS END ADD}
    Bitmap.Canvas.TextOut(CurX, CurY, ShortenedString);
    CurX := CurX + Bitmap.Canvas.TextWidth(ShortenedString);
    CurWidthValue := CurX;
    if Bitmap.Canvas.TextHeight('X') > CurrentRowHeight then
       CurrentRowHeight := Bitmap.Canvas.TextHeight('X');
    if HasNoSpace and (ShortenedString <> '') and (ShortenedString[Length(ShortenedString)]='-') then
       CutString:=Copy(CutString,Length(ShortenedString),Length(CutString))
    else
       CutString:=Copy(CutString,Succ(Length(ShortenedString)),Length(CutString));
  until CutString='';
//KJS REMOVED AS NEVER USED  inLastPos := inCurPos;
end;
