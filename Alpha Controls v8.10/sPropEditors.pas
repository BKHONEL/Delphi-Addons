...
uses sDefaults, sSkinManager, FileCtrl, sMaskData, sSkinProps, Menus,
{$IFNDEF ALITE}
...
{$ENDIF}
  ...
  {$IFNDEF BCB}{, Notebreg}{$ENDIF}, ac3rdPartyEditor, acSkinInfo, acTitleBar;
      ...
      while DosCode = 0 do begin
        if (FileInfo.Name[1] <> '.') then begin
          if (FileInfo.Attr and faDirectory = faDirectory) then begin
            if FileExists(sDir + FileInfo.Name + '\' + OptionsDatName) then Proc(FileInfo.Name); // If 'Options.dat' file exists
          end
          else if (FileInfo.Attr and faDirectory <> faDirectory) and (ExtractFileExt(FileInfo.Name) = '.' + acSkinExt) then begin
...
begin
  Obj := GetComponent(0);
{$IFDEF DELPHI6UP}    
  // Search a component if property owner is TCollectionItem
  if Obj is TCollectionItem then Obj := TCollectionItem(Obj).Collection.Owner;
{$ENDIF}  
