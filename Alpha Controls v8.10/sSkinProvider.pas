...
{$IFNDEF NOTFORHELP}
  ...
  protected
    ...
    procedure ProcessMessage(Msg : Cardinal; WParam : WPARAM = 0; LParam : LPARAM = 0);
...
{8!!!!        WM_GETTEXTLENGTH : begin
  ...
  {$ENDIF}
            ...
            if BorderForm = nil then SendMessage(Form.Handle, WM_SETREDRAW, 0, 0);
            OldWndProc(Message);
            if BorderForm = nil then begin
              SendMessage(Form.Handle, WM_SETREDRAW, 1, 0);
              FCommonData.BGChanged := True;
              ProcessMessage(WM_NCPAINT, 0, 0);
              Form.Repaint; // Update of controls which may be changed
//              RedrawWindow(Form.Handle, nil, 0, RDW_INVALIDATE or RDW_ALLCHILDREN or RDW_NOERASE); // Flickering of child controls removed
            end;
            UpdateSkinCaption(Self);
          end;
...
{            if (ListSW = nil) and DrawNonClientArea then begin // Initialize skinned scrollbars if not initialized yet
  ...
  //                BitBlt(TWMPaint(Message).DC, 0, 0, Form.Width, Form.Height, FCommonData.FCacheBmp.Canvas.Handle, OffsetX, OffsetY, SRCCOPY);
        ...
        WM_PAINT : if FDrawClientArea then begin
...
{$IFNDEF NOWNDANIMATION}
            ...
            SC_MINIMIZE : if (Form <> Application.MainForm) then begin
              bAnim := acGetAnimation;
              acSetAnimation(False);
                ...
                if (TWMWindowPosChanged(Message).WindowPos.Flags and SWP_HIDEWINDOW = SWP_HIDEWINDOW) and (SkinData.FCacheBmp <> nil {it's possible under Aero})  then begin // Window will be hidden
                  if not IsIconic(Form.Handle) then begin
                    if (SkinData.SkinManager.ShowState <> saMinimize) {and (FormState and FS_ANIMMINIMIZING <> FS_ANIMMINIMIZING) }then begin // Closing
                      if FAllowAnimation and (SkinData.SkinManager.AnimEffects.FormHide.Active) and (SkinData.SkinManager.AnimEffects.FormHide.Time > 0) and (FormState and FS_ANIMCLOSING <> FS_ANIMCLOSING) and (Form.FormStyle <> fsMDIChild) then begin
  ...
  {$IFNDEF NOWNDANIMATION}
        WM_CLOSE : begin
          if FAllowAnimation and (SkinData.SkinManager.AnimEffects.FormHide.Active) and (SkinData.SkinManager.AnimEffects.FormHide.Time > 0) and
               DrawNonClientArea and (FormState and FS_ANIMCLOSING <> FS_ANIMCLOSING) then begin
            OldWndProc(Message);
...
//  if biClicked then Exit; // If button is pressed and position of form is changed
  case CurrentHT of
...
begin
  if SystemMenu = nil then PrepareForm;
  if SystemMenu = nil then Exit; // Skin can't be loaded
...
procedure TsSkinProvider.ProcessMessage(Msg : Cardinal; WParam : WPARAM = 0; LParam : LPARAM = 0);
var
  acM : TMessage;
begin
  acM := MakeMessage(Msg, WParam, LParam, 0);
  NewWndProc(acM);
end;
...
{      // Move buttons if form is maximized
      if IsZoomed(Form.Handle) then begin
        if BorderForm = nil then inc(Btn.Rect.Top, SysBorderWidth(Form.Handle, BorderForm, False) div 2);
      end;}
    end;
...
begin
  ...
  if (BorderForm <> nil) then begin
    ...
    if IsZoomed(Form.Handle) then FBarRect.Top := 7 else FBarRect.Top := 0;
...
begin
  ...
  if not Item.FontData.UseSysColor
    ...
    else if sIndex >= 0 then DstBmp.Canvas.Font.Color := sMan.gd[sIndex].Props[iState].FontColor.Color;
...
begin
  ...
  for i := 0 to GetMenuItemCount(Menu) - 1 do begin
    ...
    if (u < $F000) and GetItemText(i, s, sCut, MF_BYPOSITION) then begin // If some external menuitems are exists
{$IFDEF TNTUNICODE}
      ...
      if SM.ThirdLists[j][i] = ClName then begin
        case j of
          ...
          ord(tpScrollControl) : begin
            ...
            ScrollWnd := TacEditWnd.Create(Control.Handle, SkinData, SM, SkinData.SkinSection);
          end;
...
{$ENDIF}
    ...
    else if (TsSkinProvider(FOwner).Form = Application.MainForm) then OldBorderProc(Message);
{$ENDIF}
