...
Uses Windows, Graphics, SysUtils, Classes, {$IFNDEF WIN64}acZLibEx{$ELSE}ZLib{$ENDIF}, Math, sConst;
...
begin
  if Buffer = nil then Result := 0 else begin
...
begin
  ...
  try
    ...
    if FTransparentColor <> Graphics.clNone then
    begin
...
//  if (Img.PixelFormat <> pf32bit) or UpdateAlphaChannell then begin
  TransColor := GetAPixel(Img, 0, 0).C; // Compare it with ¹5076
  Img.PixelFormat := pf32bit;
  for Y := 0 to Img.Height - 1 do begin
    S := MaskBitmap.ScanLine[Y];
    for X := 0 to Img.Width - 1 do begin
      C.C := GetAPixel(Img, x, y).C;
...
begin
  ...
  TPNGGraphic(Img).Draw(ColorBitmap.Canvas, Rect(0, 0, ColorBitmap.Width, ColorBitmap.Height));
  ...
  if Img.PixelFormat <> pf32bit then begin
    ...
    for Y := 0 to Img.Height - 1 do begin
      S := Img.ScanLine[Y];
      for X := 0 to Img.Width - 1 do with S[X] do begin
        if C = clFuchsia then A := 0 else A := MaxByte;
      end;
    end;
  end;
  ...
  for Y := 0 to Img.Height - 1 do begin
    S := Img.ScanLine[Y];
    D := ColorBitmap.ScanLine[Y];
    for X := 0 to Img.Width - 1 do with S[X] do begin
      D[X] := S[X];
      if A = 0 then SetPixelV(MaskBitmap.Canvas.Handle, X, Y, clWhite) else SetPixelV(MaskBitmap.Canvas.Handle, X, Y, clBlack)
    end;
  end;
...
begin
  for Y := 0 to Img.Height - 1 do begin
    S := Img.ScanLine[Y];
    for X := 0 to Img.Width - 1 do with S[X] do if (I = clWhite) then I := 0;
  end;
end;
...
begin
  if (PixelFormat = pf32bit) then begin
    ...
    if (w = Width) and (h = Height) then begin
      CopyBmp32(Classes.Rect(0, 0, w, h), Classes.Rect(0, 0, w, h), DstBmp, Self, EmptyCI, False, Graphics.clNone, 0, Reflected);
    end
      ...
      FreeAndNil(Mask);
      FreeAndNil(SrcBmp8);
      CopyBmp32(Classes.Rect(0, 0, w, h), Classes.Rect(0, 0, w, h), DstBmp, SrcBmp, EmptyCI, False, Graphics.clNone, 0, False);
      FreeAndNil(SrcBmp);
    end;
