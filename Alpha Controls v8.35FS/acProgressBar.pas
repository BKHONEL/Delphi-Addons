...
uses
  ...
  ComCtrls, sCommonData, sConst{$IFNDEF DELPHI5}, Types{$ENDIF}, ExtCtrls;
...
{$IFDEF D2009}
  if Style = pbstMarquee then Timer.Enabled := True;
{$ENDIF}
...
begin
  if (Style <> pbstMarquee) and not Smooth then begin
    if (FCommonData.FCacheBmp <> nil) and (FCommonData.FCacheBmp.Width = Width) and (FCommonData.FCacheBmp.Height = Height) then begin
      ...
      if (c > 1) and (Max > Min) and (Position > Min) and ((Max - Min) <> 0) then value := Round(c / (Max - Min) * (Position - Min)) else value := 0;
  ...
  if Style = pbstMarquee then begin
    ...
    if Orientation = pbHorizontal then begin
      ...
      else if (Max > Min) and (Position > Min) then begin
        w := WidthOf(clRect) - iNdent;
        c := w div (iSize.cx + iNdent);
        if c > 1 then begin
...
{$IFDEF DELPHI7UP}
  if (FStyle = pbstMarquee) and ThemeServices.ThemesEnabled and CheckWin32Version(5, 1) and HandleAllocated then begin
    ...
    if Timer <> nil then Timer.Interval := 12;//FMarqueeInterval;
  end;
{$ENDIF}
{$ENDIF}
end;
...
//      FMarqPos := FMarqPos + FOrient * FMarqStep * (16 / (FMarqueeInterval + 20));
{$IFDEF D2009}
