...
uses Windows, Messages, sCommonData, Controls, sSkinManager, Graphics, Classes, ExtCtrls, Grids, CommCtrl, acAlphaImageList,
  Forms{$IFNDEF DELPHI5}, types{$ENDIF}, ComCtrls, StdCtrls, sConst, TypInfo{$IFDEF DELPHI_XE3}, UITypes {$ENDIF};
...
type
  ...
  public
    ...
    function CallPrevWndProc(Ctrl : TControl; Msg : longint; WParam : WPARAM; var lParam : LPARAM) : LRESULT;
  ...
  public
    ...
    destructor Destroy; override;
    function CallPrevWndProc(Handle : hwnd; Msg : longint; WParam : WPARAM; var LParam : LPARAM) : LRESULT;
    procedure acWndProc(var Message: TMessage); virtual;
  end;
...
{$ENDIF}
  ...
  public
    ...
    IndHorzLines : integer;
    IndVertLines : integer;
    InfFillStyle : integer;

    TitleHorzLines : integer;
...
function Ac_NCDraw(sw : TacScrollWnd; Handle : hwnd; wParam : WPARAM; lParam : LPARAM; ThumbPos : integer = -1; aDC : hdc = 0) : LRESULT;
...
function Ac_NCCalcSize(sw : TacScrollWnd; Handle : hwnd; wParam : WPARAM; lParam : LPARAM) : LRESULT;
...
function Ac_Notify(sw : TacScrollWnd; Handle : hwnd; wParam : WPARAM; lParam : LPARAM) : LRESULT;
function Ac_StyleChange(sw : TacScrollWnd; Handle : hwnd; wParam : WPARAM; lParam : LPARAM) : LRESULT;
function Ac_Timer(sw : TacScrollWnd; Handle : hwnd; wTimerId : WPARAM; lParam : LPARAM) : LRESULT;
...
uses acntUtils, SysUtils, math, sGraphUtils, sStyleSimply, sSkinProps, sDefaults, sSkinProvider, acDials, Buttons, sSkinMenus,
  Menus, sBorders, acPNG, ImgList, {$IFDEF DELPHI7UP}Themes, {$ENDIF} {$IFNDEF ALITE} sTabControl, sSplitter, sFrameAdapter,{$ENDIF} sThirdParty,
  ToolWin, acGlow, sAlphaGraph, sMessages, sVclUtils {$IFDEF LOGGED}, sDebugMsgs{$ENDIF}, sMaskData, sMDIForm {$IFDEF TNTUNICODE}, TntControls,
  TntStdCtrls, TntButtons{$ENDIF} {$IFDEF USEPNG}, PngImageList, PngFunctions, PngImage{$ENDIF};
...
function ac_GetSysFont(fType : integer) : hFont;
var
  NonClientMetrics: TNonClientMetricsW;
begin
  FillChar(NonClientMetrics, SizeOf(NonClientMetrics), 0);
  NonClientMetrics.cbSize := SizeOf(NonClientMetrics);
  if SystemParametersInfoW(SPI_GETNONCLIENTMETRICS, 0, @NonClientMetrics, 0) then with NonClientMetrics do begin
    case fType of
      FONT_CAPTION   : Result := CreateFontIndirectW(NonClientMetrics.lfCaptionFont);
      FONT_SMCAPTION : Result := CreateFontIndirectW(NonClientMetrics.lfSmCaptionFont);
      FONT_MENU      : Result := CreateFontIndirectW(NonClientMetrics.lfMenuFont);
      FONT_STATUS    : Result := CreateFontIndirectW(NonClientMetrics.lfStatusFont);
      FONT_MESSAGE   : Result := CreateFontIndirectW(NonClientMetrics.lfMessageFont);
    end;
  end
  else Result := 0;
end;

procedure LoadSysFont(aCanvas : TCanvas; aFontType : integer);
var
  FontHandle : HFont;
  NewFont : TFont;
begin
...
procedure LoadCtrlFont(ListSW : TacMainWnd; DC: hdc);
var
  hf : hFont;
begin
  with ListSW do if SkinData.FOwnerControl <> nil then begin
    SkinData.FCacheBMP.Canvas.Font.Assign(TsHackedControl(SkinData.FOwnerControl).Font);
    SelectObject(SkinData.FCacheBmp.Canvas.Handle, SkinData.FCacheBmp.Canvas.Font.Handle);
  end
  else begin
    hf := Cardinal(SendMessage(CtrlHandle, WM_GETFONT, 0, 0));
    if hf <> 0
      then SelectObject(SkinData.FCacheBmp.Canvas.Handle, hf)
      else LoadSysFont(SkinData.FCacheBmp.Canvas, FONT_MESSAGE);
  end;
end;
...
var
  ...
  begin
    ...
    if ci.Ready then begin
      if (ci.Bmp.PixelFormat = pf32bit) then for sY := 0 to h do begin
        DstY := sY + Y;
          ...
          for sX := 0 to w do begin
            ...
            if (DstX <= SkinData.FCacheBmp.Width - 1) and (DstX >= 0) then begin
              ...
              if (Src[SrcX].B = sFuchsia.B) and (Src[SrcX].G = sFuchsia.G) and (Src[SrcX].R = sFuchsia.R) {if transparent pixel} then begin
                ...
                try
                  col.C := GetAPixel(ci.Bmp, ci.X + DstX, ci.Y + DstY).C;
                finally
                  NewColor.R := col.B;
                  NewColor.G := col.G;
                  NewColor.B := col.R;
  ...
  begin
    ...
    if not CI.Ready then begin
      ...
      for Y := 0 to h do begin
        ...
        dX2 := R2.Left;
        for X := 0 to w do begin
          if S2[dX2].C = sFuchsia.C then S1[dX1].C := {SwapRedBlue}(GetAPixel(ci.Bmp, dX1 + ci.X, R1.Top + ci.Y + Y).C);
          inc(dX1);
          inc(dX2);
        end;
      end;
...
begin
  if sw <> nil then begin
    ...
    if (sw.sbarHorz <> nil) and (sw.sbarVert <> nil) then begin
      ...
      if Repaint and not InAnimationProcess then Ac_NCDraw(sw, sw.CtrlHandle, 1, 0);
    end;
  end;
end;
  ...
  if (SkinData <> nil) and SkinData.BGChanged then begin
    ...
    if DlgMode and (DefaultManager.Palette[pcBorder] <> clFuchsia) then begin
      ...
      if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) then begin
        if (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TCustomListBox) and (TListBox(SkinData.FOwnerControl).BorderStyle = bsNone)             
          then PaintItemBG(SkinData, CI, integer((SkinData.FFocused or SkinData.FMouseAbove) and MayBeHot(SkinData)), Rect(0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height), p, SkinData.FCacheBmp)
          else PaintItem(SkinData, CI, True, integer((SkinData.FFocused or SkinData.FMouseAbove) and MayBeHot(SkinData)), Rect(0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height), p, SkinData.FCacheBmp, not DlgMode);
      end;
    end;
    if (SkinData.FOwnerControl <> nil) and not IsWindowEnabled(CtrlHandle) then begin
{$IFDEF TNTUNICODE}
      if (SkinData.FOwnerControl is TTntEdit) then with TTntEdit(SkinData.FOwnerControl) do begin
{$ELSE}
      if (SkinData.FOwnerControl is TCustomEdit) then with TEdit(SkinData.FOwnerControl) do begin
{$ENDIF}
      ...
      {$ENDIF}
          ...
          acWriteTextEx(SkinData.FCacheBMP.Canvas, PacChar(s), True, R, Flags {$IFDEF D2009}or GetStringFlags(SkinData.FOwnerControl, Alignment) and not DT_VCENTER{$ENDIF}, SkinData, ControlIsActive(SkinData));
        end
      {$IFDEF D2009}
        else if (TextHint <> '') then begin
          SkinData.FCacheBMP.Canvas.Brush.Style := bsClear;
          SkinData.FCacheBMP.Canvas.Font.Color := MixColors(Font.Color, Color, 0.65);
          acDrawText(SkinData.FCacheBMP.Canvas.Handle, TextHint, R, Flags {$IFDEF D2009}or GetStringFlags(SkinData.FOwnerControl, Alignment) and not DT_VCENTER{$ENDIF});
        end;
      {$ENDIF}
      end;
    end;
    if SkinData.FOwnerControl <> nil then begin
      if not SkinData.FOwnerControl.Enabled then begin
        ...
        BmpDisabledKind(SkinData.FCacheBmp, dk, SkinData.FOwnerControl.Parent, CI, Point(SkinData.FOwnerControl.Left, SkinData.FOwnerControl.Top));
      end;
    end;
...
begin
  ...
  if not ListSW.Destroyed then begin
    ...
    if Assigned(ListSW.OldWndProc) then begin
      ...
      else if ListSW.SkinData.FOwnerControl <> nil then TacWinControl(ListSW.SkinData.FOwnerControl).WindowProc := ListSW.OldWndProc;
    end
...
function GetScrollMetric(sBar : TacScrollBar; metric : integer; Btn : boolean = False) : integer;
begin
  if sbar <> nil then with sbar.sw.SkinManager do begin
    if sBar.nBarType = SB_HORZ then begin
      if metric = SM_CXHORZSB then begin
        if sBar.nArrowLength < 0 then begin
          if Btn and (ConstData.IndexScrollLeft > -1) and gd[ConstData.IndexScrollLeft].ReservedBoolean and (ConstData.MaskScrollLeft > -1)
            then Result := -sBar.nArrowLength * (acScrollBtnLength + WidthOfImage(ma[ConstData.MaskScrollLeft]) - 17)
      ...
      if metric = SM_CYVERTSB then begin
        if sBar.nArrowLength < 0 then begin
          if Btn and (ConstData.IndexScrollLeft > -1) and gd[ConstData.IndexScrollLeft].ReservedBoolean and (ConstData.MaskScrollLeft > -1)
            then Result := -sbar.nArrowLength * (acScrollBtnLength + HeightOfImage(ma[ConstData.MaskScrollTop]) - 17)
            else Result := -sbar.nArrowLength * acScrollBtnLength;
        end
        else Result := sbar.nArrowLength;
      end
...
begin
  ...
  if sw.sbarHorz.fScrollVisible and sw.sbarVert.fScrollVisible then begin
    ...
    if BGInfo.BgType = btCache then begin // If AlphaSkins are fully supported
      ...
      if sw.SkinManager.IsValidSkinIndex(sw.SkinManager.ConstData.IndexScrollBar1H) and
         (sw.SkinManager.gd[sw.SkinManager.ConstData.IndexScrollBar1H].Props[0].Transparency = 100)
        then // BGInfo.Color := BGInfo.Color
        else BGInfo.Color := sw.SkinManager.GetGlobalColor;
    ...
    case uCurrentScrollPortion of
      ...
      HTSCROLL_THUMB: begin
        if (sw is TacComboListWnd) and TacComboListWnd(sw).DBScroll then begin // Correcting a position for DBLookupCombo
          ...
          ThumbPos := pt.y - R.Top;
          btnHeight := GetScrollMetric(sb, SM_CYVERTSB);
          if ThumbPos > BtnHeight then begin
            if ThumbPos + ThumbSize >= HeightOf(R) - BtnHeight
              then nLastSBPos := 4
              else if ThumbPos + ThumbSize div 2 < HeightOf(R) div 2
                then nLastSBPos := 1
                else nLastSBPos := 3;
          end
          else nLastSBPos := 0;
...
begin
  ...
  else if not bCalled then Result := sw.CallPrevWndProc(Handle, WM_TIMER, wTimerID, LParam);
end;
...
begin
  case Message.Msg of
    ...
    CM_RECREATEWND : if not IsWindowVisible(CtrlHandle) then begin
      if (SkinData.FOwnerControl <> nil) then begin
        if (SkinData.FOwnerControl is TCustomComboBox) and (SkinData.FOwnerControl.Name = '') then Exit; // Problem with TDriveComboBox
      end;
    end;
    WM_DESTROY, WM_NCDESTROY: begin
      if (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TWinControl) then
        for i := 0 to TWinControl(SkinData.FOwnerControl).ControlCount - 1 do
          if (TWinControl(SkinData.FOwnerControl).Controls[i] is TSpeedButton)
            then TWinControl(SkinData.FOwnerControl).Controls[i].Perform(WM_DESTROY, 0, 0);
    ...
    if Message.Msg = SM_ALPHACMD then begin
      case Message.WParamHi of
        AC_GETBG : if SkinData <> nil then begin
          Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
          if PacBGInfo(Message.LParam).BgType = btUnknown then begin
        ...
        AC_UPDATING        : Skindata.FUpdating := Message.WParamLo = 1;
        AC_ENDPARENTUPDATE : if SkinData.FUpdating then begin
{$IFNDEF ALITE}
{$IFDEF D2006}
          if (SkinData.FOwnerObject <> nil) and (SkinData.FOwnerObject is TsFrameAdapter) and (csAligning in TsFrameAdapter(SkinData.FOwnerObject).Frame.ControlState) then begin
//            Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
            Exit;
          end;
{$ENDIF}
{$ENDIF}
          if not InUpdating(SkinData, True)
            then RedrawWindow(CtrlHandle, nil, 0, {RDW_ALLCHILDREN or }RDW_INVALIDATE or {RDW_ERASE or }RDW_FRAME or RDW_UPDATENOW);
          if SkinData.FOwnerControl <> nil then SetParentUpdated(TWinControl(SkinData.FOwnerControl));
          Exit;
        end;
    ...
    else case Message.Msg of
{
      ...
      WM_NCPAINT: if IsWindowVisible(CtrlHandle) then begin
        ...
        if not SkinData.FUpdating and not DontRepaint then Message.Result := integer(Ac_NCDraw(Self, CtrlHandle, Message.WParam, Message.LParam));
      end;
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_ENDPARENTUPDATE : if SkinData.FUpdating then begin
      if not InUpdating(SkinData, True) then begin
        RedrawWindow(CtrlHandle, nil, 0, RDW_ALLCHILDREN or RDW_INVALIDATE or RDW_FRAME or RDW_UPDATENOW or RDW_ERASE);
      end;
    end;
    AC_PREPARECACHE : begin
      InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
    ...
    AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
      inherited;
      if SkinData <> nil then SetSkinParams;
      Exit;
    end;
    AC_BEFORESCROLL : begin
      if GetWindowLong(CtrlHandle, GWL_EXSTYLE) and WS_EX_LEFTSCROLLBAR = WS_EX_LEFTSCROLLBAR then SendMessage(CtrlHandle, WM_SETREDRAW, 0, 0) else inherited;
      Exit;
    end;
    AC_AFTERSCROLL : begin
      if GetWindowLong(CtrlHandle, GWL_EXSTYLE) and WS_EX_LEFTSCROLLBAR = WS_EX_LEFTSCROLLBAR then begin
      ...
      if PacBGInfo(Message.LParam).BgType = btUnknown then begin
        if SkinData.CustomColor then begin // Used in ComboListWnd and similar windows
          ...
          if SkinData.FOwnerControl <> nil
            then PacBGInfo(Message.LParam).Color := TsHackedControl(SkinData.FOwnerControl).Color
            else PacBGInfo(Message.LParam).Color := Color;
        end
        else InitBGInfo(SkinData, PacBGInfo(Message.LParam), 0);
      end;
      Exit;
    end;
  end
...
//    end;
    WM_PRINT : begin
      ...
      if PRF_CLIENT and Message.LParam = PRF_CLIENT then begin // Patch for using with BE
        ...
        BitBltBorder(DC, 0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, ClientRect.Top);
        Message.Result := Ac_NCDraw(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, DC);
      end;

      SavedDC := SaveDC(DC);
      MoveWindowOrg(DC, cxLeftEdge, cxLeftEdge);
      if (sBarVert <> nil) and (sBarHorz <> nil)
        then IntersectClipRect(DC, 0, 0,
      ...
      FillDC(TWMPaint(Message).DC, Rect(0, 0, WndSize.cx, WndSize.cy), Color);
...
{$IFNDEF D2005}
      if (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TCustomListBox) then begin // Fix empty ListBox drawing bug, fixed in latest Delphi versions
        if (TCustomListBox(SkinData.FOwnerControl).Items.Count <> 0) then SendMessage(CtrlHandle, WM_PAINT, WPARAM(DC), 0);
      end
      else
{$ENDIF}
      SendMessage(CtrlHandle, WM_PAINT, WPARAM(DC), 0);
      SendMessage(CtrlHandle, WM_PRINTCLIENT, WPARAM(DC), PRF_CLIENT or PRF_OWNED);
      RestoreDC(DC, SavedDC);
      if not (SkinData.COC in [COC_TsListView]) then Exit;
    end;
    WM_SETTEXT, CM_ENABLEDCHANGED, CM_FONTCHANGED : begin
      Skindata.BGChanged := True;
      if not SkinData.FUpdating
        then RedrawWindow(CtrlHandle, nil, 0, RDW_ERASE or RDW_INVALIDATE or RDW_FRAME);
    end;
    WM_ERASEBKGND : if IsWindowVisible(CtrlHandle) and not bPreventStyleChange then begin
      ...
      if InUpdating(SkinData) then Exit;
      if not ParamsChanged then SetSkinParams;
      if SingleLineEdit and not IsWindowEnabled(CtrlHandle) then begin
        Message.Result := ERASE_OK;
        Exit;
      end
      else if (Win32MajorVersion >= 6) and InAnimationProcess and (TWMPaint(Message).DC <> SkinData.PrintDC) then begin
        Message.Result := ERASE_OK;
        Exit;
      end;
    end;
    WM_PAINT : if IsWindowVisible(CtrlHandle) and not bPreventStyleChange then begin
      ...
      else Message.Result := Ac_NCDraw(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, hdc(Message.wParam));
      if not ParamsChanged then SetSkinParams;
    end;
    WM_SETREDRAW : if (Message.WParam = 1) and CtrlListChanged then begin
    ...
    WM_NCPAINT : if IsWindowVisible(CtrlHandle) and not bPreventStyleChange then begin
      if not InAnimationProcess then begin
        ...
        try
          Message.Result := Ac_NCDraw(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, DC);
          if (DC <> 0) and (SkinData <> nil) then BitBltBorder(DC, 0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, ClientRect.Top);
        finally
          RestoreDC(DC, SavedDC);
          ReleaseDC(CtrlHandle, DC);
        end;
  ...
  inherited acWndProc(Message);
  case Message.Msg of
    CM_CONTROLLISTCHANGE : CtrlListChanged := True;
    WM_PARENTNOTIFY : if ((Message.WParam and $FFFF) in [WM_CREATE, WM_DESTROY])  then begin
      if (Message.WParamLo = WM_CREATE) and (srThirdParty in SkinData.SkinManager.SkinningRules) then CtrlListChanged := True;
    end;
...
begin
  if FClientRect.Top = -1 then begin // Not initialized
...
begin
  if aHandle <> 0 then begin
    if (Repaint or not (Self is TacComboListWnd)) and not (Self is TacGridWnd) and not (Self is TacVirtualTreeViewWnd) and (uxthemeLib <> 0) and (SkinSection <> s_Dialog) then begin
      if (ASkinData = nil) then Ac_SetWindowTheme(AHandle, ' ', ' ') else begin
        if not ((ASkinData.COC in [COC_TsListView]) and (Win32MajorVersion >= 6)) then begin
          if ASkinData.FOwnerControl <> nil then begin
            if not HasProperty(ASkinData.FOwnerControl, 'UseXPThemes') then Ac_SetWindowTheme(AHandle, ' ', ' '); // Patch for TRichViewEdit control
          end
          else Ac_SetWindowTheme(AHandle, ' ', ' ');
        end;
      end;
    end;
    CtrlHandle := AHandle;
    ...
    if SkinData.COC = 0 then SkinData.COC := COC_TsAdapterEdit;
    ...
    if SkinData.FOwnerControl <> nil then begin
      ...
      SingleLineEdit := (SkinData.FOwnerControl is TCustomEdit) and (GetWindowLong(CtrlHandle, GWL_STYLE) and ES_MULTILINE <> ES_MULTILINE);
      if not (SkinData.FOwnerControl is TCustomListView) then SkinData.FOwnerControl.ControlStyle := SkinData.FOwnerControl.ControlStyle + [csOpaque];
    end
    else SingleLineEdit := False;
  end;
  CtrlListChanged := False;
end;
...
begin
  inherited;
  if IsWindowVisible(CtrlHandle) // Scrollbars updating if exists
...
begin
  if not bPreventStyleChange then begin
    if Assigned(SkinData) then SkinData.Updating := True;
    ...
    if Assigned(SkinData) and Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) then begin
      if Assigned(SkinData.FOwnerControl) then begin // If VCL used
        if ControlIsActive(SkinData) then b := (SkinData.SkinManager.gd[SkinData.SkinIndex].States > 1) else b := False;
        ...
        if not SkinData.CustomColor then begin
          if not DlgMode then Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Color else Color := clWindow;
        end;
...
begin
  try
    ...
    CI.Ready := False;
    if SkinData.FOwnerControl <> nil then CI.FillColor := TacAccessLV(SkinData.FOwnerControl).color else CI.FillColor := ColorToRGB(clWindow);
    ...
    if NewFont <> 0 then OldFont := SelectObject(TempBmp.Canvas.Handle, NewFont) else OldFont := 0;
    TextRC := R;
...
{$ENDIF}
      ...
      if (item.iImage > 0) and (SkinData.FOwnerControl <> nil)
    ...
    if OldFont <> 0
      then SelectObject(TempBmp.Canvas.Handle, OldFont);
...
var
  Wnd : HWND;
begin
  if ASkinData <> nil then ASkinData.COC := COC_TsListView; // Prevent a disabling of native themes
  inherited;
  HoverColIndex := -1;
...
//      SendMessage(CtrlHandle, WM_PRINT, Message.WParam, PRF_CLIENT or PRF_OWNED);
  ...
  if Message.Msg = SM_ALPHACMD then begin
    case Message.WParamHi of
      AC_REMOVESKIN : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) and (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TCustomGrid) then begin
...
{$ENDIF}
    ...
    // TSMDBGrid params
    l := TObject(GetObjProp(SkinData.FOwnerControl, 'GridStyle'));
    if l <> nil then begin
      f := TObject(GetObjProp(l, 'Title'));
      if f <> nil then begin
        TryChangeIntProp(f, 'StartColor', FixedGradientFrom);
        TryChangeIntProp(f, 'EndColor', FixedGradientTo);
      end;
    end;
    // wwDBGrid params
...
{$ENDIF}
  ...
  if l <> nil then begin
    f := TObject(GetObjProp(l, 'Title'));
    if f <> nil then begin
      FixedGradientFrom := TryGetColorProp(f, 'StartColor');
      if FixedGradientFrom = clNone then FixedGradientFrom := clBtnFace;
      FixedGradientTo := TryGetColorProp(f, 'EndColor');
      if FixedGradientTo = clNone then FixedGradientTo := clBtnFace;
    end;
  end;
  // wwDBGrid params
...
{$ENDIF}
  ...
  if HasProperty(SkinData.FOwnerControl, acControlLook) then begin // (SkinData.FOwnerControl.ClassName = acTDBAdvGrid) or (SkinData.FOwnerControl.ClassName = acTAdvStringGrid) then begin
    ...
    TryChangeIntProp(SkinData.FOwnerControl, acGridFixedLineColor, SkinData.SkinManager.Palette[pcBorder]);
  ...
  if l <> nil then begin
    f := TObject(GetObjProp(l, 'Title'));
    if f <> nil then begin
      TryChangeIntProp(f, 'StartColor', MixColors(CGlobal, SkinData.SkinManager.GetActiveEditColor, 0.5));
      TryChangeIntProp(f, 'EndColor', CGlobal);
    end;
  end;
  // wwDBGrid params
  if HasProperty(SkinData.FOwnerControl, acLineColors) then begin
    l := GetObjProp(SkinData.FOwnerControl, acLineColors);
    TryChangeIntProp(l, acFixedColor, SkinData.SkinManager.Palette[pcBorder]);
    TryChangeIntProp(l, acDataColor, MixColors(DefaultManager.GetActiveEditColor, DefaultManager.GetActiveEditFontColor, 0.8));
    TryChangeIntProp(l, acShadowColor, MixColors(DefaultManager.GetActiveEditColor, DefaultManager.GetActiveEditFontColor, 0.8));
  end;
...
begin
  if SkinData <> nil then case Message.Msg of
    ...
    WM_ERASEBKGND : begin
      ...
      else if SkinData.SkinManager <> nil then begin
        SkinData.FUpdating := SkinData.Updating;
        ...
        try
          if not SkinData.CustomColor then
            ...
            if (SkinData.FOwnerControl <> nil) then
              C := TsHackedControl(SkinData.FOwnerControl).Color
            else
              C := clWindow;
          FillDC(DC, Rect(0, 0, WndSize.cx, WndSize.cy), iffi(DlgMode, clWindow, C));
        finally
  ...
  if DlgMode then TreeView_SetBkColor(CtrlHandle, ColorToRGB(clWindow)) else if (SkinData <> nil) then begin
    if (SkinData.SkinManager.gd[SkinData.SkinIndex].States > 1) and ControlIsActive(SkinData) then begin
      ...
      if not SkinData.CustomFont then
        ...
        if (SkinData.FOwnerControl <> nil) then
          fC := TsHackedControl(SkinData.FOwnerControl).Font.Color;
    end
    else begin
      if not SkinData.CustomColor then
        C := SkinData.SkinManager.gd[SkinData.SkinIndex].Color
    ...
    if (SkinData.FOwnerControl <> nil) then begin
      if (TsHackedControl(SkinData.FOwnerControl).Font.Color <> fC) then
        TsHackedControl(SkinData.FOwnerControl).Font.Color := fC;
      if (TsHackedControl(SkinData.FOwnerControl).Color <> C) then
...
begin
  ...
  case Message.Msg of
    ...
    CM_MOUSEENTER : if not SkinData.FMouseAbove then begin
      SkinData.FMouseAbove := True;
      SkinData.BGChanged := True;
      inherited;
      Invalidate;
    end;
    CM_MOUSELEAVE : if SkinData.FMouseAbove then begin
      GetWindowRect(CtrlHandle, R);
{$IFNDEF DELPHI5}
      if not SkinData.FFocused and (SkinData.FOwnerControl <> nil) and (GetFocus = TAccessCombo(SkinData.FOwnerControl).FEditHandle) then SkinData.FFocused := True;
{$ELSE}
      SkinData.FFocused := False;
{$ENDIF}
      if PtInRect(R, acMousePos) then Exit;
...
{$IFNDEF DELPHI5}
      if (SkinData.FOwnerControl <> nil) then SkinData.FFocused := (GetFocus = TAccessCombo(SkinData.FOwnerControl).FEditHandle) else
{$ENDIF}
      SkinData.FFocused := False;
      Skindata.BGChanged := True;
      Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
      if MayBeHot(SkinData) and Assigned(Skindata.FOwnerControl) and TWinControl(Skindata.FOwnerControl).CanFocus and TWinControl(Skindata.FOwnerControl).TabStop then begin
        SendMessage(TWinControl(Skindata.FOwnerControl).Handle, WM_NCPAINT, 0, 0);
      end;
      RepaintButton;
    end;
...
{$ENDIF}
      if (FListHandle = 0) then begin
        ...
        if (i > -1) and not DlgMode then begin
          ...
          Message.Result := LRESULT(CreateSolidBrush(ColorToRGB(SkinData.SkinManager.gd[i].Color)));
        end;
      end;
      LBoxOpening := False;
    end;
...
begin
{$IFDEF DELPHI7UP}
  if (SkinData.FOwnerControl <> nil) then begin // Invalidate ComboBoxEx
    InvalidateRect(CtrlHandle, nil, True);
...
begin
  ...
  s := '';
  if li > -1 then begin
    Len := SendMessage(CtrlHandle, CB_GETLBTEXT, li, LPARAM(@Text));
    ...
    if Len <> 0 then SendMessage(CtrlHandle, WM_GETTEXT, Len + 1, LPARAM(s));
  end;
...
begin
  ...
  case Message.Msg of
    WM_PARENTNOTIFY : if ((Message.WParam and $FFFF) in [WM_CREATE, WM_DESTROY])  then begin
      if (Message.WParamLo = WM_CREATE) and (srThirdParty in SkinData.SkinManager.SkinningRules) then AddToAdapter(TWinControl(SkinData.FOwnerControl));
    end;
  end;
end;
...
begin
  inherited;
  // Disable themed columns if not Win7
  if {(Win32MajorVersion < 6) and} {$IFDEF DELPHI7UP}Themes.ThemeServices.ThemesEnabled and {$ENDIF} HasProperty(SkinData.FOwnerControl, 'OptionsEh') then begin
    SetSetPropValue(SkinData.FOwnerControl, 'OptionsEh', 'dghFixed3D', Fixed3D);
  end;
  TryChangeIntProp(SkinData.FOwnerControl, 'EvenRowColor', EvenRowColor);
  TryChangeIntProp(SkinData.FOwnerControl, 'OddRowColor', OddRowColor);

  obj := GetObjProp(SkinData.FOwnerControl, 'GridLineParams');
  if obj <> nil then begin
    BrightColor := TryGetColorProp(obj, 'BrightColor');
    DataHorzColor := TryGetColorProp(obj, 'DataHorzColor');
    DataVertColor := TryGetColorProp(obj, 'DataVertColor');
    DarkColor := TryGetColorProp(obj, 'DarkColor');
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'IndicatorParams');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'Color', IndColor);
    TryChangeIntProp(obj, 'HorzLines', IndHorzLines);
    TryChangeIntProp(obj, 'VertLines', IndVertLines);
    SetOrdProp(obj, 'FillStyle', InfFillStyle);
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'TitleParams');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'Color', TitleColor);
    TryChangeIntProp(obj, 'SecondColor', FixedGradientTo);
    TryChangeIntProp(obj, 'HorzLines', TitleHorzLines);
    SetOrdProp(obj, 'FillStyle', TitleFillStyle);
    TryChangeIntProp(obj, 'VertLineColor', GridFixedLineColor);
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'RowDetailPanel');
  if obj <> nil then begin
    RowDetailColor := TryGetColorProp(obj, 'Color');
  end;
end;
...
begin
  inherited;
  if {(Win32MajorVersion < 6) and} {$IFDEF DELPHI7UP}Themes.ThemeServices.ThemesEnabled and {$ENDIF} HasProperty(SkinData.FOwnerControl, 'OptionsEh') then begin
    Fixed3D := CheckSetProp(SkinData.FOwnerControl, 'OptionsEh', 'dghFixed3D');
  end;
  EvenRowColor := TryGetColorProp(SkinData.FOwnerControl, 'EvenRowColor');
  if EvenRowColor = clNone then EvenRowColor := clWindow;
  OddRowColor := TryGetColorProp(SkinData.FOwnerControl, 'OddRowColor');
  if OddRowColor = clNone then OddRowColor := clWindow;

  obj := GetObjProp(SkinData.FOwnerControl, 'GridLineParams');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'BrightColor', BrightColor);
    TryChangeIntProp(obj, 'DataHorzColor', DataHorzColor);
    TryChangeIntProp(obj, 'DataVertColor', DataVertColor);
    TryChangeIntProp(obj, 'DarkColor', DarkColor);
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'IndicatorParams');
  if obj <> nil then begin
    IndColor := TryGetIntProp(obj, 'Color');
    IndHorzLines := TryGetIntProp(obj, 'HorzLines');
    IndVertLines := TryGetIntProp(obj, 'VertLines');
    InfFillStyle := GetOrdProp(obj, 'FillStyle');
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'TitleParams');
  if obj <> nil then begin
    TitleColor := TryGetColorProp(obj, 'Color');
    FixedGradientTo := TryGetColorProp(obj, 'SecondColor');
    TitleHorzLines := TryGetIntProp(obj, 'HorzLines');
    TitleFillStyle := GetOrdProp(obj, 'FillStyle');
    GridFixedLineColor := TryGetColorProp(obj, 'VertLineColor');
  end;                               

  obj := GetObjProp(SkinData.FOwnerControl, 'RowDetailPanel');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'Color', RowDetailColor);
  end;
end;
...
begin
  inherited;
  if {(Win32MajorVersion < 6) and} {$IFDEF DELPHI7UP}Themes.ThemeServices.ThemesEnabled and {$ENDIF} HasProperty(SkinData.FOwnerControl, 'OptionsEh') then begin
    SetSetPropValue(SkinData.FOwnerControl, 'OptionsEh', 'dghFixed3D', False);
  end;
  if EvenRowColor <> OddRowColor then begin
    TryChangeIntProp(SkinData.FOwnerControl, 'EvenRowColor', SkinManager.Palette[pcEditBG_EvenRow]);
    TryChangeIntProp(SkinData.FOwnerControl, 'OddRowColor', SkinManager.Palette[pcEditBG_OddRow]);
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'GridLineParams');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'BrightColor', MixColors(clWhite, SkinData.SkinManager.GetActiveEditColor, 0.2));
    TryChangeIntProp(obj, 'DataHorzColor', MixColors(SkinData.SkinManager.GetActiveEditFontColor, SkinData.SkinManager.GetActiveEditColor, 0.2));
    TryChangeIntProp(obj, 'DataVertColor', MixColors(SkinData.SkinManager.GetActiveEditFontColor, SkinData.SkinManager.GetActiveEditColor, 0.2));
    TryChangeIntProp(obj, 'DarkColor', MixColors(clBlack, SkinData.SkinManager.GetGlobalColor, 0.3));
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'IndicatorParams');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'Color', SkinData.SkinManager.Palette[pcMainColor]);
    TryChangeIntProp(obj, 'HorzLines', 0);
    TryChangeIntProp(obj, 'VertLines', 0);
    SetOrdProp(obj, 'FillStyle', 2);
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'TitleParams');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'Color', SkinData.SkinManager.Palette[pcMainColor]);
    TryChangeIntProp(obj, 'SecondColor', MixColors($FFFFFF, SkinData.SkinManager.Palette[pcMainColor], 0.1));
    TryChangeIntProp(obj, 'HorzLines', 0);
    SetOrdProp(obj, 'FillStyle', 3);
    TryChangeIntProp(obj, 'VertLineColor', SkinData.SkinManager.Palette[pcBorder]);
  end;

  obj := GetObjProp(SkinData.FOwnerControl, 'RowDetailPanel');
  if obj <> nil then begin
    TryChangeIntProp(obj, 'Color', SkinData.SkinManager.GetGlobalColor);
  end;
end;
...
//        ReleaseCapture;
    ...
    WM_PRINT, WM_PRINTCLIENT : begin
      if (Message.WParam <> 0) and IsWindowVisible(CtrlHandle) then begin
        ...
        if (uCurrentScrollPortion = HTSCROLL_NONE) and not Showed then BitBltBorder(DC, wR.Left, wR.Top, WidthOf(wR), HeightOf(wR), SkinData.FCacheBmp.Canvas.Handle, 0, 0, 1);
        Message.Result := Ac_NCDraw(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, DC);
      end;
      Exit;
    end;
...
{$ENDIF}
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    WM_MDICREATE : begin
      Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
      if Message.Result <> 0 then acDials.AddSupportedForm(Message.Result);
      Exit;
    end;
    WM_PRINT : if MDISkinData.Skinned and DrawSkinnedMDIWall and DrawSkinnedMDIScrolls and (TWMPaint(Message).DC <> 0) then begin
      ...
      try
        ...
        Ac_NCDraw(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, TWMPaint(Message).DC);
      finally
        RestoreDC(TWMPaint(Message).DC, SavedDC);
      end;
      Exit;
    ...
    WM_WINDOWPOSCHANGED : if SkinProvider <> nil then begin // Update menu after restoring of child windows (WindowState := wsNormal)
...
{$IFNDEF ALITE}
  if (FForm.MDIChildCount > 0) and (FForm.ActiveMDIChild <> nil) and (FForm.ActiveMDIChild.WindowState = wsMaximized) then
    for i := 0 to FForm.ControlCount - 1 do if FForm.Controls[i] is TsSplitter then begin
      ...
      SendMessage(FForm.Handle, SM_ALPHACMD, MakeWParam(0, AC_GETBG), LPARAM(@BG));
    end;
{$ENDIF}
end;
        ...
        inherited;
{
      ...
      WM_PRINT : begin
        ...
        Message.Result := Ac_NCDraw(Self, CtrlHandle, Message.wParam, Message.lParam, -1, DC);
        Exit;
      end;
      WM_NCPAINT : if IsWindowVisible(CtrlHandle) then begin
        InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
...
begin
  ...
  if Obj2 <> nil then begin
    ...
    if HeaderProp <> nil then begin
      ...
      if TWinControl(SkinData.FOwnerControl).HandleAllocated
        then RedrawWindow(TWinControl(SkinData.FOwnerControl).Handle, nil, 0, RDW_FRAME or RDW_INVALIDATE or RDW_UPDATENOW or RDW_ERASE);
    end;
//    TryChangeIntProp(SkinData.FOwnerControl, acThemed, 0); // Calls RecreateWnd in latest versions of VirtualTrees
  end;
end;
      ...
      if (hpeSortGlyph in Elements) and PaintInfo.ShowSortGlyph then begin
        ...
        if HeaderProp <> nil then begin
          if GetOrdProp(HeaderProp, 'SortDirection') = 0 then begin
            Index := DefaultManager.GetMaskIndex(DefaultManager.ConstData.IndexScrollBottom, s_ScrollBtnTop, s_ItemGlyph)
          end
          else begin
            Index := DefaultManager.GetMaskIndex(DefaultManager.ConstData.IndexScrollTop, s_ScrollBtnBottom, s_ItemGlyph)
          end;
...
begin
  ...
  if not Assigned(SkinData) or not Assigned(SkinData.SkinManager) or not SkinData.SkinManager.Active then begin
    ...
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      AC_ENDPARENTUPDATE : if SkinData.FUpdating then begin
        SkinData.FUpdating := False; // Reset
        SkinData.FUpdating := SkinData.Updating; // Get state of parent control
      ...
      AC_PREPARING : if (SkinData <> nil) then begin
        Message.Result := integer(SkinData.Updating);
        Exit;
      end;
      AC_SETSECTION : if (Message.LParam <> 0) and (SkinData <> nil) then SkinData.SkinSection := PacSectionInfo(Message.LParam).Name;
      AC_REMOVESKIN : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
      ...
      AC_GETSKINDATA : Message.Result := LRESULT(SkinData);
    end;
    Message.Result := CallPrevWndProc(CtrlHandle, Message.Msg, Message.WParam, Message.LParam);
  end;
end;

function TacMainWnd.CallPrevWndProc(Handle : hwnd; Msg : longint; WParam : WPARAM; var LParam : LPARAM) : LRESULT;
var
  M : TMessage;
begin
...
begin
  ...
  if (uxthemeLib <> 0) then begin
    if (ASkinData <> nil) and (ASkinData.FOwnerObject is TsSkinProvider) and not TsSkinProvider(ASkinData.FOwnerObject).DrawNonClientArea then begin
    end
    else if not (Self is TacMnuWnd) and (SkinSection <> s_Dialog) and not acDlgMode then Ac_SetWindowTheme(AHandle, ' ', ' ');
  end;
  NewWndProcInstance := nil;
  SkinManager := ASkinManager;
  Destroyed := False;
  ...
  if (SkinData <> nil) then begin
    if (SkinManager <> nil) then begin
      if not Destroyed then begin
        if (SkinData.FOwnerControl <> nil) and (OldCtrlStyle <> []) then begin
        ...
        if not bRemoving or SkinManager.SkinRemoving then begin
          if Assigned(OldWndProc) then begin
            ...
            if DWord(GetWindowLong(CtrlHandle, GWL_WNDPROC)) = DWord(NewWndProcInstance) then SetWindowLong(CtrlHandle, GWL_WNDPROC, LONG_PTR(oldproc));
            oldproc := nil;
        ...
        if (uxthemeLib <> 0) and (SkinData <> nil) then begin
          if Assigned(SkinData.FOwnerObject) and (SkinData.FOwnerObject is TsSkinProvider) then begin
            if not TsSkinProvider(SkinData.FOwnerObject).DrawNonClientArea then
              Ac_SetWindowTheme(CtrlHandle, nil, nil);
          end
          else if not ((SkinData.COC in [COC_TsListView]) and AeroIsEnabled) then Ac_SetWindowTheme(CtrlHandle, nil, nil);
        end;
...
begin
  if Caption <> '' then begin
    ...
    LoadCtrlFont(Self, SkinData.FCacheBMP.Canvas.Handle);

    Flags := DT_TOP or DT_EXPANDTABS or DT_WORDBREAK or DT_NOCLIP;

    if GetWindowLong(CtrlHandle, GWL_STYLE) and SS_NOPREFIX = SS_NOPREFIX then Flags := Flags or DT_NOPREFIX;
    ...
    if Application.BiDiMode = bdRightToLeft then Flags := Flags or DT_RTLREADING;
...
begin
  ...
  SetBkMode(SkinData.FCacheBMP.Canvas.Handle, TRANSPARENT);
...
var
  ...
  DrawStyle: Cardinal;
begin
  R := Rect(0, 0, MaxCaptionWidth + 2, 0);
  DrawStyle := DT_EXPANDTABS or DT_CENTER or DT_CALCRECT or DT_NOPREFIX;
  if CtrlStyle and BS_MULTILINE = BS_MULTILINE
...
begin
  ...
  try
    ...
    if SkinData.SkinManager.IsValidImgIndex(i) then begin
      ...
      BG.PleaseDraw := True;
      SendMessage(ParentWnd, SM_ALPHACMD, MakeWParam(0, AC_GETBG), LPARAM(@BG));
...
begin
  if SendMessage(CtrlHandle, TB_GETITEMRECT, Index, LPARAM(@Result)) = 0 then Result := Rect(0, 0, 0, 0);
end;
...
begin
  BeginPaint(CtrlHandle, PS);
  SkinData.FUpdating := GetBoolMsg(GetParent(CtrlHandle), AC_PREPARING) or SkinData.Updating;
...
begin
  SendMessage(CtrlHandle, TB_GETITEMRECT, Index, LPARAM(@Result));
end;
...
var
  ...
  nRect : TRect;
begin
  if (SendMessage(CtrlHandle, TB_GETBUTTON, Index, LPARAM(@Btn)) <> 0) then begin
...
{$ENDIF}
      if s <> '' then begin
        NewFont := LongWord(SendMessage(CtrlHandle, WM_GETFONT, 0, 0));
        if NewFont <> 0 then OldFont := SelectObject(BtnBmp.Canvas.Handle, NewFont) else OldFont := 0;
        ...
        if OldFont <> 0
          then SelectObject(BtnBmp.Canvas.Handle, OldFont);
      end;
    end;
    if Btn.fsState and TBSTATE_ENABLED <> TBSTATE_ENABLED then begin
...
begin
  ...
  case Message.Msg of
    ...
    WM_PARENTNOTIFY : if (Message.WParam and $FFFF = WM_CREATE) or (Message.WParam and $FFFF = WM_DESTROY) then begin
      inherited;
      if (Message.WParamLo = WM_CREATE) and (SkinData.FOwnerControl <> nil) then AddToAdapter(TWinControl(SkinData.FOwnerControl));
      exit;
    end;
...
begin
  inherited;
  if SkinSection = ''
    then ASkinData.SkinSection := s_GroupBox;
end;
...
{$ENDIF}
  ...
  case Message.Msg of
    ...
    WM_SETFOCUS, WM_KILLFOCUS : SendMessage(CtrlHandle, WM_PAINT, 0, 0);
    WM_NCHITTEST : begin
...
begin
  if not IsBmp then begin // Icon
    ...
    if not FBmp.Empty then begin
      if FBmp.PixelFormat = pf32bit
        then CopyBmp32(Rect(0, 0, WndSize.cx, WndSize.cy), Rect(0, 0, FBmp.Width, FBmp.Height), SkinData.FCacheBmp, FBmp, MakeCacheInfo(SkinData.FCacheBmp), True, clNone, 1, True)
        else BitBlt(SkinData.FCacheBmp.Canvas.Handle, 0, 0, min(WndSize.cx, FBmp.Width), min(WndSize.cy, FBmp.Height), FBmp.Canvas.Handle, 0, 0, SRCCOPY);
      Result := True;
    end
    else Result := False;
  end
...
{$ENDIF}
  ...
  case Message.Msg of
    ...
    WM_MOUSEMOVE : if (DefaultManager <> nil) and not (csDesigning in DefaultManager.ComponentState) and IsWindowEnabled(CtrlHandle) then begin
      ...
      else if DefaultManager.ActiveControl = CtrlHandle then DefaultManager.ActiveControl := 0;
    end;
  end;
end;
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    WM_WINDOWPOSCHANGING : begin
      ...
      if not bFlag then begin
        if (TWMWindowPosChanged(Message).WindowPos.Flags and SWP_NOREDRAW <> SWP_NOREDRAW)
            and (TWMWindowPosChanged(Message).WindowPos.Flags and SWP_SHOWWINDOW = SWP_SHOWWINDOW) then begin
    ...
    WM_NCPAINT : begin
      if not IsNT then inherited else begin
        ...
        if AeroIsEnabled and not Shown then RedrawWindow(CtrlHandle, nil, 0, RDW_UPDATENOW or RDW_INVALIDATE or RDW_ERASE);
        ...
        else SetWindowLong(CtrlHandle, GWL_EXSTYLE, GetWindowLong(CtrlHandle, GWL_EXSTYLE) and not WS_EX_LAYERED);
      end;
      Shown := True;
    end
    else inherited;
...
{$ENDIF}
  if (SkinData.FOwnerControl <> nil) and (csAcceptsControls in TWinControl(SkinData.FOwnerControl).ControlStyle) then begin
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      CM_TEXTCHANGED : SkinData.BGChanged := True;
      WM_WINDOWPOSCHANGED, WM_WINDOWPOSCHANGING : begin
        if SkinData.SkinManager.gd[SkinData.SkinIndex].Transparency <> 0 then SkinData.BGChanged := True;
      end;
      WM_SETTEXT : if IsWindowVisible(CtrlHandle) then begin
        SendMessage(CtrlHandle, WM_SETREDRAW, 0, 0);
        inherited;
  ...
  if not SkinData.FUpdating then begin
    ...
    try
      ...
      if SkinData.BGChanged then begin
        ...
        PaintItem(SkinData.SkinIndex, ChangedSkinSection, CI, False, 0, R, Point(WndPos.X, WndPos.Y), SkinData.FCacheBmp, SkinData.SkinManager);
        SkinData.BGChanged := False;
      end;
...
begin
  if (SkinData <> nil) then if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_SETNEWSKIN : begin
      if (Message.LParam = LPARAM(SkinData.SkinManager)) then CommonWndProc(Message, SkinData);
      AlphaBroadCastCheck(SkinData.FOwnerControl, CtrlHandle, Message);
      Exit;
    end;
...
{$ENDIF}
  ...
  if (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TPageControl) and TPageControl(SkinData.FOwnerControl).OwnerDraw and Assigned(TPageControl(SkinData.FOwnerControl).OnDrawTab) then begin
    TPageControl(SkinData.FOwnerControl).Canvas.Handle := Bmp.Canvas.Handle;
    TPageControl(SkinData.FOwnerControl).OnDrawTab(TPageControl(SkinData.FOwnerControl), Index, R, State = 2);
    TPageControl(SkinData.FOwnerControl).Canvas.Handle := 0;
  end
  else begin
    InflateRect(R, -3, -3);
    ...
    if SkinData.FOwnerControl <> nil then Bmp.Canvas.Font.Assign(TAccessControl(SkinData.FOwnerControl).Font);
    case TabPosition of
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_GETBG : begin
      ...
      if Page <> nil then begin
        PacBGInfo(Message.LParam)^.Offset.X := PacBGInfo(Message.LParam)^.Offset.X + Page.Left;
        PacBGInfo(Message.LParam)^.Offset.Y := PacBGInfo(Message.LParam)^.Offset.Y + Page.Top;
      end
      else begin
...
begin
  ...
  if ParentBG.BgType = btCache then begin
    if ParentBG.Bmp.Empty then SendMessage(ParentWnd, WM_PAINT, Longint(ParentBG.Bmp.Canvas.Handle), 0); // Prepare image
    if not ParentBG.Bmp.Empty // Image is ready
      then BitBlt(Message.DC, 0, 0, WndSize.cx, WndSize.cy, ParentBG.Bmp.Canvas.Handle, WndPos.X + ParentBG.Offset.X, WndPos.Y + ParentBG.Offset.Y, SRCCOPY)
      else Exit;
...
{$ENDIF}
  if (SkinData <> nil) and SkinData.Skinned then case Message.Msg of
    ...
    TCM_SETCURSEL : if (SkinData.FOwnerControl <> nil) then begin
      InitPages(True);
      AddToAdapter(TWinControl(SkinData.FOwnerControl));
    end;
    WM_PRINT : begin
...
var
  ...
  pw : TacPageWnd;
begin
  if (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TPageControl) then with TPageControl(SkinData.FOwnerControl) do begin
{
    if ActivePage <> nil then begin
      if not GetBoolMsg(ActivePage.Handle, AC_CTRLHANDLED) then begin
        if ServWndList = nil then ServWndList := TList.Create;
        pw := TacPageWnd.Create(ActivePage.Handle, nil, TsSkinManager(SkinManager), s_CheckBox);
        pw.Page := ActivePage;
        ServWndList.Add(pw);
      end;
    end;
    ...
    for i := 0 to PageCount - 1 do begin
      if not GetBoolMsg(Pages[i].Handle, AC_CTRLHANDLED) then begin
        if ServWndList = nil then ServWndList := TList.Create;
        pw := TacPageWnd.Create(Pages[i].Handle, nil, TsSkinManager(SkinManager), s_CheckBox);
        pw.Page := Pages[i];
        ServWndList.Add(pw);
      end;
    end;
...
begin
  ...
  if (SkinData <> nil) and SkinData.Skinned then case Message.Msg of
    ...
    CN_NOTIFY : with TWMNotify(Message) do case TWMNotify(Message).NMHdr^.code of
      TBN_DROPDOWN: with PNMToolBar(NMHdr)^ do if ToolBar.Perform(TB_GETBUTTON, iItem, LPARAM(@tbButton)) <> 0 then begin
  ...
  if (SkinData <> nil) and SkinData.Skinned then case Message.Msg of
    ...
    WM_SIZE : begin
      if not SkinData.Updating
        then RedrawWindow(CtrlHandle, nil, 0, RDW_INVALIDATE);
      Exit;
    end;
...
begin
  if SkinData.Skinned then begin
    DefaultDraw := False;
    if not (Stage in [cdPrePaint]) then begin
      DefaultDraw := False;
      Exit
    end;
    ...
    if (bWidth = 0) or (bHeight = 0) then Exit;
...
begin
      ...
      if not (SimplePanel or (Index = PartsCount - 1)) then begin;
        ...
        if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidskinIndex(si) then begin
          ...
          if SkinData.SkinManager.IsValidImgIndex(mi) then begin
            TempBmp := CreateBmp32(max(2, (SkinData.SkinManager.ma[mi].WL + SkinData.SkinManager.ma[mi].WR)), WndSize.cy - 2);
  ...
  if (StatusBar <> nil) and (Index > -1) and (Index < StatusBar.Panels.Count) and (StatusBar.Panels[Index].Style = psOwnerDraw) and Assigned(StatusBar.OnDrawPanel) then begin
    ...
    dec(aRect.Right, 4);
{
    dis.hDC := SkinData.FCacheBmp.Canvas.Handle;
    dis.rcItem := aRect;
    dis.itemID := Index;

    Msg.DrawItemStruct := @dis;
    Msg.Ctl := CtrlHandle;
}
//    if SendMessage(CtrlHandle, CN_DRAWITEM, TMessage(Msg).WParam, TMessage(Msg).LParam) = 0 then begin
...
begin
  SendMessage(CtrlHandle, SB_GETRECT, Index, LPARAM(@Result));
end;
...
begin
  if PartsCount = 0 then begin
    ...
    if SimplePanel then begin
      ...
      if Len <> 0 then SendMessage(CtrlHandle, WM_GETTEXT, Len + 1, LPARAM(Result)) else Result := '';
    end
    else begin
...
//      ControlStyle := ControlStyle - [csOpaque];
    ...
    WM_DESTROY : if not Destroyed then begin
      Destroyed := True;
...
function TacSpeedButtonHandler.CallPrevWndProc(Ctrl : TControl; Msg : longint; WParam: WPARAM; var lParam : LPARAM) : LRESULT;
var
  M : TMessage;
begin
  Result := 0;
...
begin
  try
    if not Destroyed and Assigned(Ctrl) and Assigned(PCtrl)
      then Ctrl.WindowProc := OldWndProc;
  finally
    SkinManager := nil;
  end;
{
...
begin
  if Caption <> '' then begin
    ...
    { Calculate vertical layout }
    CalcRect := R;
    DoDrawText(R, DT_EXPANDTABS or DT_WORDBREAK or DT_CENTER);//GetStringFlags(Ctrl, taLeftJustify));
  end;
end;
...
begin
  Result := Rect(0, 0, 1, 1);
  SendMessage(CtrlHandle, TBM_GETCHANNELRECT, 0, LPARAM(@Result));
  if IsVertical then begin
...
begin
  Result := Rect(0, 0, 1, 1);
  SendMessage(CtrlHandle, TBM_GETTHUMBRECT, 0, LPARAM(@Result));
end;
...
begin
  Value := longint(i);
  Result := integer(SendMessage(CtrlHandle, TBM_GETTICPOS, Value, 0));
end;
...
{$IFDEF LOGGED}
  AddToLog(Message);
{$ENDIF}
  if (Message.Msg = SM_ALPHACMD) and not (csDestroying in Control.ComponentState) then case Message.WParamHi of
    AC_CTRLHANDLED : begin Message.Result := 1; Exit end;
    AC_GETAPPLICATION : begin Message.Result := LRESULT(Application); Exit end;
    AC_REMOVESKIN : if ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager) then begin
      CommonWndProc(Message, SkinData);
  ...
  if Assigned(SkinData) then begin
    case Message.Msg of
      SBM_SETSCROLLINFO : begin
        with PScrollInfo(Message.LParam)^ do begin
          ...
          if Boolean(fMask and SIF_RANGE) and ((FSI.nMin <> nMin) or (FSI.nMax <> nMax)) then begin
            ...
            if Boolean(fMask and SIF_POS) then begin
              if (Control.Min <> FSI.nMin) then Control.Min := FSI.nMin;
              if (Cardinal(Control.PageSize) <> FSI.nPage) and (FSI.nPage < Cardinal(Control.Max)) then Control.PageSize := FSI.nPage;
              if Control.PageSize > FSI.nMax then Control.PageSize := FSI.nMax;
              if (FSI.nMax <> Control.Max) then Control.Max := FSI.nMax;
            end;
          end;
        ...
        inherited;
        while FLockCount > 0 do SendMessage(Control.Handle, WM_SETREDRAW, 1, 0); // Clear Lock
        InvalidateRect(Control.Handle, nil, True);
        RedrawWindow(Control.Handle, nil, 0, RDW_FRAME or RDW_ERASE or RDW_INVALIDATE or RDW_UPDATENOW);
        while FLockCount < i do SendMessage(Control.Handle, WM_SETREDRAW, 0, 0); // Restore Lock
        Exit;
      end;
      CM_CHANGED : begin
        i := FLockCount;
      ...
      WM_NCLBUTTONDOWN, WM_LBUTTONDOWN : begin
        ...
        while FLockCount > 0 do SendMessage(Control.Handle, WM_SETREDRAW, 1, 0);
        SendMessage(Control.Handle, WM_SETREDRAW, 0, 0);
        inherited;
        SendMessage(Control.Handle, WM_SETREDRAW, 1, 0);
      ...
      WM_NCLBUTTONUP, WM_LBUTTONUP : LMouseUp(TWMMouse(Message).XPos, TWMMouse(Message).YPos);
      WM_SETREDRAW : if not FRepainting then begin
        if Message.WParam = 0 then inc(FLockCount) else dec(FLockCount);
      end;
      ...
      WM_NCHITTEST : WMNCHitTest(TWMNCHitTest(Message));
      CM_MOUSELEAVE : begin
...
function TacSBWnd.Btn1DRect: TRect;
begin
  Result := Btn1Rect;
  with SkinData.SkinManager.ConstData, SkinData.SkinManager do if Control.Kind = sbHorizontal then begin
    if (IndexScrollLeft > -1) and gd[IndexScrollLeft].ReservedBoolean and (MaskScrollLeft > -1) then begin
      Result.Right := GetSystemMetrics(SM_CXHSCROLL) + (WidthOfImage(ma[MaskScrollLeft]) - 17 {If additional width exists});
    end;
  end
  else begin
    if (IndexScrollTop > -1) and gd[ConstData.IndexScrollTop].ReservedBoolean and (MaskScrollTop > -1) then begin
      Result.Bottom := GetSystemMetrics(SM_CYVSCROLL) + (HeightOfImage(ma[MaskScrollTop]) - 17 {If additional width exists});
    end;
  end;
end;
...
begin
  ...
  if Control.Kind = sbHorizontal then begin
    ...
    FBtn1Rect.Bottom := Control.Height;
    if WidthOf(FBtn1Rect) > Control.Width div 2 then FBtn1Rect.Right := Control.Width div 2;
  end
  else begin
    FBtn1Rect.Right := Control.Width;
    FBtn1Rect.Bottom := GetSystemMetrics(SM_CYVSCROLL);
    if HeightOf(FBtn1Rect) > Control.Height div 2 then FBtn1Rect.Bottom := Control.Height div 2;
  end;
  Result := FBtn1Rect;
end;

function TacSBWnd.Btn2DRect: TRect;
begin
  Result := Btn2Rect;
  with SkinData.SkinManager.ConstData, SkinData.SkinManager do if Control.Kind = sbHorizontal then begin
    if (IndexScrollRight > -1) and gd[IndexScrollRight].ReservedBoolean and (MaskScrollRight > -1) then begin
      Result.Left := Control.Width - (GetSystemMetrics(SM_CXHSCROLL) + (WidthOfImage(ma[MaskScrollRight]) - 17 {If additional width exists}));
    end;
  end
  else begin
    if (IndexScrollBottom > -1) and gd[IndexScrollBottom].ReservedBoolean and (MaskScrollBottom > -1) then begin
      Result.Top := Control.Height - (GetSystemMetrics(SM_CYVSCROLL) + (HeightOfImage(ma[MaskScrollBottom]) - 17 {If additional width exists}));
    end;
  end;
end;
...
begin
  if Control.Kind = sbHorizontal then begin
    FBtn2Rect.Left := Control.Width - GetSystemMetrics(SM_CXHSCROLL);
    FBtn2Rect.Top := 0;
    FBtn2Rect.Right := Control.Width;
    FBtn2Rect.Bottom := Control.Height;
    if WidthOf(FBtn2Rect) > Control.Width div 2 then FBtn2Rect.Left := Control.Width div 2;
  end
  else begin
    FBtn2Rect.Left := 0;
    FBtn2Rect.Top := Control.Height - GetSystemMetrics(SM_CYVSCROLL);
    FBtn2Rect.Right := Control.Width;
    FBtn2Rect.Bottom := Control.Height;
    if HeightOf(FBtn2Rect) > Control.Height div 2 then FBtn2Rect.Top := Control.Height div 2;
  end;
  Result := FBtn2Rect;
end;

procedure TacSBWnd.CMMouseEnter(var Msg: TMessage);
begin
  Bar1State := 1;
  Bar2State := 1;
  UpdateBar;
...
procedure TacSBWnd.CMMouseLeave(var Msg: TMessage);
begin
  Btn1State := 0;
  Btn2State := 0;
...
function TacSBWnd.CoordToPosition(p: TPoint): integer;
var
  ss : integer;
  r : real;
begin
  if Control.Enabled then begin
    ss := SliderSize;
    if Control.Kind = sbHorizontal then begin
      r := (Control.Width - 2 * GetSystemMetrics(SM_CXHSCROLL) - ss);
      ...
      r := (Control.Height - 2 * GetSystemMetrics(SM_CYVSCROLL) - ss);
...
constructor TacSBWnd.Create(AHandle: hwnd; ASkinData: TsCommonData; ASkinManager: TsSkinManager; const SkinSection: string; Repaint: boolean);
begin
  FRepainting := False;
...
procedure TacSBWnd.DrawBtnBottom(b: TBitmap);
begin
  with SkinData.SkinManager.ConstData do begin
    ...
    if SkinData.SkinManager.IsValidImgIndex(MaskArrowBottom) then
      with SkinData.SkinManager do begin
...
procedure TacSBWnd.DrawBtnLeft(b: TBitmap);
begin
  with SkinData.SkinManager.ConstData do begin
    Ci.Bmp := b;
    PaintItemFast(IndexScrollLeft, MaskScrollLeft, IndexBGScrollLeft, IndexBGHotScrollLeft, s_ScrollBtnLeft, Ci, True,
    ...
    if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidImgIndex(MaskArrowLeft) then
      with SkinData.SkinManager do begin
      if ma[MaskArrowLeft].Bmp = nil then begin // If MasterBitmap used
...
procedure TacSBWnd.DrawBtnRight(b: TBitmap);
begin
  with SkinData.SkinManager.ConstData do begin
    PaintItemFast(IndexScrollRight, MaskScrollRight, IndexBGScrollRight, IndexBGHotScrollRight, s_ScrollBtnRight, Ci, True,
      Btn2State, Btn2DRect, Point(0, 0), b, SkinData.SkinManager);
    ...
    if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidImgIndex(MaskArrowRight) then
      with SkinData.SkinManager do begin
...
procedure TacSBWnd.DrawBtnTop(b: TBitmap);
begin
  with SkinData.SkinManager.ConstData do begin
    PaintItemFast(IndexScrollTop, MaskScrollTop, IndexBGScrollTop, IndexBGHotScrollTop, s_ScrollBtnTop, Ci, True,
      Btn1State, Btn1DRect, Point(0, 0), b, SkinData.SkinManager);
    ...
    if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidImgIndex(MaskArrowTop) then
      with SkinData.SkinManager do begin
...
begin
  ...
  if (Control.Kind = sbVertical) then begin
    if HeightOf(R) > Control.Height - HeightOf(FBtn1Rect) - HeightOf(FBtn2Rect) then Exit
  end
  else if WidthOf(R) > Control.Width - WidthOf(FBtn1Rect) - WidthOf(FBtn2Rect) then Exit;

  TmpBmp := CreateBmp32(WidthOf(R), HeightOf(R));
  BitBlt(TmpBmp.Canvas.Handle, 0, 0, TmpBmp.Width, TmpBmp.Height, SkinData.FCacheBmp.Canvas.Handle, R.Left, R.Top, SRCCOPY);
  Ci := MakeCacheInfo(SkinData.FCacheBmp);
  with SkinData.SkinManager.ConstData do begin
    if Control.Kind = sbHorizontal then begin
  ...
  if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsValidImgIndex(i1) then
    with Self.SkinData.SkinManager do begin
    if Self.SkinData.SkinManager.ma[i1].Bmp = nil then begin
    ...
    else if (((Control.Kind = sbVertical) and (ma[i1].Bmp.Height div 2 < HeightOf(FSliderRect))) or
         ((Control.Kind = sbHorizontal) and (ma[i1].Bmp.Width div 2 < WidthOf(FSliderRect)))) then begin
...
function TacSBWnd.FirstPoint: integer;
begin
  if Control.Kind = sbHorizontal
...
procedure TacSBWnd.LMouseDown(X, Y: Integer);
var
  i : integer;
begin
  ...
  if PtInRect(Btn1Rect, Point(x,y)) then begin
    if Btn1State <> 2 then begin
      if Control.Kind = sbVertical then ScrollCode := SB_LINEUP else ScrollCode := SB_LINELEFT;
      Btn1State := 2;
  ...
  // If Button2 pressed...
    ...
    if Btn2State <> 2 then begin
      Btn2State := 2;
      if Control.Kind = sbVertical then ScrollCode := SB_LINEDOWN else ScrollCode := SB_LINERIGHT;
      DrawingForbidden := True;
    end;
  end
  // If slider pressed...
    ...
    if PtInRect(Bar1Rect, Point(x,y)) then begin
      if Control.Kind = sbVertical then ScrollCode := SB_PAGEUP else ScrollCode := SB_PAGELEFT;
...
//        IncPos(-Math.Max(Integer(FSI.nPage), 1));
      ...
      if Control.Kind = sbVertical then ScrollCode := SB_PAGEDOWN else ScrollCode := SB_PAGERIGHT;
...
procedure TacSBWnd.LMouseUp(X, Y: Integer);
begin
  if not Control.Enabled then Exit;
...
begin
  if ((FSI.nMax - FSI.nMin - Math.Max(Integer(FSI.nPage) - 1, 0)) <> 0) then begin
    if Control.Kind = sbHorizontal then begin
      Result := FirstPoint + SliderSize div 2 + Round((FCurrPos - FSI.nMin) * ((Control.Width - 2 * FirstPoint - SliderSize) / (FSI.nMax - FSI.nMin - Math.Max(Integer(FSI.nPage) - 1,0))));
    end
    else begin
      Result := FirstPoint + SliderSize div 2 + Round((FCurrPos - FSI.nMin) * ((Control.Height - 2 * FirstPoint - SliderSize) / (FSI.nMax - FSI.nMin - Math.Max(Integer(FSI.nPage) - 1,0))));
    end;
  end
  else if Control.Kind = sbHorizontal then Result := Control.Width div 2 else Result := Control.Height div 2;
end;
...
begin
  if Control.Kind = sbHorizontal then begin
    if Control.Max = Control.Min then FSliderRect.Left := Btn1Rect.Right else FSliderRect.Left := Round(PositionToCoord - SliderSize / 2);
    FSliderRect.Top := 0;
    ...
    if Control.Max = Control.Min
      then FSliderRect.Top := Btn1Rect.Bottom
      else if Control.Position = Control.Max - Control.PageSize
        then FSliderRect.Top := Btn2Rect.Top - SliderSize
        else FSliderRect.Top := Round(PositionToCoord - SliderSize / 2);
    FSliderRect.Left := 0;
...
begin
  if Control.Enabled then begin
    if PtInRect(SliderRect, Control.ScreenToClient(SmallPointToPoint(Message.Pos))) or (SliderState = 2) then begin
      if SliderState <> 2 then SliderState := 1 else begin
        i := CoordToPosition(Control.ScreenToClient(Point(Message.Pos.X, Message.Pos.Y))) - MouseOffset;
        if Control.Position <> i then begin
          DrawingForbidden := True;
          Control.Position := i;
        end;
      end;
    ...
    if PtInRect(Btn1Rect, Control.ScreenToClient(SmallPointToPoint(Message.Pos))) then begin
      if Btn1State <> 2 then Btn1State := 1;
    end
    else if PtInRect(Btn2Rect, Control.ScreenToClient(SmallPointToPoint(Message.Pos))) then begin
      if Btn2State <> 2 then Btn2State := 1;
    end
    else if (SliderState = 2) then begin
      i := CoordToPosition(Control.ScreenToClient(SmallPointToPoint(Message.Pos)));
      if Control.Position <> i then begin
        DrawingForbidden := True;
        Control.Position := i;
      end;
...
begin
  ...
  if Msg.DC = 0 then DC := GetDC(Control.Handle) else DC := Msg.DC;
  SavedDC := SaveDC(DC);
  try
    if {DrawingForbidden or }not ControlIsReady(Control) {or RestrictDrawing }or (csDestroying in Control.ComponentState) or (csLoading in Control.ComponentState) or SkinData.Updating then else begin
//      RepaintNeeded := False;
      InitCacheBmp(SkinData);

      if not Control.Enabled then bmp := CreateBmpLike(SkinData.FCacheBmp) else bmp := SkinData.FCacheBmp;
      ...
      with SkinData.SkinManager.ConstData do begin
        ...
        if (HeightOf(FBar1Rect) > 0) and (WidthOf(FBar1Rect) > 0) then begin
          ...
          LocalState := LocalState * integer(Control.Enabled);
          if Control.Kind = sbHorizontal then begin
            if not Assigned(SkinData.SkinManager) or not SkinData.SkinManager.IsValidSkinIndex(IndexScrollBar1H) then Exit;
            PaintItemFast(IndexScrollBar1H, MaskScrollBar1H, BGScrollBar1H, BGHotScrollBar1H,
                          s_ScrollBar1H, lCi, True, LocalState, FBar1Rect, Point(Control.Left, Control.Top), SkinData.FCacheBmp, SkinData.SkinManager);
          end
          else begin
            if not Assigned(SkinData.SkinManager) or not SkinData.SkinManager.IsValidSkinIndex(IndexScrollBar1V) then Exit;
            PaintItemFast(IndexScrollBar1V, MaskScrollBar1V, BGScrollBar1V, BGHotScrollBar1V,
                          s_ScrollBar1V, lCi, True, LocalState, FBar1Rect, Point(Control.Left, Control.Top), SkinData.FCacheBmp, SkinData.SkinManager);
          end;
        end;
        Bar2Rect;
        if (HeightOf(FBar2Rect) > 0) and (WidthOf(FBar2Rect) > 0) then begin
          ...
          if Control.Kind = sbHorizontal then begin
            PaintItemFast(IndexScrollBar2H, MaskScrollBar2H, BGScrollBar2H, BGHotScrollBar2H,
                          s_ScrollBar2H, lCi, True, LocalState, FBar2Rect, Point(Control.Left + FBar2Rect.Left, Control.Top + FBar2Rect.Top), SkinData.FCacheBmp, SkinData.SkinManager);
          end
          else begin
            PaintItemFast(IndexScrollBar2V, MaskScrollBar2V, BGScrollBar2V, BGHotScrollBar2V,
                          s_ScrollBar2V, lCi, True, LocalState, FBar2Rect, Point(Control.Left + FBar2Rect.Left, Control.Top + FBar2Rect.Top), SkinData.FCacheBmp, SkinData.SkinManager);
          end;
        end;
      end;
      BitBlt(bmp.Canvas.Handle, 0, 0, bmp.Width, bmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY);
      ...
      if Control.Enabled then DrawSlider(bmp);

      BG.PleaseDraw := False;
      if not Control.Enabled then begin
        lCI := GetParentCache(SkinData);
        BmpDisabledKind(bmp, [dkBlended], Control.Parent, lCI, Point(Control.Left, Control.Top));
      end;

      BitBlt(DC, 0, 0, bmp.Width, bmp.Height, bmp.Canvas.Handle, 0, 0, SRCCOPY);
    end;
    if not Control.Enabled and Assigned(bmp) then FreeAndNil(bmp);
  finally
    RestoreDC(DC, SavedDC);
    if Msg.DC = 0 then ReleaseDC(Control.Handle, DC);
...
begin
  if Control.Kind = sbHorizontal then Result := Control.Width - 2 * GetSystemMetrics(SM_CXHSCROLL) else Result := Control.Height - 2 * GetSystemMetrics(SM_CYVSCROLL);
end;
...
begin
  ...
  try
    PrepareCache;
    Ac_NCDraw(Self, Self.CtrlHandle, integer(DC), 0, -1, DC);
    w := (WidthOf(WndRect) - WidthOf(ClRect)) div 2;
    if w > 0 then BitBltBorder(DC, 0, 0, WndSize.cx, WndSize.cy, SkinData.FCacheBmp.Canvas.Handle, 0, 0, w);
  finally
    if aDC = 0 then ReleaseDC(CtrlHandle, DC);
...
procedure TacScrollBoxWnd.AC_WMPaint(var aDC: hdc);
var
  b : boolean;
  DC, SaveIndex : HDC;
  R : TRect;
...
begin
  ...
  if not SkinData.Updating then begin
    InitCtrlData(CtrlHandle, ParentWnd, WndRect, ParentRect, WndSize, WndPos);
    if aDC = 0 then DC := GetWindowDC(Ctrl.Handle) else DC := aDC;
    SaveIndex := SaveDC(DC);
    try
      // If transparent and form resizing processed
      b := SkinData.HalfVisible or SkinData.BGChanged;
      ...
      GetClientRect(CtrlHandle, ClRect);
      w := 2;
      CopyWinControlCache(Ctrl, SkinData, Rect(w, w, 0, 0), Rect(0, 0, Ctrl.Width - 2 * w, Ctrl.Height - 2 * w), DC, True);
      sVCLUtils.PaintControls(DC, Ctrl, b and SkinData.RepaintIfMoved, Point(0, 0));
      SetParentUpdated(Ctrl);
    finally
      RestoreDC(DC, SaveIndex);
      if aDC = 0 then ReleaseDC(Ctrl.Handle, DC);
    end;
  end;
end;
...
{      WM_NCPAINT : begin
      ...
      WM_NCPAINT : if IsWindowVisible(CtrlHandle) and not bPreventStyleChange then begin
        if not InAnimationProcess then begin
          ...
          try
            Message.Result := Ac_NCDraw(Self, Self.CtrlHandle, Message.wParam, Message.lParam, -1, DC);
            if (DC <> 0) and (SkinData <> nil) then BitBltBorder(DC, 0, 0, SkinData.FCacheBmp.Width, SkinData.FCacheBmp.Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, 2);
          finally
            RestoreDC(DC, SavedDC);
            ReleaseDC(CtrlHandle, DC);
          end;
      ...
      WM_SIZE, CM_TEXTCHANGED : SkinData.BGChanged := True;
      WM_WINDOWPOSCHANGED, WM_WINDOWPOSCHANGING : begin
        if SkinData.SkinManager.gd[SkinData.SkinIndex].Transparency <> 0 then SkinData.BGChanged := True;
      end;
      WM_SETTEXT : if IsWindowVisible(CtrlHandle) then begin
