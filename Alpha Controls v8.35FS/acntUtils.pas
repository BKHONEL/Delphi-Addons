...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, sConst, IniFiles{$IFDEF DELPHI_XE3}, UITypes {$ENDIF}
...
function WordToByte_(const Value : word) : byte; register; //Nick
...
function CopyFiles(SrcMask, SrcDir: ACString; DstDir: ACString) : boolean;
function MoveFiles(SrcMask, SrcDir: ACString; DstDir: ACString) : boolean;
...
function WordToByte_(const Value : word) : byte; register;
begin
  Result := lo(Value);
end;
...
begin
  ...
  for I := 1 to Length(Value) do
    if not CharInSet(Value[I], [{$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator, '-', '+', '0'..'9', 'e', 'E']) then
      Exit;
  Result := TextToFloat(StrPLCopy(Buffer, Value,
    SizeOf(Buffer) - 1), RetValue {$IFDEF WIN32}, fvExtended{$ELSE}, fvExtended {$ENDIF});
end;
...
begin
  ...
  I := Pos({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator, S);
  ...
  for I := MaxSym downto MinSym do begin
    ...
    if (Group = 3) and Thousands and (I > MinSym) then begin
      Group := 0;
      Result := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ThousandSeparator + Result;
    end;
  end;
...
begin
  Result := True;
  if not acDirExists(DstDir) then {$IFDEF DELPHI6UP}ForceDirectories(DstDir){$ELSE}acCreateDir(DstDir){$ENDIF};
  ...
  try
    while DosCode = 0 do begin
      if (FileInfo.Name[1] <> '.') and (FileInfo.Attr <> faVolumeID) then begin
        if (FileInfo.Attr and faVolumeID <> faVolumeID)
          then Result := acCopyFile(SrcDir + FileInfo.Name, DstDir + FileInfo.Name, False);
      end;
    ...
    while DosCode = 0 do begin
      if (FileInfo.Name[1] <> '.') and (FileInfo.Attr <> faVolumeID) then begin
        if (FileInfo.Attr and faVolumeID <> faVolumeID) then begin
          Old := SrcDir + FileInfo.Name;
          New := DstDir + FileInfo.Name;
          if FileExists(New) then DeleteFile(New);
          if not MoveFile(PChar(Old), PChar(New)) then Result := False;// Alert(SysErrorMessage(GetLastError))
        end;
      end;
      DosCode := acFindNext(FileInfo);
    end;
