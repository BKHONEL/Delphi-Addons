...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, {$IFNDEF DELPHI5}Types,{$ENDIF}
    ComCtrls, sConst, ExtCtrls, acTinyJpg, math{$IFNDEF ACHINTS}, sMaskData, sCommonData{$ENDIF};
...
begin
  ...
  if TransparentColor <> clNone then begin
    for Y := 0 to h do begin
      ...
      for X := 0 to w do if TsColor_(S[R.Left + X]).C <> TransparentColor then TsColor_(S[R.Left + X]).A := Value;
    end
  end
  else begin
    for Y := 0 to h do begin
