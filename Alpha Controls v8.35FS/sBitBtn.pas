...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, Imglist, StdCtrls, Buttons, sCommonData, {$IFNDEF DELPHI5}Types, {$ENDIF}
  sConst, sDefaults, sFade {$IFDEF LOGGED}, sDebugMsgs{$ENDIF} {$IFDEF TNTUNICODE}, TntButtons{$ENDIF};
...
begin
  ...
  try
    ...
    if not FCommonData.FUpdating and not (Assigned(FadeTimer) and FadeTimer.Enabled) then begin
      ...
      else if (FCommonData.BGChanged or (csDesigning in ComponentState {for glyph changing})) then begin
        if not PrepareCache then Exit;
      end;
      SavedDC := SaveDC(DC);
      try
...
begin
  ...
  if sm.IsValidImgIndex(FCommonData.BorderIndex) then begin
    if RegionChanged and (sm.ma[FCommonData.BorderIndex].CornerType = 2) and not (csDesigning in ComponentState) and not InAnimationProcess and (FadeTimer = nil) then begin
  ...
  if not FDrawOverBorder and sm.IsValidImgIndex(FCommonData.BorderIndex) then
    if (State <> 0) or (sm.ma[FCommonData.BorderIndex].DrawMode and BDM_ACTIVEONLY <> BDM_ACTIVEONLY) then begin
      if (sm.ma[FCommonData.BorderIndex].CornerType = 2) and not (csDesigning in ComponentState)
        then PaintRgnBorder(FCommonData.FCacheBmp, FRegion, True, sm.ma[FCommonData.BorderIndex], State)
        else DrawSkinRect(FCommonData.FCacheBmp, Rect(0, 0, FCommonData.FCacheBmp.Width, FCommonData.FCacheBmp.Height), True, CI, sm.ma[FCommonData.BorderIndex], State, True, sm);
    end;
...
{$ENDIF}
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_PREPARECACHE : begin
      Message.Result := integer(not PrepareCache);
      Exit;
    end;
