    ...
    if SkinData.SkinManager.ma[SkinData.BorderIndex].MaskType = 1 then begin
      if ParentBG.BgType = btCache then begin
        ...
        BitBltBorder(SkinData.FCacheBmp.Canvas.Handle, aRect.Left, aRect.Top, WidthOf(aRect), HeightOf(aRect),
          ParentBG.Bmp.Canvas.Handle, ParentBG.Offset.X + SkinData.FOwnerControl.Left + aRect.Left, ParentBG.Offset.Y + SkinData.FOwnerControl.Top + aRect.Top, l);

//        BitBlt(SkinData.FCacheBmp.Canvas.Handle, aRect.Left, aRect.Top, WidthOf(aRect), HeightOf(aRect),
//          ParentBG.Bmp.Canvas.Handle, ParentBG.Offset.X + SkinData.FOwnerControl.Left + aRect.Left, ParentBG.Offset.Y + SkinData.FOwnerControl.Top + aRect.Top, SRCCOPY);
    ...
    DrawSkinRect(SkinData.FCacheBmp, aRect, False, CI, SkinData.SkinManager.ma[SkinData.BorderIndex], State, True);
    if SkinData.FOwnerControl <> nil then begin
      CI.X := CI.X - SkinData.FOwnerControl.Left;
      CI.Y := CI.Y - SkinData.FOwnerControl.Top;
    end;
      ...
      if FillRgn then for Y := 0 to h do begin
        ...
        for X := 0 to w do begin
          ...
          if src.C = clFuchsia then begin // If transparent pixel..
            ...
            dst := S1[DstX1 + X];
            msk := M[SrcX1 + X];
            dst.R := ((dst.R - src.R) * msk.R + src.R shl 8) shr 8;
            dst.G := ((dst.G - src.G) * msk.G + src.G shl 8) shr 8;
            dst.B := ((dst.B - src.B) * msk.B + src.B shl 8) shr 8;
            S1[DstX1 + X] := dst;
          end;
        end;
        if (RegRect.Left <> -1) and (Region <> 0) then begin
...
begin
  ...
  if h > 0 then begin
    y := wt;
    if Stretch then StretchBltMask(0, y, wl, Bmp.Height - wb - wt, dw, MaskData.R.Top + wt, wl, h, Bmp, BmpSrc, dhm) else begin
  ...
  if w > 0 then begin
    x := wl;
    if Stretch then StretchBltMask(x, 0, Bmp.Width - wr - x, wt, dw + wl, MaskData.R.Top, w, wt, Bmp, BmpSrc, dhm) else begin
  ...
  if w > 0 then begin
    x := wl;
    if Stretch then StretchBltMask(x, Bmp.Height - wb, Bmp.Width - wr - x, wb, dw + wl, MaskData.R.Top + dh - wb, w, wb, Bmp, BmpSrc, dhm) else begin
      while x < Bmp.Width - w - wr do begin
  ...
  if h > 0 then begin
    y := wt;
    if Stretch then StretchBltMask(Bmp.Width - wr, y, wr, Bmp.Height - wb - y, dw + w + wl, MaskData.R.Top + wt, wr, h, Bmp, BmpSrc, dhm) else begin
      while y < Bmp.Height - h - wb do begin
