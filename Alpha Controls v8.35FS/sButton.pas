...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, ImgList, Menus, {$IFNDEF DELPHI5}Types, {$ENDIF}
  StdCtrls, sCommonData, Buttons, sConst, {$IFDEF DELPHI_XE3}UITypes, {$ENDIF}sDefaults, sFade{$IFDEF LOGGED}, sDebugMsgs{$ENDIF}
...
begin
  with Button do Result := Width - 2 * FContentMargin;
end;
...
begin
  ...
  if Style = bsSplitButton then aWidth := Width - DropWidth else aWidth := Width;
  if Style = bsCommandLink then begin
  ...
  else if GlyphExist and (ImageAlignment <> iaCenter) then begin
...
const
  BS_MASK = $000F;
begin
  if ((SendMessage(Handle, BM_GETSTATE, 0, 0) and BST_PUSHED = BST_PUSHED) or fGlobalFlag) and (SkinData.FMouseAbove or not (csLButtonDown in ControlState)) or FDown or DroppedDown
    then Result := 2
    else if not (csDesigning in ComponentState) and ControlIsActive(FCommonData)
      then Result := 1
      else if Default
        then Result := 3
        else Result := 0
...
begin
  ...
  try
    if not InUpdating(FCommonData) and not (Assigned(FadeTimer) and FadeTimer.Enabled) then begin
      ...
      FRegion := 0;
      if RegionChanged and FCommonData.SkinManager.IsValidImgIndex(FCommonData.BorderIndex) and (SkinData.SkinManager.ma[SkinData.BorderIndex].CornerType = 2) and not (csDesigning in ComponentState) then begin
        ...
        if (DC <> SkinData.PrintDC) and not (csAlignmentNeeded in ControlState) then begin // Set region
      ...
      else if FCommonData.BGChanged then if not PrepareCache then Exit;
      SavedDC := SaveDC(DC);
      try
...
begin
  ...
  if (CurrentState = 3 {Def/Focused}) and (sm.gd[FCommonData.SkinIndex].States < 4)
    then State := 1
    else State := min(CurrentState, sm.gd[FCommonData.SkinIndex].States - 1);
  ...
  if sm.IsValidImgIndex(FCommonData.BorderIndex) then begin
    if RegionChanged and (sm.ma[FCommonData.BorderIndex].CornerType = 2) and not InAnimationProcess and (FadeTimer = nil) then begin
      if FRegion > 1 then DeleteObject(FRegion);
      FRegion := CreateRectRgn(0, 0, Width, Height);
      if (State <> 0) or (sm.ma[FCommonData.BorderIndex].DrawMode and BDM_ACTIVEONLY <> BDM_ACTIVEONLY)
        then PaintRgnBorder(FCommonData.FCacheBmp, FRegion, True, sm.ma[FCommonData.BorderIndex], State);
    end
    else begin // Empty region
      if (State <> 0) or (sm.ma[FCommonData.BorderIndex].DrawMode and BDM_ACTIVEONLY <> BDM_ACTIVEONLY) then begin
        inc(CI.X, Left);
        inc(CI.Y, Top);
        DrawSkinRect(FCommonData.FCacheBmp, Rect(0, 0, FCommonData.FCacheBmp.Width, FCommonData.FCacheBmp.Height), True, CI, sm.ma[FCommonData.BorderIndex], State, True, sm);
        dec(CI.X, Left);
        dec(CI.Y, Top);
      end;
    end;
  ...
  if Style = bsSplitButton then begin
    ...
    DrawColorArrow(FCommonData.FCacheBmp.Canvas, FCommonData.FCacheBmp.Canvas.Pen.Color, Rect(Width - DropWidth, 0, Width, Height), BF_BOTTOM);
  end;
  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned(True) then begin
    ...
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      AC_ENDPARENTUPDATE : if FCommonData.Updating then begin
        if not InUpdating(FCommonData, True)
          then Repaint;
        Exit
      end;
      ...
      WM_UPDATEUISTATE : if Visible or (csDesigning in ComponentState) then begin
        Message.Result := 1;
        Exit;
      end;
      WM_ERASEBKGND : if Visible or (csDesigning in ComponentState) then begin
        if (TWMPaint(Message).DC <> 0) and (Skindata.FCacheBmp <> nil) and not FCommonData.BGChanged and ((FadeTimer = nil) or not FadeTimer.Enabled) then begin
          CopyWinControlCache(Self, FCommonData, Rect(0, 0, 0, 0), Rect(0, 0, Width, Height), TWMPaint(Message).DC, False);
        end;
        Message.Result := 0;
        Exit;
      end;
...
{$IFDEF DELPHI7UP}
  if ThemeServices.ThemesEnabled then begin
    ...
    if IsFocused and IsDefault then begin
      ...
      if Style = bsSplitButton then R.Right := R.Right - DropWidth + 2;
...
{$ENDIF}
  ...
  if Style = bsSplitButton then begin
    ...
    DrawColorArrow(Canvas, Font.Color, Rect(Width - DropWidth, 0, Width, Height), BF_BOTTOM);
  end;
  Canvas.Handle := 0;
  Canvas.Free;
end;
...
procedure TsButton.Notification(AComponent: TComponent;
