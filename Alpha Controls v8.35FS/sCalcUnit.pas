...
uses sCurrEdit, sCustomComboEdit, acntUtils, sSkinManager, sConst, sGraphUtils, sMessages;
...
begin
  ...
  case Key of
    '.', ',': begin
      CheckFirst;
      if Pos({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator, FText) = 0 then SetText(FText + {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator);
      Exit;
    end;
...
begin
  if Clipboard.HasFormat(CF_TEXT) then begin
    try
      SetDisplay(StrToFloat(Trim(ReplaceStr(Clipboard.AsText, {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}CurrencyString, ''))));
    except
      SetText('0');
    end;
  end;
