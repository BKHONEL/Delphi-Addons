...
begin
  ...
  SavedDC := SaveDC(DC);
  try
    if not InUpdating(FCommonData) then PaintControl(DC);
  finally
...
begin
  ...
  if not (csLoading in ComponentState) then begin
    ...
    if b and assigned(FOnValueChanged) then FOnValueChanged(self);
  end;
end;
...
{$ENDIF}
        ...
        CM_CHANGED : begin
          if not (csDesigning in ComponentState) and (FadeTimer <> nil) and FadeTimer.Enabled then begin
            if Checked
        ...
        WM_KEYUP : if not (csDesigning in ComponentState) and Enabled and (TWMKey(Message).CharCode = VK_SPACE) then begin
          if ReadOnly then Exit;
