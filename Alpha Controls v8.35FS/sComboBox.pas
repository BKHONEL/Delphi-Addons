...
var
  w : integer;
begin
  if (Style <> csSimple) and FShowButton then w := GetSystemMetrics(SM_CXVSCROLL) + 1 else w := 0;
...
{$ENDIF}
  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned then begin
    inherited;
    if not (csLoading in ComponentState) then case Message.Msg of
      CM_FONTCHANGED : begin
        if IsWindowVisible(Handle) then ItemHeight := ItemHeight; // Update control size
        Invalidate;
      end;
    end;
  end
...
{$ENDIF}
      ...
      WM_PRINT : begin
        ...
        if (Style = csDropDown) and SkinData.FFocused then begin
  ...
  if SkinData.Skinned then begin
    ...
    if Assigned(OnDrawItem) and (Style in [csOwnerDrawFixed, csOwnerDrawVariable]) then begin
      ...
      if (csDropDown = Style) and (odComboBoxEdit in State) then begin
        ...
        else acWriteTextEx(Bmp.Canvas, PACChar(s), True, aRect, DrawStyle, sNdx, True, SkinData.SkinManager);
...
begin
  ...
  else Result := inherited GetItemHt;
end;
{$ENDIF}
