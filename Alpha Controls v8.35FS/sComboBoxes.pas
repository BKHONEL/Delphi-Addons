...
begin
  if FReadOnly then case Message.Msg of
    ...
    WM_LBUTTONDOWN, WM_LBUTTONDBLCLK : if ReadOnly then begin
      if not Focused then SetFocus;
      Exit;
    end;
  end;
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    AC_MOUSELEAVE : if not (csDesigning in ComponentState) then begin
  ...
  if not (csDestroying in ComponentState) and FCommonData.Skinned then case Message.Msg of
    WM_MOUSEMOVE : if not (csDesigning in ComponentState) and (SkinData.SkinManager.ActiveControl <> ExHandle) then begin
      SkinData.SkinManager.ActiveControl := ExHandle;
    end;
...
{$ENDIF}
  if HandleAllocated and FCommonData.Skinned then begin
    if not FCommonData.CustomColor then begin
      Color := FCommonData.SkinManager.gd[FCommonData.SkinIndex].Props[0].Color;//GetControlColor(Self);
    end;
...
begin
  ...
  if bEdit then CI := MakeCacheInfo(FCommonData.FCacheBmp, aRect.Left, aRect.Top) else begin
    CI.Bmp := nil;
    CI.Ready := False;
    CI.FillColor := FCommonData.SkinManager.GetActiveEditColor;// Color;
  end;
  ...
  if bSelected then begin
    if (Style = csExDropDown) then begin
      ...
      if sNdx < 0
        ...
        else PaintItem(sNdx, s_Selection, CI, True, 1{integer(odFocused in aState)}, R, Point(0, 0), Bmp, SkinData.SkinManager);
    end
    else begin
      if sNdx < 0
        then FillDC(Bmp.Canvas.Handle, Rect(0, 0, Bmp.Width, Bmp.Height), SkinData.SkinManager.GetHighLightColor)
        else PaintItem(sNdx, s_Selection, CI, True, 1{integer(odFocused in aState)}, Rect(0, 0, Bmp.Width, Bmp.Height), Point(0, 0), Bmp, SkinData.SkinManager);
    end;
  end
  else begin
    if CI.Ready then BitBlt(Bmp.Canvas.Handle, 0, 0, Bmp.Width, Bmp.Height, CI.Bmp.Canvas.Handle, CI.X, CI.Y, SRCCOPY) else FillDC(Bmp.Canvas.Handle, rText, CI.FillColor);
  end;
...
//    DrawStyle := DrawStyle and not DT_VCENTER or DT_BOTTOM;
    WriteTextEx(Bmp.Canvas, PChar(Text), True, rText, DrawStyle, sNdx, True{(odFocused in aState)}, SkinData.SkinManager);
  end
  else begin
    if (aIndex >= 0) and (aIndex < Items.Count) then begin
      if sNdx < 0 then begin
        if odSelected in aState then Bmp.Canvas.Font.Color := SkinData.SkinManager.GetHighLightFontColor(True{odFocused in aState}) else Bmp.Canvas.Font.Color := FCommonData.SkinManager.GetActiveEditFontColor;//Font.Color;
        Bmp.Canvas.Brush.Style := bsClear;
        AcDrawText(Bmp.Canvas.Handle, Items[aIndex], rText, DrawStyle);
      end
      else WriteTextEx(Bmp.Canvas, PChar(Items[aIndex]), True, rText, DrawStyle, sNdx, True{(odFocused in aState)}, SkinData.SkinManager);
    end;
  end;
  ...
  else begin
{
    if lboxhandle <> 0 then begin
      ...
      GetWindowRect(lboxhandle, ClnR);
      FCommonData.FCacheBmp.Height := ClnR.Top - WndR.Top;
    end
    else FCommonData.FCacheBmp.Height := ItemHeight + 8;
}
...
begin
  ...
  if not FDropDown then begin
    ...
    else BitBltBorder(DC, 0, 0, Width, Height, SkinData.FCacheBmp.Canvas.Handle, 0, 0, 3);
  end
  else begin
    btnRect := ButtonRect;
...
{$ENDIF}
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
      if FDropDown and (ExHandle <> 0) then begin
        // Close up
      ...
      if FCommonData.Skinned then begin
        ...
        if ListSW <> nil then begin
          ...
          if not FCommonData.CustomColor then TacComboListWnd(ListSW).Color := FCommonData.SkinManager.GetActiveEditColor else TacComboListWnd(ListSW).Color := Color;
        end;
      end;
  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned then inherited else begin
    case Message.Msg of
      WM_CTLCOLOREDIT : if (Win32MajorVersion < 6) and (Style = csExDropDownList) then begin // Forbid a filling by system
        if Message.WParam <> 0 then SetBkMode(hdc(Message.WParam), TRANSPARENT);
        Message.Result := LRESULT(GetStockObject(NULL_BRUSH));
        Exit;
      end;
{$IFDEF D2010} // Catch when control is hot
      WM_SETCURSOR : if not (csDesigning in ComponentState) and (SkinData.SkinManager.ActiveControl <> ExHandle) and (SkinData.SkinManager.ActiveControl <> Handle) then begin
...
{$ENDIF}
      ...
      CN_COMMAND : case TWMCommand(Message).NotifyCode of
        CBN_CLOSEUP : begin
          ...
          if ExHandle <> 0 then RedrawWindow(ExHandle, nil, 0, RDW_INVALIDATE or RDW_UPDATENOW or RDW_ALLCHILDREN);
        end;
        CBN_DROPDOWN : begin
          FDropDown := True;
          FCommonData.BGChanged := True;
      ...
      WM_DRAWITEM : begin
        if not SkinData.FUpdating then WMDrawItem(TWMDrawItem(Message));
        if Message.Result = 1 then Exit
      end;
      WM_ERASEBKGND : begin
        Message.Result := 1;
      ...
      CM_MOUSEENTER, CM_MOUSELEAVE : if not (csDesigning in ComponentState) then begin
        if not DroppedDown or (Style = csExSimple) then begin
          ...
          if (Message.Msg = CM_MOUSELEAVE) then begin
            P := acMousePos;
            if PtInRect(R, P) then Exit; // If mouse is in Edit
          end
          else begin
...
begin
  ...
  if (odSelected in aState) and (ShowcolorName or not (odComboBoxEdit in aState)) then begin
    if Skinned then begin
      if (sNdx > -1)
        then PaintItem(sNdx, s_Selection, CI, True, 1{integer(odFocused in aState)}, Rect(0, 0, Bmp.Width, Bmp.Height), Point(0, 0), Bmp, SkinData.SkinManager)
  ...
  if ShowcolorName or not (odComboBoxEdit in aState) then begin
    ...
    if (aIndex >= 0) and (aIndex < Items.Count) then begin
      if Skinned and (sNdx > -1) then WriteTextEx(Bmp.Canvas, PChar(Items[aIndex]), True, rText, DrawStyle, sNdx, odSelected in aState, SkinData.SkinManager) else begin
        ...
        if (odSelected in aState) then begin
          if Skinned
            then Bmp.Canvas.Font.Color := SkinData.SkinManager.GetHighLightFontColor(True{odFocused in aState})
...
begin
  if AStyle <> Style then begin
    FStyle := AStyle;
//    Enabled := ([cbStandardColors, cbExtendedColors, cbSystemColors, cbCustomColor, cbCustomColors] * FStyle) <> [];
    PopulateList;
    SkinData.Invalidate;
  end;
end;
