...
uses
  windows, Graphics, Classes, Controls, SysUtils, StdCtrls,  Dialogs, sSkinManager, acntUtils, {$IFNDEF DELPHI5}Types, {$ENDIF}
...
{$ENDIF} // NOTFORHELP
    ...
    procedure Invalidate(UpdateNow : boolean = False);
...
function InUpdating(SkinData : TsCommonData; Reset : boolean = False) : boolean;
function IsCacheRequired(SkinData : TsCommonData) : integer;
...
function GetFontIndex(const Ctrl : TControl; const DefSkinIndex : integer; const SkinManager : TsSkinManager; State : integer = 0) : integer;
...
function FullRepaintOnly(const SkinData : TsCommonData; ResizeDirection : TScrollStyle) : boolean;
...
function IsCached(SkinData : TsCommonData) : boolean;
begin
  Result := (SkinData.CtrlSkinState and ACS_FAST <> ACS_FAST) or ControlIsActive(SkinData);
end;

function InUpdating(SkinData : TsCommonData; Reset : boolean = False) : boolean;
begin
  if Reset then SkinData.FUpdating := False;
  if SkinData.Updating then begin
    SkinData.FUpdating := True;
    Result := True; // Change to True in Beta
  end
  else Result := False;
end;
...
begin
  with SkinData do if Skinned and (COC in sEditCtrls) then begin
    if (FOwnerControl is TWinControl) and not TWinControl(FOwnerControl).HandleAllocated then Exit;
...
function IsCacheRequired(SkinData : TsCommonData) : integer; // Used for non-active controls only, active controls have cache always
begin
  Result := 0;
  if (SkinData.SkinManager <> nil) and (SkinData.SkinIndex > -1) then begin
    with SkinData.SkinManager.gd[SkinData.SkinIndex] do begin
      if (SkinData.BorderIndex > -1) then begin // If border exists
        if SkinData.SkinManager.ma[SkinData.BorderIndex].DrawMode and BDM_FILL = BDM_FILL then Exit; // If center image is drawn by border mask
      end;
      if (Transparency = 100) then begin // Get cachestate of the parent
        if SkinData.SkinManager.gd[SkinData.SkinIndex].TextureNormal >= 0 then begin
          if acFillModes[SkinData.SkinManager.ma[SkinData.SkinManager.gd[SkinData.SkinIndex].TextureNormal].DrawMode] = fmDisTiled then Exit; // If special texture used
        end;
        if Assigned(SkinData.FOwnerControl) and Assigned(SkinData.FOwnerControl.Parent) { and SkinData.Skinned} then begin
          if not SkinData.FOwnerControl.Parent.HandleAllocated // If parent is not ready
            then Result := ACS_BGUNDEF
            else begin
              if SendAMessage(SkinData.FOwnerControl.Parent, AC_CTRLHANDLED) <> 1 // If parent is not skinned
                then Result := ACS_FAST
                else Result := SendAMessage(SkinData.FOwnerControl.Parent, AC_GETSKINSTATE); // and ACS_FAST <> ACS_FAST;
            end;
          Exit;
        end
        else Exit;
      end
      else if (Transparency <> 0) {If control is semi-transparent} then Exit;
      if (SkinData.SkinManager.gd[SkinData.SkinIndex].ImagePercent <> 0) or (SkinData.SkinManager.gd[SkinData.SkinIndex].GradientPercent <> 0) then Exit;
      if (SkinData.FOwnerObject is TsSkinProvider) and Assigned(TsSkinProvider(SkinData.FOwnerObject).Form.OnPaint) then Exit;
      if (ImgTL >= 0) or (ImgTR >= 0) or (ImgBL >= 0) or (ImgBR >= 0) then Exit;
      Result := ACS_FAST;
    end;
  end
  else Result := ACS_FAST;
end;
...
begin
  ...
  if SkinData.Skinned and not SkinData.CustomColor then begin
    if State >= Skindata.SkinManager.gd[SkinData.SkinIndex].States then aState := Skindata.SkinManager.gd[SkinData.SkinIndex].States - 1 else aState := State;
    if aState > 1 then aState := 1;
    iTransparency := Skindata.SkinManager.gd[SkinData.SkinIndex].Props[aState].Transparency;
    iGradient := Skindata.SkinManager.gd[SkinData.SkinIndex].Props[aState].GradientPercent;
    iTexture := Skindata.SkinManager.gd[SkinData.SkinIndex].Props[aState].ImagePercent;
    case iTransparency of
      0 : begin
        if (iGradient > 0) or (iTexture > 0) then begin
          PBGInfo^.Color := SkinData.SkinManager.GetGlobalColor;
          if (SkinData.FCacheBmp = nil) or SkinData.FCacheBmp.Empty or ((SkinData.FOwnerControl <> nil) and
               ((SkinData.FCacheBmp.Width <> SkinData.FOwnerControl.Width) or (SkinData.FCacheBmp.Height <> SkinData.FOwnerControl.Height))) then begin
//          if (SkinData.FCacheBmp = nil) or SkinData.FCacheBmp.Empty then begin
            PBGInfo^.BgType := btNotReady;//Fill;
            Exit;
          end;
          ...
          PBGInfo^.Bmp := nil;//SkinData.FCacheBmp;
          PBGInfo^.Color := GetBGColor(SkinData, State);
          if SkinData.SkinManager.IsValidImgIndex(SkinData.BorderIndex) and (SkinData.SkinManager.ma[SkinData.BorderIndex].DrawMode and BDM_ACTIVEONLY <> BDM_ACTIVEONLY)then begin
            with SkinData.SkinManager.ma[SkinData.BorderIndex] do PBGInfo^.FillRect := Rect(WL, WT, WR, WB);
            PBGInfo^.Bmp := SkinData.FCacheBmp;
            PBGInfo^.Offset := Point(0, 0);
          end;
      ...
      100 : begin
        if (SkinData.CtrlSkinState and ACS_FAST <> ACS_FAST) and (SkinData.FCacheBmp <> nil) then begin
          if SkinData.BGChanged and (SkinData.FOwnerControl <> nil) then SkinData.FOwnerControl.Perform(SM_ALPHACMD, MakeWParam(0, AC_PREPARECACHE), 0);
          PBGInfo^.BgType := btCache;
          if PBGInfo^.PleaseDraw then begin
            BitBlt(PBGInfo^.DrawDC, PBGInfo^.R.Left, PBGInfo^.R.Top, WidthOf(PBGInfo^.R), HeightOf(PBGInfo^.R),
                     SkinData.FCacheBmp.Canvas.Handle, PBGInfo^.Offset.X, PBGInfo^.Offset.Y, SRCCOPY);
...
begin
  Result := DefSkinIndex;
  if (Ctrl = nil) or (Ctrl.Parent = nil) then Exit;
  cd := GetObjProp(Ctrl.Parent, s_SkinData);
  if (cd <> nil) and (cd is TsCommonData) and (TsCommonData(cd).SkinIndex >= 0) and (TsCommonData(cd).SkinIndex < Length(SkinManager.gd)) then begin
    if (SkinManager.gd[TsCommonData(cd).SkinIndex].Props[min(integer(State > 0), SkinManager.gd[TsCommonData(cd).SkinIndex].States - 1)].Transparency > 50) and
       ((SkinManager.gd[TsCommonData(cd).SkinIndex].BorderIndex < 0) or (SkinManager.ma[SkinManager.gd[TsCommonData(cd).SkinIndex].BorderIndex].DrawMode and BDM_FILL <> BDM_FILL)) and

       (Ctrl.Parent.Parent <> nil)
      then Result := GetFontIndex(Ctrl.Parent.Parent, DefSkinIndex, SkinManager, 0)
      else if SkinManager.gd[TsCommonData(cd).SkinIndex].GiveOwnFont then Result := TsCommonData(cd).SkinIndex else Result := DefSkinIndex;
  end
  else begin
    si.SkinIndex := -1;
    SendMessage(Ctrl.Parent.Handle, SM_ALPHACMD, MakeWParam(0, AC_GETSKININDEX), integer(@si));
    if SkinManager.IsValidSkinIndex(si.SkinIndex) then begin
      if (SkinManager.gd[si.SkinIndex].Props[min(integer(State > 0), SkinManager.gd[si.SkinIndex].States - 1)].Transparency > 50) and (Ctrl.Parent <> nil)
        then Result := GetFontIndex(Ctrl.Parent, DefSkinIndex, SkinManager, 0)
...
begin
  Result := False;
  if ResizeDirection in [ssHorizontal, ssBoth] then begin
    Result := True;
  end
  else if ResizeDirection in [ssVertical] then begin
    Result := True;
  end;
end;
  ...
  if SkinData.SkinManager.Effects.AllowGlowing and SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) then begin
    ...
    if (SkinData.FOwnerControl <> nil) then begin
      if SkinData.FMouseAbove and not (csLButtonDown in SkinData.FOwnerControl.ControlState) and (not Clicked or (SkinData.GlowID = -1)) then begin
        ...
        if (SkinData.GlowID <> -1) then Exit;

        if (SkinData.FOwnerControl is TWinControl) then begin
          if not GetWindowRect(TWinControl(SkinData.FOwnerControl).Handle, R) then Exit;
...
begin
  with SkinData do if SkinSection = '' then case COC of
    ...
    COC_TsImage, COC_TsRadioButton : SkinSection := s_CheckBox;
  ...
  if SkinData.FOwnerControl <> nil then begin
    if (SkinData.FOwnerControl is TFrame) or (SkinData.FOwnerControl is TPanel) or (SkinData.FOwnerControl is TScrollingWinControl) or (SkinData.FOwnerControl is TPageControl) then begin
      ...
      if i <> ACS_BGUNDEF then begin
        if i and ACS_FAST = ACS_FAST then SkinData.CtrlSkinState := SkinData.CtrlSkinState or ACS_FAST else SkinData.CtrlSkinState := SkinData.CtrlSkinState and not ACS_FAST;
...
begin
  ...
  with SkinData do begin
    ...
    if FOwnerControl.Enabled and not (csDesigning in FOwnerControl.ComponentState) then begin
      if FFocused
        then Result := True
        else if (FOwnerControl is TWinControl) and TWinControl(SkinData.FOwnerControl).HandleAllocated and (TWinControl(FOwnerControl).Handle = GetFocus)
...
begin
  Dec(FUpdateCount);
//  If FUpdateCount > 0 then Exit; Uncomment in Beta
  FUpdating := False;
  CtrlSkinState := CtrlSkinState and not ACS_LOCKED;
  If FUpdateCount > 0 then Exit; // Comment in Beta
  if FOwnerControl <> nil then begin
...
procedure TsCommonData.Invalidate(UpdateNow : boolean = False);
var
  Flags : Cardinal;
begin
  if Assigned(FOwnerControl) then begin
    if not ((csDestroying in FOwnerControl.ComponentState) or (csLoading in FOwnerControl.ComponentState)) then begin
      ...
      if ControlIsReady(FOwnerControl) then begin
        if not (FOwnerControl is TWinControl)
          ...
          else if TWinControl(FOwnerControl).HandleAllocated then begin
            Flags := RDW_FRAME or RDW_ERASE or RDW_INVALIDATE or RDW_ALLCHILDREN;
            if UpdateNow then Flags := Flags or RDW_UPDATENOW;
...
begin
  ...
  try
    SM := SkinManager;
    if (FSkinSection = '') or (SM = nil) then Exit;
    if (csDestroying in SM.ComponentState) or not SM.IsValidSkinIndex(SkinIndex) then Exit;
...
begin
  ...
  if SkinData <> nil then with SkinData do if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_CLEARCACHE : if SkinData.FCacheBmp <> nil then begin
      if (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TWinControl) then with SkinData.FOwnerControl as TWinControl do begin
    ...
    AC_CHILDCHANGED : if (SkinData.SkinIndex > -1) then begin
      if (SkinData.SkinManager.gd[SkinData.SkinIndex].Transparency > 0{= 100}) and (FOwnerControl <> nil) and (FOwnerControl.Parent <> nil)
        then Message.LParam := integer(ParentTextured(SkinData))
        else if (SkinData.SkinManager.gd[SkinData.SkinIndex].GradientPercent + SkinData.SkinManager.gd[SkinData.SkinIndex].ImagePercent > 0) or SkinData.RepaintIfMoved then begin
            Message.LParam := 1;
          end
          else begin
      ...
      if SkinData.Skinned then begin
        case SkinData.SkinManager.gd[SkinData.Skinindex].Transparency of
          ...
          100 : begin if FOwnerControl.Parent <> nil
            then begin
              if GetBoolMsg(FOwnerControl.Parent, AC_CTRLHANDLED)
                then Message.Result := SendMessage(FOwnerControl.Parent.Handle, SM_ALPHACMD, MakeWParam(0, AC_GETCONTROLCOLOR), 0)
                else Message.Result := TsHackedControl(FOwnerControl.Parent).Color;
...
//              if Message.Result = clFuchsia {if AlphaMessage is not supported} then Message.Result := TsHackedControl(FOwnerControl.Parent).Color
            ...
            C1.R := ((C1.R - C2.R) * SkinData.SkinManager.gd[SkinData.Skinindex].Transparency + C2.R shl 8) shr 8;
            C1.G := ((C1.G - C2.G) * SkinData.SkinManager.gd[SkinData.Skinindex].Transparency + C2.G shl 8) shr 8;
            C1.B := ((C1.B - C2.B) * SkinData.SkinManager.gd[SkinData.Skinindex].Transparency + C2.B shl 8) shr 8;
            Message.Result := C1.C;
          end
        end;
      end
    ...
    AC_PREPARECACHE : SkinData.BGChanged := False;
    AC_ENDPARENTUPDATE : if SkinData.FUpdating and (SkinData.FOwnerControl <> nil) and (SkinData.FOwnerControl is TWinControl) then begin
      if not InUpdating(SkinData, True) then
        RedrawWindow(TWinControl(SkinData.FOwnerControl).Handle, nil, 0, RDW_ALLCHILDREN or RDW_INVALIDATE or RDW_ERASE or RDW_FRAME or RDW_UPDATENOW);
//        InvalidateRect(TWinControl(SkinData.FOwnerControl).Handle, nil, False);
    ...
    AC_GETSKINSTATE : begin
      if Message.LParam = 1 then begin
        ...
        if CtrlSkinState = ACS_BGUNDEF {If not initialized yet}
          then UpdateSkinState(SkinData, False);
...
{$ENDIF}
      ...
      CM_VISIBLECHANGED : if ac_OptimizeMemory and (SkinData.FOwnerControl <> nil) and not (csDestroying in SkinData.FOwnerControl.ComponentState) and Assigned(SkinData) and Assigned(SkinData.FCacheBmp) then begin
        if Message.WParam = 0 then begin
          SkinData.FCacheBmp.Width := 0;
          SkinData.FCacheBmp.Height := 0;
          if (SkinData.FOwnerControl is TWinControl) then with SkinData.FOwnerControl as TWinControl do begin
            for i := 0 to ControlCount - 1 do begin
              if (Controls[i] is TWinControl) and TWinControl(Controls[i]).HandleAllocated then begin
                SendAMessage(TWinControl(Controls[i]).Handle, AC_CLEARCACHE);
              end
              else Controls[i].Perform(SM_ALPHACMD, MakeWPAram(0, Word(AC_CLEARCACHE)), 0);
            end;
          end;
        end
        else begin
      ...
      WM_SETFOCUS: if Assigned(FOwnerControl) and (FOwnerControl is TWinControl) and TWinControl(FOwnerControl).CanFocus and TWinControl(FOwnerControl).TabStop then begin
        BGChanged := True;
        FFocused := True;
        if SkinData.Skinned and (COC in sEditCtrls) and (SkinData.SkinManager.gd[SkinData.SkinIndex].States > 1) then begin
          if (TsHackedControl(FOwnerControl).Color <> SkinData.SkinManager.gd[SkinData.SkinIndex].Props[1].Color) and not SkinData.CustomColor
            then TsHackedControl(FOwnerControl).Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[1].Color;
      ...
      WM_KILLFOCUS : if Assigned(FOwnerControl) and (FOwnerControl is TWinControl) and TWinControl(FOwnerControl).CanFocus and TWinControl(FOwnerControl).TabStop then begin
        BGChanged := True;
        FFocused := False;
        if (COC in sEditCtrls) and FOwnerControl.Visible then begin
          if (TsHackedControl(FOwnerControl).Color <> SkinData.SkinManager.gd[SkinData.SkinIndex].Props[0].Color) and not SkinData.CustomColor
            then TsHackedControl(FOwnerControl).Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[0].Color;
          InvalidateRect(TWinControl(FOwnerControl).Handle, nil, True);
        end;
      end;
      ...
      CM_MOUSEENTER : if not (csDesigning in FOwnerControl.ComponentState) and Skindata.Skinned then begin
        if (FOwnerControl <> nil) and (FOwnerControl is TWinControl) and (DefaultManager <> nil) then begin
          ...
          if (Skindata.COC in sEditCtrls) and (SkinData.SkinManager.gd[Skindata.SkinIndex].States > 1) then begin
            BGChanged := True;
            if (TsHackedControl(FOwnerControl).Color <> SkinData.SkinManager.gd[SkinData.SkinIndex].Props[1].Color) and not SkinData.CustomColor then begin
              TsHackedControl(FOwnerControl).Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[1].Color;
              InvalidateRect(TWinControl(FOwnerControl).Handle, nil, True);
            end;
      ...
      CM_MOUSELEAVE : if not (csDesigning in FOwnerControl.ComponentState) and Skindata.Skinned then begin
        ...
        if not (COC in sCanNotBeHot) and not (COC in [COC_TsButton, COC_TsBitBtn, COC_TsSpeedButton]) then begin
          if (SkinData.GlowID <> -1) then begin
            HideGlow(SkinData.GlowID);
            SkinData.GlowID := -1;
          end;
        end;
        if (Skindata.COC in sEditCtrls) and not FFocused and (TsHackedControl(FOwnerControl).Color <> SkinData.SkinManager.gd[SkinData.SkinIndex].Props[0].Color) and not SkinData.CustomColor then begin
          TsHackedControl(FOwnerControl).Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[0].Color;
          InvalidateRect(TWinControl(FOwnerControl).Handle, nil, True);
        end;
...
begin
  if FTheLabel <> nil then case FLayout of
    sclTopLeft, sclBottomLeft, sclLeftTop : TsLabel(FTheLabel).Alignment := taLeftJustify;
    sclTopCenter, sclBottomCenter : TsLabel(FTheLabel).Alignment := taCenter;
    sclTopRight, sclBottomRight, sclLeft : TsLabel(FTheLabel).Alignment := taRightJustify;
  end;
end;
