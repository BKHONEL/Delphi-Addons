...
uses SysUtils, Windows,  Messages, Classes, Graphics, Controls, Menus, Forms, StdCtrls, Mask, Buttons, sCustomComboEdit,
  acntUtils, {$IFNDEF ALITE} sCalcUnit,{$ENDIF} sConst;
...
begin
  ...
  S := TextToValText(S);
  DecPos := Pos({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator, S);
...
begin
  if Assigned(FPopupWindow) and IsWindowVisible(FPopupWindow.Handle) then begin
    ...
    if CharInSet(AnsiChar(Key), ['.', ','] - [{$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ThousandSeparator]) then Key := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator;
    inherited KeyPress(Key);
    if not (Key in [#3, #22]) then begin // Ctrl-C, Ctrl-V
      if (Key in [#43{'+'}, #45{'-'}, #47{'/'}, #42{'*'}, #37{'%'}]) then begin
        if (Key = #45) and (Value = 0) then Exit; 
        EditButtonClick(Self);
        if Assigned(FPopupWindow) and IsWindowVisible(FPopupWindow.Handle)
          then TsCalcForm(FPopupWindow).OnKeyPress(FPopupWindow, Key);
        Key := #0;
      end
      else if not (Key in [#8]) and not IsValidChar(Key) then begin
...
begin
  if DisplayFormat <> '' then begin
    ...
    while (Length(Result) > 0) and (Result[1] = ' ') do Delete(Result, 1, 1); // Del leading spaces
  end
...
begin
  ...
  FValue := CheckValue(StrToFloat(TextToValText('0' + s)) * Minus, False);
  Exit;
  Text := DelChars(Text, '?');
...
begin
  ...
  if {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator <> {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ThousandSeparator then Result := DelChars(Result, {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ThousandSeparator);
  if ({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator <> '.') and ({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ThousandSeparator <> '.') then Result := ReplaceStr(Result, '.', {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator);
  if ({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator <> ',') and ({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ThousandSeparator <> ',') then Result := ReplaceStr(Result, ',', {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator);
...
begin
  ...
  if SkinData.Skinned then begin
    TsCalcForm(FPopupWindow).FMainPanel.BorderStyle := bsNone;
    TsCalcForm(FPopupWindow).FDisplayPanel.BorderStyle := bsNone;
  end
  else begin
    TsCalcForm(FPopupWindow).FMainPanel.BorderStyle := bsSingle;
    TsCalcForm(FPopupWindow).FDisplayPanel.BorderStyle := bsSingle;
  end;
...
begin
  ...
  if Focused then al := taLeftJustify else al := Alignment;
...
{  case Message.Msg of
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    AC_SETNEWSKIN, AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
      if Assigned(FPopupWindow) then AlphaBroadCast(FPopupWindow, Message);
    end;
  end;
end;
