...
uses sStyleSimply, {$IFNDEF ALITE} sCurrencyEdit, {$ENDIF} {$IFDEF CHECKXP} UxTheme, Themes, {$ENDIF}
  sToolEdit, sMaskData, sVCLUtils, sMessages, sAlphaGraph, sSkinManager, math {$IFDEF LOGGED}, sDebugMsgs{$ENDIF};
  ...
  if (FPopupWindow <> nil) and not (ReadOnly or DroppedDown) then begin
    ...
    if Form <> nil then begin
      ...
      if Application.MainForm <> nil
        then SetWindowPos(FPopupWindow.Handle, HWND_TOPMOST, P.X, Y, FPopupWindow.Width, FPopupWindow.Height, Flags)
        else SetWindowPos(FPopupWindow.Handle, HWND_TOPMOST, P.X, Y, FPopupWindow.Width, FPopupWindow.Height, Flags);
    end;
...
begin
  ...
  Loc.Top := 0;
  if (BorderStyle <> bsNone) then Loc.Left := 2 * integer(not Ctl3d) else Loc.Left := 0;
...
begin
  if (Text <> '') then SendMessage(Handle, EM_SETSEL, 0, -1);
end;
...
begin
  if FAlignment <> Value then begin
    FAlignment := Value;
    if HandleAllocated and Visible then SkinData.Invalidate;
  end;
end;
{$ENDIF}
...
{$ENDIF}
  if Message.MSG = SM_ALPHACMD then case Message.WParamHi of
    ...
    WM_PRINT : if SkinData.Skinned then begin
      ...
      if ShowButton then FButton.PaintTo(DC, Point(FButton.Left + BtnOffset(Self), FButton.Top + BtnOffset(Self)));
      FButton.Visible := b;
      Exit;
    end;
    WM_KILLFOCUS : if (Self is TsDateEdit) and (FPopupWindow <> nil) then TForm(FPopupWindow).Close;
  end;
  inherited;
  if Message.MSG = SM_ALPHACMD then case Message.WParamHi of
  ...
  try
    if SkinData.Skinned then begin
      ...
      if not SkinData.Updating then begin
        ...
        if (not Focused{ControlIsActive(SkinData)} or (SkinData.PrintDC = NewDC)) and FButton.Visible
          then FButton.PaintTo(NewDC, Point(FButton.Left + BtnOffset(Self), FButton.Top + BtnOffset(Self)));
{$IFDEF DYNAMICCACHE}
...
begin
  ...
  if Text <> '' then begin
    if PasswordChar <> #0 then acFillString(aText, Length(aText), acChar(PasswordChar));
