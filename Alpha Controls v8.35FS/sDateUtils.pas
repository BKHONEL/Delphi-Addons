...
begin
  ...
  if not (ScanNumber(S, MaxInt, Pos, N1) and ScanChar(S, Pos, {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DateSeparator) and
    ScanNumber(S, MaxInt, Pos, N2)) then Exit;
  if ScanChar(S, Pos, {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DateSeparator) then begin
  ...
  ScanChar(S, Pos, {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DateSeparator);
  ScanBlanks(S, Pos);
...
begin
  if Length(S) > 0 then
    for Result := 1 to 12 do begin
      if (Length({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}LongMonthNames[Result]) > 0) and
        (AnsiCompareText(Copy(S, 1, MaxLen),
        Copy({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}LongMonthNames[Result], 1, MaxLen)) = 0) then Exit;
    end;
  Result := 0;
end;
...
initialization
  NormalYears := Pos('YYYY', AnsiUpperCase({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ShortDateFormat)) > 0;
