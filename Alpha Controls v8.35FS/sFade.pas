...
begin
  if CommonData.SkinManager.gd[CommonData.SkinIndex].FadingEnabled then begin
    ...
    if CommonData.FOwnerControl.Perform(SM_ALPHACMD, MakeWParam(0, AC_PREPARECACHE), 0) <> 0 then begin
      // BG not ready
      Timer.Enabled := False;
    end;
...
begin
  ...
  b := TBitmap.Create;
  Alpha := lo(MaxByte - (FadeLevel * MaxByte) div Iterations);
  b.Assign(BmpFrom);
  b.PixelFormat := pf32bit;
  b.HandleType := bmDIB;
  try
    SumBitmaps(b, FOwnerData.FCacheBmp, Alpha);
    ...
    try
      if FOwnerData.FOwnerControl.Perform(SM_ALPHACMD, MakeWParam(0, AC_DRAWANIMAGE), LPARAM(b)) = 0
...
begin
  try
    ...
    Alpha := lo(MaxByte - (FadeLevel * MaxByte) div Iterations);
    SumBitmaps(TmpBmp, FOwnerData.FCacheBmp, Alpha);
    Repaint;
    if Assigned(acMagnForm) then SendMessage(acMagnForm.Handle, SM_ALPHACMD, MakeWParam(0, AC_REFRESH), 0);
  finally
    inc(FadeLevel);
