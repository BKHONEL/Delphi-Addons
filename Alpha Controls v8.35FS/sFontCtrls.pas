...
uses
 Windows, Messages, Classes, Graphics, Controls, StdCtrls, sListBox, sComboBox {$IFNDEF DELPHI5}, types{$ENDIF};
...
begin
  case Message.Msg of
    WM_FONTCHANGE : GetFonts(Self);
    WM_SETTEXT : if not (csLoading in ComponentState) and not (csCreating in ControlState) and HandleAllocated and not SkinData.FUpdating {Control is not ready} then begin // Change a font when text is changed
