...
{$ENDIF}
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_REMOVESKIN : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
      ...
      CommonWndProc(Message, FCommonData);
      if (csDesigning in Frame.ComponentState) and // Updating of form color in design-time
           Assigned(Frame.Parent) and (Frame.Parent.ClassName = sWinControlForm) then TsHackedControl(Frame.Parent).Color := clBtnFace;
      AlphaBroadcast(Frame, Message);
...
//        Message.Result := integer((SkinData.CtrlSkinState and ACS_FAST <> ACS_FAST) and ({FCommonData.BGChanged or} FCommonData.FUpdating));
        if (Frame = nil) or (FCommonData.FCacheBmp = nil) then
          Message.Result := 0
        else
          Message.Result := integer(InUpdating(FCommonData) or ((FCommonData.FCacheBmp.Width <> Frame.Width) or (FCommonData.FCacheBmp.Height <> Frame.Height)) and IsCached(FCommonData) and not SkinData.CustomColor);
        Exit
      end;
      AC_UPDATING : begin
...
{      AC_GETBG : if (SkinData.SkinManager <> nil) and SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) then begin
        if SkinData.BGChanged and not SkinData.FUpdating and IsCached(SkinData) then PrepareCache;
        if SkinData.FUpdating
          then PacBGInfo(Message.LParam).BgType := btNotReady
          else InitBGInfo(SkinData, PacBGInfo(Message.LParam), 0);
        Exit;
      end;          }
      AC_ENDPARENTUPDATE : begin
        if FCommonData.FUpdating {or not IsCached(FCommonData) }{$IFDEF D2006} and not (csRecreating in Frame.ControlState) and not (csAligning in Frame.ControlState) {$ENDIF} then begin
...
//            RedrawWindow(Frame.Handle, nil, 0, RDW_ALLCHILDREN or RDW_INVALIDATE or RDW_UPDATENOW or RDW_ERASE)
        ...
        if SkinData.Skinned then begin
          if not IsCached(SkinData) then begin
            case SkinData.SkinManager.gd[SkinData.Skinindex].Transparency of
              0 : Message.Result := LRESULT(SkinData.SkinManager.gd[SkinData.Skinindex].Color);
              100 : begin if Frame.Parent <> nil
                then begin
                  ...
                  if Message.Result = clFuchsia {if AlphaMessage not supported} then Message.Result := LRESULT(TsHackedControl(Frame.Parent).Color)
                end
            ...
            if SkinData.CtrlSkinState and ACS_BGUNDEF = ACS_BGUNDEF then UpdateSkinState(SkinData, False);
            Message.LParam := LRESULT(clFuchsia);
          end;
        end
        else if Assigned(Frame) then Message.LParam := LPARAM(ColorToRGB(TsHackedControl(Frame).Color));
      end
      else begin
        CommonWndProc(Message, FCommonData);
        Exit;
      ...
      WM_WINDOWPOSCHANGED, WM_SIZE, WM_MOVE : begin
        ...
        if Message.Msg <> WM_MOVE then InvalidateRect(Frame.Handle, nil, True);
        OldWndProc(Message);
      end;
...
{        if (Message.WParamLo = WM_CREATE) and Assigned(Frame) and Frame.HandleAllocated then begin
      ...
      WM_PRINT : if FCommonData.Skinned then begin
        ...
        if ControlIsReady(Frame) then begin
          ...
          Ac_NCDraw(ListSW, Frame.Handle, Message.wParam, Message.lParam, -1, TWMPaint(Message).DC); // Scrolls painting
        end;
      end
      else OldWndProc(Message);
...
begin
  if (aDC <> 0) and (not InAnimationProcess or (aDC = SkinData.PrintDC)) then begin
    ...
    if IsCached(FCommonData) then begin
      SavedDC := SaveDC(DC);
      if not InUpdating(FCommonData) then begin
        if (FCommonData.BorderIndex < 0) and (FCommonData.SkinManager.gd[FCommonData.SkinIndex].Props[0].Transparency = 100) then begin // Parent BG is taken
          BGInfo.DrawDC := 0;
          ...
          if (BGInfo.BgType = btCache) and (BGInfo.Bmp <> nil) then begin
            ...
            FillBG(Rect(0, 0, Frame.Width, Frame.Height));
            RestoreDC(DC, SavedDC);
          end
        end
        else begin
          ...
          if Changed and not FCommonData.UrgentPainting then if not PrepareCache then begin
            RestoreDC(DC, SavedDC);
            Exit;
          end;
...
begin
  ...
  if IsCached(SkinData) then begin
    if Frame.Parent <> nil
      then GetBGInfo(@BGInfo, Frame.Parent.Handle, False)
      else BGInfo.BgType := btNotReady;
    ...
    PaintItem(SkinData, BGInfoToCI(@BGInfo), False, 0, Rect(0, 0, Frame.Width, Frame.Height), Point(Frame.Left, Frame.Top), SkinData.FCacheBMP, False);
  end;
