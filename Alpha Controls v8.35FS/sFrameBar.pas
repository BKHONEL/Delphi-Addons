...
{$ENDIF} // NOTFORHELP
    ...
    property ActiveFrameIndex : integer read FActiveFrame write SetActiveFrame;
  end;
...
begin
  if not visible or Arranging or (csReading in ComponentState) or (Items.Count = 0) then begin
    FActiveFrame := -1;
    Exit;
  end;
      ...
      case Items[i].State of
        ...
        stClosing : begin
          ...
          if (ii = Steps) then begin
            ...
            if Assigned(Items[i].FOnFrameDestroy) then Items[i].FOnFrameDestroy(Items[i], Items[i].Frame, CanDestroy);
...
//          inc(sHeight, FSpacing);
        ...
        stClosed : begin
          if FActiveFrame = i then FActiveFrame := -1;
      ...
      if (Items[i].Frame <> nil) and (Items[i].State in [stOpened, stOpening, stClosing]) then begin
        if Items[i].Frame.Parent = nil then begin
  ...
  Arranging := False;
  UpdateWidths;
  if Showing then RedrawWindow(Handle, nil, 0, RDW_ALLCHILDREN or RDW_INVALIDATE or RDW_ERASE or RDW_FRAME);
  if Assigned(acMagnForm) then SendMessage(acMagnForm.Handle, SM_ALPHACMD, MakeWParam(0, AC_REFRESH), 0);
...
begin
  if SkinData.Skinned then begin
    ...
    sbi.fMask := SIF_POS;
    if GetScrollInfo(Handle, SB_VERT, sbi) then Result := sbi.nPos else Result := 0;
  end;
end;
...
var
  i : integer;
begin
  if (Index = -1) or (Index >= Items.Count) then raise Exception.Create('Open Index Out of Range: ' + IntToStr(Index));
  FActiveFrame := Index;
  if AllowAnimation then Items[Index].State := stOpening else Items[Index].State := stOpened;
...
procedure TsFrameBar.SetActiveFrame(const Value: integer);
begin
  if (Value >= 0) and (Value < Items.Count) then OpenItem(Value, True);
  FActiveFrame := Value;
end;
...
{      bCanChanging := False;
    ...
    if (Items[i].Frame <> nil) then begin
      if bCanChange and Assigned(FOnChange) then FOnChange(Self, Items[i]);
      bCanChange := False;
    end;
  end;
  if (Items[i].Frame <> nil) then begin
    ...
    if h = 0 then begin
      ...
      if Assigned(Items[i].FOnFrameShow) then Items[i].FOnFrameShow(Items[i]);
    end
    else begin
...
begin
  ...
  for i := 0 to Items.Count - 1 do if (Items[i].TitleButton <> nil) and Items[i].TitleButton.Visible and Items[i].Visible then begin
...
//        SendMessage(Handle, WM_NCPAINT, 0, 0);
  ...
  if Message.Msg = cardinal(SM_ALPHACMD) then case Message.WParamHi of
    AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) and SkinData.Skinned then UpdateWidths
  end;
end;
...
begin
  ...
  if Assigned(FOnTitleItemDestroy) then FOnTitleItemDestroy(self);
  inherited Destroy;
...
begin
  ...
  case State of
    ...
    stOpened : begin
      ...
      if FOwner.FOwner.AllowAllClose then FOwner[Index].State := stClosing;
    end;
  end;
  FOwner.FOwner.ArrangeTitles;
end;
