...
uses Consts, sStyleSimply, sMaskData, sBorders, sSkinProps, sAlphaGraph,
  sMessages, sSKinManager, acPNG, math{$IFDEF LOGGED}, sDebugMsgs{$ENDIF};
...
begin
  ...
  if Assigned(FOnPaintText) then FOnPaintText(Self, S);
...
{$ENDIF}
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_SETNEWSKIN : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin CommonWndProc(Message, FCommonData); exit end;
...
begin
  if FAnimated <> Value then begin
    ...
    if not (csDesigning in ComponentState) then begin
      if (FTmpProgressBmp <> nil) then FreeAndNil(FTmpProgressBmp);
      Timer.Enabled := Value;
      if not Value then SkinData.Invalidate;
    end;
  end;
end;
