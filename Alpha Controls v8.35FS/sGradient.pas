        ...
        if MaxY - CurrentY > 0 then begin
          ...
          if Color1.C = Color2.C then begin
            ...
            for Y := CurrentY to MaxY do begin
              SSrc := Bmp.ScanLine[Y];
{$IFDEF WIN64}
              for X := aRect.Left to aRect.Left + dX - 1 do SSrc[X] := CurrentColor;
{$ELSE}
...
//          w := min(WidthOf(aRect) + aRect.Left, bmp.Width) - 1;
    ...
    1 : begin
      ...
      if CurrentY < aRect.Bottom
        then BitBlt(Bmp.Canvas.Handle, aRect.Left, CurrentY, w, aRect.Bottom - CurrentY, Bmp.Canvas.Handle, aRect.Left, aRect.Top, SRCCOPY);
    end;
  end;
end;
