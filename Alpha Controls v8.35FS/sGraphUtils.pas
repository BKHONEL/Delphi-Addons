...
uses
  ...
  StdCtrls, ComCtrls, sConst{$IFNDEF DELPHI5}, Types{$ENDIF}, ExtCtrls, acTinyJpg, acntUtils, math, {$IFDEF DELPHI_XE3}UITypes, {$ENDIF}
  Buttons{$IFDEF LOGGED}, sDebugMsgs{$ENDIF} {$IFNDEF ACHINTS}, imglist, sMaskData, sCommonData{$ENDIF};
...
//procedure DrawColorArrow(Bmp : TBitmap; Color : TColor; R : TRect; Direction : integer); overload;
procedure DrawColorArrow(Canvas : TCanvas; Color : TColor; R : TRect; Direction : integer); overload;
...
function CreateAlphaBmp(SrcMaskedBmp : TBitmap; SrcRect : TRect) : TBitmap;
...
begin
  DrawColorArrow(Bmp.Canvas, Color, R, Direction);
{  i := 0;
  case Direction of
    BF_BOTTOM : begin
...
begin
  Result.A := 0;
  if S = 0 then begin Result.R := lo(Round(V * MaxByte)); Result.G := Result.R; Result.B := Result.R end else begin
...
begin
  ...
  if (Control is TWinControl) and (TWinControl(Control)).HandleAllocated
    then SendMessage(TWinControl(Control).Handle, SM_ALPHACMD, MakeWParam(0, AC_GETBG), LPARAM(BGInfo))
    else Control.Perform(SM_ALPHACMD, MakeWParam(0, AC_GETBG), LPARAM(BGInfo));
    ...
    if CustomColor = clFuchsia then begin
      ...
      if Transparency <> 100 then begin
        ...
        if (ImagePercent > 0) then begin
          if TextureIndex < 0 then TextureIndex := gd[SkinIndex].Props[State].TextureIndex;
        ...
        if GradientPercent <> 0 then begin GradientData := gd[SkinIndex].Props[State].GradientData; GradientArray := gd[SkinIndex].Props[State].GradientArray end;
      end;
    end
    else begin
      Color := CustomColor;
    ...
    if ci.Ready or (ci.FillColor <> clFuchsia) then case Transparency of
      100 : begin
        ...
        if ci.Ready and (ci.Bmp <> nil) then begin
          if (ItemBmp <> ci.Bmp) then BitBlt(ItemBmp.Canvas.Handle, aRect.Left, aRect.Top, w, h, ci.Bmp.Canvas.Handle, ci.X + pP.X, ci.Y + pP.Y, SRCCOPY)
        end
        else begin
          bRect := Rect(0, 0, w, h);
          ...
          if ci.Bmp <> nil then begin
            ...
            if (ci.FillRect.Right > 0) and (ci.Bmp.Width - ci.FillRect.Right < bRect.Right) then begin
            ...
            if (ci.FillRect.Bottom > 0) and (ci.Bmp.Height - ci.FillRect.Bottom < bRect.Bottom) then begin
      ...
      0 : PaintAddons(ItemBmp);
        ...
        if ci.Ready or (GradientPercent + ImagePercent <> 0) then begin
          ...
          try
            ...
            SumBmpRect(ItemBmp, TempBmp, lo(Round(Transparency * 2.55 {255 / 100})), Rect(0, 0, WidthOf(aRect, True), HeightOf(aRect, True)), Point(aRect.Left, aRect.Top));
          finally
            FreeAndNil(TempBmp);
          end
        end
    ...
    if (GradientPercent > 0) then begin
      if iDrawed then begin
        ...
        try
          ...
          SumBmpRect(aBmp, Bmp, ImagePercent * integer(MaxByte) div 100, Rect(0, 0, Bmp.Width, Bmp.Height), Point(aRect.Left, aRect.Top));
        finally
          FreeAndNil(Bmp);
        end;
      end
  ...
  with TsSkinManager(SkinManager) do begin
    ...
    if ci.Ready and (Transparency = 100) then begin
      ...
      try
        ...
        SumBmpRect(ItemBmp, TempBmp, lo(Transparency * integer(MaxByte) div 100), Rect(0, 0, WidthOf(aRect, True), HeightOf(aRect, True)), Point(aRect.Left, aRect.Top));
      finally
        FreeAndNil(TempBmp);
      end;
    end;
...
begin
  if (SkinManager = nil) then SkinManager := DefaultManager;
  if not Assigned(SkinManager) or not TsSkinManager(SkinManager).IsValidSkinIndex(SkinIndex) or (R.Left < 0) or (R.Top < 0) or (R.Right - R.Left < 1) or (R.Bottom - R.Top < 1) then Exit;
  SavedDC := SaveDC(DC);
...
begin
  with TsSkinManager(Manager) do begin
    ...
    if IsValidSkinIndex(Result) then begin
      ...
      PaintItem(Result, Section, CI, True, State, Rect(0, 0, Bmp.Width, Bmp.Height), Point(0, 0), Bmp, Manager);
    end;
  end;
end;
...
var
  Dst, Src : PByteArray;
  X, Y : integer;
  X1, Y1, hSrc, wSrc : integer;
begin
  hSrc := HeightOf(SrcRect, True) - 1;
  if hSrc + SrcRect.Top >= SrcBmp.Height then hSrc := SrcBmp.Height - SrcRect.Top - 1;
  if hSrc + DstRect.Top >= DstBmp.Height then hSrc := DstBmp.Height - DstRect.Top - 1;
  if SrcRect.Top < 0 then Y1 := -SrcRect.Top else Y1 := 0;
  if DstRect.Top < 0 then Y1 := min(Y1, -SrcRect.Top);

  wSrc := WidthOf(SrcRect, True) - 1;
  if wSrc + SrcRect.Left >= SrcBmp.Width then wSrc := SrcBmp.Width - SrcRect.Left - 1;
  if wSrc + DstRect.Left >= DstBmp.Width then wSrc := DstBmp.Width - DstRect.Left - 1;
  if SrcRect.Left < 0 then X1 := -DstRect.Left else X1 := 0;
  if DstRect.Left < 0 then X1 := max(X1, -DstRect.Left);

  for Y := Y1 to hSrc + Y1 do begin
    Dst := DstBmp.ScanLine[Y + DstRect.Top];
    Src := SrcBmp.ScanLine[Y + SrcRect.Top];
    for X := X1 to wSrc + X1 do Dst[(X + DstRect.Left) * 4 + Channel] := Src[(X + SrcRect.Left) * 4 + Channel];
  end;
end;
...
//Exit;
  ...
  // Left border
  if (h <> 0) then case DrawModes.Left of
    0 : begin
      X := DstRect.Left;
  ...
  // Top border
  if (w <> 0) then case DrawModes.Top of
    0 : begin
  ...
  // Right border
  if (h <> 0) then case DrawModes.Right of
    0 : begin
      X := DstRect.Right - dr;
      Y := DstRect.Top + BorderWidths.Top;
      while Y < DstRect.Bottom - BorderWidths.Bottom - h do begin
  ...
  // Bottom border
  if (w <> 0) then case DrawModes.Bottom of
    0 : begin
      X := DstRect.Left + BorderWidths.Left;
      Y := DstRect.Bottom - db;
      while X < DstRect.Right - BorderWidths.Right - w do begin
    ...
    1 : begin
      ...
      if not Enabled then GlyphIndex := 0 else begin
        case State of
          ...
          3 : if NumGlyphs > 2 then GlyphIndex := 3 else GlyphIndex := 0;
        end;
      end;
...
begin
  if (Text <> '') then begin
    ...
    if not Assigned(SkinManager) or not TsSkinManager(SkinManager).IsValidSkinIndex(SkinIndex) then begin
...
begin
  if Text <> '' then begin
    ...
    if Hot and (SkinData.SkinIndex >= 0) and (SkinData.SkinIndex = SM.ConstData.IndexWebBtn) then Canvas.Font.Style := Canvas.Font.Style + [fsUnderline];
    ...
    try
      ...
      if SkinData.Skinned and not SkinData.CustomFont then begin
        ...
        if (SkinData.FOwnerControl <> nil) and ((SkinData.COC in [COC_TsGroupBox, COC_TsCheckBox, COC_TsRadioButton]) or (SkinData.FOwnerControl is TGraphicControl)) and
          ...
          if SM.gd[SkinData.SkinIndex].Props[State].Transparency > 0
            then SkinIndex := GetFontIndex(SkinData.FOwnerControl, SkinData.SkinIndex, SM)
        ...
        if SM.IsValidSkinIndex(SkinIndex) then begin
          if SkinData.FCacheBmp <> nil then begin
            ...
            if i <> 0 then begin
              FGlowMask := nil;
              C := SM.gd[SkinIndex].Props[State].GlowColor;
          ...
          if Enabled then begin
            ...
            if not SkinData.CustomFont then begin
              if SM.gd[SkinIndex].Props[State].FontColor.Left <> -1 then begin
                C := ChangeSaturation(ChangeHUE(SkinData.HUEOffset, SM.gd[SkinIndex].Props[State].FontColor.Left), SkinData.Saturation);
                Rd := Rect(R.Left - 1, R.Top, R.Right - 1, R.Bottom);
              ...
              if SM.gd[SkinIndex].Props[State].FontColor.Top <> -1 then begin
                C := ChangeSaturation(ChangeHUE(SkinData.HUEOffset, SM.gd[SkinIndex].Props[State].FontColor.Top), SkinData.Saturation);
                Rd := Rect(R.Left, R.Top - 1, R.Right, R.Bottom - 1);
                SetTextColor(Canvas.Handle, C);
                DrawText(Canvas.Handle, Text, nLength, Rd, Flags);
              end;
              ...
              if SM.gd[SkinIndex].Props[State].FontColor.Right <> -1 then begin
                C := ColortoRGB(ChangeSaturation(ChangeHUE(SkinData.HUEOffset, SM.gd[SkinIndex].Props[State].FontColor.Right), SkinData.Saturation));
                Rd := Rect(R.Left + 1, R.Top, R.Right + 1, R.Bottom);
                SetTextColor(Canvas.Handle, C);
                DrawText(Canvas.Handle, Text, nLength, Rd, Flags);
              end;
              ...
              if SM.gd[SkinIndex].Props[State].FontColor.Bottom <> -1 then begin
                C := ChangeSaturation(ChangeHUE(SkinData.HUEOffset, SM.gd[SkinIndex].Props[State].FontColor.Bottom), SkinData.Saturation);
                Rd := Rect(R.Left, R.Top + 1, R.Right, R.Bottom + 1);
                SetTextColor(Canvas.Handle, C);
                DrawText(Canvas.Handle, Text, nLength, Rd, Flags);
              end;
              // Center
              if not SkinData.CustomFont then begin
                C := SM.gd[SkinIndex].Props[State].FontColor.Color;
                if SkinData.HUEOffset <> 0 then C := ChangeHUE(SkinData.HUEOffset, C);
                if SkinData.Saturation <> 0 then C := ChangeSaturation(C, SkinData.Saturation);
                SetTextColor(Canvas.Handle, C);
              end;
            ...
            Canvas.Font.Color := ChangeSaturation(ChangeHUE(SkinData.HUEOffset, MixColors(SM.gd[SkinIndex].FontColor[1], SM.GetGlobalColor, DefDisabledBlend)), SkinData.Saturation);
            DrawText(Canvas.Handle, Text, nLength, R, Flags);
          end
        end
        else DrawText(Canvas.Handle, Text, nLength, R, Flags);
...
begin
  if Text <> '' then begin
    nLength := Length(Text);
    R := aRect;
    if Hot and (SkinData.SkinIndex >= 0) and (SkinData.SkinIndex = SkinData.SkinManager.ConstData.IndexWebBtn) then Canvas.Font.Style := Canvas.Font.Style + [fsUnderline];
    SavedDC := SaveDC(Canvas.Handle);
    try
      IntersectClipRect(Canvas.Handle, aRect.Left, aRect.Top, aRect.Right, aRect.Bottom);
      if SkinData.Skinned then begin
        ...
        if Enabled then Tnt_DrawTextW(Canvas.Handle, Text, nLength, R, Flags) else begin
          OffsetRect(R, 1, 1);
          Canvas.Font.Color := clBtnHighlight;
          Tnt_DrawTextW(Canvas.Handle, Text, nLength, R, Flags);
          OffsetRect(R, -1, -1);
...
var
  ...
  i : integer;
  c : TsColor;
  SavedBmp, SavedSrc, SavedDst: hdc;
begin
...
begin
  ...
  for Y := 0 to SrcBmp.Height - 1 do begin
    ...
    for X := 0 to SrcBmp.Width - 1 do begin
      S1[X].R := ((S1[X].R - S2[X].R) * AlphaValue + S2[X].R shl 8) shr 8;
      S1[X].G := ((S1[X].G - S2[X].G) * AlphaValue + S2[X].G shl 8) shr 8;
      S1[X].B := ((S1[X].B - S2[X].B) * AlphaValue + S2[X].B shl 8) shr 8;
    end
  end;
end;
...
begin
  ...
  if SrcRect.Bottom >= SrcBmp.Height then SrcRect.Bottom := SrcBmp.Height - 1;
  if DstPoint.x + WidthOf(SrcRect) >= DstBmp.Width then begin
    SrcRect.Right := SrcRect.Left + (DstBmp.Width - DstPoint.x) - 1;
  end;
  if DstPoint.y + HeightOf(SrcRect) >= DstBmp.Height then begin
    SrcRect.Bottom := SrcRect.Top + (DstBmp.Height - DstPoint.y) - 1;
  end;

  Yd := DstPoint.Y;
  for Y := SrcRect.Top to SrcRect.Bottom {-1} do begin
    S1 := DstBmp.ScanLine[Yd];
    S2 := SrcBmp.ScanLine[Y];
    Xd := DstPoint.X;
    for X := SrcRect.Left to SrcRect.Right - 1 do begin
      S1[Xd].R := ((S1[Xd].R - S2[X].R) * AlphaValue + S2[X].R shl 8) shr 8;
      S1[Xd].G := ((S1[Xd].G - S2[X].G) * AlphaValue + S2[X].G shl 8) shr 8;
      S1[Xd].B := ((S1[Xd].B - S2[X].B) * AlphaValue + S2[X].B shl 8) shr 8;
      inc(Xd);
    end;
    inc(Yd);
  end;
    ...
    if Reflected then begin
      ...
      for Y := 1 to h do begin
        ...
        for X := 0 to w do begin
          ...
          if Col_.C <> clFuchsia then begin
            if Blend = 0
              then MaskValue := Col_.A * (h - Y) div h * Col.R shr 10
              else MaskValue := Col_.A * (h - Y) div h * Col.R shr 10 * (100 - Blend) div 100;
            gMaskValue := (col_.R + col_.G + col_.B) div 3;
            S1[dX1].R := (((min(round(gMaskValue * dR), MaxByte) - S1[dX1].R) * MaskValue + S1[dX1].R shl 8) shr 8) and MaxByte;
            S1[dX1].G := (((min(round(gMaskValue * dG), MaxByte) - S1[dX1].G) * MaskValue + S1[dX1].G shl 8) shr 8) and MaxByte;
            S1[dX1].B := (((min(round(gMaskValue * dB), MaxByte) - S1[dX1].B) * MaskValue + S1[dX1].B shl 8) shr 8) and MaxByte;
      ...
      if not CI.Ready then begin
        for Y := 0 to h do begin
          ...
          for X := 0 to w do begin
            ...
            S1[dX1].A := max(S1[dX1].A, S3[dX2].A);
            inc(dX1);
            inc(dX2);
          end;
        end;
      ...
      for Y := 0 to h do begin
        ...
        for X := 0 to w do begin
          ...
          if cur.C <> clFuchsia then begin
            ...
            C := GetAPixel(CI.Bmp, k2 + X, ci.Y + R1.Top + Y);
            S1[dX1].R := C.B;
            S1[dX1].G := C.G;
            S1[dX1].B := C.R;
          end;
          ...
          for sX := 0 to w do begin
            if (DstX <= dw) and (DstX >= 0) then begin
              ...
              if Cur.C <> TransColor then Dst[DstX] := Cur else begin
                ...
                col := GetAPixel(CI.Bmp, ci.X + DstX, ci.Y + DstY);
                Dst[DstX].R := col.B;
                Dst[DstX].G := col.G;
                Dst[DstX].B := col.R;
                Dst[DstX].A := col.A;//MaxByte;
              end;
            end;
            inc(DstX);
            inc(SrcX);
...
begin
  ...
  if Bmp.PixelFormat = pf32bit then begin
    ...
    if CW
      then for Y := 0 to h do for X := 0 to w do begin
        C := GetAPixel(Bmp, X, Y);
        SetAPixel(Result, h - Y, X, C);
      end
      else for Y := 0 to h do for X := 0 to w do begin
...
begin
  ...
  for Y := 0 to h - 1 do begin
    S := SrcMaskedBmp.ScanLine[SrcRect.Top + Y + h];
    D := Result.ScanLine[Y];
    for X := 0 to w - 1 do D[X].A := MaxByte - S[SrcRect.Left + X].R;
  end;
end;
...
{$IFDEF NOSLOWDETAILS} Exit;{$ENDIF}
  ...
  else if Graphic is TacTinyJPGImage then begin
    SavedDC := SaveDC(Canvas.Handle);
    IntersectClipRect(Canvas.Handle, aRect.Left, aRect.Top, aRect.Right, aRect.Bottom);
...
{$R-}
  ...
  for A := 0 to Size - 1 do begin
    ...
    Contrib[A].Data := Data;
    Data := PContributorList(DWord(Data) + CSize);
  end;
{$R+}
{$T+}
end;
...
{$R-}
  ...
  for X := 0 to Count - 1 do begin
    ...
    if DWORD(Dest) + dstDelta < MAXDWORD then Dest := PRGBTriple(DWORD(Dest) + dstDelta)
  end;
{$R+}
end;
...
{$R-}
  ...
  for X := 0 to Count - 1 do begin
    ...
    if DWORD(Dest) + dstDelta < MAXDWORD then Dest := PRGBQUAD(DWORD(Dest) + dstDelta)
  end;
{$R+}
end;
    ...
    try
      ...
      try
        for A := 0 to Width - 1 do begin
          ...
          SourceLine := Pointer(DWORD(SourceLine) + BytePerPixel);
          DestLine := Pointer(DWORD(DestLine) + BytePerPixel);
        end;
      finally
        FreeContributors(Contrib);
      end;
