...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms{$IFDEF DELPHI7UP}, Types{$ENDIF},
  sGraphUtils, sConst, acntUtils, {$IFDEF TNTUNICODE} TntForms, {$ENDIF} ExtCtrls, IniFiles, sHtmlParse;
    ...
    if (HintKind.GradientPercent > 0) then begin
      if iDrawed then begin
        ...
        try
          ...
          SumBmpRect(aBmp, Bmp, lo(HintKind.TexturePercent shl 8 div 100), Rect(0, 0, Bmp.Width - 1, Bmp.Height{ - 1}), Point(aRect.Left, aRect.Top));
        finally
          FreeAndNil(Bmp);
        end;
      end
...
begin
  ...
  if ci.Ready and (Transparency = 100) then begin
    ...
    try
      ...
      SumBmpRect(BGBmp, TempBmp, lo(Transparency shl 8 div 100), Rect(0, 0, WidthOf(aRect), HeightOf(aRect)), Point(aRect.Left, aRect.Top));
...
begin
  ...
  If Assigned(FTempHint) Then With FTempHint Do Begin
    ...
    if Assigned(OnShowHint) then begin
      ...
      OldText := HintText;
      OnShowHint(HintText, b, HintInfo, F);
      if HintText <> OldText then HintRect := CalcHintRect(HintKind.MaxWidth, HintText, NIL);
...
{$ENDIF}
  ...
  begin
    ...
    if (DefaultManager <> nil) and DefaultManager.IsValidSkinIndex(SkinIndex) and (DefaultManager.gd[SkinIndex].Transparency = 100) and (BorderIndex > -1) then begin // Used BorderMask
...
{$ENDIF}
    ...
    else acDrawText(Manager.FCacheBmp.Canvas.Handle, PacChar(HintText), Result,
...
begin
    ...
    if (DefaultManager <> nil) and DefaultManager.Active then begin // If frame is skinned and WM_PRINT is supported
      ...
      for i := 0 to HintFrame.ControlCount - 1 do begin
        if HintFrame.Controls[i] is TWinControl then begin
          ...
          HintFrame.Controls[i].Perform(WM_PAINT, WPARAM(TempBmp.Canvas.Handle), 0);
          MoveWindowOrg(TempBmp.Canvas.Handle, -HintFrame.Controls[i].Left, -HintFrame.Controls[i].Top);
        end;
      end;
      TempBmp.Canvas.UnLock;
  ...
  {$IFNDEF ACHINTS}
      if Manager.Skinned then begin
        Flags := DT_EXPANDTABS or DT_WORDBREAK or DT_CENTER or DrawTextBiDiModeFlagsReadingOnly;
...
{$ENDIF}
  ...
  {$ENDIF}
{$IFDEF TNTUNICODE}
        if TntApplication <> nil then acDrawText(BMP.Canvas.Handle, PacChar(TntApplication.Hint), R, DT_CENTER or DT_NOPREFIX or DT_WORDBREAK or DrawTextBiDiModeFlagsReadingOnly);
{$ELSE}
...
{$IFNDEF ACHINTS}
  ...
  h := Result.Height - 1;
  w := Result.Width - 1;
  for y := 0 to h do for x := 0 to w do if GetAPixel(Result, x, y).I = CI.FillColor then SetAPixel(Result, x, y, White) else SetAPixel(Result, x, y, Black);
{$ENDIF}
end;
