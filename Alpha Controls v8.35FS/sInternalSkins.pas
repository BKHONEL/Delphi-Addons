...
uses
  ...
  ExtCtrls, sPanel, sButton, sListBox, sLabel;
...
begin
  for i := 0 to SkinManager.InternalSkins.Count - 1 do begin
    ...
    p := Pos(' ' + acs_InternalSkin, sFileName);
    if p > 0 then Delete(sFileName, p, Length(' ' + acs_InternalSkin));
    sFileName := NormalDir(NewDir) + sFileName + '.' + acSkinExt;
  ...
  if sn <> ''
    then ShowWarning('These skins are not updated:'#13#10 + sn)
    else MessageDlg('All internal skins were updated completely!', mtInformation, [mbOk], 0);
end;
