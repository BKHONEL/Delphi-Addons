...
{  case Message.Msg of // Is not ready for Aero
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_ENDPARENTUPDATE : if not SkinData.FUpdating and FCommonData.Skinned then begin
      SkinData.PrintDC := 0;
...
begin
  ...
  if (Stage in [cdPrePaint, cdPostPaint]) then begin
      ...
      if (StateImages <> nil) then begin
        if (Item.StateIndex >= 0) and (Item.StateIndex < StateImages.Count) then begin
          ...
          if Bmp <> nil then begin
            if not bSelected then FillDC(Bmp.Canvas.Handle, fText, Canvas.Brush.Color);
            StateImages.Draw(Bmp.Canvas, 2, 0, Item.StateIndex, True);
          end
          else begin
            Bmp := CreateBmp32(cw, fText.Bottom - fText.Top);
            FillDC(Bmp.Canvas.Handle, Rect(0, 0, Bmp.Width, Bmp.Height), Canvas.Brush.Color);
    ...
    {$ENDIF}
      ...
      cw := Columns[0].Width;
      for sNdx := 1 to Columns.Count - 1 do if Columns[sNdx].Width <> 0 then begin
...
{        aRect.Bottom := min(aRect.Bottom, iRect.Bottom);
      ...
      if (Item.ImageIndex > -1) then begin
        ...
        if (ImgList <> nil) and (Item.ImageIndex < ImgList.Count) then begin
          if ListView_GetItemRect(Handle, Item.Index, aRect, LVIR_ICON) and not IsRectEmpty(aRecT) then begin
            ...
            if not bSelected then FillDC(TAccessCanvas(Canvas).FHandle, aRect, Canvas.Brush.Color);
{$IFDEF DELPHI6UP}
