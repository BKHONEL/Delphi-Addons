  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned then inherited else begin
    case Message.Msg of
      WM_PAINT : begin
        ...
        if not SkinData.CustomColor and (SkinData.SkinIndex >= 0) then begin
          State := integer((SkinData.FFocused or SkinData.FMouseAbove) and (SkinData.SkinManager.gd[SkinData.SkinIndex].States > 1));
          if (Color <> SkinData.SkinManager.gd[SkinData.SkinIndex].Props[State].Color) then Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[State].Color;
          if (Font.Color <> SkinData.SkinManager.gd[SkinData.SkinIndex].Props[State].FontColor.Color) then Font.Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[State].FontColor.Color;
        end;
