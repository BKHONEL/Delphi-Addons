...
uses Windows, Classes, Controls, SysUtils, Graphics, buttons, grids, {$IFNDEF DELPHI5}types,{$ENDIF}
  messages, acntUtils, sPanel, sGraphUtils, sConst, StdCtrls, forms, comctrls{$IFDEF DELPHI_XE3}, UItypes{$ENDIF},
...
uses Consts, sToolEdit, ExtCtrls, sStyleSimply, sSkinProps, sSkinManager, sPopupClndr, sVclUtils, math{$IFDEF DELPHI6UP}, DateUtils{$ENDIF}
...
begin
  if ARow = 0 then begin
    if ShowWeeks and (ACol = 0) then Result := s_WeeksTitle else Result := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}ShortDayNames[(FirstDay + ACol - Integer(ShowWeeks) + 1) mod 7 + 1]
  end
...
{$ENDIF}
    ...
    if not DayOnly then begin
      FDragBar.Caption := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}LongMonthNames[AMonth] + ' ' + IntToStr(AYear);
      FGrid.Invalidate;
    end;
  finally
    FUpdating := False;
...
begin
  ...
  if Message.MSG = SM_ALPHACMD then case Message.WParamHi of
    AC_REMOVESKIN, AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
  ...
  begin
    for i := aYear to aYear + 9 do begin
      ...
      for j := 1 to 12 do begin
        ...
        si.Caption := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}LongMonthNames[j];
...
procedure TsCalendGrid.CreateParams(var Params: TCreateParams);
begin
  inherited;
...
begin
  ...
  if not (FOwner.ShowWeeks and (ACol = 0)) and FOwner.ShowCurrentDate and IsToday and (ACol <> 7 + Integer(FOwner.ShowWeeks)) then begin
    ...
    if FOwner.SkinData.Skinned and Assigned(FOwner.SkinData.SkinManager) and (FOwner.SkinData.SkinManager.GetGlobalFontColor <> clFuchsia)
      then begin
        if FOwner.SkinData.SkinIndex >= 0
          then begin
            if (FOwner.SkinData.BorderIndex < 0) and (FOwner.SkinData.SkinManager.gd[FOwner.SkinData.SkinIndex].Props[0].Transparency = 100)
              then Canvas.Font.Color := GetControlColor(Parent.Parent.Parent)
              else Canvas.Font.Color := FOwner.SkinData.SkinManager.gd[FOwner.SkinData.SkinIndex].Props[0].FontColor.Color
          end
          else Canvas.Font.Color := FOwner.SkinData.SkinManager.GetGlobalFontColor
      end
...
{
    VK_RETURN : if FOwner.Parent is TsPopupCalendar then begin
      Clicked := True;
      TsPopupCalendar(FOwner.Parent).CalendarClick;
      Clicked := False;
    end;
}
    VK_RETURN: if FOwner.Parent is TsPopupCalendar then with TsPopupCalendar(FOwner.Parent) do begin
    ...
    VK_ESCAPE: if FOwner.Parent is TsPopupCalendar then with TsPopupCalendar(FOwner.Parent) do begin
      if FEditor.Visible then FEditor.SetFocus;
      Close;
      CloseUp;
    end;
  end;
...
begin
  case Message.Msg of
    ...
    WM_LBUTTONDBLCLK : begin
      inherited;
      if Assigned(FOwner.OnDblClick) and (csDoubleClicks in FOwner.ControlStyle) then begin
        if FOwner.Parent is TsPopupCalendar
          then begin
            if (TsPopupCalendar(FOwner.Parent).FEditor = nil) then FOwner.OnDblClick(FOwner)
          end
          else FOwner.OnDblClick(FOwner);
      end;
    end;
    WM_PAINT : begin
      inherited;
    ...
    WM_LBUTTONDOWN : if FOwner.Parent is TsPopupCalendar then begin
      if (FOwner.Parent is TsPopupCalendar) then begin
        if not FOwner.Parent.Visible then Exit;
        if Clicked then Exit;
        Clicked := True;

        MouseToCell(TWMMouse(Message).XPos, TWMMouse(Message).YPos, X, Y);
        if Y > 0 then begin // If not a day name
{$IFDEF DELPHI6UP}        
          FocusCell(X, Y, False);
{$ELSE}
          Row := Y;
    ...
    // Search a localization of "Today :"
{
    if Win32MajorVersion >= 6
      then ResStringRec.Identifier := 4432
      else
}
