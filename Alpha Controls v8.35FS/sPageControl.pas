...
uses
  ...
  ComCtrls, sCommonData, sConst, sFade, extctrls, sSpeedButton, acSBUtils, {$IFDEF DELPHI_XE3}UITypes, {$ENDIF}menus
...
{$ENDIF}
  ...
  if not OwnerDraw then begin
    ...
    if (TabPosition in [tpLeft, tpRight]) and not RotateCaptions or (TabPosition in [tpTop, tpBottom]) and RotateCaptions // If vertical text
      then OldFont := MakeAngledFont(TempBmp.Canvas.Handle, TempBmp.Canvas.Font, -2700); // Rotated font initialization
    ...
    if Page.TabIndex <> ActivePageIndex then
      BitBlt(DstDC, aRect.Left, aRect.Top, TempBmp.Width, TempBmp.Height, TempBmp.Canvas.Handle, 0, 0, SRCCOPY);
    DC := Canvas.Handle;
    Canvas.Handle := DstDC;
...
var
  ...
  R : TRect;
begin
  if (i < 0) or (i >= PageCount) or FCommonData.FUpdating then Exit;
...
begin
  ...
  if not InUpdating(FCommonData, False) then FCommonData.FUpdating := GetBoolMsg(Parent, AC_PREPARING); // Transparent BG may be not ready if PageControl haven't cached BG
...
{$ENDIF}
  case Message.Msg of
    WM_PAINT : if (Visible or (csDesigning in ComponentState)) then begin
      ...
      if FCommonData.Skinned then begin
        if IsCached(FCommonData) and not InAnimationProcess or (csDesigning in ComponentState) then
    ...
    WM_MOUSELEAVE, CM_MOUSELEAVE : if not (csDesigning in ComponentState) {and (HotTrack or FShowCloseBtns) }then begin
      ...
      if (CurItem >= 0) and (CurItem < PageCount) and (DroppedDownItem = -1) {and ((Pages[CurItem] <> ActivePage) {or (TsTabSheet(Pages[CurItem]).TabType <> ttTab))}  then begin
        if (Pages[CurItem] <> ActivePage) then begin
          ...
          if ShowCloseBtns then RepaintTab(CurItem, 2); // Repaint old tab in normal state
        end;
          ...
          if (NewItem <> CurItem) then begin // If hot item is changed
            if (CurItem > -1) and (CurItem < PageCount) {and (ActivePage <> Pages[CurItem]) }then begin
              if ActivePage <> Pages[CurItem] then begin
                ...
                if Assigned(FOnTabMouseLeave) then FOnTabMouseLeave(Self, CurItem);
                if (CurItem <> ActivePageIndex) or ShowCloseBtns then RepaintTab(CurItem, 2); // Repaint old tab in normal state
              end;
            end;
...
//              FCommonData.FUpdating := False;
  ...
  if (Message.Msg = cardinal(SM_ALPHACMD)) and Assigned(FCommonData) then case Message.WParamHi of
    ...
    AC_PREPARING : begin
      Message.Result := integer(InUpdating(FCommonData) or ((FCommonData.FCacheBmp.Width <> Width) or (FCommonData.FCacheBmp.Height <> Height)) and IsCached(FCommonData) and not SkinData.CustomColor);
      Exit;
{    end
    AC_PREPARING : begin
    ...
    AC_ENDPARENTUPDATE : if FCommonData.FUpdating then begin
      FCommonData.FUpdating := False;
      if not GetBoolMsg(Parent, AC_PREPARING) then begin
    ...
    AC_GETBG : begin
      ...
      PacBGInfo(Message.LParam).FillRect := Rect(0, 0, 0, 0);
      Exit;
    end;
...
//          RedrawWindow(SpinWnd.CtrlHandle, nil, 0, RDW_INVALIDATE or RDW_FRAME);
      ...
      WM_NCPAINT : begin
        ...
        if (SpinWnd <> nil) and not FCommonData.BGChanged then RedrawWindow(SpinWnd.CtrlHandle, nil, 0, RDW_INVALIDATE or RDW_FRAME or RDW_UPDATENOW);
        Message.Result := 0;
      end;
      WM_ERASEBKGND : begin
  ...
  if (PageControl <> nil) and TsPageControl(PageControl).SkinData.Skinned then case Message.Msg of
    CM_VISIBLECHANGED : if ac_OptimizeMemory and not (csDestroying in ComponentState) then begin
      if not Visible then begin
        ...
        if not TsPageControl(PageControl).SkinData.SkinManager.AnimEffects.PageChange.Active then
          RedrawWindow(Parent.Handle, nil, 0, RDW_ERASE or RDW_ALLCHILDREN or RDW_INVALIDATE or RDW_FRAME);
