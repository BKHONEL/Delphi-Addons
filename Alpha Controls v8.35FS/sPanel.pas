...
begin
  if Showing and Assigned(FCommonData) and FCommonData.Skinned {and not acGraphPainting }then begin
    ...
    if IsCached(FCommonData) and not SkinData.CustomColor or (Self is TsColorsPanel) or InAnimationProcess then begin
      ...
      if (i = 0) {or IsRectEmpty(R) is not redrawn while resizing }then Exit;
      if not InUpdating(FCommonData, False) then begin
        ...
        if b and not FCommonData.UrgentPainting then
...
{      if FCommonData.FCacheBmp.Empty then begin
      ...
      if (FCommonData.SkinManager.gd[FCommonData.SkinIndex].Transparency = 100) and (ParentBG.BgType = btCache) then begin
        ...
        if i = 0 then FillDC(DC, R, C) else begin
          if FCommonData.FCacheBmp = nil then FCommonData.FCacheBmp := CreateBmp32(Width, Height);
          if (FCommonData.SkinManager.gd[FCommonData.SkinIndex].Transparency <> 100) or FCommonData.SkinManager.IsValidImgIndex(FCommonData.BorderIndex) and (FCommonData.SkinManager.ma[FCommonData.BorderIndex].DrawMode and BDM_FILL <> BDM_FILL) then begin
            R := PaintBorderFast(NewDC, R, i, FCommonData, 0);
            FillDC(NewDC, R, C);
            BitBltBorder(NewDC, 0, 0, Width, Height, FCommonData.FCacheBmp.Canvas.Handle, 0, 0, i);
          end
          // If filled by BorderMask
          else begin
...
//      if FCommonData.FCacheBmp <> nil then FreeAndNil(FCommonData.FCacheBmp);
    ...
    if SendUpdated and not (csPaintCopy in ControlState)
...
begin
  if IsCached(FCommonData) or InAnimationProcess or (Self is TsColorsPanel) then begin
    ...
    if not PaintSkinControl(FCommonData, Parent, True, 0, Rect(0, 0, Width, Height), Point(Left, Top), FCommonData.FCacheBMP, True) then begin
      SkinData.FUpdating := True;
      Result := False;
      Exit;
    end;
...
{$ENDIF}
  if Message.Msg = SM_ALPHACMD
    ...
    AC_REFRESH : begin
      if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
        CommonWndProc(Message, FCommonData);
        if Assigned(OnPaint) then SkinData.CtrlSkinState := SkinData.CtrlSkinState and not ACS_FAST;
...
//          SkinData.FUpdating := True;
  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned then begin
    ...
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      case Message.Msg of
        ...
        WM_ERASEBKGND : begin
          if (Message.WParam <> Message.LParam {PerformEraseBackground, TntSpeedButtons}) then begin
            ...
            if not IsCached(SkinData) and not FCommonData.FUpdating
              then OurPaint(TWMPaint(Message).DC) // TODO : drawing of graphic controls must be added also
              else if csDesigning in ComponentState then inherited;
          end
        ...
        CM_SHOWINGCHANGED : if ac_OptimizeMemory and Visible then FCommonData.FUpdating := False;
        WM_WINDOWPOSCHANGING : begin
          if SkinData.SkinManager.gd[SkinData.SkinIndex].Transparency <> 0 then FCommonData.BGChanged := True;
        end;
      ...
      case Message.Msg of
        WM_WINDOWPOSCHANGED,
        WM_SIZE : if FCommonData.BGChanged then begin
          Repaint;
        end;
