...
uses sToolEdit, sStyleSimply, sSkinManager, sGraphUtils{$IFDEF LOGGED}, sDebugMsgs{$ENDIF};
...
begin
  if not (csDesigning in ComponentState) then begin
    try
      if (Trim(ReplaceStr(VarToStr(Value), {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DateSeparator, '')) = '') or VarIsNull(Value) or VarIsEmpty(Value)
...
begin
  if Assigned(FCloseUp) then FCloseUp(FEditor);
end;
...
begin
  ...
  if FEditor <> nil then begin
    ...
    if CanAccept then begin
      ...
      if Assigned(TsCustomDateEdit(FEditor).OnChange) then TsCustomDateEdit(FEditor).OnChange(TsCustomDateEdit(FEditor));
      Visible := False;
      if sPopupCalendar = Self
        then sPopupCalendar := nil;
      if Assigned(FEditor) and FEditor.Visible and FEditor.Enabled then begin
        FEditor.SetFocus;
        CloseUp;
        if FEditor.Focused and FEditor.AutoSelect then FEditor.SelectAll;
        Exit; // Closed already
      end;
    end;
  end;
...
begin
  ...
  if sPopupCalendar = Self then begin
    sPopupCalendar := nil;
  end;
  Inherited;
end;
