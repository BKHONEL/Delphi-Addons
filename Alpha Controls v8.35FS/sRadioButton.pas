...
begin
  ...
  if FGlyphChecked.Width > 0 then begin
    if GetControlsAlignment = taRightJustify
      then Result := Rect(Margin, GlyphTop, Margin + GlyphWidth, GlyphHeight + GlyphTop)
      else Result := Rect(Width - GlyphWidth - Margin, GlyphTop, Width - Margin, GlyphHeight + GlyphTop);
  end
  else begin
    i := GlyphMaskIndex(Checked);
    if SkinData.SkinManager.IsValidImgIndex(i) then Result := SkinCheckRect(i) else  Result := Rect(0, 0, 16, 16);  end;
end;
...
//  BGInfo.DrawDC := FCommonData.FCacheBmp.Canvas.Handle;
  ...
  PaintItem(FCommonData, BGInfoToCI(@BGInfo), True, integer(ControlIsActive(FCommonData) and not ReadOnly),
              Rect(0, 0, FCommonData.FCacheBmp.Width, Height), Point(Left, Top), FCommonData.FCacheBmp, True);
  DrawCheckText;
  DrawCheckArea;
...
{$ENDIF}
        ...
        WM_KEYUP : if Enabled and not (csDesigning in ComponentState) and (TWMKey(Message).CharCode = VK_SPACE) then begin
          if ReadOnly then Exit;
      ...
      CM_FONTCHANGED, CM_TEXTCHANGED : if AutoSize then AdjustSize;
...
begin
  CurValue:=Checked;
  inherited;
  if Assigned(FOnValueChanged) and (CurValue <> Checked) then FOnValueChanged(self);
end;
