...
begin
  if SkinData.Skinned then begin
    ...
    if BorderStyle <> bsNone then begin
      ...
      Ac_NCDraw(ListSW, Handle, WPARAM(Message.DC), Message.Unused, -1, TWMPaint(Message).DC);
    end;
    Message.Result := 2;
  end;
end;
...
{$ENDIF}
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) and Visible then begin
      CommonWndProc(Message, FCommonData);
  ...
  if not ControlIsReady(Self) or not Assigned(FCommonData) or not FCommonData.Skinned then inherited else begin
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      AC_ENDPARENTUPDATE : if FCommonData.Updating then begin
        ...
        if not FCommonData.FUpdating then Perform(WM_NCPAINT, 0, 0);
        Exit;
      end;
    end;
    CommonWndProc(Message, FCommonData);
