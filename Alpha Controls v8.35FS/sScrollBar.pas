...
begin
  ...
  SavedDC := SaveDC(DC);
  try
    if not (DrawingForbidden or not ControlIsReady(Self) or RestrictDrawing or (csDestroying in ComponentState) or (csLoading in ComponentState) or FCommonData.Updating) then begin
      RepaintNeeded := False;
...
{$ENDIF}
  ...
  if Assigned(FCommonData) then begin
    case Message.Msg of
      ...
      WM_NCHITTEST : if MustBeRecreated and not InAnimationProcess then begin // Control must be recreated for the skinned mode using without std blinking
        MustBeRecreated := False;
        SkinnedRecreate := True;
        OldPos := Position;
        if Parent <> nil then SendMessage(Parent.Handle, WM_SETREDRAW, 0, 0);
        RecreateWnd;
        if Parent <> nil then SendMessage(Parent.Handle, WM_SETREDRAW, 1, 0);
        Position := OldPos;
        SkinnedRecreate := False;
      end;
    end;
        ...
        with PScrollInfo(Message.LParam)^ do begin
          ...
          if Boolean(fMask and SIF_RANGE) and ((FSI.nMin <> nMin) or (FSI.nMax <> nMax)) then begin
            ...
            if Boolean(fMask and SIF_POS) then begin
              ...
              if PageSize > FSI.nMax then PageSize := FSI.nMax;
...
//        UpdateBar;
        inherited;
        SendMessage(Handle, WM_SETREDRAW, 1, 0);
        if not DrawingForbidden then Repaint;
//        if not DrawingForbidden then FCommonData.Invalidate;
        Exit;
      end;
...
begin
  if DrawingForbidden or (not (csCreating in Controlstate) and Assigned(SkinData.SkinManager) and SkinData.SkinManager.SkinData.Active and not (csDestroying in Componentstate)) then begin
    if MustBeRecreated then CheckRecreate;
    Self.FCommonData.FUpdating := Self.FCommonData.Updating;
    Paint(Msg.DC);
  end
  else inherited;
end;
...
begin
  ...
  with FCommonData.SkinManager.ConstData, FCommonData.SkinManager do if Kind = sbHorizontal then begin
    if (FCommonData.SkinManager.ConstData.IndexScrollLeft > -1) and
          FCommonData.SkinManager.gd[FCommonData.SkinManager.ConstData.IndexScrollLeft].ReservedBoolean and
          (FCommonData.SkinManager.ConstData.MaskScrollLeft > -1) then begin
...
begin
  ...
  with FCommonData.SkinManager.ConstData, FCommonData.SkinManager do if Kind = sbHorizontal then begin
    if (IndexScrollRight > -1) and gd[IndexScrollRight].ReservedBoolean and (MaskScrollRight > -1) then begin
      Result.Left := Width - (GetSystemMetrics(SM_CXHSCROLL) + (WidthOfImage(ma[MaskScrollRight]) - 17 {If additional width exists}));
    end;
  end
  else begin
    if (IndexScrollBottom > -1) and gd[IndexScrollBottom].ReservedBoolean and (MaskScrollBottom > -1) then begin
      Result.Top := Height - (GetSystemMetrics(SM_CYVSCROLL) + (HeightOfImage(ma[MaskScrollBottom]) - 17 {If additional width exists}));
    end;
  end;
end;
...
begin
  ...
  else m.Pos := SmallInt(FCurrPos);

  if m.ScrollCode in [SB_THUMBTRACK, SB_THUMBPOSITION] then m.Pos := SmallInt(FCurrPos) else m.Pos := 0;
...
begin
  CheckRecreate;
  if not SkinnedRecreate then inherited;
end;
