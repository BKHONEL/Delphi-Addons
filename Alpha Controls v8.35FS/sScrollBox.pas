...
begin
  ...
  if IsCached(FCommonData) and not SkinData.CustomColor or Assigned(FOnPaint) then begin
    if not InUpdating(FCommonData, False) then begin
      ...
      if b or (FCommonData.FCacheBmp = nil) then if not PrepareCache then Exit;
...
begin
  ...
  if not PaintSkinControl(FCommonData, Parent, False, 0, Rect(0, 0, width, Height), Point(Left, Top), FCommonData.FCacheBMP, True) then begin
  ...
  if FCommonData.Skinned and (BorderStyle <> bsNone) and Visible then begin
    ...
    if IsCached(FCommonData) then begin
      if ControlIsReady(Self) and not InUpdating(FCommonData, False) then begin
        ...
        if Assigned(ListSW) and Assigned(ListSW.sBarVert) then Ac_NCDraw(ListSW, Handle, 1, 0, -1, DC);
        RestoreDC(DC, SavedDC);
        ReleaseDC(Handle, DC);
      end;
    end
...
begin
  if SkinData.Skinned then begin
    if InAnimationProcess and (Message.DC <> SkinData.PrintDC) or not ControlIsReady(Self) then Exit; // Prevent of BG drawing in Aero
    if (FCommonData.SkinManager.gd[FCommonData.SkinIndex].Transparency = 0) or
         (SendAMessage(Parent, AC_GETSKINSTATE) and ACS_FAST = ACS_FAST) then Paint(Message.DC);
    Message.Result := 1;
  end
  else inherited;
end;
...
//      if Assigned(OnPaint) then OnPaint(FCommonData.FCacheBmp);
      ...
      // And scrolls
      Ac_NCDraw(ListSW, Handle, WPARAM(Message.DC), 0, -1, Message.DC);
...
{$ENDIF}
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_REFRESH : begin
      if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
        CommonWndProc(Message, FCommonData);
        if not InUpdating(SkinData) and not InAnimationProcess
    ...
    AC_ENDPARENTUPDATE : begin
      if not InUpdating(FCommonData, True) then begin
        RedrawWindow(Handle, nil, 0, RDW_INVALIDATE or RDW_FRAME);// or RDW_UPDATENOW or RDW_ERASE);
      end;
      SetParentUpdated(Self);
      Exit;
    end
  ...
  if not ControlIsReady(Self) then begin
    ...
    CommonWndProc(Message, FCommonData);
    inherited;
    if (Message.Result >= 0) {Avoiding an issue when ScrollBox is killed by child control already} and (FCommonData <> nil) and FCommonData.Skinned then case Message.Msg of
  ...
  if FAutoMouseWheel then begin
    ...
    if Form <> nil then begin
      ...
      if m.Result = 0 then begin
        ...
        if Assigned(FOnAfterScroll) then FOnAfterScroll(Self);
        if not SkinData.Skinned then SendMessage(Handle, WM_NCPAINT, 0, 0);
      end;
    end;
    Result := True;
  end
