...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, sDefaults,{$IFDEF DELPHI_XE3}UITypes, {$ENDIF}
  sConst, IniFiles, sMaskData, sSkinMenus, acTinyJpg, sStyleSimply, acSkinPack, menus, {$IFNDEF DELPHI5}Types, {$ENDIF}acntUtils
...
{$IFNDEF NOTFORHELP}
  ...
  private
    ...
    FThirdScrollControl: string;
    FThirdUpDownBtn    : string;
{$IFDEF ADDWEBBROWSER}
    FThirdWebBrowser   : string;
{$ENDIF}
    FThirdScrollBar    : string;
  ...
  published
    ...
    property ThirdScrollControl: string index ord(tpScrollControl)read GetString write SetString stored True;
    property ThirdUpDown       : string index ord(tpUpDownBtn  ) read GetString write SetString stored True;
{$IFDEF ADDWEBBROWSER}
    property ThirdWebBrowser   : string index ord(tpWebBrowser ) read GetString write SetString stored True;
{$ENDIF}
    property ThirdScrollBar    : string index ord(tpScrollBar  ) read GetString write SetString stored True;
  end;
  ...
  TacPaletteArray = array [pcMainColor..pcBorder] of TColor;
{$ENDIF} // NOTFORHELP
...
uses sMessages, sStoreUtils, sVclUtils, sCommonData, acPNG, acGlow, sThirdParty, {$IFNDEF ALITE}sDialogs,{$ENDIF}
...
function GetCornerType(sm : TsSkinManager; mask : TsMaskData) : integer;
var
  Bmp : TBitmap;
  X, Y, w, h, l, t : integer;
  C : TsColor;
...
begin
  if (mask.ClassName = s_ButtonHuge) and (mask.PropertyName = s_BordersMask) then Result := 2 {Region changed} else begin
    Result := 0; // Normal corner
    if HeightOf(mask.R) <= 0 then Exit;
    if Mask.Bmp <> nil then Bmp := Mask.Bmp else Bmp := sm.MasterBitmap;
    // Left top
    l := mask.R.Left;
    t := mask.R.Top;
  ...
  with sm do begin
    ...
    if (s = s_Pattern) or ( s = s_HotPattern) then begin
      ...
      // If loaded image is Jpeg, then working with massive of JPegs
        ...
        l := Length(pa);
        if l > 0 then for i := 0 to l - 1 do if (pa[i].PropertyName = s) and (pa[i].ClassName = UpperCase(skinSection)) then begin
          if not Assigned(pa[i].Img) then pa[i].Img := TacTinyJPGImage.Create;
        ...
        if not Result then begin
          ...
          SetLength(pa, l);
          try
            pa[l - 1].Img := TacTinyJPGImage.Create;
...
begin
  ...
  with sManager do begin
    ...
    with sManager do begin
      SkinData.Shadow1Color := ChangeSaturation(SkinData.Shadow1Color, Value);
      for pc := Low(Palette) to High(Palette) do if (pc in ChangeableColors) then Palette[pc] := ChangeSaturation(Palette[pc], Value);
    end;
  end;
end;
...
var
  i, l, j, w, x, y : integer;
  bmpClose, bmpCloseAlone, bmpMin, bmpMax, bmpNorm, bmpHelp, bmpCloseSmall, bmpMaxSmall, bmpMinSmall, bmpNormSmall : TBitmap;
...
begin
  ...
  with sManager do begin
    ...
    if SkinData.BIKeepHUE <> 0 then begin
      ...
      SaveImage(bmpCloseAlone, s_BorderIconCloseAlone);
    ...
    with sManager do begin
      SkinData.Shadow1Color := ChangeHue(Value, SkinData.Shadow1Color);
      for pc := Low(Palette) to High(Palette) do if (pc in ChangeableColors) then Palette[pc] := ChangeHue(Value, Palette[pc]);
    end;
  end;
  SetLength(ExceptNdx, 0);
end;
...
begin
  if SkinData.Active then begin
    s := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator;
    {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator := '.';
    Result := FloatToStr(SkinData.Version);
    {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator := s;
  end
  else Result := 'N/A';
end;
...
begin
  with ConstData do begin
    ...
    if IndexGlobalInfo > -1 then begin
      ...
      // Grips
      GripRightBottom := GetMaskIndex(IndexGlobalInfo, s_GlobalInfo, s_GripImage);
      GripHorizontal := GetMaskIndex(IndexGlobalInfo, s_GlobalInfo, s_GripHorz);
      GripVertical := GetMaskIndex(IndexGlobalInfo, s_GlobalInfo, s_GripVert);
    end
    else begin
      CheckBoxChecked        := -1;
      CheckBoxUnChecked      := -1;
    ...
    // Sys icons
    MaskCloseBtn      := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_BorderIconClose);
    MaskCloseAloneBtn := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_BorderIconCloseAlone);
    MaskMaxBtn        := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_BorderIconMaximize);
    MaskNormBtn       := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_BorderIconNormalize);
    MaskMinBtn        := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_BorderIconMinimize);
    MaskHelpBtn       := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_BorderIconHelp);

    MaskCloseSmall := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_SmallIconClose);
    MaskMaxSmall   := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_SmallIconMaximize);
    MaskNormSmall  := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_SmallIconNormalize);
    MaskMinSmall   := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_SmallIconMinimize);
    MaskHelpSmall  := GetMaskIndex(IndexGLobalInfo, s_GlobalInfo, s_SmallIconHelp);
    ...
    IndexWebBtn := GetSkinIndex(s_WebBtn);
  end;
  InitMaskIndexes;
  CheckShadows;
end;
...
begin
  for i := 0 to Length(gd) - 1 do if i <> ConstData.IndexGlobalInfo then begin
    ...
    gd[i].TextureNormal := GetTextureIndex(i, gd[i].ClassName, s_Pattern);
    gd[i].Props[0].TextureIndex := GetTextureIndex(i, gd[i].ClassName, s_Pattern);
    if gd[i].States > 1 then begin
      gd[i].Props[1].TextureIndex := GetTextureIndex(i, gd[i].ClassName, s_HotPattern);
    end;
//    gd[i].TextureHot    := GetTextureIndex(i, gd[i].ClassName, s_HotPattern);
  end;
end;
...
begin
  ...
  if not (csLoading in ComponentState) and not (csReading in ComponentState) and Assigned(InitDevEx) then InitDevEx(Active and (SkinName <> ''));
  if Assigned(FOnActivate) and FActive and not (csDesigning in ComponentState) then FOnActivate(Self);
end;
...
var
  ...
  i : integer;
begin
  if IsDefault and Assigned(InitDevEx) then InitDevEx(False);
  SkinRemoving := True;
  ClearGlows;
  UninstallHook;
  SkinData.Active := False;
  ...
  if FActive <> Value then begin
    ...
    if not Value then begin
      if FSkinName <> '' then begin
        ...
        InitConstantIndexes;
        UpdateCommonDlgs(Self);
        if Assigned(FOnDeactivate) and not (csDesigning in ComponentState) then FOnDeactivate(Self);
      end;
    end
    else begin
      SkinName := FSkinName;
...
begin
  if FSkinDirectory <> Value then begin
    ...
    if Assigned(FOnSkinListChanged) then FOnSkinListChanged(Self);
  end;
end;
...
begin
  ...
  if FActive then begin
    ...
    if Length(Value) > 4 then begin
      ...
      if UpperCase(s1) = UpperCase('.' + acSkinExt) then begin
        Delete(s, Length(Value) - 3, 4);
        FSkinName := s;
      end;
    end;
    s := NormalDir(SkinDirectory) + s + '.' + acSkinExt;
    SkinIsPacked := False;
    ...
    if FActive then begin
      ...
      if Assigned(FOnActivate) and not (csDesigning in ComponentState) and not (csLoading in ComponentState) then FOnActivate(Self);
    end
    else begin
      SendRemoveSkin;
      if Assigned(FOnActivate) and not (csDesigning in ComponentState) then FOnActivate(Self);
    end;
    aSkinChanging := False;
...
begin
  if Focused
    then Result := Palette[pcSelectionBG_Focused]
    else Result := Palette[pcSelectionBG];
end;

function TsSkinManager.GetHighLightFontColor(Focused : boolean = True): TColor;
begin
  if Focused
    then Result := Palette[pcSelectionText_Focused]
    else Result := Palette[pcSelectionText];
end;
...
begin
  ...
  if SkinFile <> nil then begin
    ...
    OldSeparator := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator;
    {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator := '.';
    fVersion := 7.4; // Stretched masks added in this version
    SkinData.Version := sf.ReadFloat(s_GlobalInfo, s_Version, 0);
    {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator := OldSeparator;
    ...
    if MasterBitmap.PixelFormat <> pf32bit then begin
      ...
      FillAlphaRect(MasterBitmap, Rect(0, 0, MasterBitmap.Width, MasterBitmap.Height), MaxByte, clFuchsia);
    end;
        ...
        if (s <> '') then begin
          case s[1] of
            TexChar : begin // Texture loading
              ...
              if (ma[l].MaskType > 0) and (SkinData.Version < fVersion)
//                then ma[l].DrawMode := ma[l].DrawMode or BDM_STRETCH; // Correct old skin
                then ma[l].DrawMode := ma[l].DrawMode and not BDM_STRETCH; // Correct old skin
              if Length(s) > 24
                then ma[l].MaskType := StrToInt(Copy(s, 25, 1))
                else ma[l].MaskType := 0;
            end;
            CharGlyph : begin // Glyph loading
              l := Length(ma);
              SetLength(ma, l + 1);
              ma[l].PropertyName := Values[j];
              ma[l].ClassName := Sections[i];
            ...
            CharMask : begin // Border mask
              ...
              if ((ma[l].MaskType > 0) and (SkinData.Version < fVersion)) then begin // Correct old skin
                if (((WidthOf(ma[l].R) div ma[l].ImageCount) - ma[l].WL - ma[l].WR) = 1) and (((HeightOf(ma[l].R) div (ma[l].MaskType + 1)) - ma[l].WT - ma[l].WB) = 1)
                  then ma[l].DrawMode := ma[l].DrawMode or BDM_STRETCH
                  else ma[l].DrawMode := ma[l].DrawMode and not BDM_STRETCH;
              end;
              ...
              ma[l].CornerType := GetCornerType(Self, ma[l]);
            end;
            CharExt : begin // External picture
                ...
                if SkinIsPacked then begin
                  for l := 0 to sc.ImageCount - 1 do if UpperCase(sc.Files[l].FileName) = s1 then begin
                    ...
                    if b then { if is bitmap } TempBmp.LoadFromStream(sc.Files[l].FileStream) else begin // If is PNG
                      ...
                      TempBmp.Assign(Png);
                      if Skindata.Version > 7.45 {New mode of layered wnd}
                        then UpdateAlpha(TempBmp)
                        else UpdateTransPixels(TempBmp);
                      FreeAndNil(Png);
                    end;
                    break;
                  end;
                  ...
                  if FileExists(s1) then begin
                    ...
                    if b then { if is bitmap } TempBmp.LoadFromFile(s1) else begin // If is PNG
                      ...
                      TempBmp.Assign(Png);
                      if Skindata.Version > 7.45 {New mode of layered wnd}
                        then UpdateAlpha(TempBmp{, (Sections[i] = s_GLOBALINFO) and (Values[j] = s_ExBorder)})
                        else UpdateTransPixels(TempBmp);
                      FreeAndNil(Png);
                    end;
                  end;
                ...
                if (TempBmp <> nil) and (TempBmp.Width > 0) then begin
                  ...
                  try
                    ...
                    if ma[l].Bmp.PixelFormat <> pf32bit then begin
                      ...
                      FillAlphaRect(ma[l].Bmp, Rect(0, 0, ma[l].Bmp.Width, ma[l].Bmp.Height), MaxByte, clFuchsia);
                    end;
                  finally
                    ma[l].PropertyName := Values[j];
                    ma[l].ClassName := Sections[i];
                    ...
                    if WordCount(s, [CharExt]) > 8 then ma[l].DrawMode := ExtInt(9, s, [CharExt]) else ma[l].DrawMode := BDM_STRETCH;
                    if ma[l].ImgType = itisaBorder then ma[l].CornerType := GetCornerType(Self, ma[l]);
                  end;
                end;
              end;
            end
              ...
              if (pos(CharDiez, s) > 0) then begin // Reading of the MasterBitmap item
                ...
                if s1 <> ''
                  ...
                  else ma[l - 1].DrawMode := BDM_STRETCH; // Stretching of borders is allowed if possible
                ...
                if (TempBmp <> nil) and (TempBmp.Width > 0) then begin
                  ...
                  try
                    if ma[l - 1].Bmp.PixelFormat <> pf32bit then begin
                      ...
                      FillAlphaRect(ma[l - 1].Bmp, Rect(0, 0, ma[l - 1].Bmp.Width, ma[l - 1].Bmp.Height), MaxByte, clFuchsia);
                    end;
                  finally
                    ma[l - 1].PropertyName := UpperCase(Values[j]);
                    ma[l - 1].ClassName := UpperCase(Sections[i]);
                    ...
                    ma[l - 1].WR := ma[l - 1].WL;
                    ma[l - 1].WB := ma[l - 1].WT;
                    ma[l - 1].CornerType := GetCornerType(Self, ma[l - 1]);
                  end;
                end;
              end;
            end;
    ...
    for i := 0 to Sections.Count - 1 do begin
      ...
      for j := 0 to Values.Count - 1 do begin
        ...
        if (pos('.JPG', s) > 0) or (pos('.JPEG', s) > 0) then begin
          ...
          if SkinIsPacked then begin
            for l := 0 to sc.ImageCount - 1 do if UpperCase(sc.Files[l].FileName) = s then begin
              TempJpg := TacTinyJPGImage.Create;
              sc.Files[l].FileStream.Seek(0, 0);
          ...
          if (TempJpg <> nil) and (TempJpg.Width > 0) then begin
            ...
            if pa[l - 1].Img.Width * pa[l - 1].Img.Height < 900000 then begin // convert to bitmap
              ...
              if ma[n - 1].Bmp.PixelFormat <> pf32bit then begin
                ...
                FillAlphaRect(ma[n - 1].Bmp, Rect(0, 0, ma[n - 1].Bmp.Width, ma[n - 1].Bmp.Height), MaxByte, clFuchsia);
              end;
...
begin
  if SkinFile <> nil then begin
    ...
    if SkinData.ExTitleHeight = 0 then SkinData.ExTitleHeight := 30;
    ...
    SkinData.UseAeroBluring := sf.ReadInteger(s_GlobalInfo, s_UseAeroBluring, 0) <> 0;
//    SkinData.ExtShadowColor := sf.ReadInteger(s_GlobalInfo, s_ExtShadowColor, 0);

    // Load colors palette
    // pcEditTextOk, pcEditTextWarning, pcEditTextAlert, pcEditTextHithglight1, pcEditTextHithglight2, pcEditTextHithglight3    
...
{$ENDIF}
    ...
    for i := 0 to Sections.Count - 1 do begin
      ...
      // State count
      gd[i].States := FindInteger(Sections[i], s_States, 3);
...
{$ENDIF}
  ...
  // ---- PaintingOptions -----
        ...
        int := acntUtils.ReadIniInteger(Ini, Sections, Sections[i], s_States, -1);
        ...
        gData.States := FindInteger(Sections[i], s_States, 3);
...
//  Palette[pcEditBG] := sf.ReadInteger(s_GlobalInfo, s_HotColor, clBlack);
  ...
  if (Palette[pcSelectionText] = -1) or (Palette[pcSelectionText] = clFuchsia) then begin
    ...
    if IsValidSkinIndex(i) then begin
      Palette[pcSelectionText] := gd[i].HotFontColor[1];
      Palette[pcSelectionBG] := gd[i].HotColor;
      Palette[pcSelectionBG_Focused] := gd[i].HotColor;
      if (Palette[pcSelectionText] <> -1) and (Palette[pcSelectionText] <> clFuchsia)
        then Palette[pcSelectionText] := ColorToRGB(Palette[pcSelectionText]) else Palette[pcSelectionText] := clHighLightText;
  ...
  Palette[pcEditBG_Warning] := sf.ReadInteger(s_GlobalInfo, s_EditBGWarning, $40ffFF); // $00C0FF);
  Palette[pcEditBG_Alert]   := sf.ReadInteger(s_GlobalInfo, s_EditBGAlert, $95a7e9); // $8597D9);
...
(*
  Palette[pcEditText_Ok] := sf.ReadInteger(s_GlobalInfo, s_EditTextOk, 113153 {Green});
  Palette[pcEditText_Warning] := sf.ReadInteger(s_GlobalInfo, s_EditTextWarning, 40703 {Orange});
  Palette[pcEditText_Alert] := sf.ReadInteger(s_GlobalInfo, s_EditTextAlert, 255 {Red});
*)
...
begin
  ...
  with sp as TsSkinProvider do begin
    ...
    else TmpBmp := GetFormImage(sp as TsSkinProvider);
  end;
...
{$IFNDEF NOWNDANIMATION}
    WM_WINDOWPOSCHANGED : if acLayered then begin
//    WM_WINDOWPOSCHANGED : if acLayered {$IFDEF D2011}and Application.MainFormOnTaskBar{$ENDIF} then begin
      if (TWMWindowPosChanged(Message).WindowPos.Flags and SWP_HIDEWINDOW = SWP_HIDEWINDOW) then begin
        if (AnimEffects.FormHide.Active) and (AnimEffects.FormHide.Time > 0) and not IsIconic(Application.Handle) and Application.Terminated then begin
          ...
          sp := TsSkinProvider(SendAMessage(Application.MainForm.Handle, AC_GETPROVIDER));
          if sp = nil then Exit;
          if not sp.DrawNonClientArea or sp.SkipAnimation or not sp.AllowAnimation then Exit;
          sp.SkipAnimation := True;

          sp.FormState := sp.FormState or FS_ANIMCLOSING;
          if sp.BorderForm <> nil then SetWindowRgn(sp.BorderForm.AForm.Handle, sp.BorderForm.MakeRgn, False);
...
{$IFNDEF NOWNDANIMATION}
          if (Application.MainForm <> nil) then begin
            ...
            else if sp.SkinData.SkinManager.AnimEffects.Minimizing.Active and Application.MainForm.Visible {and IsIconic(Application.Handle) }then begin
              if sp <> nil then begin
                if not sp.DrawNonClientArea then Exit;
...
{$IFDEF DELPHI6UP}
      TempSkinFile.CaseSensitive := True;
{$ENDIF}
      TempSkinFile.SetStrings(acMemSkinFile);
    end
    else begin
//      if Assigned(TempSkinFile) then FreeAndNil(TempSkinFile);
...
//    if not
    OldSeparator := {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator;
    {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator := '.';
    SkinData.Version := TempSkinFile.ReadFloat(s_GlobalInfo, s_Version, 0);
    {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator := OldSeparator;
    CheckVersion;
...
begin
  if (FActiveControl <> Value) then begin
    ...
    if (FActiveControl <> 0) and (FActiveGraphControl <> nil) then begin
      ...
      if (FActiveGraphControl <> nil) then Exit; // If control is hot still
    end;
    OldHwnd := FActiveControl;
    FActiveControl := Value;
    if OldHwnd <> 0 then SendAMessage(OldHwnd, AC_MOUSELEAVE, LPARAM(Self));
  ...
  if FActive and ExtendedBorders then begin
    ...
    if ConstData.ExBorder > -1 then begin
      if SkinData.ExDrawMode = 0 then begin // Shadow only
        // Calc a width from begin of image to begin of content
    ...
    if Self.SkinData.Version < 7.6 then begin // Update old skins
...
begin
  if (SkinData.Version < CompatibleSkinVersion) then begin
    ...
    ShowMessage('You are using an old version of the "' + SkinName + '" skin. Please, update a skins to latest or link with the AlphaControls support for upgrading of existing skin.'#13#10#13#10 +
                'This notification occurs in design-time only for your information and will not occur in real-time.')
  end
  else if SkinData.Version > MaxCompSkinVersion then begin
    b := srStdDialogs in SkinningRules;
    SkinningRules := SkinningRules - [srStdDialogs];
    MessageDlg('This version of the skin have not complete support by used AlphaControls package release.'#13#10 +
               'Components must be updated up to latest version for using this skin.', mtWarning, [mbOk], 0);
    if b then SkinningRules := SkinningRules + [srStdDialogs];
  end;
end;
...
begin
  if Value <> FActiveGraphControl then begin
    ...
    if (Value = nil) and (FActiveGraphControl <> nil) then begin
      ...
      if l <> 0 then if TsCommonData(l).FMouseAbove then Exit;
    end;
    OldControl := FActiveGraphControl;
    FActiveGraphControl := Value;
    if (FActiveGraphControl <> nil) and (FActiveControl <> 0) then FActiveControl := 0;
    if OldControl <> nil then OldControl.Perform(SM_ALPHACMD, MakeWParam(0, AC_MOUSELEAVE), LPARAM(Self));
  end;
end;
...
begin
  inherited;
  if Assigned(FOwner.OnSkinListChanged) then FOwner.OnSkinListChanged(FOwner);
end;
