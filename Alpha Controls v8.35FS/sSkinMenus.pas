...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, sConst, {$IFDEF DELPHI6UP}Types,{$ENDIF}{$IFDEF DELPHI_XE3}UITypes, {$ENDIF}
...
{        if GetMenuItemRect(PopupList.Window, Item.Parent.Handle, Item.MenuIndex, R) then begin
      ...
      if Item.Bitmap.Empty and ((Item.GetImageList = nil) or (Item.ImageIndex < 0)) then begin
        ...
        if j > -1 then begin
          ...
          if RTL then begin
            ...
            cRect.Left   := 2;
            cRect.Right  := WidthOfImage(TsSkinManager(FOwner).ma[j]) + 2;
          end;
    ...
    if not Item.Bitmap.Empty then begin
      ...
      if RTL
        ...
        else gRect.Left := min(3, gRect.Top);
      ...
      if RTL
        then gRect.Left := ItemBmp.Width - gRect.Top - GlyphSize(Item, False).cx
        else gRect.Left := min(3, gRect.Top);
    ...
    else if (Item.GetParentMenu <> nil) and (pos(s_SysMenu, Item.GetParentMenu.Name) = 1) then begin
      gChar := #0;
    ...
    if Assigned(TsSkinManager(FOwner).OnGetPopupItemData) then begin
      ...
      if ItemBmp.Canvas.Font.Color <> c then begin
        i := -1;
      end;
    end;
    R := TextRect;
...
{$ENDIF}
    ...
    DrawStyle := DrawStyle or DT_EXPANDTABS {DT_SINGLELINE} or DT_VCENTER or DT_NOCLIP;
    if pos(#13, Text) > 0 then DrawStyle := DrawStyle or DT_WORDBREAK else DrawStyle := DrawStyle or DT_SINGLELINE;
    if RTL then begin
      DrawStyle := DrawStyle or DT_RIGHT;
      dec(R.Right, ExtraWidth(mi));
    end;
    if odNoAccel in State then DrawStyle := DrawStyle or DT_HIDEPREFIX;
    if RTL then DrawStyle := DrawStyle or DT_RTLREADING;
...
begin
  ...
  if it in [smDivider, smCaption]
    then R := Rect(0, 0, 1, 0)
    else R := Rect(0, 0, 800, 0);
...
{$ENDIF}
  if Text = '' then Width := Margin * 3 + GlyphSize(Item, False).cx * 2 + Spacing else begin
    acDrawText(ACanvas.Handle, PacChar(Text), R, DT_EXPANDTABS {or DT_SINGLELINE }or DT_NOCLIP or DT_CALCRECT or DT_WORDBREAK);
...
begin
  ...
  if IsDivider then Text := '' else begin
    Text := Item.Caption;
    if not IsDivText(Item) then Text := Text + iff(ShortCutToText(Item.ShortCut) = '', '', ShortCutToText(Item.ShortCut));
  end;

  if IsDivider then Result := 2 else begin
    R := Rect(0, 0, 800, 0);
    acDrawText(ACanvas.Handle, PacChar(Text), R, DT_EXPANDTABS {or DT_SINGLELINE }or DT_NOCLIP or DT_CALCRECT or DT_WORDBREAK);
...
begin
  ...
  if BorderIndex > -1 then begin
    ...
    if TsSkinManager(FOwner).Palette[pcBorder] <> clFuchsia then aCanvas.Pen.Color := TsSkinManager(FOwner).Palette[pcBorder] else aCanvas.Pen.Color := clBtnShadow;
...
begin
  ...
  if TsSkinManager(FOwner).IsValidSkinIndex(i) then PaintItem(i, SkinSection, MakeCacheInfo(BG, 4, 4), True, 0, R, Point(aRect.Left + ExtraWidth(mi), aRect.Top), ItemBmp.Canvas.Handle, FOwner);
  ...
  if sp = nil then inherited else begin
    ...
    if ACanvas = sp.SkinData.FCacheBmp.Canvas then begin // If paint in form cache
      if sp.BorderForm <> nil then begin
        ...
        if (GetWindowLong(sp.Form.Handle, GWL_STYLE) and WS_CAPTION = WS_CAPTION)
          then t := aRect.Top - SysCaptHeight(sp.Form) - SysBorderWidth(sp.Form.Handle, sp.BorderForm, False)
          else t := 0;
      end;
      ...
      if sp.BorderForm <> nil
        then t := aRect.Top - sp.CaptionHeight(True) + DiffTitle(sp.BorderForm) + integer(sp.FSysExHeight) * 4
          ...
          if (GetWindowLong(sp.Form.Handle, GWL_STYLE) and WS_CAPTION = WS_CAPTION)
            then t := aRect.Top - SysCaptHeight(sp.Form) + DiffTitle(sp.BorderForm) - SysBorderWidth(sp.Form.Handle, sp.BorderForm, False)
            else t := 0;
        end;
    ...
    ItemSelected := Item.Enabled and ((odSelected in State) or (odHotLight in State) or IsOpened(Item));
...
{$ENDIF}
    ...
    if Assigned(Bmp) then begin
      if not Item.Enabled then begin
        ...
        SumBmpRect(Bmp, sp.MenuLineBmp, lo(Round(DefDisabledBlend * MaxByte)), R, Point(0, 0));
      end;
...
begin
  ...
  if Menu.Items.Count > 0 then Menu.OwnerDraw := A;
end;
...
var
  ...
  begin
    ...
    VertFont.lfHeight := TsSkinManager(FOwner).MenuSupport.ExtraLineFont.Height;
    ...
    if fsBold in TsSkinManager(FOwner).MenuSupport.ExtraLineFont.Style
      then VertFont.lfWeight := FW_BOLD
      else VertFont.lfWeight := FW_NORMAL;
...
begin
  ...
  if TsSkinManager(FOwner).IsValidSkinIndex(i) then begin
    ...
    if SysLocale.MiddleEast and (Item.GetParentMenu.BiDiMode <> bdLeftToRight) then begin // Flip BG
    ...
    if TsSkinManager(FOwner).IsValidImgIndex(i) then begin
      ...
      AddRgn(ArOR, mi.Bmp.Width, TsSkinManager(FOwner).ma[i], mi.Bmp.Height - TsSkinManager(FOwner).ma[i].WB, True);
    end;
    if Wnd <> 0 then DrawWndBorder(Wnd, mi.Bmp);
  end;
