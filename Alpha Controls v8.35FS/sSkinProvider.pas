...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Forms, Dialogs, sDefaults, menus, sSkinMenus, sSkinManager, acPNG,
  ExtCtrls, acGlow, Buttons, acThdTimer, {$IFDEF DELPHI_XE3}UITypes, {$ENDIF}
  sConst, sCommondata, Controls, acSBUtils{$IFDEF DELPHI7UP}, Types{$ENDIF}{$IFDEF TNTUNICODE}, TntWideStrUtils,
  TntMenus, TntStdCtrls, TntControls{$ENDIF},
  acTitleBar;
...
{$IFNDEF NOTFORHELP}
  ...
  protected
    ...
    procedure PaintTitleContent;
    procedure PaintTitleItem(Item : TacTitleBarItem; R : TRect; Bmp : TBitmap; NoAlpha : boolean = True);
...
//    procedure AC_WMDrawItem(var Message : TWMDrawItem);
    ...
    procedure AC_NCMouseMove(var Message : TWMNCMouseMove);
//    procedure AC_WMWindowPosChanged(const Msg : TWMWindowPosMsg);
    ...
    function ActualTitleHeight : integer;
    function GetTopCoord : integer;
  ...
  public
    ...
    procedure DropSysMenu(x, y : integer); virtual;
...
const
  ...
  FS_MAXWIDTH       = $4000;
  FS_ACTIVATE       = $8000;
  FS_LOCKED         = $10000;    // Lock for for animation
  FS_SCROLLSUPDATING = $20000;   // Update ListSW object
...
uses math, sVclUtils, sBorders, sGraphUtils, sSkinProps, sGradient, sLabel, FlatSB, StdCtrls,
  ...
  sMDIForm{$IFDEF CHECKXP}, UxTheme, Themes{$ENDIF}, sAlphaGraph, ComCtrls, Grids, acDials, {$IFDEF ADDWEBBROWSER} acWB, {$ENDIF}
...
begin
  ...
  if (Win32MajorVersion >= 6) and (Width <> 0) and (Height <> 0) then begin
    ...
    if (sp.FormTimer <> nil) and (TAccessProvider(sp).FormTimer is TacMintimer) and TacMintimer(TAccessProvider(sp).FormTimer).Minimized
...
begin
  if hDWMAPI <> 0 then begin
    with _BlurBehind do begin
      if ARGN <> 0 then dwFlags := DWM_BB_ENABLE or DWM_BB_BLURREGION else dwFlags := DWM_BB_ENABLE;
      fTransitionOnMaximized := False;
      hRgnBlur := ARGN;
      fEnable := AEnabled;
    end;
    if @_DwmEnableBlurBehindWindow <> nil then _DwmEnableBlurBehindWindow(AHandle, @_BlurBehind);
  end;
end;
  ...
  if sp.SkinData.SkinManager.AnimEffects.Minimizing.Active then begin
    ...
    if (TacMinTimer(sp.FormTimer).CurrentAlpha >= TacMinTimer(sp.FormTimer).AlphaFrom) then begin // If not in animation already
      if sp.BorderForm <> nil then begin
        ...
        if TacMinTimer(sp.FormTimer).SavedImage <> nil then FreeAndNil(TacMinTimer(sp.FormTimer).SavedImage);
...
{$ENDIF}
      ...
      sp.FormTimer.Enabled := True;
      sp.BorderForm.LastTopLeft := Point(-1, -1);
      sp.BorderForm.AForm.Perform(WM_SYSCOMMAND, SC_DRAGMOVE, 0);
      if (sp.BorderForm.LastTopLeft.X <> -1) and (sp.BorderForm.LastTopLeft.Y <> -1) then begin
        if (sp.BorderForm.LastTopLeft.Y < 0) and (Win32MajorVersion >= 6) // Avoid of a form auto aligning
          then sp.BorderForm.AForm.SetBounds(sp.BorderForm.LastTopLeft.X, sp.BorderForm.LastTopLeft.Y, sp.BorderForm.AForm.Width, sp.BorderForm.AForm.Height);
      end;
    end
    else begin
{$IFDEF DELPHI7UP}
...
{$ENDIF}
    ...
    if AeroIsEnabled and (sp.Form.Menu <> nil) then SendMessage(sp.Form.Handle, WM_NCPAINT, 0, 0); // Menu repaint
    sp.BorderForm.ExBorderShowing := False;
//    sp.FormState := sp.FormState or FS_BLENDMOVING; // Avoid a blinking
...
begin
  if sp.SkinData.FCacheBmp <> nil then begin
    Result := CreateBmp32(sp.SkinData.FCacheBmp.Width, sp.SkinData.FCacheBmp.Height);
    Result.Canvas.Lock;
    try
      SkinPaintTo(Result, sp.Form, 0, 0, sp);
      if sp.BorderForm = nil
        then FillAlphaRect(Result, Rect(0, 0, Result.Width, Result.Height), MaxByte);
    finally
      Result.Canvas.UnLock;
    end;
    if CacheReplaced then FreeAndNil(sp.SkinData.FCacheBmp);
  end
end;
...
function SkinBorderWidth(const BorderForm : TacBorderForm) : integer;
begin
  if (BorderForm <> nil) and (BorderForm.SkinData.SkinManager <> nil) then Result := BorderForm.SkinData.SkinManager.SkinData.ExBorderWidth else Result := 0;
end;
...
//  if IsIDERunning then Exit; // Avoiding of DbgBreakPoint issue
  if (sp.Form.FormStyle = fsMDIChild) and (MDISkinProvider <> nil) and (TsSkinProvider(MDISkinProvider).LockCount > 0) or (sp.FormState and FS_BLENDMOVING = FS_BLENDMOVING) then Exit;
...
var
  ...
  sbw, i : integer;
begin
  if sp.DrawNonClientArea and sp.Form.HandleAllocated and not sp.InMenu and (HaveBorder(sp) or IsIconic(sp.Form.Handle) or IsSizeBox(sp.Form.Handle)) then with sp do begin
...
begin
  if not (csDestroying in SkinProvider.ComponentState) and SkinProvider.Form.HandleAllocated and
        TForm(SkinProvider.Form).AutoScroll and (SkinProvider.FormState and FS_SCROLLSUPDATING <> FS_SCROLLSUPDATING) then begin
    SkinProvider.FormState := SkinProvider.FormState or FS_SCROLLSUPDATING;
    ...
    SkinProvider.FormState := SkinProvider.FormState and not FS_SCROLLSUPDATING;
  end;
end;
...
begin
  ...
  if MDIAlso and (TForm(sp.Form).FormStyle = fsMDIChild) and Assigned(MDISkinProvider) and TsSkinProvider(MDISkinProvider).DrawNonClientArea then begin
...
begin
  ...
  if (TForm(Form).FormStyle = fsMDIChild) and (Form.WindowState = wsMaximized) then begin
    ...
    FCommonData.BGChanged := True;
    if SkinProvider.Form.Menu <> nil then SkinProvider.ProcessMessage(WM_NCPAINT);
    ...
    if not TitleIcon.Glyph.Empty then begin
      if TitleIcon.Glyph.PixelFormat <> pf32bit then begin
        ...
        for y := 0 to TitleIcon.Glyph.Height - 1 do begin
          S := TitleIcon.Glyph.ScanLine[y];
          for x := 0 to TitleIcon.Glyph.Width - 1 do begin
            if (sFuchsia.R <> S[x].R) or (sFuchsia.G <> S[x].G) or (sFuchsia.B <> S[x].B) then S[x].A := MaxByte
          end;
        end;
      end;
...
begin
  if acPreviewNeeded and (acPreviewHandle = 0) and ((Application.MainForm <> nil) and Application.MainForm.HandleAllocated and (Application.MainForm.Handle = Handle) or (Application.MainForm = nil)) then begin
    ...
    for Count := 0 to 100 do begin
      ...
      if (h <> 0) then begin
        if (SendMessage(h, ASE_MSG, ASE_HELLO, LPARAM(Handle)) = 1) then Break;
      end
      else Break;
    end;
  end;
...
{$ENDIF}
  ...
  if (csDesigning in AOwner.ComponentState) then begin
    for i := 0 to Form.ComponentCount - 1 do if (Form.Components[i] is TsTitleBar) then begin
...
begin
  ...
  case i of
    ...
    else if Between(i, HTUDBTN, (HTUDBTN + TitleButtons.Count - 1)) then begin
      if TitleButtons.Items[i - HTUDBTN].Visible then CurButton := @TitleButtons.Items[i - HTUDBTN].Data;
    end
    else if Assigned(FTitleBar) and Between(i, HTITEM, (HTITEM + FTitleBar.Items.Count - 1)) then begin
      if FTitleBar.Items[i - HTITEM].Visible then BarItem := FTitleBar.Items[i - HTITEM];
    end;
  end;
  if (i in [HTCHILDCLOSE]) and not MDIButtonsNeeded then Exit;
  if i >= HTITEM then BtnLayered := True else begin // Bar items may be layered only
  ...
  if not BtnLayered then begin
    if (CurButton <> nil) and (CurButton^.State <> -1) then begin
      BtnDisabled := False;
      if CurButton^.Rect.Left <= FIconRect.Right then Exit;
      ...
      if FCommonData.SkinManager.IsValidImgIndex(ind) then begin // Drawing of the button from skin
        if i < HTUDBTN // if not user defined
          then DrawSkinGlyph(FCommonData.FCacheBmp, Point(CurButton^.Rect.Left, CurButton^.Rect.Top),
            ...
            if (TitleButtons.Items[i - HTUDBTN].UseSkinData)
               then DrawSkinGlyph(FCommonData.FCacheBmp, Point(CurButton^.Rect.Left, CurButton^.Rect.Top),
...
//          TitleButtons.Items[i - HTUDBTN].Glyph.PixelFormat := pf32bit;
        ...
        if (CurButton^.State = 1) and (i in [HTCLOSE, HTMAXBUTTON, HTMINBUTTON]) then begin
          ...
          if x > 0 then begin
            ...
            if SkinData.SkinManager.Effects.AllowGlowing and (Form.Parent = nil) and (Form.FormStyle <> fsMDIChild) then begin
              if BorderForm <> nil
                then CurButton^.GlowID := ShowGlow(R, R, s_GlobalInfo, FCommonData.SkinManager.ma[CurButton.ImageIndex].PropertyName + s_Glow, y, MaxByte, BorderForm.AForm.Handle, FCommonData.SkinManager)
                else CurButton^.GlowID := ShowGlow(R, R, s_GlobalInfo, FCommonData.SkinManager.ma[CurButton.ImageIndex].PropertyName + s_Glow, y, MaxByte, Form.Handle, FCommonData.SkinManager);
            end
          end;
        end
    ...
    if (CurButton <> nil) and (CurButton^.State <> -1) then begin
      case CurButton^.State of
        1 : StartSBAnimation(CurButton, CurButton^.State, 10, True, Self);
        2 : StartSBAnimation(CurButton, CurButton^.State, 1, True, Self);
        else StartSBAnimation(CurButton, CurButton^.State, 10, False, Self);
      end;
    end
    // Title Bar Items
    else if BarItem <> nil then begin
      if BorderForm <> nil then case BarItem.State of
        1 : StartItemAnimation(BarItem, BarItem.State, 10, True, BorderForm.AForm);
        2 : StartItemAnimation(BarItem, BarItem.State, 10, True, BorderForm.AForm);
        else StartItemAnimation(BarItem, BarItem.State, 10, False, BorderForm.AForm);
      end
      else case BarItem.State of
        1 : StartItemAnimation(BarItem, BarItem.State, 10, True);
        2 : StartItemAnimation(BarItem, BarItem.State, 10, True);
        else StartItemAnimation(BarItem, BarItem.State, 10, False);
      end
    end;
  end;
end;
...
var
  ...
  begin
    ...
    if SystemMenu = nil then PrepareForm;
    if SystemMenu = nil then Exit;
    ...
    if Assigned(FTitleBar) then for i := 0 to FTitleBar.Items.Count - 1 do begin
      inc(c);
      if not FTitleBar.Items[i].Visible then Continue;
      if PtInRect(FTitleBar.Items[i].Rect, p) then begin
        if FTitleBar.Items[i].Style in [bsSpacing, bsInfo] then Result := 0 else Result := c;
        Exit;
      end;
    end;
  end;
begin
  ...
  if (BtnIndex > 0) then begin
    ...
    if (BtnIndex <= SysBtnCount) then begin
      case BtnIndex of
        1 : if HaveSysMenu then Result := HTCLOSE;
        2 : begin
          if SystemMenu.VisibleMax then begin
    ...
    else if FTitleBar <> nil then begin
      BtnIndex := BtnIndex - SysBtnCount - TitleButtons.Count - 1;
      if FTitleBar.Items[BtnIndex].Enabled and
           ((FTitleBar.Items[BtnIndex].Style <> bsTab) or not FTitleBar.Items[BtnIndex].Down)
        then Result := HTITEM + BtnIndex;
    end;                                                      

{    else if (BtnIndex <= TitleButtons.Count + SysBtnCount) then begin // Title Bar button
      BtnIndex := BtnIndex - SysBtnCount - 1;
      if TitleButtons.Items[BtnIndex].Enabled then Result := HTITEM + BtnIndex;
    end;}
...
begin
  if Assigned(MDISkinProvider) then with TsSkinProvider(MDISkinProvider) do begin
    if DrawNonClientArea then begin
      ...
      if Assigned(MDIForm) then TsMDIForm(MDIForm).UpdateMDIIconItem;
      FCommonData.BGChanged := True;
      ...
      if UpdateNow and (BorderForm <> nil) then BorderForm.UpdateExBordersPos;
    end
    else RedrawWindow(Form.Handle, nil, 0, RDW_FRAME or RDW_INVALIDATE or RDW_UPDATENOW);
  end
end;
...
begin
{$IFDEF DELPHI6UP}
  if Form.FormStyle = fsStayOnTop then begin
    Result := Rect(Form.Monitor.Left, Form.Monitor.Top, Form.Monitor.Width, Form.Monitor.Height);
  end
  else Result := Form.Monitor.WorkareaRect;
{$ELSE}
...
begin
  if (sp.SkinData <> nil) and (sp.SkinData.SkinManager <> nil) and sp.SkinData.SkinManager.ExtendedBorders and sp.AllowExtBorders and (not sConst.ac_NoExtBordersIfMax or (sp.Form.WindowState <> wsMaximized))
    then Result := True
    else Result := False
end;
...
{$ENDIF}
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    ...
    AC_SETNEWSKIN : if not (csDestroying in Form.ComponentState) and Assigned(SkinData) then begin
      if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) and not Assigned(SystemMenu) then PrepareForm
    end;
    ...
    AC_GETBG : if (FCommonData <> nil) and FCommonData.Skinned then begin
      ...
      with PacBGInfo(Message.LParam)^.Offset do begin
        X := X + OffsetX;
        if X <> 0 then PacBGInfo(Message.LParam)^.FillRect.Left := max(0, PacBGInfo(Message.LParam)^.FillRect.Left - X);
        Y := Y + OffsetY;
        if Y <> 0 then PacBGInfo(Message.LParam)^.FillRect.Top  := max(0, PacBGInfo(Message.LParam)^.FillRect.Top - Y);
      end;
      Exit;
    end;
    AC_GETSKINSTATE : begin
      if FCommonData.CtrlSkinState and ACS_BGUNDEF = ACS_BGUNDEF then UpdateSkinState(FCommonData, False);
  ...
  if (csDestroying in Form.ComponentState) or not FCommonData.Skinned(True) then begin
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      AC_GETPROVIDER : begin Message.Result := LRESULT(Self); Exit end; // Used for menuline init
      AC_SETNEWSKIN : if not (csDestroying in Form.ComponentState) and Assigned(SkinData) then begin
        if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
          KillAnimations;
          DeleteUnusedBmps(True);
...
//              if SkinData.SkinManager.SkinData.ExMaxHeight <> 0 then i := SkinData.SkinManager.SkinData.ExMaxHeight else i := SkinData.SkinManager.SkinData.ExTitleHeight;
            ...
            if not (csLoading in SkinData.SkinManager.ComponentState) then begin
              if (Form.Menu <> nil) and FMenuOwnerDraw then MenusInitialized := True;
              InitMenuItems(True); // Update after skinning in run-time
              if not MenusInitialized then MenusInitialized := UpdateMenu;
            end;
            // Menu Line refresh
            FCommonData.BGChanged := True;
            FLinesCount := -1;
...
//              if not Assigned(MDIForm) then
            ...
            InitDwm(Form.Handle, DrawNonClientArea);//True);
          end;
...
{$IFNDEF NOWNDANIMATION}
      WM_WINDOWPOSCHANGED : if Assigned(SkinData.SkinManager) and acLayered then begin // Patch for BDS
        if (TWMWindowPosChanged(Message).WindowPos.Flags and SWP_HIDEWINDOW = SWP_HIDEWINDOW) {$IFDEF D2011} and not Application.Terminated{$ENDIF} then begin
          if FAllowAnimation and not IsIconic(Form.Handle) and not SkipAnimation and
...
{$ENDIF}
    ...
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      Ac_NCPAINT : begin
        if Message.LParam = 1 then FCommonData.BGChanged := True;
      ...
      AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
        if Adapter = nil then AdapterCreate else TacCtrlAdapter(Adapter).AddAllItems;
        if FDrawNonClientArea and FDrawClientArea then RefreshFormScrolls(Self, ListSW, False);
      ...
      AC_CHILDCHANGED : begin
        if (FCommonData.SkinManager.gd[SkinData.SkinIndex].GradientPercent + FCommonData.SkinManager.gd[SkinData.SkinIndex].ImagePercent > 0) then begin
          Message.LParam := 1;
        end
        else begin
          Message.LParam := integer((SkinData.BorderIndex >= 0) and (SkinData.SkinManager.ma[SkinData.BorderIndex].DrawMode and BDM_FILL = BDM_FILL))
        end;
      ...
      case Message.Msg of
        WM_NCMOUSEMOVE : if Form.Enabled then begin
          FCommonData.SkinManager.ActiveControl := Form.Handle;
          AC_NCMouseMove(TWMNCMouseMove(Message));
          Exit;
        end;
  {$IFNDEF ALITE}
        CM_MOUSEWHEEL : begin
  ...
  //      WM_NCCALCSIZE : AC_WMNCCalcSize(TWMNCCalcSize(Message));
        WM_INITMENUPOPUP : begin
          if SkinData.SkinManager.SkinnedPopups then AC_WMInitMenuPopup(TWMInitMenuPopup(Message));
        end;
  ...
  {$ENDIF}
              TryInitMenu;
              if Adapter <> nil then TacCtrlAdapter(Adapter).AddAllItems(Form);
              fAnimating := FAllowAnimation and ((Form.FormStyle = fsMDIChild) or (Form.Parent = nil)) and SkinData.SkinManager.AnimEffects.FormShow.Active{$IFNDEF DISABLEPREVIEWMODE} and (acPreviewHandle <> Form.Handle){$ENDIF};
              lInted := False;
              ...
              if (BorderForm <> nil) and ((not SkinData.SkinManager.AnimEffects.FormHide.Active or (SkinData.SkinManager.AnimEffects.FormHide.Time div acTimerInterval = 0)) or not FAllowAnimation)
                then FreeAndNil(BorderForm);
              OldWndProc(Message);
            end;
          end
          else begin // Backgrounds must be drawn by all controls for a blinking prevent
            OldWndProc(Message);
            if AeroIsEnabled and Form.Showing then SendMessage(Form.Handle, WM_SETREDRAW, 1, 0);
          ...
          if (Message.WParam = 1) then begin
            ...
            if (Form.FormStyle = fsMDIForm) then begin
              for i := 0 to Form.MDIChildCount - 1 do
                if not GetBoolMsg(Form.MDIChildren[i].Handle, AC_CTRLHANDLED) then AddSupportedForm(Form.MDIChildren[i].Handle);
            end;
...
{8!!!!        WM_GETTEXTLENGTH : begin
        ...
        WM_SETTEXT : if Form.HandleAllocated and IsWindowVisible(Form.Handle) and not (csCreating in Form.ControlState) and FDrawNonClientArea then begin
          if Assigned(FGlow1) then FreeAndNil(FGlow1);
          if Assigned(FGlow2) then FreeAndNil(FGlow2);
          if (Form.FormStyle = fsMdiChild) and (Form.WindowState = wsMaximized) then begin
            ...
            KillCaptForm(Self);
  {$IFDEF DELPHI6UP}
            {$IFNDEF DELPHI7}if Form.AlphaBlend then{$ENDIF} SendMessage(Form.Handle, WM_NCPAINT, 0, 0);
  {$ENDIF}
          end
          else begin
            if BorderForm = nil then MakeCaptForm(Self);
            OldWndProc(Message);
            UpdateSkinCaption(Self);
            if BorderForm = nil then KillCaptForm(Self);
          end;
        ...
        CM_ENABLEDCHANGED : begin
          ...
          if FDrawNonClientArea then begin
            if (BorderForm <> nil) and (BorderForm.AForm <> nil) then BorderForm.AForm.Enabled := Form.Enabled;
            ...
            if Form.Showing then begin
              if FormChanged and (not (IsIconic(Form.Handle) and InAero)) then begin
                ...
                if FCommonData.FCacheBmp <> nil
                  ...
                  else UpdateClient := False;

                if FDrawNonClientArea then begin
                  if ac_NoExtBordersIfMax and (Form.WindowState = wsMaximized) and (BorderForm <> nil)
                    then FreeAndNil(BorderForm);
                  ...
                  if (BorderForm <> nil) and ((ListSW = nil) or (ListSW.sBarVert = nil) or (ListSW.sBarHorz = nil) or (not ListSW.sBarVert.fScrollVisible and not ListSW.sBarHorz.fScrollVisible)) then begin // Pre-paint the form while is under the BorderForm
                    ...
                    else UpdateRgn(Self, False);
                  end
                  else RedrawWindow(Form.Handle, nil, 0, RDW_NOFRAME or RDW_NOERASE or RDW_INVALIDATE or RDW_UPDATENOW);
                  SetParentUpdated(Form);
                end
                else if UpdateClient {and not InAero }then InvalidateRect(Form.Handle, nil, False);
                LastClientRect := Rect(0, 0, Form.ClientWidth, Form.ClientHeight);
                Exit;
              end
              else if (Form.FormStyle = fsMDIForm) and IsCached(SkinData) or (Form.FormStyle = fsMDIChild) then begin
                // Upfate region
                ProcessMessage(WM_NCPAINT);
              end
              else begin
  ...
  //      WM_DRAWITEM : if not FDrawNonClientArea then OldWndProc(Message) else AC_WMDrawItem(TWMDrawItem(Message));
        WM_NCHITTEST : begin
          ...
          if (CaptionHeight = 0) or not FDrawNonClientArea then OldWndProc(Message) else begin
            ...
            if (Message.Result in [HTMAXBUTTON, HTCLOSE, HTMINBUTTON]) and (Form.FormStyle = fsMDIChild)
              then CheckSysMenu(True);
        ...
        WM_MOUSELEAVE, CM_MOUSELEAVE : begin
          ...
          if (CurrentHT < HTITEM) or (BorderForm = nil) then SetHotHT(-1);
          biClicked := False;
          OldWndProc(Message);
        end;
        CM_MOUSEENTER : begin
          OldWndProc(Message);
          FCommonData.SkinManager.ActiveControl := Form.Handle;
          if (BorderForm <> nil) and BorderForm.ResetRgn and (FormState and FS_SIZING <> FS_SIZING) then begin
...
//          SetWindowLong(Form.Handle, GWL_STYLE, GetWindowLong(Form.Handle, GWL_STYLE) or WS_SYSMENU);
          OldWndProc(Message);
{          if (SystemMenu <> nil) and SystemMenu.ItemClose.Visible then begin
            SetWindowLong(Form.Handle, GWL_STYLE, GetWindowLong(Form.Handle, GWL_STYLE) or WS_SYSMENU);
          end;}
//          HaveSysMenu := False;
          ...
          if FDrawNonClientArea then begin
            ...
            case TWMNCLButtonDown(Message).HitTest of
              ...
              HTSYSMENU : begin
                ...
                else DropSysMenu(FormLeftTop.x + SysBorderWidth(Form.Handle, BorderForm, False), FormLeftTop.y + BorderHeight + HeightOf(FIconRect) + SysBorderHeight(Form.Handle, BorderForm, False));
              end
              ...
              else if BetWeen(TWMNCLButtonDown(Message).HitTest, HTITEM, HTITEM + MaxByte) then begin
                if Assigned(FTitleBar) then begin
                  SetPressedHT(TWMNCLButtonDown(Message).HitTest);
                  FTitleBar.Items[TWMNCHitMessage(Message).HitTest - HTITEM].DoMouseDown(mbLeft, [], TWMNCHitMessage(Message).XCursor, TWMNCHitMessage(Message).YCursor);
                end;
              end
              else begin
  ...
  {$ENDIF}
                    ...
                    if not Form.Active and Form.Enabled then Form.SetFocus; // Focusing if maximized
                  end;
                end;
              end
            end
            ...
            case TWMNCLButtonDown(Message).HitTest of
              HTSYSMENU : begin
                ...
                else DropSysMenu(FormLeftTop.x + SysBorderWidth(Form.Handle, BorderForm), FormLeftTop.y + BorderHeight + HeightOf(FIconRect) + SysBorderHeight(Form.Handle, BorderForm, False));
              end
              else OldWndProc(Message);
            end
          end;
  ...
  //            FreeAndNil(FormTimer);
        ...
        WM_NCLBUTTONUP, WM_LBUTTONUP : if FDrawNonClientArea and Form.Enabled then begin
          if (BorderForm <> nil) and (TWMNCLButtonDown(Message).HitTest = HTTRANSPARENT)
            then TWMNCLButtonDown(Message).HitTest := BorderForm.Ex_WMNCHitTest(TWMNCHitTest(Message));
...
//                  FormState := FormState or FS_ANIMMINIMIZING;
                  SendMessage(Form.Handle, WM_SYSCOMMAND, SC_MINIMIZE, 0);
//                  FormState := FormState and not FS_ANIMMINIMIZING;
                end;
                FCommonData.BGChanged := True;
            ...
            else if BetWeen(TWMNCHitMessage(Message).HitTest, HTITEM, HTITEM + MaxByte) then begin
              if biClicked and Assigned(FTitleBar)
                then FTitleBar.Items[TWMNCHitMessage(Message).HitTest - HTITEM].DoMouseUp(mbLeft, [], TWMNCHitMessage(Message).XCursor, TWMNCHitMessage(Message).YCursor);
              biClicked := False;
              SetHotHT(TWMNCHitMessage(Message).HitTest);
            end
            else begin
        ...
        WM_PRINT : begin
          ...
          if Message.LParam and PRF_ERASEBKGND = PRF_ERASEBKGND then begin
          ...
          if Form.FormStyle = fsMDIForm then SendMessage(Form.ClientHandle, WM_PRINT, WPARAM(DC), 0);
  ...
  //      147, Std menu haven't captions if disabled
        ...
        WM_NCPAINT : begin
          if SystemMenu = nil then PrepareForm;
          if DrawNonClientArea{ and HaveBorder(Self)} then begin
          ...
          if RTInit then begin
            RTInit := False;
            LoadInit;
{            if (ListSW = nil) and DrawNonClientArea then begin // Initialize skinned scrollbars if not initialized yet
              RefreshFormScrolls(Self, ListSW, False);
            end;    CPU Window is shown sometimes}
          end;
          ...
          if DrawNonClientArea and {HaveBorder(Self) and} not (InAnimationProcess and (Form.FormStyle = fsMDIChild){liondev}) then begin
        ...
        WM_ERASEBKGND : if Form.Showing and FDrawClientArea then begin
            ...
            if not IsCached(FCommonData) {and (Form.Parent = nil) }or ((Form.FormStyle = fsMDIForm) and (Form.ActiveMDIChild <> nil) and (Form.ActiveMDIChild.WindowState = wsMaximized)) then begin
              ...
              if not FCommonData.FUpdating then begin
                if (FCommonData.FCacheBmp <> nil) and not FCommonData.BGChanged then begin
                  CopyWinControlCache(Form, FCommonData, Rect(OffsetX, OffsetY, OffsetX + Form.ClientWidth, OffsetY + Form.ClientHeight), Rect(0, 0, Form.ClientWidth, Form.ClientHeight), TWMPaint(Message).DC, False);
  //                BitBlt(TWMPaint(Message).DC, 0, 0, Form.Width, Form.Height, FCommonData.FCacheBmp.Canvas.Handle, OffsetX, OffsetY, SRCCOPY);
        ...
        WM_PAINT : if FDrawClientArea {and (Form.WindowState <> wsMinimized) }then begin
          if (csPaintCopy in Form.ControlState) then Exit; // Implemented in WM_ERASEBKGND
          if (BorderForm <> nil) and not InAnimation(Self) and ((BorderForm.AForm = nil) or not IsWindowVisible(BorderForm.AForm.Handle))
            then BorderForm.UpdateExBordersPos(False);
          ...
          if not DrawNonClientArea and (Form.Menu <> nil) then SendMessage(Form.Handle, WM_NCPAINT, 0, 0);
        end
        else OldWndProc(Message);
  ...
  {$ENDIF}
        ...
        WM_NCACTIVATE : if not (csDestroying in ComponentState) and DrawNonClientArea and Form.HandleAllocated then begin
          if SystemMenu = nil then PrepareForm; // If not intitialized (occurs when Scaled = False)
          FormState := FormState or FS_ACTIVATE;
          if FormState and FS_BLENDMOVING <> FS_BLENDMOVING then FormActive := TWMNCActivate(Message).Active;
          if not FormActive and (ShowAction = saMaximize) then begin // <<<<< Caption blinking removing
            ...
            if SkinData.SkinIndex >= 0 then begin
              if SkinData.BorderIndex >= 0 then begin
                if (SkinData.SkinManager.ma[SkinData.BorderIndex].ImageCount > 1) or (SkinData.SkinManager.gd[SkinData.SkinIndex].States > 1) then FCommonData.BGChanged := True;
              end
              else if (SkinData.SkinManager.gd[SkinData.SkinIndex].States > 1) then FCommonData.BGChanged := True;
            end
            else FCommonData.BGChanged := True;
  ...
  {$ENDIF}
              ...
              else if (SkinData.FCacheBmp <> nil) and (GetLinesCount > 0) then begin // Menu line hiding
                if BorderForm.AForm = nil then BorderForm.CreateNewForm;
                SetWindowRgn(BorderForm.AForm.Handle, BorderForm.MakeRgn, False);
  {$IFDEF DELPHI7UP}
                if Form.AlphaBlend then begin
  ...
  {$ENDIF}
            ...
            if (BorderForm <> nil) and not fAnimating and not AeroIsEnabled then begin // Forbid a borders painting by system
              UpdateRgn(Self, False);
            end;
  ...
  //          if SystemMenu = nil then PrepareForm; // If not intitialized (occurs when Scaled = False)
          ...
          if (GetWindowLong(Form.Handle, GWL_EXSTYLE) and WS_EX_LAYERED = WS_EX_LAYERED) and (FormTimer = nil) then ProcessMessage(WM_NCPAINT);

          if (csCreating in Form.ControlState) then Exit;

          if HaveBorder(Self) and (FormState and FS_BLENDMOVING <> FS_BLENDMOVING) then begin
            if MDIButtonsNeeded then begin
          ...
          if (FormTimer = nil) or not FormTimer.Enabled {and DrawNonClientArea} then begin
            ...
            if (BorderForm <> nil) then begin
              case Form.WindowState of
                wsMaximized : begin
                  if ac_NoExtBordersIfMax then begin
                    FreeAndNil(BorderForm);
                    UpdateRgn(Self, False);
                  end
                  else {if Form.Menu <> nil then }BorderForm.UpdateExBordersPos;
                end;
                wsNormal : BorderForm.UpdateExBordersPos;
              end;
            end;
          end;
        ...
        WM_ACTIVATEAPP : begin
          OldWndProc(Message);
          if (sPopupCalendar <> nil) and sPopupCalendar.Visible then sPopupCalendar.Close;
          if (Message.WParam = 1) and (SkinData.SkinManager.AnimEffects.Minimizing.Active) and (FormState and FS_ANIMMINIMIZING <> FS_ANIMMINIMIZING) then begin
            if (BorderForm <> nil) then BorderForm.UpdateExBordersPos(False) // Update z-order ( BDS )
            else if (Application.MainForm = Form) and (Message.WParam = 1) and (Application.MainForm.WindowState = wsMinimized)
                and ((FormTimer = nil) or not TacMinTimer(FormTimer).Minimized) then begin // Restore a main form if is not restored still
...
{$IFNDEF NOWNDANIMATION}
            ...
            SC_MINIMIZE : if (Form <> Application.MainForm) and SkinData.SkinManager.AnimEffects.Minimizing.Active then begin
              bAnim := acGetAnimation;
              acSetAnimation(False);
...
{$ENDIF}
          ...
          case Form.FormStyle of
            fsMDIChild : begin
              ...
              if UpdateClient and (MDISkinProvider <> nil) then TsSkinProvider(MDISkinProvider).FCommonData.BeginUpdate; // Speed rising
              case $FFF0 and Message.WParam of
                SC_CLOSE : if MDISkinProvider <> nil then begin
                ...
                SC_RESTORE : begin
                  ...
                  if DrawNonClientArea then SetWindowLong(Form.Handle, GWL_STYLE, GetWindowLong(Form.Handle, GWL_STYLE) and not WS_SYSMENU);
                  RedrawWindow(Form.Handle, nil, 0, RDW_FRAME or RDW_ERASE or RDW_INVALIDATE or RDW_UPDATENOW);

                  // Updating of childs
                  if (MDISkinProvider <> nil) then for i := 0 to TsSkinProvider(MDISkinProvider).Form.MDIChildCount - 1 do if TsSkinProvider(MDISkinProvider).Form.MDIChildren[i] <> Form then begin
                ...
                SC_MAXIMIZE : begin
                  ...
                  if (MDISkinProvider <> nil) and Assigned(TsSkinProvider(MDISkinProvider).MDIForm) then UpdateMainForm;
                  SystemMenu.UpdateItems(True);
                  Exit;
                end
                else OldWndProc(Message);
            ...
            fsMDIForm : begin
              ...
              case Message.WParam of
                SC_MAXIMIZE, SC_RESTORE : if (BorderForm <> nil) then begin
                  if ac_NoExtBordersIfMax or (Message.WParam = SC_RESTORE) then FreeAndNil(BorderForm);
                end;
              end;

              OldWndProc(Message);
              case Message.WParam of
                SC_KEYMENU : if (TWMSysCommand(Message).Key = VK_SPACE) and HaveSysMenu and HaveBorder(Self) then begin
                  if IsIconic(Form.Handle)
                    then DropSysMenu(FormLeftTop.x + SysBorderWidth(Form.Handle, BorderForm), FormLeftTop.y + BorderHeight + HeightOf(FIconRect) - 16)
                    else DropSysMenu(FormLeftTop.x + SysBorderWidth(Form.Handle, BorderForm), FormLeftTop.y + BorderHeight + HeightOf(FIconRect));
                end;
                SC_MAXIMIZE, SC_RESTORE : begin
                  ...
                  if (Message.WParam = SC_RESTORE) then begin
                    FCommonData.BGChanged := True;
                    RegionChanged := True;
                    if not HaveBorder(Self) then SetWindowRgn(Form.Handle, 0, True) else begin
                      if (SkinData.SkinManager <> nil) and SkinData.SkinManager.ExtendedBorders and
                           AllowExtBorders and DrawNonClientArea and (Form.Parent = nil) and (Form.FormStyle <> fsMDIChild) then begin
...
{                SC_MINIMIZE : if Assigned(FCommonData.FCacheBmp) then begin
                  ...
                  if (BorderForm <> nil) and (BorderForm.AForm <> nil) and (Form <> Application.MainForm) then FreeAndNil(BorderForm.AForm);
                end;}
              end;
            end;
          end;
        ...
        WM_WINDOWPOSCHANGING : begin
          if not RgnChanging and not (csLoading in Form.ComponentState) then begin
            if not IsZoomed(Form.Handle) and (TWMWindowPosChanged(Message).WindowPos.Flags and SWP_NOMOVE <> SWP_NOMOVE) and
              ...
              if sConst.ac_NoExtBordersIfMax and (Form.WindowState = wsNormal) and (BorderForm = nil) and ExtBordersNeeded(Self) and IsWindowVisible(Form.Handle) then begin
                Self.InitExBorders(True);
              end;
  ...
  {$IFNDEF NOWNDANIMATION}
            if Assigned(SkinData.SkinManager) and acLayered and DrawNonClientArea then begin
              if not SkipAnimation then begin
                if ((TWMWindowPosChanged(Message).WindowPos.Flags and SWP_HIDEWINDOW = SWP_HIDEWINDOW) or (TWindowPos(TWMWindowPosChanged(Message).WindowPos^).x = -32000))
                     and (SkinData.FCacheBmp <> nil {it's possible under Aero}) then begin // Window will be hidden
                  if not IsIconic(Form.Handle) then begin
                    if (SkinData.SkinManager.ShowState <> saMinimize) {and (FormState and FS_ANIMMINIMIZING <> FS_ANIMMINIMIZING) }then begin // Closing
                      if FAllowAnimation and (SkinData.SkinManager.AnimEffects.FormHide.Active) {and (SkinData.SkinManager.AnimEffects.FormHide.Time > 0)} and (FormState and FS_ANIMCLOSING <> FS_ANIMCLOSING) and (Form.Parent = nil) then begin
  ...
  {$ENDIF}
                  ...
                  else if (Form.WindowState <> wsMinimized) and (TWindowPos(TWMWindowPosChanged(Message).WindowPos^).x = -32000) then begin
                    OldWndProc(Message);
  ...
  {$ENDIF}
        ...
        WM_QUERYENDSESSION : begin
          ...
          if not Application.Terminated then while acAnimCount > 0 do Application.ProcessMessages;
          OldWndProc(Message);
        end;
            ...
            if not SkipAnimation{ and (Form = Application.MainForm)} and (SkinData.FCacheBmp <> nil {it's possible under Aero}) then begin // Window will be hidden
              if not IsIconic(Form.Handle) then begin
                if (SkinData.SkinManager.ShowState = saMinimize) and (Form = Application.MainForm) then begin
                  ...
                  if FAllowAnimation and (SkinData.SkinManager.AnimEffects.FormHide.Active) and (Form.Parent = nil) and (Form.FormStyle <> fsMDIChild) {and (SkinData.SkinManager.AnimEffects.FormHide.Time > 0) }then begin
                  ...
                  else begin // Closing not main form
  ...
  {$ENDIF}
          OldWndProc(Message);
          if not FInAnimation and (BorderForm <> nil) and Form.Visible and not IsIconic(Form.Handle) then begin
  ...
  {$IFNDEF NOWNDANIMATION}
        WM_CLOSE : begin
          if FAllowAnimation and (SkinData.SkinManager.AnimEffects.FormHide.Active) and //(SkinData.SkinManager.AnimEffects.FormHide.Time > 0) and
               DrawNonClientArea and (FormState and FS_ANIMCLOSING <> FS_ANIMCLOSING) and (Form.Parent = nil) then begin
            OldWndProc(Message);
            if csDestroying in Form.ComponentState then Exit; // Sometimes form may be destroyed already
            FormState := FormState or FS_ANIMCLOSING;
            SkinData.FCacheBmp := GetFormImage(Self, True);
            FormState := FormState and not FS_ANIMCLOSING;
...
procedure TsSkinProvider.PaintTitleContent;
var
  ...
  begin
    if Btn.ImageIndex > -1 then DrawSkinGlyph(FCommonData.FCacheBmp, Point(Btn.Rect.Left, Btn.Rect.Top),
      Btn.State * integer(Btn.Timer = nil), 1 + integer(not FormActive or not BtnEnabled) * integer((Btn.State = 0) or (Btn.Timer <> nil)),
...
begin
  ...
  // Paint sys buttons
  if Assigned(SystemMenu) and HaveSysMenu then begin
    PaintButton(ButtonClose, SystemMenu.VisibleClose);
    if SystemMenu.VisibleMax then PaintButton(ButtonMax, SystemMenu.EnabledMax);
    if SystemMenu.VisibleMin then PaintButton(ButtonMin, SystemMenu.EnabledMin);
    if biHelp in Form.BorderIcons then PaintButton(ButtonHelp, True);
  end;
  // Paint title buttons
  if TitleButtons.Count > 0 then begin // TitleButtons will be removed soon
    for index := 0 to TitleButtons.Count - 1 do if TitleButtons.Items[index].Visible then begin
      ...
      if Assigned(TitleButton.Glyph) and not TitleButton.Glyph.Empty then begin
        if TitleButton.Glyph.PixelFormat = pf32bit then begin
          ...
          CI := MakeCacheInfo(FCommonData.FCacheBmp, 0, 0);
          CopyByMask(Rect(x, y, x + TitleButton.Glyph.Width, y + TitleButton.Glyph.Height),
                     ...
                     FCommonData.FCacheBmp, TitleButton.Glyph, CI, True);
        end
        else begin
  ...
  if Assigned(FTitleBar) and (FTitleBar.Items.Count > 0) then begin
    for index := FTitleBar.Items.Count - 1 downto 0 do if ((FTitleBar.Items[index].Style <> bsTab) or not FTitleBar.Items[index].Down) and FTitleBar.Items[index].Visible then begin
      if (FTitleBar.Items[index].Rect.Left > 0) and (FTitleBar.Items[index].Rect.Right < Captionwidth)
        then PaintTitleItem(FTitleBar.Items[index], FTitleBar.Items[index].Rect, FCommonData.FCacheBmp);
    end;
    // Paint active tab if exists
    for index := FTitleBar.Items.Count - 1 downto 0 do if (FTitleBar.Items[index].Style = bsTab) and FTitleBar.Items[index].Down and FTitleBar.Items[index].Visible then begin
      if (FTitleBar.Items[index].Rect.Left > 0) and (FTitleBar.Items[index].Rect.Right < Captionwidth)
        then PaintTitleItem(FTitleBar.Items[index], FTitleBar.Items[index].Rect, FCommonData.FCacheBmp);
    end;
  end;
  ...
  if MDIButtonsNeeded then begin
    ...
    if Assigned(TsSkinProvider(MDISkinProvider).Form.ActiveMDIChild) and { Draw norm. button when maximized } (TsSkinProvider(MDISkinProvider).Form.ActiveMDIChild.WindowState <> wsMaximized) then begin
      ...
      i := sMan.GetMaskIndex(sMan.ConstData.IndexGlobalInfo, s_GlobalInfo, s_SmallIconNormalize);
      if i < 0 then i := sMan.GetMaskIndex(FCommonData.SkinIndex, s_GlobalInfo, s_BorderIconNormalize);
    end;
    ...
    if sMan.IsValidImgIndex(i) then begin
      DrawSkinGlyph(FCommonData.FCacheBmp, Point(MDIMax.Rect.Left, MDIMax.Rect.Top),
        MDIMax.State, 1 + integer(not FormActive or not ChildProvider.SystemMenu.EnabledRestore), sMan.ma[i], MakeCacheInfo(SkinData.FCacheBmp));
    end;
    if sMan.IsValidImgIndex(MDIClose.ImageIndex) then begin
      ...
      DrawSkinGlyph(FCommonData.FCacheBmp, Point(MDIClose.Rect.Left, MDIClose.Rect.Top),
        MDIClose.State, 1 + integer(not FormActive), sMan.ma[MDIClose.ImageIndex], MakeCacheInfo(SkinData.FCacheBmp));
    end;
  end;
end;
...
begin
  ...
  w := CaptionWidth;
  if hh <> 0
    then BitBlt(DC, 0, 0, w, hh, FCommonData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY) // Title and menu line update
    else BitBlt(DC, 0, 0, w, hmnu, FCommonData.FCacheBmp.Canvas.Handle, 0, 0, SRCCOPY); // Only menu line update
...
begin
  ...
  if (SkinData.CtrlSkinState and ACS_FAST = ACS_FAST) then begin
    if (Form.FormStyle <> fsMDIForm) then begin
      if Form.BorderStyle = bsSingle then begin
        if SkinData.BorderIndex >= 0 then i := GetSystemMetrics(SM_CXFIXEDFRAME) - SkinData.SkinManager.ma[SkinData.BorderIndex].WL;
      end
      else i := 0;
      FillDC(DC, R, FormColor);
      if DrawNonClientArea and (FCommonData.FCacheBmp <> nil) and (i > 0) then begin
        // Top border
        BitBlt(DC, R.Left, R.Top, WidthOf(R), SkinData.SkinManager.ma[SkinData.BorderIndex].WT - i, FCommonData.FCacheBmp.Canvas.Handle, OffsetX, OffsetY, SRCCOPY);
        // Left
        BitBlt(DC, R.Left, R.Top, SkinData.SkinManager.ma[SkinData.BorderIndex].WL - i, HeightOf(R), FCommonData.FCacheBmp.Canvas.Handle, OffsetX, OffsetY, SRCCOPY);
        // Right
        w := R.Right - (SkinData.SkinManager.ma[SkinData.BorderIndex].WR - i);
        BitBlt(DC, w, R.Top, SkinData.SkinManager.ma[SkinData.BorderIndex].WL - i, HeightOf(R), FCommonData.FCacheBmp.Canvas.Handle, OffsetX + w, OffsetY, SRCCOPY);
        // Bottom
        w := R.Bottom - (SkinData.SkinManager.ma[SkinData.BorderIndex].WB - i);
        BitBlt(DC, R.Left, w, R.Right, SkinData.SkinManager.ma[SkinData.BorderIndex].WB - i, FCommonData.FCacheBmp.Canvas.Handle, OffsetX, OffsetY + w, SRCCOPY);
      end;
    end;
{    if (Form.FormStyle <> fsMDIForm) then begin
      PaintControls(DC, Form, ControlsChanged or Changed, Point(0, 0), Form.Handle, False);
    end;}
  end
  else begin
...
begin
  ...
  if not ((CurrentHT = i) or (CurrentHT = -1) and (i = 0)) then begin
    if (CurrentHT > 0) then begin
      case CurrentHT of
        ...
        else if BetWeen(CurrentHT, HTITEM, HTITEM + MaxByte) then begin
          if Assigned(FTitleBar) and (CurrentHT - HTITEM < FTitleBar.Items.Count) then begin
            if Assigned(FTitleBar.Items[CurrentHT - HTITEM].OnMouseLeave) then FTitleBar.Items[CurrentHT - HTITEM].OnMouseLeave(FTitleBar.Items[CurrentHT - HTITEM]);
  ...
  CurrentHT := i;
  if biClicked or (CurrentHT <= 0) then Exit; // If button is pressed and position of form is changed
  case CurrentHT of
    ...
    HTITEM..HTITEM + MaxByte : if Assigned(FTitleBar) then if (CurrentHT - HTITEM < FTitleBar.Items.Count) then begin
      if Assigned(FTitleBar.Items[CurrentHT - HTITEM].OnMouseEnter) then FTitleBar.Items[CurrentHT - HTITEM].OnMouseEnter(FTitleBar.Items[CurrentHT - HTITEM]);
      FTitleBar.Items[CurrentHT - HTITEM].State := 1;
      ShowHintWnd(FTitleBar.Items[CurrentHT - HTITEM]);
    end;
  end;
  if Repaint then RepaintButton(CurrentHT);
end;
    ...
    HTCHILDMIN : MDIMin.State := 2
      ...
      if Assigned(FTitleBar) then begin
        ...
        if FTitleBar.Items[CurrentHT - HTITEM].Style = bsTab then begin
          if FTitleBar.Items[CurrentHT - HTITEM].Timer <> nil then FreeAndNil(FTitleBar.Items[CurrentHT - HTITEM].Timer);
          CurrentHT := -1;
          Exit; // No animation
        end;
      end;
  ...
  begin
    if (CaptionHeight <> 0) and FDrawNonClientArea then begin // Out the title text
      ...
      if Assigned(FTitleBar) then begin
        ...
        R.Left := SysBorderWidth(Form.Handle, BorderForm) + integer(IconVisible) * WidthOf(FIconRect) + 4 + sMan.SkinData.BILeftMargin + ShadowSize.Left + integer(FCaptionSkinIndex > -1) * 2;
      ...
      if Assigned(FTitleBar) then ShowCaption := FTitleBar.ShowCaption else ShowCaption := AddedTitle.ShowMainCaption;
      if not IsRectEmpty(R) then begin
        ...
        if ShowCaption then begin
          ...
          if ac_UseSysCharSet
            then FCommonData.FCacheBmp.Canvas.Font.Charset := GetDefFontCharSet
            else FCommonData.FCacheBmp.Canvas.Font.Charset := Form.Font.Charset;
...
//          R.Top := R.Top + (HeightOf(R) - ts.cy) div 2;
          R.Top := VCenter - ts.cy div 2;
        end
        else begin
          R.Top := TacAccessTitleBar(FTitleBar).FBarVCenter - ts.cy div 2;
        end;
        ...
        if Form.UseRightToLeftAlignment{ (Form.BiDiMode = bdRightToLeft) and SysLocale.MiddleEast} then begin
          if TextAlign = Classes.taLeftJustify then TextAlign := Classes.taRightJustify else if TextAlign = Classes.taRightJustify then TextAlign := Classes.taLeftJustify;
        end;
        ...
        if ShowCaption then begin
          ...
          // Draw a text glowing
          if not x64woAero or (SkinData.SkinManager.SkinData.Version > 7.50) then begin
...
begin
  if (csCreating in Form.ControlState) or (FormState and FS_BLENDMOVING = FS_BLENDMOVING) or (FormState and FS_ANIMCLOSING = FS_ANIMCLOSING) or (SkinData.Skinindex < 0) then Exit;
    ...
    if sMan.IsValidSkinIndex(FCommonData.SkinIndex) then begin
      ...
      if (SkinData.CtrlSkinState and ACS_FAST <> ACS_FAST) then begin // If cache is required
        ...
        if (BorderForm = nil) then begin // If not extended borders
          ...
          if not exBorders and (FCommonData.BorderIndex >= 0) then PaintBorderFast(FCommonData.FCacheBmp.Canvas.Handle, rForm, BorderWidth, FCommonData, iDrawState);
        end;
      end;
      ...
      if IsMenuVisible(Self) and MenuPresent and (MenuHeight > 0) and DrawNonClientArea and sMan.SkinnedPopups then begin
        ...
        if ChildIconPresent and (MDISkinProvider = Self) then begin
          if Form.ActiveMDIChild.Icon.Handle <> 0
            then DrawIconEx(FCommonData.FCacheBmp.Canvas.Handle, sbw + 1 + ShadowSize.Left, fCaptHeight + 1, Form.ActiveMDIChild.Icon.Handle, MenuHeight - 2, MenuHeight - 2, 0, 0, DI_NORMAL)
            else if AppIcon <> nil
        ...
        while Index < Form.Menu.Items.Count do begin
          ...
          if ((CurrentItem.GroupIndex = ChangedIndex) or ProcessMerged(CurrentItem.GroupIndex)) then begin
            ...
            if Assigned(CurrentItem.OnAdvancedDrawItem) then CurrentItem.OnAdvancedDrawItem(CurrentItem, FCommonData.FCacheBmp.Canvas, R, [odNoAccel, odReserved1]);
            inc(x, w);
            inc(Index);
            inc(VisIndex);
          end;
      ...
      if (BorderForm <> nil) then with sMan do begin // Painting of form shadow if required
        if FormActive then BorderForm.ShadowTemplate := sMan.ShdaTemplate else BorderForm.ShadowTemplate := sMan.ShdiTemplate;
        if BorderForm.ShadowTemplate <> nil then begin
          ...
          if exBorders then begin
            ...
            PaintControlByTemplate(FCommonData.FCacheBmp, BorderForm.ShadowTemplate, Rect(0, 0, fWidth, fHeight),
                ...
                Rect(ShadowSize.Left + BorderWidth, ShadowSize.Top + i, ShadowSize.Right + BorderWidth, ShadowSize.Bottom + BorderWidth + 2),
                Rect(iDrawMode, iDrawMode, iDrawMode, iDrawMode), False, False); // Uncomment in BETA

            PaintTitle;
            PaintText;

            if x64woAero and (SkinData.Version < 7.51) then begin // Fix a problem with glowing glitches on x64 PC w/o Aero
            ...
            if sMan.ConstData.ExBorder >= 0 then begin
              PaintControlByTemplate(FCommonData.FCacheBmp, BorderForm.ShadowTemplate, Rect(0, 0, fWidth, fHeight),
                Rect(0, 0, BorderForm.ShadowTemplate.Width, BorderForm.ShadowTemplate.Height),
                Rect(sMan.ma[sMan.ConstData.ExBorder].WL, ma[sMan.ConstData.ExBorder].WT, ma[sMan.ConstData.ExBorder].WR, ma[sMan.ConstData.ExBorder].WB),
                ShadowSize, // Offsets to content
                Rect(sMan.ma[sMan.ConstData.ExBorder].DrawMode, sMan.ma[sMan.ConstData.ExBorder].DrawMode, sMan.ma[sMan.ConstData.ExBorder].DrawMode, sMan.ma[sMan.ConstData.ExBorder].DrawMode), sMan.ma[sMan.ConstData.ExBorder].DrawMode <> 0, False);
            end
...
begin
  if SystemMenu = nil then PrepareForm;
  if (SystemMenu = nil) { Skin can't be loaded} {$IFDEF D2005}or (Application.ActiveFormHandle <> Form.Handle) {$ENDIF} then Exit;
...
begin
  if FLinesCount <> -1 then Result := FLinesCount else begin
    ...
    Result := 1;
    if GetMenuItemRect(Form.Handle, Form.Menu.Handle, 0, R) then y1 := R.Bottom else begin
      Result := 0;
      Exit;
    end;
    ...
    if y2 > y1 then inc(Result, (y2 - y1) div (GetSystemMetrics(SM_CYMENU) - 2));
    FLinesCount := Result;
  end;
end;
...
begin
  ...
  else if (AComponent is TsTitleBar) then case Operation of
    opInsert : begin
      if FTitleBar = nil then FTitleBar := TsTitleBar(AComponent);
    end;
    opRemove : FTitleBar := nil;
  end;
end;
...
begin
  Result := TitleIcon.Visible and ((Form.BorderStyle = bsSizeable) or (Form.BorderStyle = bsSingle)) and FShowAppIcon and (GetSystemMenu(Form.Handle, False) <> 0);
end;
...
begin
  if FUseGlobalColor <> Value then begin
    ...
    if FCommonData.Skinned and Assigned(Form) and Value and not SkinData.CustomColor
      ...
      else if Form.Color <> clBtnFace
        then Form.Color := clBtnFace;
  end;
end;
...
begin
  if not MenusInitialized then begin
    if Form.Menu <> nil then SkinData.SkinManager.SkinableMenus.InitMenuLine(Form.Menu, FDrawNonClientArea);
...
begin
  if RTInit or RTEmpty or (FCommonData.SkinManager = nil) or (Form = nil) or not Form.HandleAllocated or not FCommonData.SkinManager.SkinData.Active then Exit;
  if (Form.Menu <> nil) and Form.Menu.OwnerDraw then begin
    FMenuOwnerDraw := Form.Menu.OwnerDraw;
    MenusInitialized := True;
  end
  else FMenuOwnerDraw := False;
  FCommonData.Loaded;
...
begin
  ...
  if Assigned(SystemMenu) and HaveSysMenu then begin
    ...
    if SystemMenu.VisibleMax then inc(Result, SysButtonWidth(ButtonMax) + integer(BigButtons) * FCommonData.SkinManager.SkinData.BISpacing);
    if SystemMenu.VisibleMin then inc(Result, SysButtonWidth(ButtonMin) + integer(BigButtons) * FCommonData.SkinManager.SkinData.BISpacing);
    if (biHelp in Form.BorderIcons) then inc(Result, SysButtonWidth(ButtonHelp) + integer(BigButtons) * FCommonData.SkinManager.SkinData.BISpacing);
  end;

  if TitleButtons.Count > 0 then inc(Result, UserButtonsOffset);
  for i := 0 to TitleButtons.Count - 1 do begin
    inc(Result, UserButtonWidth(TitleButtons[i]) + integer(BigButtons) * FCommonData.SkinManager.SkinData.BISpacing);
  end;
end;
...
begin
  if (CaptForm <> nil) and not (csDestroying in CaptForm.ComponentState) and (SkinData.FCacheBmp <> nil)
...
begin
  ...
  if (aDC = 0) or (SkinData.PrintDC <> 0) and (aDC <> SkinData.PrintDC)
    then DC := GetDC(Form.Handle)
    else DC := aDC;
  try
    FCommonData.FUpdating := False;
...
begin
  ...
  if not RgnChanging and RgnChanged then begin
    ...
    if (HaveBorder(Self) or IsSizeBox(Form.Handle) or bIconic) then begin
      if bIconic then begin
        // If form is iconic but shown on taskbar only
        if GetWindowLong(Form.Handle, GWL_EXSTYLE) and WS_EX_APPWINDOW = WS_EX_APPWINDOW then Exit;
      end;

      if (BorderForm = nil) {or } then FillArOR(Self);
      RgnChanged := False;
      if (fsShowing in Form.FormState) and (Form.Position <> poDefault) and (Form.WindowState <> wsMaximized) then UpdateRgn(Self, False) else begin
        ...
        if not SkinData.BGChanged then Exit; // If not repainted
      end;
    end
    else SetWindowRgn(Form.Handle, 0, False);
  end;
  ...
  if FCommonData.BGChanged then if not PaintAll then Exit;
  DC := GetWindowDC(Form.Handle);
  SavedDC := SaveDC(DC);
  try
    if (BorderForm <> nil) then begin
...
//    if (GetWindowLong(Form.Handle, GWL_STYLE) and WS_SYSMENU = WS_SYSMENU) or HaveSysMenu then begin
      if FDrawNonClientArea then begin
        if GetWindowLong(Form.Handle, GWL_STYLE) and WS_SYSMENU = WS_SYSMENU
...
//    else //HaveSysMenu := False;
    ...
    if HaveSysMenu then begin
      ...
      SendMessage(Form.Handle, WM_SETICON, 0, SendMessage(Form.Handle, WM_GETICON, 0, 0));
    end;
//    HaveSysMenu := False;
  end;
end;
...
begin
  ...
  if (Form.FormStyle = fsMDIForm) and (Form.MDIChildCount > 0) and (Form.WindowMenu <> nil) then begin
    if Form.WindowMenu.Handle = Message.MenuPopup then begin
      for i := 0 to Form.WindowMenu.Count - 1 do begin
      ...
      for i := 0 to Form.MDIChildCount - 1 do if Form.MDIChildren[i].Visible then begin
        TmpItem := TMenuItem.Create(Self);
...
begin
  if Active and HaveBorder(Self) and DrawNonClientArea and (Form.Parent = nil) and (Form.FormStyle <> fsMDIChild) and
        FAllowExtBorders and not ((Form.WindowState = wsMaximized) and ac_NoExtBordersIfMax) then begin
    if BorderForm = nil then begin
  ...
  if FDrawNonClientArea and (Form.BorderStyle <> bsNone) and (Form.FormStyle <> fsMDIChild) and (Form.Parent = nil) then begin
    ...
    if BorderForm <> nil then begin
      if ((GetWindowLong(Form.Handle, GWL_STYLE) and WS_CAPTION = WS_CAPTION)) then begin // If for caption is visible
        FSysExHeight := IsZoomed(Form.Handle) and (ActualTitleHeight > (SysCaptHeight(Form) + 4));
...
begin
  ...
  if (TitleButtons <> nil) then for i := 0 to TitleButtons.Count - 1 do begin
    if TitleButtons[i].Data.Timer <> nil then FreeAndNil(TitleButtons[i].Data.Timer);
  end;

  if not (csDestroying in ComponentState) and Assigned(FTitleBar) then for i := 0 to FTitleBar.Items.Count - 1 do begin
    if FTitleBar.Items[i].Timer <> nil
      then FreeAndNil(FTitleBar.Items[i].Timer);
  end;
end;
...
var
  mi, i : integer;
begin
  if Form.HandleAllocated then InitDwm(Form.Handle, DrawNonClientArea);//True);
  PrepareForm;
  ...
  for i := 0 to TitleButtons.Count - 1 do
    if TitleButtons.Items[i].UseSkinData
      then TitleButtons.Items[i].Data.ImageIndex := mi
      else TitleButtons.Items[i].Data.ImageIndex := -1;
...
procedure TsSkinProvider.AC_NCMouseMove(var Message: TWMNCMouseMove);
var
  M : TWMNCHitTest;
begin
  M.Msg := WM_NCHitTest;
  ...
  case Message.HitTest of
    HTCLOSE, HTMINBUTTON, HTMAXBUTTON, HTHELP, HTUDBTN..HTUDBTN + 100, HTITEM..HTITEM + 100 : SetHotHT(Message.HitTest); // Title icon hovered
    HTCHILDCLOSE, HTCHILDMAX, HTCHILDMIN : SetHotHT(Message.HitTest); // MDI icon hovered
    else begin
      SetHotHT(0);
      OldWndProc(TMessage(Message));
    end;
...
var
  ...
  begin
    ...
    // New right Offset
    FBarRect.Right := Btn.Rect.Left - integer(BigButtons) * FCommonData.SkinManager.SkinData.BISpacing;
    inc(Index);
  end;
begin
  ...
  if (BorderForm <> nil) then begin
    ...
    VCenter := ShadowSize.Top + ActualTitleHeight div 2 + CaptionHeight - ActualTitleHeight;
    FHeaderHeight := CaptionHeight + ShadowSize.Top;
    // Center of header (vertical)
    if (Form.WindowState = wsMaximized) and (FCommonData.SkinManager.SkinData.ExTitleHeight <> FCommonData.SkinManager.SkinData.ExMaxHeight) then begin
      OffsetRect(FIconRect, 0, (FCommonData.SkinManager.SkinData.ExTitleHeight - FCommonData.SkinManager.SkinData.ExMaxHeight) div 2 - 2);
    end
    else VCenter := VCenter + FCommonData.SkinManager.SkinData.ExCenterOffs;
  end
  else begin
    VCenter := CaptionHeight div 2;
    if IsZoomed(Form.Handle) then begin
      FBarRect.Top := GetSystemMetrics(SM_CXSIZEFRAME) - 1;
    end
    else FBarRect.Top := 0;
  ...
  if IsZoomed(Form.Handle)
    then inc(VCenter, 2);
  // Calc Icon Rect
  FIconRect.Left := SysBorderWidth(Form.Handle, BorderForm) + SkinData.SkinManager.SkinData.BILeftMargin + ShadowSize.Left;
  ...
  else FIconRect.Right := FIconRect.Left; // Icon is not shown

  // Calc sys buttons and right offset of bar
  i := 1;
  FullRight := CaptionWidth - FCommonData.SkinManager.SkinData.BIRightMargin - ShadowSize.Right - SysBorderWidth(Form.Handle, BorderForm);
  ...
  if Assigned(SystemMenu) and HaveSysMenu then begin
    ...
    // Max button
    if SystemMenu.VisibleMax then begin
      ButtonMax.ImageIndex := iffi(Form.WindowState <> wsMaximized, SkinData.SkinManager.ConstData.MaskMaxBtn, SkinData.SkinManager.ConstData.MaskNormBtn);
      ProcSysButton(ButtonMax, i);
    end
    else ButtonMax.Rect.Right := ButtonMax.Rect.Left; // Is not shown
    // Min button
    if SystemMenu.VisibleMin then begin
      ButtonMin.ImageIndex := iffi((Form.WindowState = wsMinimized) and IsIconic(Form.Handle) and (Application.MainForm <> Form), FCommonData.SkinManager.ConstData.MaskNormBtn, FCommonData.SkinManager.ConstData.MaskMinBtn); // If form is minimized then changing to Normalize
      ProcSysButton(ButtonMin, i);
    end
    else ButtonMin.Rect.Right := ButtonMin.Rect.Left; // Is not shown
    if biHelp in Form.BorderIcons then begin
//      ButtonHelp.ImageIndex := SkinData.SkinManager.ConstData.MaskHelpBtn;
      dec(FBarRect.Right, 6); // Spacing between help button and others
      ProcSysButton(ButtonHelp, i);
    end
    else ButtonHelp.Rect.Right := ButtonHelp.Rect.Left; // Is not shown
  end
  else ButtonClose.Rect.Right := ButtonClose.Rect.Left; // Is not shown

  if TitleButtons.Count > 0 then begin
    FBarRect.Right := FBarRect.Right - UserButtonsOffset; // Spacing between old buttons and others
    mi := UserBtnIndex;
    Index := 0;
    for i := 0 to TitleButtons.Count - 1 do if TitleButtons.Items[i].Visible then begin
      TitleButtons.Items[i].Data.Rect.Right := FBarRect.Right;
      TitleButtons.Items[i].Data.ImageIndex := iffi(TitleButtons.Items[i].UseSkinData and FCommonData.SkinManager.IsValidImgIndex(mi), mi, -1);
  ...
  if Assigned(FTitleBar) then with FTitleBar do begin
    TacAccessTitleBar(FTitleBar).FFullRight := FullRight;
    FBarRect.Left := FIconRect.Right + integer(FIconRect.Right <> FIconRect.Left) * UserButtonsOffset;
    FBarRect.Top := GetTopCoord;
    FBarRect.Bottom := FHeaderHeight;// + 1;
    FBarRect.Right := FBarRect.Right;
    TacAccessTitleBar(FTitleBar).FBarVCenter := VCenter;

    dec(FBarRect.Right, UserButtonsOffset); // Spacing between sys buttons and others
    TacAccessTitleBar(FTitleBar).FBarRect := FBarRect;
    CalcSizes;
  end;
end;

procedure TsSkinProvider.PaintTitleItem(Item : TacTitleBarItem; R : TRect; Bmp : TBitmap; NoAlpha : boolean = True);
var
  sSection : string;
  ...
  begin
    ...
    if not Item.Enabled then ImgNdx := Item.DisabledImageIndex else case Item.State of
      0 : ImgNdx := Item.ImageIndex;
      1 : ImgNdx := Item.HotImageIndex;
      2 : ImgNdx := Item.PressedImageIndex;
    end;
    if ImgNdx < 0 then ImgNdx := Item.ImageIndex;
    if Assigned(FTitleBar.Images) and Between(ImgNdx, 0, FTitleBar.Images.Count - 1) then begin
      y := (Item.Height - FTitleBar.Images.Height);
      y := R.Top + y div 2;
      FTitleBar.Images.Draw(DstBmp.Canvas, x, y, ImgNdx);
      inc(x, integer(Item.Caption <> '') * Item.Spacing + FTitleBar.Images.Width);
    end
    else if Assigned(Item.Glyph) and not Item.Glyph.Empty then begin
      ...
      if Item.Glyph.PixelFormat = pf32bit
        then CopyBmp32(Rect(x, y, x + Item.Glyph.Width, y + Item.Glyph.Height), Rect(0, 0, Item.Glyph.Width, Item.Glyph.Height), DstBmp, Item.Glyph, EmptyCI, False, clNone, 0, False)
        else DstBmp.Canvas.Draw(x, y, Item.Glyph);
    ...
    if Item.Caption <> '' then begin
      ...
      if gSize <> 0 then acDrawGlowForText(DstBmp, PacChar(s), rText, Flags, BF_RECT, gSize, gColor, Item.FontData.GlowMask);
    ...
    if (Item.Style in [bsMenu, bsButton]) and (Item.DropDownMenu <> nil) then begin
      rText.Left := rText.Left + ts.cx + 1 + 3 * integer(Item.Style = bsMenu);
      s := acDownChar;
  ...
  procedure PaintItemSection(sIndex : integer; const sSection : string; ci : TCacheInfo; Filling : boolean; State : integer; R : TRect; pP : TPoint; ItemBmp : TBitmap; IsTab : boolean; SkinManager : TObject = nil);
  var
    LayeredWndMode : boolean;
    DrawState : integer;
  begin
    if not TsSkinManager(SkinManager).IsValidSkinIndex(sIndex) then Exit;
    with TsSkinManager(SkinManager) do begin
      LayeredWndMode := True;
      DrawState := min(min(State, TsSkinManager(SkinManager).gd[sIndex].States), 1);
      if TsSkinManager(SkinManager).gd[sIndex].Props[DrawState].Transparency = 100 then begin
        if TsSkinManager(SkinManager).gd[sIndex].BorderIndex < 0 then LayeredWndMode := False else if State = 0 then begin
          LayeredWndMode := TsSkinManager(SkinManager).ma[TsSkinManager(SkinManager).gd[sIndex].BorderIndex].DrawMode and BDM_ACTIVEONLY <> BDM_ACTIVEONLY;
        end;
      end;
    end;
    if not LayeredWndMode
      then PaintItem(sIndex, sSection, ci, Filling, State, R, pP, ItemBmp.Canvas.Handle, SkinManager)
      else PaintItem32(sIndex, Filling, State, R, pP, ItemBmp, SkinManager);
  end;
begin
  h := Item.Height;
  bR := R;
  ...
  if Item.Enabled then DstBmp := Bmp else begin
    DstBmp := TBitmap.Create;
    DstBmp.Assign(Bmp); // Copy an image for disabled (semitransparent Btn drawing)
  end;

  if Item.ContentSize.cx = 0 then begin
    Item.UpdateSize(not Item.AutoSize);
  end;

  DstBmp.Canvas.Font.Assign(Item.FontData.UsedFont);

  if sIndex >= 0 then iState := min(Item.State, min(sMan.gd[sIndex].States - 1, 1)) else iState := 0;

  if not Item.FontData.UseSysColor
    then DstBmp.Canvas.Font.Color := Item.FontData.Font.Color
    else if sMan.IsValidSkinIndex(sIndex) then DstBmp.Canvas.Font.Color := sMan.gd[sIndex].Props[iState].FontColor.Color;
  if not Item.FontData.UseSysFontName then DstBmp.Canvas.Font.Name := Item.FontData.Font.Name;
  if not Item.FontData.UseSysGlowing then begin
    ...
    if sMan.IsValidSkinIndex(sIndex) then begin
      gSize := sMan.gd[sIndex].Props[iState].GlowSize;
  ...
  if not Item.FontData.UseSysSize then DstBmp.Canvas.Font.Height := Item.FontData.Font.Height;
  if not Item.FontData.UseSysStyle then DstBmp.Canvas.Font.Style := Item.FontData.Font.Style;
  if ac_UseSysCharSet then DstBmp.Canvas.Font.Charset := GetDefFontCharSet else DstBmp.Canvas.Font.Charset := Form.Font.Charset;

  // Clear bmp for a layered window
  if FCommonData.FCacheBmp <> Bmp then FillRect32(DstBmp, bR, 0, 0);
  ...
  case Item.Style of
    bsMenu : begin
      if sIndex < 0 then begin
        ...
        if Item.FontData.UseSysColor then DstBmp.Canvas.Font.Color := clWhite;
        if Item.FontData.UseSysGlowing then begin
          gSize := 2;
          gColor := $000E5A8D;
        end;
      end
      else begin // Section is defined in a skin
        PaintItemsection(sIndex, sSection, MakeCacheInfo(SkinData.FCacheBmp), True, Item.State, bR, Point(bR.Left, bR.Top), DstBmp, False, sMan)
      end;
    end;
    bsTab : begin
      ...
      if sIndex >= 0 then begin
        ...
        PaintItemsection(sIndex, isd^.SkinSection, MakeCacheInfo(Bmp), True, Item.State, bR, Point(bR.Left, bR.Top), DstBmp, True, sMan);
        if Item.FontData.UseSysGlowing then begin
          gSize := sMan.gd[sIndex].Props[min(iState, 1)].GlowSize;
          gColor := sMan.gd[sIndex].Props[min(iState, 1)].GlowColor;
        end
      end;
    ...
    bsInfo, bsButton : begin
      PaintItemsection(sIndex, isd^.SkinSection, MakeCacheInfo(Bmp), True, Item.State, bR, Point(bR.Left, bR.Top), DstBmp, False, sMan);
      // Init default font props
      if (Item.SkinSection = '') and (Item.State = 0) then begin // Get font color from title if button is not active
        ...
        if Item.FontData.UseSysColor then DstBmp.Canvas.Font.Color := sMan.gd[TitleSkinIndex].Props[min(iState, 1)].FontColor.Color;
        if Item.FontData.UseSysGlowing then begin
          gSize := sMan.gd[TitleSkinIndex].Props[min(iState, 1)].GlowSize;
          gColor := sMan.gd[TitleSkinIndex].Props[min(iState, 1)].GlowColor;
        end;
        sIndex := TitleSkinIndex;
      end;
    end
    else iState := integer(FormActive);
  end;
  PaintContent(bR);
...
var
  ...
  begin
    ...
    if FTitleBar.Items[i].SkinSection <> '' then begin
      ...
      if isd^.SkinIndex >= 0 then Defined := True;
    end;
    // Search default skin section
    if not Defined then case FTitleBar.Items[i].Style of
      bsButton : begin
    ...
    if not Defined then case FTitleBar.Items[i].Style of
      ...
      bsTab : begin
        ...
        isd^.SkinIndex := SkinData.SkinManager.ConstData.IndexTabTop;
      end;
    end;
    if isd^.SkinIndex >= 0 then begin
      isd^.BorderIndex := SkinData.SkinManager.GetMaskIndex(isd^.SkinIndex, isd^.SkinSection, s_BordersMask);
    end;
  end;
begin
  if Assigned(FTitleBar) then begin
...
begin
  if (BorderForm <> nil) then begin
    if IsZoomed(Form.Handle) then begin
      addY := ShadowSize.Top + 4;
      Result := addY;
      if (FCommonData.SkinManager.SkinData.ExMaxHeight <> 0) and (FCommonData.SkinManager.SkinData.ExMaxHeight <> FCommonData.SkinManager.SkinData.ExTitleHeight) then begin
        inc(Result, max(0, FCommonData.SkinManager.SkinData.ExTitleHeight - FCommonData.SkinManager.SkinData.ExMaxHeight));
      end;
    end
    else begin
      Result := ShadowSize.Top + FCommonData.SkinManager.SkinData.BITopMargin;
    end;
  end
  else begin
    if IsZoomed(Form.Handle) then Result := 6 else Result := 0;
  end;
end;

function TsSkinProvider.ActualTitleHeight: integer;
begin
  if (FCommonData.SkinManager.SkinData.ExMaxHeight <> 0) 
    then Result := FCommonData.SkinManager.SkinData.ExMaxHeight
    else Result := FCommonData.SkinManager.SkinData.ExTitleHeight;
end;
...
begin
  Result := (biMinimize in FOwner.Form.BorderIcons) and not (IsIconic(FForm.Handle) and (FForm <> Application.MainForm)); // (FForm.WindowState <> wsMinimized);
end;
...
begin
  with SourceItem Do Begin
    ...
    for i := 0 to Count - 1 do Result.Add(CloneMenuItem(Items[i]));
  End;
End;
...
{$ENDIF}
  ...
  begin
    if (FOwner.SysSubMenu.Position = Position) and (FOwner.SysSubMenu.Caption <> '') and (FOwner.SysSubMenu.PopupMenu <> nil) then begin
      ...
      if Position = ipBottom then begin
        Self.Items.Add(CreateSystemItem('-', 'ac_BtmDiv', nil));
      end;
      ...
      if Position in [ipTop, ipBeforeClose] then begin
        Self.Items.Add(CreateSystemItem('-', 'ac_TopDiv', nil));
      end;
    end;
  end;
begin
...
begin
  ...
  if FOwner.HaveSysMenu then begin
    ...
    if hMenuHandle <> 0 then begin
      i := GetMenuState(hMenuHandle, SC_CLOSE, MF_BYCOMMAND);
      Result := (i <> -1);
    end;
  end
end;
...
begin
  Result := False;
  if (Self = nil) or not FOwner.HaveSysMenu then Exit;

  Result := (IsIconic(Self.FForm.Handle) {$IFDEF D2009}and not (Application.MainFormOnTaskBar and (FForm = Application.MainForm)){$ENDIF}) or (TForm(FForm).FormStyle = fsMDIChild) or
    ...
    (FForm.BorderStyle <> bsToolWindow) and FOwner.HaveSysMenu) and (biMaximize in FOwner.Form.BorderIcons);
end;
...
begin
  ...
  if IsIconic(FForm.Handle) or (TForm(FForm).FormStyle = fsMDIChild) then begin
    ...
    Result :=
      ...
      (FForm.BorderStyle <> bsSizeToolWin) and
      (FForm.BorderStyle <> bsToolWindow) and (biMinimize in FOwner.Form.BorderIcons) and FOwner.HaveSysMenu;
  end;
end;
...
begin
  ...
  Result := (TForm(FForm).FormStyle = fsMDIChild) or
    ...
    (FForm.BorderStyle <> bsNone) and
    (FForm.BorderStyle <> bsSizeToolWin) and
    (FForm.BorderStyle <> bsToolWindow)) and FOwner.HaveSysMenu;
end;
...
begin
  if FVisible <> Value then begin
    FVisible := Value;
    if not (csLoading in FOwner.ComponentState) and not (csDesigning in FOwner.ComponentState) then UpdateSkinCaption(FOwner);
  end;
end;
...
{$ENDIF}
  ...
  if Ctrl is TForm then begin
    ...
    if s = '' then sp.SkinData.SkinSection := s_Form else sp.SkinData.SkinSection := s;
    sp.PrepareForm;
...
begin
  ...
  if (Control is TWinControl) and GetBoolMsg(TWinControl(Control), AC_CTRLHANDLED) then Exit else begin
    if not (Control is TGraphicControl) then begin
      ...
      else if (Control is TForm) then begin
        CanAdd := True;
        s := s_Form;
        if Assigned(Provider.OnSkinItem) then Provider.FOnSkinItem(Control, CanAdd, s);
        Result := CanAdd;
...
var
  ...
  ClName : string;
begin
  if (Control.Tag = ExceptTag) or not CtrlIsReadyForHook(Control) or (Control.Parent = nil) or not Control.HandleAllocated or GetBoolMsg(Control, AC_CTRLHANDLED) then Exit;
    ...
    for i := 0 to SM.ThirdLists[j].Count - 1 do begin
      if SM.ThirdLists[j][i] = ClName then begin
        case j of
          ...
          ord(tpScrollControl) : begin
            if SkinData.SkinSection = '' then SkinData.SkinSection := s_PanelLow;
            ScrollWnd := TacScrollBoxWnd.Create(Control.Handle, SkinData, SM, SkinData.SkinSection);
          end;
          ord(tpUpDownBtn) : ScrollWnd := TacSpinWnd.Create(Control.Handle, SkinData, SM, SkinData.SkinSection);
          ord(tpScrollBar) : begin
            if SkinData.SkinSection = '' then SkinData.SkinSection := s_CheckBox;
            ScrollWnd := TacSBWnd.Create(Control.Handle, SkinData, SM, SkinData.SkinSection);
            if Control is TScrollBar then begin
              TacSBWnd(ScrollWnd).Control := TScrollBar(Control);
              Control.TabStop := False;
              TacSBWnd(ScrollWnd).InitScrollData;
            end;
          end;
{$IFDEF ADDWEBBROWSER}
          ord(tpWebBrowser) : begin
            if SkinData.SkinSection = '' then SkinData.SkinSection := s_CheckBox;//s_Edit;
            ScrollWnd := TacWBWnd.Create(Control.Handle, SkinData, SM, SkinData.SkinSection);
          end;
{$ENDIF}          
        end;
        Exit;
...
{$ENDIF}
  ...
  case Message.Msg of
    WM_NCMOUSEMOVE : begin
      if SkinData.SkinManager.ActiveGraphControl <> nil then begin
      ...
      case Message.Result of
        HTCLOSE, HTMINBUTTON, HTMAXBUTTON, HTHELP, HTUDBTN..HTITEM + 100 : SetHotHT(Message.Result); // Title icon hovered
        else SetHotHT(0);
      end;
    end;
    WM_MOUSEMOVE, WM_LBUTTONDOWN..WM_MBUTTONDBLCLK : if MouseAboveTheShadow(TWMMouse(Message)) then begin
      if SkinData.SkinManager.ActiveGraphControl <> nil then begin
        SkinData.SkinManager.ActiveGraphControl := nil;
      end;
      acInMouseMsg := True;
      ...
      if (FOwner is TsSkinProvider) then begin
        if not (fsModal in TAccessForm(TsSkinProvider(FOwner).Form).FormState {Report builder repainting bug} ) then SetWindowRgn(AForm.Handle, Rgn, False) else DeleteObject(Rgn);
      end
    ...
    WM_NCHITTEST : if MouseAboveTheShadow(TWMMouse(Message)) then begin
      ...
      OldBorderProc(Message);
      Message.Result := integer(Windows.HTOBJECT);
    end;
    WM_NCLBUTTONDBLCLK : begin
      if (FOwner is TsSkinProvider) then begin
      ...
      if (FOwner is TsSkinProvider) then case TWMNCHitMessage(Message).HitTest of
        ...
        HTCAPTION : with TsSkinProvider(FOwner) do begin
          if HaveSysMenu and (SystemMenu.EnabledMax or SystemMenu.EnabledRestore) or not HaveBorder(TsSkinProvider(FOwner)) and IsIconic(Form.Handle) then begin
            if (Form.WindowState = wsMaximized) or IsIconic(Form.Handle)
              then SendMessage(Form.Handle, WM_SYSCOMMAND, SC_RESTORE, 0)
              else SendMessage(Form.Handle, WM_SYSCOMMAND, SC_MAXIMIZE, 0);
            SystemMenu.UpdateItems;
        ...
        HTITEM..HTITEM + MaxByte : if (FOwner is TsSkinProvider) and Assigned(TsSkinProvider(FOwner).FTitleBar) then begin
          if TsSkinProvider(FOwner).FTitleBar.Items[TWMNCHitMessage(Message).HitTest - HTITEM].Timer <> nil
            then FreeAndNil(TsSkinProvider(FOwner).FTitleBar.Items[TWMNCHitMessage(Message).HitTest - HTITEM].Timer);
          TsSkinProvider(FOwner).SetHotHT(TWMNCHitMessage(Message).HitTest);
          TsSkinProvider(FOwner).SetPressedHT(TWMNCHitMessage(Message).HitTest);
          TsSkinProvider(FOwner).FTitleBar.Items[TWMNCHitMessage(Message).HitTest - HTITEM].DoClick;
        end;
      end;
    end;
    WM_SETCURSOR : begin
      if not Ex_WMSetCursor(TWMSetCursor(Message)) then OldBorderProc(Message);
    end;
...
{$IFNDEF NOWNDANIMATION}
    WM_SHOWWINDOW : if acLayered and (FOwner is TsSkinProvider) and not InAnimationProcess{$IFDEF D2007} and Application.MainFormOnTaskBar {$ENDIF} and TsSkinProvider(FOwner).DrawNonClientArea and (TsSkinProvider(FOwner).Form = Application.MainForm) then begin
      if (Message.WParam = 0) and (Message.LParam in [0, SW_PARENTCLOSING])then begin
        if (FormState and FS_ANIMMINIMIZING = FS_ANIMMINIMIZING) then begin
          ...
          if TsSkinProvider(FOwner).AllowAnimation and (SkinData.SkinManager.AnimEffects.FormHide.Active) and //(SkinData.SkinManager.AnimEffects.FormHide.Time > 0) and
...
{$ENDIF}
    ...
    else if (TsSkinProvider(FOwner).Form = Application.MainForm) then begin
{$IFDEF D2010}
    ...
    WM_WINDOWPOSCHANGED : begin
      ...
      if (sp <> nil) and not (csDestroying in sp.ComponentState) and not (csDestroying in sp.Form.ComponentState) and (AForm <> nil) and not (csDestroying in AForm.ComponentState) then begin
        if (sp.FormTimer = nil) and IsIconic(sp.Form.Handle) then
          if (sp.FormState and FS_ANIMMINIMIZING <> FS_ANIMMINIMIZING)         { Form.WindowState = wsMinimized}
            then FreeAndNil(AForm);

        if (sp.Form.FormStyle = fsStayOnTop) then begin
...
var
  ...
  begin
    ...
    if (FOwner is TsSkinProvider) then with FOwner as TsSkinProvider do begin
      ...
      // Title bar items
      if FTitleBar <> nil then for i := 0 to FTitleBar.Items.Count - 1 do begin
        inc(c);
        if not FTitleBar.Items[i].Visible then Continue;
        if PtInRect(FTitleBar.Items[i].Rect, p) then begin
          if FTitleBar.Items[i].Style in [bsSpacing, bsInfo] then Result := 0 else Result := c;
...
begin
  ...
  if FOwner is TsSkinProvider then begin
    ...
    CloseIconVisible := TsSkinProvider(FOwner).HaveSysMenu;
  end
  else begin
  ...
  with FOwner do begin
    ...
    if (BtnIndex > 0) then begin
      ...
      if (BtnIndex <= SysBtnCount) then begin
        case BtnIndex of
          1 : if CloseIconVisible then begin
            if FOwner is TsSkinProvider then begin
              if TsSkinProvider(FOwner).SystemMenu.VisibleClose then Result := HTCLOSE else Result := Windows.HTNOWHERE;
            end
            else if TacDialogWnd(FOwner).EnabledClose then Result := HTCLOSE else Result := Windows.HTNOWHERE;
          end;
          2 : begin
      ...
      else if (FOwner is TsSkinProvider) then begin
        if (BtnIndex <= TsSkinProvider(FOwner).TitleButtons.Count + SysBtnCount) then begin // UDF button
          BtnIndex := BtnIndex - SysBtnCount - 1;
          if TsSkinProvider(FOwner).TitleButtons.Items[BtnIndex].Enabled then Result := HTUDBTN + BtnIndex;
        end
        else if TsSkinProvider(FOwner).FTitleBar <> nil then begin
          BtnIndex := BtnIndex - SysBtnCount - TsSkinProvider(FOwner).TitleButtons.Count - 1;
          if TsSkinProvider(FOwner).FTitleBar.Items[BtnIndex].Enabled and
               ((TsSkinProvider(FOwner).FTitleBar.Items[BtnIndex].Style <> bsTab) or not TsSkinProvider(FOwner).FTitleBar.Items[BtnIndex].Down)
            then Result := HTITEM + BtnIndex;
        end;
      end;
      if Result <> DefRESULT then begin
    ...
    if not IsZoomed(Handle) and (GetWindowLong(Handle, GWL_STYLE) and WS_SIZEBOX = WS_SIZEBOX)
        and PtInRect(Rect(ShadowSize.Left - SkinData.SkinManager.SkinData.ExShadowOffs, ShadowSize.Top - SkinData.SkinManager.SkinData.ExShadowOffs, AForm.Width - ShadowSize.Right + SkinData.SkinManager.SkinData.ExShadowOffs, AForm.Height - ShadowSize.Bottom + SkinData.SkinManager.SkinData.ExShadowOffs), p) then begin
      if Between(p.Y, ShadowSize.Top - SkinData.SkinManager.SkinData.ExShadowOffs, 4{SysBorderWidth(Handle, Self, False)} + ShadowSize.Top - SkinData.SkinManager.SkinData.ExShadowOffs) then Result := HTTOP;
      if Between(p.Y, AForm.Height - SysBorderWidth(Handle, Self, False) - ShadowSize.Bottom + SkinData.SkinManager.SkinData.ExShadowOffs, AForm.Height - ShadowSize.Bottom + SkinData.SkinManager.SkinData.ExShadowOffs) then Result := HTBOTTOM;
      if Between(p.X, ShadowSize.Left - SkinData.SkinManager.SkinData.ExShadowOffs, SysBorderWidth(Handle, Self, False) + ShadowSize.Left - SkinData.SkinManager.SkinData.ExShadowOffs) then if Result = HTTOP then Result := HTTOPLEFT else if Result = HTBOTTOM then Result := HTBOTTOMLEFT else Result := HTLEFT;
      if Between(p.X, AForm.Width - SysBorderWidth(Handle, Self, False) - ShadowSize.Right + SkinData.SkinManager.SkinData.ExShadowOffs, AForm.Width - ShadowSize.Right + SkinData.SkinManager.SkinData.ExShadowOffs) then if Result = HTTOP then Result := HTTOPRIGHT else if Result = HTBOTTOM then Result := HTBOTTOMRIGHT else Result := HTRIGHT;
      if Result <> DefRESULT then begin
        Exit;
      end;
    end;
    ...
    if Between(p.Y, 0, CaptionHeight + SysBorderHeight(OwnerHandle, Self) + ShadowSize.Top) then begin
      ...
      else if PtInRect(Rect(ShadowSize.Left, ShadowSize.Top, AForm.Width - ShadowSize.Right, AForm.Height - ShadowSize.Bottom), p) then begin // Grip?
        GripVisible := False;
...
begin
  ...
  if not IsZoomed(OwnerHandle) then if (p.Y < ShadowSize.Top - SkinData.SkinManager.SkinData.ExShadowOffs) or (p.Y > AForm.Height - ShadowSize.Bottom + SkinData.SkinManager.SkinData.ExShadowOffs) or (p.X < ShadowSize.Left - SkinData.SkinManager.SkinData.ExShadowOffs) or (p.X > AForm.Width - ShadowSize.Right + SkinData.SkinManager.SkinData.ExShadowOffs) then begin
    Result := True;
...
var
  ...
  var
    ...
    var
      X, Y : integer;
    begin
      for Y := R.Top to R.Bottom do for X := R.Left to R.Right do with GetAPixel(SkinData.FCacheBmp, X, Y) do if (R = 0) and (G = 0) and (B = 0) then begin // Black color is not blurred
  ...
  begin
    fR := Rect(ShadowSize.Left - 1, ShadowSize.Top - 1, AForm.Width - ShadowSize.Right + 1, AForm.Height - ShadowSize.Bottom + 1);
    bRgn := CreateRectRgn(fR.Left, fR.Top, fR.Right, fR.Bottom);
    Offset := SkinData.SkinManager.SkinData.ExContentOffs;
    // Top left
...
begin
  with FOwner do begin
    ...
    if (sp <> nil) and (fR.Left <> sp.Form.Left) then Exit; // Window Rect is not correct
...
{      with sp.Form do begin
        fR.Left := Left;
        fR.Top := Top;
        fR.Right := fR.Left + Width;
        fR.Bottom := fR.Top + Height;
      end;}
    ...
    if FormState and FS_SIZING = FS_SIZING then Flags := Flags or SWP_NOZORDER;
...
{$ENDIF}
      ...
      if AForm.HandleAllocated then begin
        if SkinData.SkinManager.Effects.AllowAeroBluring and SkinData.SkinManager.SkinData.UseAeroBluring then begin
          MakeBluringRgn;
          SetBlurBehindWindow(AForm.Handle, True, bRgn);
        end
        else SetBlurBehindWindow(AForm.Handle, False, 0);

        if FormState and FS_BLENDMOVING = FS_BLENDMOVING then SetWindowRgn(AForm.Handle, MakeRgn, False);
        DC := GetDC(0);
        UpdateLayeredWindow(AForm.Handle, DC, @p, @FBmpSize, SkinData.FCacheBmp.Canvas.Handle, @FBmpTopLeft, clNone, @FBlend, ULW_ALPHA);
        ReleaseDC(0, DC);
        if FormState and FS_FULLPAINTING <> FS_BLENDMOVING then SetWindowRgn(AForm.Handle, MakeRgn, False); // Change rgn after blend-moving
    ...
    else SetWindowPos(AForm.Handle, iInsAfter, p.X, p.Y, 0, 0, Flags or SWP_NOSIZE);
  end;
  ExBorderShowing := False;
end;
...
var
  ...
  begin
    if sp <> nil then begin
      ...
      if (sp.LinesCount > 0) and (FormState and FS_ACTIVATE <> FS_ACTIVATE) then begin
...
procedure TacMinTimer.InitData;
  function GetWndAlpha : integer;
  begin
{$IFDEF DELPHI7UP}
    if sp.Form.AlphaBlend then Result := sp.Form.AlphaBlendValue else
{$ENDIF}
    Result := MaxByte;
  end;
begin
...
{$ENDIF}
      ...
      if (sp.Form.Menu <> nil) and (sp.BorderForm = nil) and AeroIsEnabled then sp.SkinData.BGChanged := True; // Menu repaint
...
destructor TacSysSubMenu.Destroy;
begin
  inherited;
end;
