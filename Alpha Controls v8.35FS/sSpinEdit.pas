...
uses Windows, Classes, StdCtrls, ExtCtrls, Controls, Messages, SysUtils,
  Forms, Graphics{$IFDEF DELPHI7UP}, Types{$ENDIF}, Menus, sEdit, acntUtils, sConst, buttons, sGraphUtils, sSpeedButton;
...
{$IFNDEF NOTFORHELP}
  ...
  protected
    ...
    procedure UpClick (Sender: TObject); override;
    procedure DownClick (Sender: TObject); override;
    procedure SetDecimalPlaces(New:Integer); virtual;
  public
    constructor Create(AOwner: TComponent); override;
    procedure CreateWnd; override;
  published
...
{$IFDEF DELPHI7UP}
      if BordWidth = 0 then begin
        if FOwner.BevelInner <> bvNone then inc(BordWidth);
        if FOwner.BevelOuter <> bvNone then inc(BordWidth);
      end;
{$ENDIF}
    ...
    WM_PAINT : if FOwner.SkinData.Skinned and not FOwner.Enabled then begin
...
begin
  Result := (FAllowNegative or (Key <> '-')) and
    CharInSet(Key, [{$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator, '+', '-', '0'..'9']) or ((Key < #32) and (Key <> Chr(VK_RETURN)));
...
begin
  ...
  Loc.Right := Width - (FButton.Width * Ord(FButton.Visible)) - 3;
  Loc.Top := 0;
  Loc.Left := 0;
...
begin
  if (NewValue < 0) and not FAllowNegative then NewValue := max(0, FMinValue);
...
begin
  if (NewValue < 0) and not FAllowNegative then Result := max(0, FMinValue) else Result := NewValue;
...
begin
  ...
  case Key of
    ...
    Char(VK_BACK), Chr(VK_ESCAPE) : begin
      Result := True;
      Exit;
    end;
    '-' : if not fAllowNegative then begin
      Key := #0;
      Exit;
    end
    else if (Pos('-', Text) <= 0) or (SelLength >= Length(Text) - 1) then begin
...
begin
  ...
  if not TextChanging then begin
    SendMessage(Handle, WM_SETTEXT, 0, LPARAM(PChar(FloatToStrF(CheckValue(FValue), ffFixed, 18, fDecimalPlaces))));
...
begin
  ...
  if FUseSystemDecSeparator then bIsDecSeparator := Key = AnsiChar({$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator) else bIsDecSeparator := CharInSet(Key, ['.', ',']);
...
begin
  ...
  if not (csLoading in ComponentState) and not ValueChanging then begin
    TextChanging := True;
    if (Text = '') or (Text = '-') then Value := 0 else begin
...
{$ENDIF}
  inherited;
  if Message.MSG = SM_ALPHACMD then case Message.WParamHi of
    AC_REMOVESKIN, AC_SETNEWSKIN, AC_REFRESH : if (ACUInt(Message.LParam) = ACUInt(SkinData.SkinManager)) then begin
    ...
    WM_PAINT : if SkinData.Skinned then begin
      ...
      if not SkinData.FUpdating and Enabled then begin
...
begin
  if (Glyph = nil) or Glyph.Empty then begin
    if SkinData.SkinManager.IsValidSkinIndex(SkinData.SkinIndex) and not FOwner.FOwner.SkinData.CustomFont then begin
...
begin
  Result := False;
  if not FEditorEnabled or ReadOnly or CharInSet(Key, [Chr(VK_ESCAPE), Chr(VK_RETURN), #0]) then begin
    Key := #0;
    Exit;
  end;
...
begin
  ...
  if BorderStyle = bsSingle
    then PaintItem(SkinData, BGInfoToCI(@BGInfo), True, integer(ControlIsActive(SkinData)), Rect(0, 0, Width, Height), Point(Left, top), SkinData.FCacheBmp, False)
    else PaintItemBG(SkinData, BGInfoToCI(@BGInfo), integer(ControlIsActive(SkinData)), Rect(0, 0, Width, Height), Point(Left, Top), SkinData.FCacheBmp);
  ...
  PaintText;
  if not Enabled or not ControlIsActive(SkinData) then begin
    bw := integer(BorderStyle <> bsNone) * (1 + integer(Ctl3d));
    SkinData.FCacheBmp.Canvas.Lock;
    if ShowSpinButtons then FButton.PaintTo(SkinData.FCacheBmp.Canvas.Handle, Point(FButton.Left + bw, FButton.Top + bw));
    SkinData.FCacheBmp.Canvas.UnLock;
    if not Enabled then BmpDisabledKind(SkinData.FCacheBmp, DisabledKind, Parent, GetParentCache(SkinData), Point(Left, Top));
  end;
...
begin
  ...
  inherited;
  if Key <> #0 then ReplaceAtPos(FPos, Key);
  Key := #0;
  IncPos;
end;
...
begin
  inherited;
  Message.Result := Message.Result and not DLGC_WANTALLKEYS;// or DLGC_WANTTAB;
end;
...
begin
  inherited;
  if not ReadOnly and (Message.Result = 0) then begin
    if Message.WheelDelta < 0 then DownClick(Self) else if Message.WheelDelta > 0 then UpClick(Self);
  end;
end;
...
begin
  if ControlCount <> 0 then begin
    ...
    for i := 0 to ControlCount - 1 do if not (Controls[i] is TsSpinButton) then
