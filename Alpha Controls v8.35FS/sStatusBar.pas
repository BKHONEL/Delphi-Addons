...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms{$IFNDEF DELPHI5}, Types{$ENDIF},
  Dialogs, sConst, {$IFDEF LOGGED}sDebugMsgs,{$ENDIF} ComCtrls, sCommonData{$IFDEF TNTUNICODE}, TntComCtrls{$ENDIF};
...
begin
  ...
  if Assigned(FCommonData.SkinManager) and FCommonData.SkinManager.IsValidImgIndex(FCommonData.SkinManager.ConstData.GripRightBottom) then begin
    GripPos := point(Width - WidthOfImage(FCommonData.SkinManager.ma[FCommonData.SkinManager.ConstData.GripRightBottom]) - BorderWidth, Height - HeightOfImage(FCommonData.SkinManager.ma[FCommonData.SkinManager.ConstData.GripRightBottom]) - BorderWidth);
    DrawSkinGlyph(FCommonData.FCacheBmp, GripPos, 0, 1, FCommonData.SkinManager.ma[FCommonData.SkinManager.ConstData.GripRightBottom], MakeCacheInfo(FCommonData.FCacheBmp));
  end;
end;
...
{$ENDIF}
  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned then inherited else begin
    ...
    inherited;
    case Message.Msg of
      WM_USER + 11 {Text changed}, DM_SETDEFID, 1033, 1034 : SkinData.BGChanged := True;
