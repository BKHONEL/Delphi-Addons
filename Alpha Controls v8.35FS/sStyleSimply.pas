...
//    ExtShadowColor : TColor;
  ...
  TConstantSkinData = record
    ...
    // Sys icons
    MaskCloseBtn      : smallint;
    MaskCloseAloneBtn : smallint;
    MaskMaxBtn        : smallint;
    MaskNormBtn       : smallint;
    MaskMinBtn        : smallint;
    MaskHelpBtn       : smallint;
    MaskCloseSmall : smallint;
    MaskMaxSmall   : smallint;
    MaskNormSmall  : smallint;
    MaskMinSmall   : smallint;
    MaskHelpSmall  : smallint;
    // Arrows masks
    ...
    if (HookedComponents[i] <> nil) and (HookedComponents[i] is TCustomForm) and (TCustomForm(HookedComponents[i]).WindowState <> wsMinimized) and
      ...
      if (sp <> nil) and (sp.SkinData.SkinManager = SkinManager) then begin
        if (sp.BorderForm <> nil) and (sp.BorderForm.AForm <> nil) then begin
          ...
          if sp.CoverBmp <> nil then FreeAndNil(sp.CoverBmp);
...
begin
  ...
  if TsSkinManager(SkinManager).IsDefault and (TsSkinManager(SkinManager).AnimEffects.SkinChanging.Active) then InAnimationProcess := True;
  while i <= Length(HookedComponents) - 1 do begin
    if (HookedComponents[i] <> nil) and (HookedComponents[i] is TForm) and (TCustomForm(HookedComponents[i]).WindowState <> wsMinimized) and
         (TForm(HookedComponents[i]).FormStyle <> fsMDIChild) and (TForm(HookedComponents[i]).Parent = nil) and
         TForm(HookedComponents[i]).HandleAllocated and IsWindowVisible(TForm(HookedComponents[i]).Handle){TControl(HookedComponents[i]).Visible} then begin
      sp := TAccessSkinProvider(SendAMessage(TWinControl(HookedComponents[i]), AC_GETPROVIDER));
      if (sp <> nil) and (sp.SkinData.SkinManager = SkinManager) then begin
        sp.FInAnimation := True;
    ...
    if (HookedComponents[i] <> nil) and (HookedComponents[i] is TCustomForm) and (TForm(HookedComponents[i]).FormStyle <> fsMDIChild) and (TForm(HookedComponents[i]).Parent = nil) and
      ...
      if (sp <> nil) and (sp.SkinData.SkinManager = SkinManager) then begin
        if sp.FormState and FS_LOCKED <> FS_LOCKED then begin
          inc(i);
          Continue; // FS_LOCKED is defined in the LockForms procedure
        end;
...
{$ENDIF}
            ...
            if (sp.BorderForm <> nil) and (sp.Form.WindowState = wsMaximized) then begin // Title height may be changed
              if iLatestTitleHeight <> TAccessProvider(sp).ActualTitleHeight then begin // If height of title is really changed
                iLatestTitleHeight := TAccessProvider(sp).ActualTitleHeight;
            ...
            if sp.DrawNonClientArea then begin
              ...
              RedrawWindow(TWinControl(HookedComponents[i]).Handle, nil, 0, Flags); // Redraw main form under layered form
              if TAccessSkinProvider(sp).BorderForm <> nil then begin
                SetWindowRgn(TAccessSkinProvider(sp).BorderForm.AForm.Handle, TAccessSkinProvider(sp).BorderForm.MakeRgn, False); // Avoiding a flickering
                TAccessSkinProvider(sp).BorderForm.UpdateExBordersPos;
              end;
            end
