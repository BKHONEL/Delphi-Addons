...
begin
  if SendMessage(Handle, TB_GETITEMRECT, Index, LPARAM(@Result)) = 0
...
{$ENDIF}
      acWriteTextEx(BtnBMP.Canvas, PacChar(Button.Caption), Enabled and (DisabledImages = nil), cRect, DT_CENTER or DT_VCENTER or DT_SINGLELINE, GetFontIndex(Button, SkinIndex, SkinData.SkinManager), Mode > 0);
    end;
  end;
...
begin
  if FCommonData.Skinned then begin
    ...
    if (Stage in [cdPrePaint]) then begin
      ...
      if Flat then Section := s_TOOLBUTTON else Section := s_SPEEDBUTTON;
      SkinIndex := FCommonData.SkinManager.GetSkinIndex(Section);
      ...
      PaintItemBg(SkinIndex, Section, ci, Mode, R, Point(Button.Left, Button.Top), BtnBmp, FCommonData.SkinManager);
      R.Right := bWidth - AddedWidth;

      ci.X := ci.X + Button.Left;
      ci.Y := ci.Y + Button.Top;
      BorderIndex := FCommonData.SkinManager.GetMaskIndex(SkinIndex, Section, s_BordersMask);
...
begin
  if FCommonData.Skinned then begin
    if InUpdating(FCommonData) or (csDestroying in ComponentState) then Exit;
...
{$IFDEF LOGGED}
  AddToLog(Message);
{$ENDIF}
  if HandleAllocated and (Message.Msg = SM_ALPHACMD) then case Message.WParamHi of
...
//        if FCommonData.BGChanged then
        ...
        if aSkinChanging then SendMessage(Parent.Handle, WM_SETREDRAW, 0, 0); // Preventing of blinking when skin is changed
        SendMessage(Handle, WM_PAINT, Message.WParam, 0);
        if aSkinChanging then SendMessage(Parent.Handle, WM_SETREDRAW, 1, 0);
      ...
      CN_NOTIFY : with TWMNotify(Message) do case TWMNotify(Message).NMHdr^.code of
        TBN_DROPDOWN: with PNMToolBar(NMHdr)^ do if Perform(TB_GETBUTTON, iItem, LPARAM(@tbButton)) <> 0 then begin
...
{$IFDEF DELPHI7UP}
      WM_LBUTTONDBLCLK : if Customizable then begin // Remove a custom item drawing for avoiding a bug in VCL
        if not (csDesigning in ComponentState) then inherited OnAdvancedCustomDrawButton := nil;
      end;
{$ENDIF}
    end;
    ...
    case Message.Msg of
{$IFDEF DELPHI7UP}
      WM_LBUTTONDBLCLK : if Customizable then begin // Restore a custom drawing
        UpdateEvents;
      end;
{$ENDIF}
      WM_PAINT : if not InAnimationProcess then begin
        ...
        if SkinData.SkinManager.IsValidSkinIndex(w) then begin
