...
uses ShellAPI, Consts, sMessages, sStyleSimply, acPathDialog, acShellCtrls, sSkinManager, sGlyphUtils,
  ExtDlgs, sGraphUtils;
...
begin
  Result := not IsEmptyStr(Text, [#0, ' ', {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DateSeparator, FBlanksChar]);
end;
...
begin
  ...
  if TextStored then try
    FFormatting := True;
    try
      if FourDigitYear then i := 4 else i := 2;
      l := Length(Text) - i + 1;
      if (Copy(Text, l, 1) = ' ') or (Copy(Text, l + 1, 1) = ' ') then begin
        s := Text;
        Delete(s, l, i);
        l := ExtractYear(SysUtils.Date);
        Text := s + IntToStr(l);
      end
      else if FourDigitYear then begin
        if (Copy(Text, l + 2, 1) = ' ') or (Copy(Text, l + 3, 1) = ' ') then begin
          s := Text;
...
{$IFDEF DELPHI7UP}
      ...
      if Result then SetDate(D) else Raise EConvertError.CreateFmt(acs_InvalidDate, [Text]);
{$ELSE}
...
begin
  ...
  s := Text;
  if Pos(' ', s) > 0
    then Result := NullDate
    else Result := StrToDateFmtDef(FDateFormat, s, Result);
end;
...
var
  i : integer;
begin
  if (FPopupWindow <> nil) and (TsPopupCalendar(FPopupWindow).FCalendar <> nil) then with TsPopupCalendar(FPopupWindow) do begin
    ...
    if Assigned(FOnGetCellParams)
      then sMonthCalendar1.OnGetCellParams := FOnGetCellParams
      else sMonthCalendar1.OnGetCellParams := nil;

    if Assigned(FOnDrawDay)
      then sMonthCalendar1.OnDrawDay := FOnDrawDay
      else sMonthCalendar1.OnDrawDay := nil;

    if Assigned(FCloseUp)
      then OnCloseUp := FCloseUp
      else OnCloseUp := nil;

    if SkinData.Skinned and (DefaultManager <> nil)
      then Color := DefaultManager.GetGlobalColor
...
begin
  ...
  else if (sPopupCalendar <> nil) and (sPopupCalendar <> FPopupWindow) then begin
...
begin
  inherited;
  if not AutoSelect or (CaretPos(Message.XPos) < Length(Text)) then case CaretPos(Message.XPos) of
    1..3 : begin
      SelStart := 0;
      SelLength := 2;
    end;
...
begin
  pos := 0;
  if FourDigitYear then l := 10 else l := 8;
  for i := 1 to l do begin
    case i of
      3, 6 : begin
        w := GetStringSize(Font.Handle, ',').cx;
      end
      else begin
        w := GetStringSize(Font.Handle, '0').cx;
      end;
    end;
...
function StrToDateFmt(const DateFormat, S: string): TDateTime;
begin
  if not InternalStrToDate(DateFormat, S, Result) then Raise EConvertError.CreateFmt(acs_InvalidDate, [S]);
end;
