...
uses
  ...
  sFade{$IFDEF LOGGED}, sDebugMsgs{$ENDIF}{$IFDEF TNTUNICODE}, TntComCtrls {$ENDIF};
...
{$ENDIF}
  ...
  if not ControlIsReady(Self) or not FCommonData.Skinned(True) then inherited else begin
    case Message.Msg of
      ...
      WM_SIZE : if FadeTimer <> nil then StopFading(FadeTimer, FCommonData);
