...
begin
  ...
  if not (csDesigning in ComponentState) then begin
    if Assigned(OnAdvancedCustomDrawItem) and not Assigned(FOldDrawItem) then begin
      ...
      if addr(TempEvent) <> addr(OnAdvancedCustomDrawItem)
        then FOldDrawItem := OnAdvancedCustomDrawItem;
    end;
...
var
  ...
  cx : integer;
begin
  if (csDesigning in ComponentState) or not DefaultDraw then Exit;
  DefaultDraw := True;
  if Assigned(FOldDrawItem) then FOldDrawItem(Sender, Node, State, Stage, PaintImages, DefaultDraw);
  if not DefaultDraw then Exit;
{
  if HotTrack and (Stage in [cdPrePaint]) then begin
    // Clear bg for image redrawing
  ...
  if SkinData.Skinned and (Stage in [cdPostPaint]) then begin
    DefaultDraw := False;
    ...
    if ((Selected = Node) or HotTrack and (cdsHot in State)) and (Focused or not HideSelection) then begin
      ...
      if sNdx < 0
        ...
        else PaintItem(sNdx, s_Selection, CI, True, integer(Focused), aRect, Point(0, 0), Bmp, SkinData.SkinManager);
...
{$IFDEF TNTUNICODE}
      AcDrawText(Bmp.Canvas.Handle, TTntTreeNode(Node).Text, aRect, DrawStyle);
{$ELSE}
...
{$ENDIF}
        else if ListSW <> nil then TacTreeViewWnd(ListSW).Color := Color;
