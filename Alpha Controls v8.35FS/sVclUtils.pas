...
uses
  ...
  comctrls, sConst, Menus, inifiles, acntUtils, sCommonData, acDials, acThdTimer{$IFNDEF DELPHI5}, Types{$ENDIF},
{$IFNDEF ALITE}
  sEdit, sMemo, sComboBox, sCurrEdit, sDateUtils,
  sCustomComboEdit, sRadioButton, 
{$ENDIF}
...
function SendAMessage(Handle : hwnd; Cmd : Integer; LParam : LPARAM = 0) : LRESULT; overload; // may be removed later
function SendAMessage(Control : TControl; Cmd : Integer; LParam : LPARAM = 0) : LRESULT; overload;
...
{$ENDIF}
  ...
  if sp.BorderForm <> nil then sp.BorderForm.ExBorderShowing := True; // Do not update extended borders
  ...
  FreeAndNil(acDstBmp);
  if AeroIsEnabled then Sleep(4 * acTimerInterval); // Blinking in Aero removing
  SetWindowPos(AnimForm.Handle, sp.Form.Handle, 0, 0, 0, 0 {fR.Left - cx, fr.Top - cy, FBmpSize.cx, FBmpSize.cy}, Flags or SWP_NOSIZE or SWP_NOMOVE);
  if sp.BorderForm = nil then begin
    FreeAndNil(AnimForm);
  end
  else begin
    SetWindowRgn(AnimForm.Handle, sp.BorderForm.MakeRgn, False);
    if sp.SkinData.SkinManager.SkinData.UseAeroBluring then sp.BorderForm.UpdateExBordersPos(True);
  end;
...
begin
  ...
  if GetWindowLong(Form.Handle, GWL_EXSTYLE) and WS_EX_TOPMOST = WS_EX_TOPMOST then begin
    h := HWND_TOPMOST
  end
  else begin
    h := GetWindow(Form.Handle, GW_HWNDPREV);
  end;
...
{$IFDEF DELPHI7}
  if sp.Form.AlphaBlend then b := sp.Form.AlphaBlendValue else
{$ENDIF}
  b := MaxByte;   
  ...
  if sp.BorderForm <> nil then begin
    if sp.BorderForm.AForm = nil then sp.BorderForm.CreateNewForm;
    AnimForm := sp.BorderForm.AForm;
    if sp.BorderForm.ParentHandle <> 0 then SetWindowLong(sp.BorderForm.AForm.Handle, GWL_HWNDPARENT, LONG_PTR(sp.BorderForm.ParentHandle));
...
//  if b < MaxByte then
  DoLayered(sp.Form.Handle, False, b);
  if (sp.BorderForm <> nil) then begin
...
begin
  ...
  if ListSW.BorderForm <> nil then begin
    ...
    if ListSW.BorderForm.ParentHandle <> 0 then SetWindowLong(ListSW.BorderForm.AForm.Handle, GWL_HWNDPARENT, LONG_PTR(ListSW.BorderForm.ParentHandle));
...
begin
  ...
  if ListSW.BorderForm <> nil then begin
    ...
    ListSW.BorderForm.UpdateExBordersPos(True);
  end
...
begin
  ...
  if acLayered and (Ctrl is TCustomForm) then begin
    ...
    if sp.BorderForm <> nil then begin
      ...
      if sp.BorderForm.ParentHandle <> 0 then SetWindowLong(sp.BorderForm.AForm.Handle, GWL_HWNDPARENT, LONG_PTR(sp.BorderForm.ParentHandle)); // Patch for ReportBuilder and similar windows
...
var
  i : integer;
begin
  if (csDesigning in wc.ComponentState) then Exit;
  if not InAnimationProcess then begin
    i := 0;
    while i < wc.ControlCount do begin
      if {not (wc.Controls[i] is TGraphicControl) and }not (csDestroying in wc.Controls[i].ComponentState) and Assigned(wc.Controls[i]) then begin
        ...
        else if wc.Controls[i] is TControl then begin
...
var
  ...
  function ControlIsReady(Control : TControl) : boolean;
  begin
    Result := (Control.Visible or (csDesigning in Control.ComponentState)) and (Control is TGraphicControl) and (Control.Width > 0) and (Control.Height > 0);
    Result := Result and not (csNoDesignVisible in Control.ControlStyle) and not (csDestroying in Control.ComponentState) and
             not ((Control is TToolButton) and (TToolButton(Control).Style in [tbsCheck, tbsButton, tbsDropDown]));
    Result := Result and RectVisible(DC, Control.BoundsRect);
  end;
begin
          ...
          for j := 0 to Count - 1 do // Copy parent BG
            if ControlIsReady(OwnerControl.Controls[J]) then begin
              if not (csOpaque in OwnerControl.Controls[J].ControlStyle) {???or (OwnerControl.Controls[J] is TGraphicControl)} then begin
                if (BGInfo.BgType = btCache) and (BGInfo.Bmp <> nil) // If with cache
                  then BitBlt(MemDC, OwnerControl.Controls[J].Left, OwnerControl.Controls[J].Top, OwnerControl.Controls[J].Width, OwnerControl.Controls[J].Height, BGInfo.Bmp.Canvas.Handle, OwnerControl.Controls[J].Left + BGInfo.Offset.X, OwnerControl.Controls[J].Top + BGInfo.Offset.Y, SRCCOPY)
                  else begin
                    if (BGInfo.Bmp <> nil) then begin
                      ...
                      if bRect.Bottom > (BGInfo.Bmp.Height - BGInfo.FillRect.Bottom) then begin
                      ...
                      if bRect.Right > (BGInfo.Bmp.Width - BGInfo.FillRect.Right) then begin
                    ...
                    else FillDC(MemDC, OwnerControl.Controls[J].BoundsRect, BGInfo.Color);
                  end;
              end
...
function SendAMessage(Handle : hwnd; Cmd : Integer; LParam : LPARAM = 0) : LRESULT; overload;
begin
...
//    if Control is TWinControl
    Control.Perform(SM_ALPHACMD, MakeWParam(0, AC_SETSECTION), LPARAM(@si));
  end;
end;
...
initialization
  ...
  if uxthemeLib <= HINSTANCE_ERROR then uxthemeLib := 0;
