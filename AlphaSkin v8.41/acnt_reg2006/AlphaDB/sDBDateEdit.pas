...
begin
  ...
  if ((Key = VK_DELETE) or ((Key = VK_INSERT) and (ssShift in Shift))) or
    ...
    ((GetKeyState(VK_Down) and 128) = 128) then begin
    if not ReadOnly then FDataLink.Edit else begin
      Reset;
      Key := 0;
    end;
  end;
...
begin
  inherited;
  if CharInSet(Key, [#32..#255]) and (FDataLink.Field <> nil) and not CharInSet(Key, ['0'..'9']) and (Key <> {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DateSeparator) then begin
    SysUtils.Beep;
    Key := #0;
  end;
  case Key of
  ...
  if ReadOnly and ((Key=^V) or (Key=^X)) then
  begin
    Reset;
    Key := #0;
  end;
