...
//  if Column.Alignment = taLeftJustify then Alignment := Column.Title.Alignment else
  ...
  else if (SkinData.SkinIndex >= 0) and not SkinData.CustomFont then Canvas.Font.Color := SkinData.SkinManager.gd[SkinData.SkinIndex].Props[0].FontColor.Color;
  ...
  if FDefaultDrawing then begin
    if (gdSelected in State) and Focused and not bgChanged then begin
      if SkinData.Skinned then begin
...
begin
  ...
  case Message.Msg of
    ...
    WM_PRINT : if SkinData.Skinned then begin
      ...
      try
        if BorderStyle = bsNone then MoveWindowOrg(TWMPaint(Message).DC, -2, -2);
  ...
  if Message.ScrollCode = SB_THUMBTRACK then begin
    ...
    begin
      ...
      if SI.nTrackPos <= 1 then
        NewRecNo := 1
      else if SI.nTrackPos >= DataSource.DataSet.RecordCount then
...
procedure TsDBGrid.UpdateScrollBar;
var
  SIOld, SINew: TScrollInfo;
  ScrollBarVisible: Boolean;
begin
  if Datalink.Active and HandleAllocated then with Datalink.DataSet do
    begin
