...
uses sGlyphUtils, acntUtils{$IFNDEF DELPHI5}, MaskUtils{$ENDIF};
...
begin
  ...
  if CharInSet(Key, [#32..#255]) and (FDataLink.Field <> nil) and not CharInSet(Key, ['0'..'9']) and (Key <> {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator) and (Key <> '-') then begin
    MessageBeep(16);
    Key := #0;
  end;
  if Key = {$IFDEF DELPHI_XE3}FormatSettings.{$ENDIF}DecimalSeparator then FDataLink.Edit;
  case Key of
...
begin
  if (FDataLink <> nil) and (FDataLink.Field <> nil) and FDataLink.Field.IsNull
    then Result := ''
    else Result := FormatFloat(DisplayFormat, Value);
end;
