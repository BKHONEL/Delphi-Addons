...
//    property The3DRect: Boolean read FDrawCellByThemes write FDrawCellByThemes;
  end;
...
//  TCustomDBAxisGridEh = class(TCustomDBAxisGridEh, IMTEventReceiverEh,
...
uses DBConsts, Dialogs, DBGridEhImpExp, Clipbrd,
...
//var
//  VarColCellParamsEh: TAxisColCellParamsEh;
...
//    if AxisBar.UseRightToLeftAlignment
...
//        FPopupMonthCalendar.Font := Font;
      {try
        ...
        FPopupMonthCalendar.Date := Grid.AxisBars[CurColumn.Field.AsDateTime;
      end;}
        try
          if Text = '' then
      ...
      if Assigned(AxisBar.Field) then
        ...
        { Show the dropdown button only if the field is editable }
          ...
          //MasterField := Dataset.FieldByName(KeyFields);
            ...
            if Assigned(AxisBar.Picklist) and (AxisBar.PickList.Count > 0) and
              not AxisBar.Readonly {and not Assigned(AxisBar.ImageList)}
              then Result := esPickList
            else if (DataType in [ftDate, ftDateTime{$IFDEF EH_LIB_6}, ftTimeStamp{$ENDIF}]) and not AxisBar.Readonly
...
begin
  if (Grid <> nil) and (Grid.AxisBars <> nil) and
   (Grid.AxisBars.Count > 0) and (Grid.SelectedIndex < Grid.AxisBars.Count)
...
(*  for i := AxisBar.EditButtons.Count downto 0 do
    with FEditButtonControlList[i] do
    begin
...
//        AxisBar.EditButtons[i - 1].OnClick(Sender, Handled);
      end;
...
//    if Msg.wParam = Integer(Sender) then
  ...
  begin
    ...
    for i := 1 to Length(FEditButtonControlList) - 1 do
      if (Sender = FEditButtonControlList[i].EditButtonControl) then
      begin
        if Assigned(AxisBar.EditButtons[i - 1].OnDown) then
          AxisBar.EditButtons[i - 1].OnDown(Sender, TopButton, AutoRepeat, Handled);
        if not Handled then
          if Assigned(AxisBar.EditButtons[i - 1].DropdownMenu) then
          begin
            P := TControl(Sender).ClientToScreen(Point(0, TControl(Sender).Height));
            if AxisBar.EditButtons[i - 1].DropdownMenu.Alignment = paRight then
              Inc(P.X, TControl(Sender).Width);
            AxisBar.EditButtons[i - 1].DropdownMenu.Popup(p.X, p.y);
...
begin
  ...
  if Result then
  begin
    Result := Result and (SelectedIndex < AxisBars.Count);
...
//      InflateRect(ATextRect, -1, -1);
  ...
  begin
    ...
    begin
      ...
      with ColCellParamsEh do
        WriteCellText {WriteTextEh}(AxisBar, Canvas, ARect1, not IsCellFilled,
...
//TODO TODO  if not ColCellParamsEh.BlankCell then
...
{function TCustomDBAxisGridEh.IsActiveControl: Boolean;
var
  H: Hwnd;
...
begin
  ...
  begin
    ...
    Params.HintPos.X := Params.HintPos.X + (3 - 2); // + (Style.GetDataCellHorzOffset(Self, Column) - 2);
    if AxisBar.UseRightToLeftAlignment then
...
(*
procedure TCustomDBAxisGridEh.AdjustClientRect(var Rect: TRect);
begin
