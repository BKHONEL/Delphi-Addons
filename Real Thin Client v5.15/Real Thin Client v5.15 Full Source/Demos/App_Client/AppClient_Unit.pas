...
procedure TForm1.RtcClientConnect(Sender: TRtcConnection);
  ...
  if not Sender.inMainThread then
    ...
    Label10.Caption:='Connected to '+Sender.sPeerAddr+':'+Sender.sPeerPort;
    end;
  end;
...
procedure TForm1.eModuleChange(Sender: TObject);
  begin
  if (RtcClientModule1.ModuleHost<>AnsiString(eModuleHost.Text)) or
     (RtcClientModule1.ModuleFileName<>AnsiString(eModule.Text))then
    begin
    RtcClient.SkipRequests;

    RtcClientModule1.ModuleFileName:=AnsiString(eModule.Text);
    RtcClientModule1.ModuleHost:=AnsiString(eModuleHost.Text);
  ...
  if assigned(Sender) then
    ...
    for a:=0 to cnt-1 do
      ...
      with CliCon[a] do
        ...
        if (ServerAddr<>AnsiString(eServer.Text)) or
           (ServerPort<>AnsiString(ePort.Text)) or
  ...
  if LoopCnt>=1000 then // log max once every 1000 requests
    ...
    if LoopTime=0 then
      ...
      if LoopTime<1000 then // do not log more often than once every 5 seconds
        ...
        // Log ...
        Log(AnsiString( IntToStr((LoopDiff-LoopStart) div 1000 )+ // Time (sec)
...
procedure TForm1.MultiResultReturn(Sender: TRtcConnection; Data,Result: TRtcValue);
  ...
  if Result.isNull then
    ...
    begin
    Log('Exception from Server!'#13#10+AnsiString(Result.asException));
    end
