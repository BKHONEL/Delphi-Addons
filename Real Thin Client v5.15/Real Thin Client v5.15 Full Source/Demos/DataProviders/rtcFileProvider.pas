...
type
  ...
  protected
    function GetDocRoot(Host: AnsiString): String;
    function GetContentType(FName: String): AnsiString;
...
function TFile_Provider.GetContentType(FName: String): AnsiString;
  var
    loc:integer;
    ext:String;
  begin
  ...
  if loc>=0 then
    Result:=AnsiString(TStringObject(ExtList.Objects[loc]).value)
  else
    begin
    loc:=ExtList.IndexOf('*');
    if loc>=0 then
...
function TFile_Provider.GetDocRoot(Host: AnsiString): String;
  var
    loc:integer;
  begin
  Host:=UpperCase(Host);
...
procedure TFile_Provider.FileProviderCheckRequest(Sender: TRtcConnection);
  ...
  procedure CheckDiskFile(Sender: TRtcDataServer);
    var
      ...
      rang_type, rang_from, rang_to:AnsiString;
    ...
    function RepairFileName(FileName:String):String;
      ...
      if (Pos('\..',FullName)>0) or
        ...
        XLog('DENY '+Sender.PeerAddr+' > '+Sender.Request.Host+
             ...
             ' AGENT "'+Sender.Request.Agent+'" > Invalid FileName: "'+AnsiString(FullName)+'".');
    ...
    procedure ParseRange(s:AnsiString; var r_type, r_from, r_to:AnsiString);
      var
        a:integer;
      begin
      r_type:=''; r_from:=''; r_to:='';
      ...
      if MyFileName<>'' then
        ...
        if fsize>0 then
          ...
          XLog('FAIL '+Sender.PeerAddr+' > '+Sender.Request.Host+
               ...
               ' 0'+
               ' REF "'+Sender.Request.Referer+'"'+
               ' AGENT "'+Sender.Request.Agent+'" > File not found: "'+AnsiString(MyFileName)+'".');
      ...
      if Response.DataOut<Response.DataSize then // need to send more content
        ...
        if s='' then // Error reading file.
          ...
          XLog('CRC! '+PeerAddr+' > '+Request.Host+
               ' "'+Request.Method+' '+Request.URI+'"'+
               ' > Error reading File: "'+AnsiString(DocRoot+Request.Info.asText['fname'])+'".');
          Disconnect;
          end
        else
          Write(s);
  ...
  with TRtcDataServer(Sender) do
    if Request.Complete then
      Write('<html><body>Your IP: '+PeerAddr+'<br>'+
            'Server Time: '+AnsiString(TimeToStr(Now))+'<br>'+
            'Connection count: '+Int2Str(TotalConnectionCount)+'<br>'+
            'Memory in use: '+AnsiString(FormatFloat('0.### MB', Get_AddressSpaceUsed / 1024))+'</body></html>');
  end;
