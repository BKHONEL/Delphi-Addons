...
function TIsapiDLLs.Get(FileName:String):TIsapi;
  ...
  try
    ...
    if i>=0 then
      ...
      begin
      XLog('----------------------------------------------------------'#13#10+
           'Loading ISAPI Extension "'+AnsiString(FileName)+'" ...','ISAPI');

      h:=LoadLibrary(PChar(FileName));
      if h=0 then
        begin
        XLog('Error loading ISAPI Extension "'+AnsiString(FileName)+'".','ISAPI');
        Result:=nil;
        Exit;
        end;
      ...
      XLog('ISAPI Extension "'+AnsiString(FileName)+'" loaded.','ISAPI');
      end;
  finally
    CS.Release;
    end;
...
procedure TIsapiDLLs.Close(isapi:TIsapi);
  var
    Buff:array [0..64] of AnsiChar;
  begin
  CS.Acquire;
  try
    if Assigned(isapi.Term) then
      begin
      XLog('Asking ISAPI Extension "'+AnsiString(isapi.FName)+'" to terminate ...','ISAPI');
      isapi.Term(0);
      end;
    if GetModuleFileNameA(isapi.Handle, Buff,63)>0 then
...
function GetServerVariable(hConn: HCONN; VariableName: PAnsiChar; Buffer: Pointer; var Size: DWORD ): BOOL stdcall;
  ...
  if v='SERVER_SOFTWARE' then
    ...
    s:='RealThinClient SDK (c) Danijel Tkalcec'
    ...
    for i:=0 to con.Request.ItemCount-1 do
      begin
      vname:=AnsiString(UpperCase(StringReplace(String(con.Request.ItemName[i]),'-','_',[rfReplaceAll])));
    ...
    s:=AnsiString(con.Request.Info['ROOT'])
  else
    begin
    for i:=0 to con.Request.ItemCount-1 do
      begin
  ...
  if s='' then
    ...
    Result:=False;
    Size:=0;
    XLog(AnsiString('GetServerVariable(')+VariableName+') = <empty>','ISAPI');
    end
  else if Size<=len then
    begin
    Result:=False;
    Size:=length(s)+1;
    XLog(AnsiString('GetServerVariable(')+VariableName+') = <buffer too small>','ISAPI');
    end
  else
    begin
    Result:=True;
    ...
    XLog(AnsiString('GetServerVariable(')+VariableName+')="'+s+'"','ISAPI');
    end;
  end;
...
class procedure TRtcISAPI.Execute(Con:TRtcDataServer);
  ...
  try
    ...
    if isapi=nil then
      ...
      con.Write('Error loading ISAPI Extension "'+AnsiString(FName)+'".');
      Exit;
      end;
    ...
    if assigned(isapi.ExtVer) then
      ...
      if not isapi.init then
        ...
        isapi.init:=True;
        XLog('----------------------------------------------------'#13#10+
             'InitDLL('+AnsiString(FName)+'): '+Con.Request.Host+' > '+Con.Request.URI+' (Ver '+
    ...
    with TRtcECB(Con.Request.Info.Obj['ECB']) do
      ...
      ecb^.lpszMethod            := PAnsiChar(Con.Request.Method);

      ecb^.lpszQueryString       := PAnsiChar(Con.Request.Query.Text);
      ecb^.lpszPathInfo          := PAnsiChar(Con.Request.Info.asString['PATH']);
      ecb^.lpszPathTranslated    := PAnsiChar(Con.Request.Info.asString['PATH']);

      ecb^.lpbData               := PAnsiChar(con.Read);
      ...
      XLog('Execute('+AnsiString(FName)+'): '+con.Request.Method+' '+con.Request.URI+#13#10+
    ...
    on E:Exception do
      ...
      Con.Write(#13#10+AnsiString(E.Message));
      XLog('Internal Exception in ISAPI "'+Con.Request.FileName+'": '+AnsiString(E.Message),'ISAPI');
      end;
    end;
  end;
