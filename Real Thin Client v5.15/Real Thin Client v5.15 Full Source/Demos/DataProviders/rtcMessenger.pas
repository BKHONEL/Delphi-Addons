...
const
  MSG_LIST_CRYPT_KEY:AnsiString='$RtcMessenger_UserList';
  MSG_DATA_CRYPT_KEY:AnsiString='$RtcMessenger_UserData';
  MSG_INFO_CRYPT_KEY:AnsiString='$RtcMessenger_UserInfo';
...
type
  ...
  private
    ...
    procedure doLogIn(uname:string; sessid:AnsiString);
    procedure doLogIn2(uname:string; sessid:AnsiString); // 2nd connnection
    procedure doLogOut(uname:string; sessid:AnsiString);
  ...
  public
    ...
    function isLoggedIn(uname:string; sessid:AnsiString):boolean; overload;
    function isLoggedIn(uname:string):boolean; overload;

    procedure Login(uname,upass:string; sessid:AnsiString);
    procedure Login2(uname,upass:string; sessid:AnsiString); // 2nd connection
    procedure Logout(uname:string; sessid:AnsiString);

    procedure RegUser(uname,upass:string; sessid:AnsiString);
...
procedure TRtcMessengerUsers.doLogIn2(uname:string; sessid:AnsiString);
  begin
  userCS.Acquire;
  try
    ...
    if UserInfo.Child[uname]=nil then
      ...
      asString['Session2']:=AnsiString(sessid);
  finally
    userCS.Release;
    end;
  end;

procedure TRtcMessengerUsers.doLogIn(uname:string;sessid:AnsiString);
  var
...
procedure TRtcMessengerUsers.doLogOut(uname:String;sessid:AnsiString);
  var
    info,rec:TRtcRecord;
...
function TRtcMessengerUsers.isLoggedIn(uname:string;sessid:AnsiString):boolean;
  begin
  userCS.Acquire;
  try
    Result:=False;
...
procedure TRtcMessengerUsers.Login(uname,upass:string;sessid:AnsiString);
  begin
  userCS.Acquire;
  try
    if uname='' then
...
procedure TRtcMessengerUsers.Login2(uname,upass:string;sessid:AnsiString);
  begin
  userCS.Acquire;
  try
    if uname='' then
...
procedure TRtcMessengerUsers.RegUser(uname,upass:string;sessid:AnsiString);
  begin
  userCS.Acquire;
  try
    if uname='' then
...
procedure TRtcMessengerUsers.Logout(uname:string; sessid: AnsiString);
  begin
