...
type
  ...
  public
    ...
    property IniFolder:AnsiString read FIniFolder write FIniFolder;
  end;
...
procedure TPHP_Provider.PhpProviderListenStart(Sender: TRtcConnection);
  ...
  try
    ...
    on E:Exception do
      XLog('Error starting PHP Library: '+AnsiString(E.Message));
    end;
  end;
...
procedure TPHP_Provider.PhpProviderListenStop(Sender: TRtcConnection);
  ...
  if FReady then
    ...
    try
      ...
      on E:Exception do
        XLog('Error shuting down PHP Library: '+AnsiString(E.Message));
      end;
    FReady:=False;
    end;
  end;
        ...
        if MyFileName<>'' then
          ...
          for a:=0 to ExtList.Count-1 do
            ...
            if UpperCase(Copy(MyFileName,
              ...
              Request.FileName:=AnsiString(MyFileName);
              Break;
              end;
            end;
          end;
...
procedure TPHP_Provider.PhpProviderDataReceived(Sender: TRtcConnection);
  ...
  procedure SendPHP(SendContent:boolean=True);
    ...
    function Parse_PHP(Sender:TRtcDataServer; const filename:string):AnsiString;
      var
        PHP:TRtcPHP;
        s:AnsiString;
        a,TC:longword;
      ...
      try
        ...
        PHP.DocumentRoot:=AnsiString(GetDocRoot(PHP.Request.Host));

        PHP.FileName:=AnsiString(filename);
        ...
        try
          ...
          on E:Exception do
            begin
            XLog('Request.Query error: '+AnsiString(E.Message),'PHP');
            s:='';
            end;
          end;
        s2:=s2+s;
        ...
        try
          ...
          on E:Exception do
            begin
            XLog('Request.Cookie error: '+AnsiString(E.Message),'PHP');
            s:='';
            end;
          end;
        s2:=s2+s;
        ...
        if PHP.RequestBody<>'' then
          ...
          try
            ...
            on E:Exception do
              begin
              XLog('Request.Params error: '+AnsiString(E.Message),'PHP');
              s:='<'+PHP.RequestBody+'>';
              end;
            end;
          s2:=s2+s;
        ...
        try
          ...
          on E:Exception do
            begin
            XLog('Response.Cookie error: '+AnsiString(E.Message),'PHP');
            s:='';
            end;
          end;
        s2:=s2+s;
