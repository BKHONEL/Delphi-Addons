{
  ...
  @html(</b>)
  - Copyright (c) 2010 by Danijel Tkalcec (http://www.realthinclient.com)
  @html(<br><br>)
...
type
  ...
  public
    FCliServerAddr:AnsiString;
    FCliServerPort:AnsiString;
  ...
  public
    ...
    function FindDataRequestID(Sender:TRtcDataServer; var sid:AnsiString; var AddToQueue:integer):integer;
    procedure GetDataRequest(Sender:TRtcDataServer; var DataRequest:TRtcDataRequest; var AddToQueue:integer; const tip:AnsiString);
    ...
    function GetRequestSessionID(Sender:TRtcDataServer):AnsiString;

    function NeedResponseContentBody(Sender:TRtcDataClient):boolean;
    function GetResponseSessionID(Sender:TRtcDataClient; Content:TRtcRouterContentBody; var IsClosing:boolean):AnsiString;

    procedure CreatePool;

    procedure LogStatus(Sender:TRtcDataServer; const Stat:AnsiString); overload;
    procedure LogStatus(Sender:TRtcDataClient; const Stat:AnsiString); overload;
    procedure LogStatus(const Stat:AnsiString); overload;
...
function TRtcLoadBalancerMainForm.GetResponseSessionID(Sender: TRtcDataClient; Content:TRtcRouterContentBody; var IsClosing:boolean): AnsiString;
  var
    loc:integer;
  begin
...
function TRtcLoadBalancerMainForm.GetRequestSessionID(Sender: TRtcDataServer): AnsiString;
  var
    s:AnsiString;
  begin
...
function TRtcLoadBalancerMainForm.FindDataRequestID(Sender:TRtcDataServer; var sid:AnsiString; var AddToQueue:integer):integer;
  var
    a,idx,
    cnt1,cnt2,
    cnt3,cnt4:integer;
...
procedure TRtcLoadBalancerMainForm.GetDataRequest(Sender:TRtcDataServer; var DataRequest: TRtcDataRequest; var AddToQueue:integer; const tip:AnsiString);
  var
    idx:integer;
    con:TRtcClientConPool;
    sid:AnsiString;
    ...
    if FChangeURLs then
      if Sender.Request.Referer<>'' then
        if UpperCase(Copy(Sender.Request.Referer,1,8+9))='HTTP://LOCALHOST/' then
          Sender.Request.Referer:=AnsiString(StringReplace(String(Sender.Request.Referer),'http://localhost/',
  ...
  try
    ...
    if FChangeHost then
      ...
      if Sender.Response['LOCATION']<>'' then
        Sender.Response['LOCATION']:=AnsiString(StringReplace(String(Sender.Response['LOCATION']),
                                                              String('http://'+Sender.ServerAddr+FToURI+'/'),'/',[rfReplaceAll,rfIgnoreCase]));
      if Sender.Response['REFRESH']<>'' then
        Sender.Response['REFRESH']:=AnsiString(StringReplace(String(Sender.Response['REFRESH']),
                                                             String('http://'+Sender.ServerAddr+FToURI+'/'),'/',[rfReplaceAll,rfIgnoreCase]));
      end;
    ...
    if assigned(Content) then // Response Content Buffering was enabled in "DataRouterResponseBegin" event
      ...
      if FChangeURLs and (length(Content.Body)>0) then
        if (Copy(UpperCase(Sender.Response.ContentType),1,5)='TEXT/') and
           (UpperCase(Sender.Response['CONTENT-ENCODING'])<>'GZIP') then
          begin
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+Sender.ServerAddr+FToURI+'"'),'/"',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+Sender.ServerAddr+FToURI+'/'),'/',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+Sender.ServerAddr+FToURI+'<'),'/<',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+Sender.ServerAddr+FToURI+' '),'/ ',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+Sender.ServerAddr+FToURI+#13),'/'#13,[rfReplaceAll,rfIgnoreCase]));
          end;
      end
...
procedure TRtcLoadBalancerMainForm.LogStatus(Sender: TRtcDataServer; const Stat: AnsiString);
  begin
...
procedure TRtcLoadBalancerMainForm.LogStatus(Sender: TRtcDataClient; const Stat: AnsiString);
  begin
  if not FEventLog then Exit;
  ReqCS.Acquire;
  try
...
procedure TRtcLoadBalancerMainForm.LogStatus(const Stat: AnsiString);
  begin
  if not FEventLog then Exit;
  ReqCS.Acquire;
  try
...
procedure TRtcLoadBalancerMainForm.CreatePool;
  var
    ...
    tmp:AnsiString;
    l:integer;
  function GetToken(delim:AnsiChar):AnsiString;
    var
      i:integer;
    begin
    i:=PosEx(delim,lin);
  ...
  txt:=AnsiString(eServerList.Text);
  repeat
    l:=PosEx2(#13#10,txt);
    if l>0 then
      begin
...
procedure TRtcLoadBalancerMainForm.bConnectClick(Sender: TObject);
  ...
  try
    if not Server.isListening then
      ...
      Server.ServerPort:=AnsiString(eFromPort.Text);
