...
procedure TForm1.StartLogin;
  begin
  Client.Disconnect;
  Client.ServerAddr:=AnsiString(eServerAddr.Text);
  Client.ServerPort:=AnsiString(eServerPort.Text);
  ...
  with ClientModule do
    ...
    if xUseXMLRPC.Checked then
      ...
      DataFormat:=fmt_RTC;
    ModuleFileName:=AnsiString(eModulePath.Text);
    if copy(ModuleFileName,length(ModuleFileName),1)<>'/' then
      ModuleFileName:=ModuleFileName+'/';
    ModuleFileName:=ModuleFileName+'$MSG';

    ModuleHost:=AnsiString(eServerAddr.Text);
    end;

  TimerClient.ServerAddr:=AnsiString(eServerAddr.Text);
  TimerClient.ServerPort:=AnsiString(eServerPort.Text);
