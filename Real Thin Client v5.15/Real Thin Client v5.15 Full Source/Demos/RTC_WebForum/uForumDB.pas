...
type
  ...
  private
    ...
    function GetSectionDirName(SectionID: integer): AnsiString;
    function GetRepliesFileName(SectionID, TopicID: integer): AnsiString;
    function GetTopicsFileName(SectionID: integer): AnsiString;
  ...
  public
    ...
    constructor Create(folder:AnsiString);
...
function NormalDir(const DirName: AnsiString): AnsiString;
begin
  Result := DirName;
  if (Result <> '') and
{$IFDEF RX_D3}
...
function ClearDir(const Path: AnsiString; Delete: Boolean): Boolean;
const
{$IFDEF WIN32}
  FileNotFound = 18;
{$ELSE}
...
function _ExcludeTrailingPathDelimiter(const S: AnsiString): AnsiString;
begin
  Result := S;
...
function TRtcForumData.GetSectionDirName(SectionID: integer): AnsiString;
begin
...
function TRtcForumData.GetRepliesFileName(SectionID, TopicID: integer): AnsiString;
begin
...
function TRtcForumData.GetTopicsFileName(SectionID: integer): AnsiString;
begin
  Result := Format('%s\%s',
    [
      GetSectionDirName(SectionID),
...
constructor TRtcForumData.Create(folder: AnsiString);
  begin
