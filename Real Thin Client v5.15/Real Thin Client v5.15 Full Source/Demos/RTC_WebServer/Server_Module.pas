...
procedure TData_Server.Start;
  ...
  XLog('Read LOG "'+AnsiString(IniName)+'"');
    ...
    {$IFDEF WIN32}
      if web_UsePHP then
        ...
        with GetPHPProvider do
          ...
          IniFolder := AnsiString(Ini.ReadString('PHP','IniFolder',''));
...
{$IFDEF StreamSecII}
  ...
  AddServerPFXFile('server.pfx','abc'); // use our demo server PFX file
...
procedure TData_Server.ServerHTTPListenError(Sender: TRtcConnection; E: Exception);
  begin
  XLog('Error starting Web Server!'#13#10 + AnsiString(E.ClassName+'>'+E.Message) );
