...
type
  ...
  public
    ...
    procedure GetDataRequest(Sender:TRtcDataServer; var DataRequest:TRtcDataRequest; const tip:AnsiString);
  ...
  try
    ...
    try
      if not Server.isListening then
        ...
        FToHost:=AnsiString(eToHost.Text);
        ...
        Req.FCliServerAddr:=AnsiString(eToAddr.Text);
        Req.FCliServerPort:=AnsiString(eToPort.Text);
...
procedure TRtcRouter2MainForm.GetDataRequest(Sender:TRtcDataServer; var DataRequest: TRtcDataRequest; const tip:AnsiString);
  begin
  try
    ...
    if FChangeURLs then
      if Sender.Request.Referer<>'' then
        if UpperCase(Copy(Sender.Request.Referer,1,8+9))='HTTP://LOCALHOST/' then
          Sender.Request.Referer:=AnsiString(StringReplace(String(Sender.Request.Referer),'http://localhost/',String('http://'+FToHost+'/'),[rfReplaceAll,rfIgnoreCase]));
  except
    on E:Exception do
      begin
  ...
  try
    ...
    if FChangeHost then
      ...
      if Sender.Response['LOCATION']<>'' then
        Sender.Response['LOCATION']:=AnsiString(StringReplace(String(Sender.Response['LOCATION']),String('http://'+FToHost+FToURI+'/'),'/',[rfReplaceAll,rfIgnoreCase]));
      if Sender.Response['REFRESH']<>'' then
        Sender.Response['REFRESH']:=AnsiString(StringReplace(String(Sender.Response['REFRESH']),String('http://'+FToHost+FToURI+'/'),'/',[rfReplaceAll,rfIgnoreCase]));
      end;
    ...
    if assigned(Content) then // Response Content Buffering was enabled in "DataRouterResponseBegin" event
      ...
      if FChangeURLs and (length(Content.Body)>0) then
        if (Copy(UpperCase(Sender.Response.ContentType),1,5)='TEXT/') and
           (UpperCase(Sender.Response['CONTENT-ENCODING'])<>'GZIP') then
          begin
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+FToHost+FToURI+'"'),'/"',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+FToHost+FToURI+'/'),'/',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+FToHost+FToURI+'<'),'/<',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+FToHost+FToURI+' '),'/ ',[rfReplaceAll,rfIgnoreCase]));
          Content.Body:=AnsiString(StringReplace(String(Content.Body),String('http://'+FToHost+FToURI+#13),'/'#13,[rfReplaceAll,rfIgnoreCase]));
          end;
      end
...
procedure TRtcRouter2MainForm.LogStatus(Sender: TRtcDataServer; const Stat: AnsiString);
  begin
...
procedure TRtcRouter2MainForm.LogStatus(Sender: TRtcDataClient; const Stat: AnsiString);
  begin
  if not FEventLog then Exit;
  ReqCS.Acquire;
  try
...
procedure TRtcRouter2MainForm.LogStatus(const Stat: AnsiString);
  begin
  if not FEventLog then Exit;
  ReqCS.Acquire;
  try
