...
type
  ...
  public
    ...
    ClientKey,ServerKey,
    ControlKey:AnsiString;

    Read,Write:TRtcEncryptionClass;
  ...
  private
    ...
    FModuleHost:AnsiString;
    FCryptSesName:AnsiString;
    FObjManSesName:AnsiString;
    FAutoRepost:integer;
    ...
    function GetModuleHost: AnsiString;
    procedure SetModuleHost(const Value: AnsiString);
  ...
  public
    ...
    function Prepare(const FunctionName:UString):TRtcFunctionInfo;
  ...
  published
    ...
    property SecureKey:AnsiString read FSecureKey write FSecureKey;
    ...
    property ModuleHost:AnsiString read GetModuleHost write SetModuleHost;
...
destructor TRtcClientModule.Destroy;
  ...
  try
    ...
    FreeAndNil(FMainThrData);
    FreeAndNil(FMyData);
    FreeAndNil(FExecuteResult);

    FreeAndNil(FExecuteHandler);
    FreeAndNil(FLoginResult);
    FreeAndNil(FPingResult);
    FreeAndNil(FObjectLinkResult);
...
function RandomKey(len:integer):AnsiString;
  var
    a:integer;
  begin
  SetLength(Result,len);
  for a:=1 to len do
    Result[a]:=AnsiChar(random(256));
  end;

procedure CryptRead(Crypt:TRtcCryptClient; var Data:AnsiString);
  begin
  if assigned(Crypt) and assigned(Crypt.Read) then
    Crypt.Read.DeCrypt(Data);
  end;

procedure CryptWrite(Crypt:TRtcCryptClient; var Data:AnsiString);
  begin
  if assigned(Crypt) and assigned(Crypt.Write) then
    Crypt.Write.Crypt(Data);
  end;

function GenerateControlKey(var Counter:integer):AnsiString;
  var
    len,a,b,c:integer;
  begin
  Inc(Counter);
  ...
  for a:=5 to len+4 do
    begin
    c:=random(10); Inc(b,c);
    Result[a]:=AnsiChar(c+Ord('0'));
    end;
  Result[1]:=AnsiChar(b div 10 + Ord('0'));
  Result[2]:=AnsiChar(b mod 10 + Ord('0'));
  Result[3]:=AnsiChar(Counter div 10 + Ord('0'));
  Result[4]:=AnsiChar(Counter mod 10 + Ord('0'));
  end;
        ...
        if FDataFormat=fmt_RTC then
          ...
          if assigned(crypt) then
            if Request.Query.ValueCS['ACTION']='RESET' then
              begin
              FreeAndNil(crypt.Write);
              crypt.Write:=TRtcEncryptionClass.Create;
              crypt.Write.Key:=crypt.ServerKey + crypt.ClientHello;

              FreeAndNil(crypt.Read);
              crypt.Read:=TRtcEncryptionClass.Create;
...
function TRtcClientModule.GetObjectManager(xCreate:boolean=False): TRtcRemoteObjectManager;
  ...
  if not assigned(FObjectManager) and xCreate then
    ...
    FObjectManager.BroadcastGroup:=UString(FObjManSesName);
    ...
    if AutoSyncEvents and not Sender.inMainThread then
      ...
      try
        ...
        on E:Exception do
          ...
          Response.StatusText:=AnsiString(E.Message);
          Call_WrongResponse(Sender);
          end;
        end;
      end;
...
function TRtcClientModule.GetModuleFileName: AnsiString;
  begin
  Result:=FModuleFileName;
  end;

procedure TRtcClientModule.SetModuleFileName(const Value: AnsiString);
  begin
...
function TRtcClientModule.GetModuleHost: AnsiString;
  begin
  Result:=FModuleHost;
  end;

procedure TRtcClientModule.SetModuleHost(const Value: AnsiString);
  begin
...
function TRtcClientModule.Prepare(const FunctionName: UString): TRtcFunctionInfo;
  begin
  Data.Clear;
