...
uses
  SysUtils, Classes,
  rtcConn, rtcDataCli, rtcInfo, rtcSyncObjs,
...
type
  ...
  public
    ...
    procedure OpenOrUpdateSession(const SID:AnsiString; ExpireTime:TDateTime);
    ...
    function OpenSession(const SID:AnsiString; ExpireTime:TDateTime):boolean;

    { If Session "SID" does NOT exist, return FALSE.
      If Session "SID" exists, update its expiration time to "ExpireTime" and return TRUE. }
    function UpdateSession(const SID:AnsiString; ExpireTime:TDateTime):boolean;

    { If Session "SID" does NOT exist, return FALSE.
      If Session "SID" exists and expires before "BeforeTime",
      update its expiration time to "ExpireTime" and return TRUE. }
    function UpdateSessionIfExpiresBefore(const SID:AnsiString; BeforeTime,ExpireTime:TDateTime):boolean;
    ...
    function SessionExpiresBefore(const SID:AnsiString; ExpireTime:TDateTime):boolean;

    { If Session "SID" exist, return TRUE.
      If Session "SID" does NOT exist, return FALSE. }
    function HaveSession(const SID:AnsiString):boolean;

    { If Session "SID" exist, Close the Session and return TRUE.
      If Session "SID" did NOT exist, return FALSE. }
    function CloseSession(const SID:AnsiString):boolean;
  ...
  public
    ...
    function UpdateSession(const SID:AnsiString; ExpireTime:TDateTime):integer;
    ...
    function SessionExpiresBefore(const SID:AnsiString; ExpireTime:TDateTime):integer;
    ...
    function UpdateSessionIfExpiresBefore(const SID:AnsiString; BeforeTime,ExpireTime:TDateTime):integer;
    ...
    function HaveSession(const SID:AnsiString):integer;

    { Close Session "SID".
      Returns the index of the Pool where the Session was found,
      or -1 if the Session "SID" could not be found. }
    function CloseSession(const SID:AnsiString):integer;
...
procedure TRtcClientPool.OpenOrUpdateSession(const SID: AnsiString; ExpireTime: TDateTime);
  var
    exp:AnsiString;
  begin
...
function TRtcClientPool.OpenSession(const SID: AnsiString; ExpireTime: TDateTime):boolean;
  var
    exp:AnsiString;
  begin
  exp:=FSessList.search(SID);
...
function TRtcClientPool.UpdateSession(const SID: AnsiString; ExpireTime: TDateTime):boolean;
  var
    exp:AnsiString;
  begin
  exp:=FSessList.search(SID);
...
function TRtcClientPool.CloseSession(const SID: AnsiString): boolean;
  var
    exp:AnsiString;
  begin
  exp:=FSessList.search(SID);
...
function TRtcClientPool.HaveSession(const SID: AnsiString): boolean;
  begin
...
function TRtcClientPool.SessionExpiresBefore(const SID: AnsiString; ExpireTime: TDateTime): boolean;
  var
    exp,exp2:AnsiString;
  begin
  exp:=FSessList.search(SID);
...
function TRtcClientPool.UpdateSessionIfExpiresBefore(const SID: AnsiString; BeforeTime, ExpireTime: TDateTime): boolean;
  var
    exp,exp2:AnsiString;
  begin
  exp:=FSessList.search(SID);
...
function TRtcMultiClientPool.CloseSession(const SID: AnsiString): integer;
  var
    a:integer;
  begin
  Result:=-1;
...
function TRtcMultiClientPool.HaveSession(const SID: AnsiString): integer;
  var
    a:integer;
  begin
  Result:=-1;
...
function TRtcMultiClientPool.UpdateSession(const SID: AnsiString; ExpireTime: TDateTime): integer;
  var
    a:integer;
  begin
  Result:=-1;
...
function TRtcMultiClientPool.SessionExpiresBefore(const SID: AnsiString; ExpireTime: TDateTime): integer;
  var
    a:integer;
  begin
  Result:=-1;
...
function TRtcMultiClientPool.UpdateSessionIfExpiresBefore(const SID: AnsiString; BeforeTime, ExpireTime: TDateTime): integer;
  var
    a:integer;
  begin
  Result:=-1;
