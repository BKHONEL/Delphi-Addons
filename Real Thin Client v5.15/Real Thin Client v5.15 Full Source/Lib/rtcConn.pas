...
{ --- TRtcConnection --- }
  ...
  private
    ...
    function GetVersionSDK:AnsiString;

    procedure SetVersionSDK(const s:AnsiString);
  ...
  public
    ...
    function Read:AnsiString; virtual;
  ...
  published
    ...
    property Version_SDK:AnsiString read GetVersionSDK write SetVersionSDK stored False;
    ...
    property ServerAddr:AnsiString read FAddr write FAddr;

    { Port on the Host to connect to for the Client,
      Listening Port (Bind) for the Server (never leave empty). }
    property ServerPort:AnsiString read FPort write FPort;
...
{ --- TRtcClient --- }
  ...
  public
    ...
    procedure Status(Code:integer; const Text:AnsiString); overload;
    // Set Status code and Text by using a AnsiString like "200 OK"
    procedure Status(const CodeText:AnsiString); overload;
...
function TRtcConnection.GetVersionSDK: AnsiString;
  begin
  Result:=RTCSDK_VERSION;
  end;

procedure TRtcConnection.SetVersionSDK(const s: AnsiString);
  begin
...
function TRtcConnection.Read: AnsiString;
  begin
...
procedure TRtcConnection.Write(const s: AnsiString);
  begin
  if assigned(Con) then
    begin
    Timeout.DataSending;
...
function TRtcConnection.LocalAddr: AnsiString;
  begin
  if assigned(Con) then
...
function TRtcConnection.LocalPort: AnsiString;
  begin
  if assigned(Con) then
...
function TRtcConnection.PeerAddr: AnsiString;
  begin
  if assigned(Con) then
...
function TRtcConnection.PeerPort: AnsiString;
  begin
  if assigned(Con) then
...
procedure TRtcServerResponse.Status(Code: integer; const Text: AnsiString);
  begin
  StatusCode:=Code;
  StatusText:=Text;
  end;

procedure TRtcServerResponse.Status(const CodeText: AnsiString);
  var
    c,s:AnsiString;
    p:integer;
  begin
