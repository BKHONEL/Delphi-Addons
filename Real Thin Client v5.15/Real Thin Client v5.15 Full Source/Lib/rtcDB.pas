{
  ...
  @html(</b>)
  - Copyright (c) 2006-2012 by Danijel Tkalcec (http://www.realthinclient.com)
  @html(<br><br>)
...
{$IF Defined(UNICODE) and Defined(IDE_2009up)}
  {$DEFINE RTC_UNICODE}
{$ELSEIF Defined(FPC)}
  {$DEFINE RTC_WIDECHAR}
{$ELSEIF not Defined(IDE_2009up)}
  {$DEFINE RTC_ANSICHAR}
{$IFEND}
...
uses
  ...
  {$IFDEF IDE_2009up} WideStrings, {$ENDIF}
  {$IFDEF RTC_DATASET}
          ...
          if (OnlyDataFields=False) or (field.FieldKind=fkData) then
            if field.isBlob then
              ...
              case RTC_FIELD2VALUE_TYPES[rtcDS.FieldType[fldname]] of
                ...
                rtc_String: rtcDS.asString[fldname]:=AnsiString(field.AsString);
...
function TRtcBaseDataSet.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
begin
  Result := False;
  // ??
end;

procedure TRtcBaseDataSet.SetFieldData(Field: TField; Buffer: Pointer);
begin
  // ??
end;
...
function TRtcBaseDataSet.Translate(Src, Dest: PAnsiChar; ToOem: Boolean): Integer;
begin
  if (Src <> nil) then
  begin
...
procedure TRtcMemDataSet.InternalOpen;
  ...
  if not assigned(FData) then
    if (FileName<>'') and File_Exists(FileName) then
      FData := TRtcDataSet.FromCode(ZDecompress_Str(Read_File(FileName)))
    else
...
procedure TRtcMemDataSet.InternalClose;
  ...
  { Write any edits to disk and free the managing AnsiString list }
  if (FileName<>'') and assigned(FData) then
    if FData.FieldCount>0 then
      Write_File(FileName, ZCompress_Str(FData.toCode,zcDefault));
...
procedure TRtcMemDataSet.InternalInitFieldDefs;
  var
    ...
    ftype:TFieldType;
    fsize:integer;
{  TFieldType = (ftUnknown, ftAnsiString, ftSmallint, ftInteger, ftWord,
    ftBoolean, ftFloat, ftCurrency, ftBCD, ftDate, ftTime, ftDateTime,
    ftBytes, ftVarBytes, ftAutoInc, ftBlob, ftMemo, ftGraphic, ftFmtMemo,
    ftParadoxOle, ftDBaseOle, ftTypedBinary, ftCursor, ftFixedChar, ftWideAnsiString,
...
{$IFDEF RTC_WIDECHAR}
procedure rtcStrLCopy(Dest: PWideChar; const Source: WideString; MaxLen:Cardinal);
  var
    Len,a:integer;
  begin
  Len:=length(Source);
...
procedure rtcStrLCopy2(var Dest: WideString; const Source: PWideChar; MaxLen:Cardinal);
  var
    Len,a:integer;
  begin
  if Source=nil then
...
procedure rtcStrLCopy3(Dest: PWideChar; const Source: PWideChar; MaxLen:Cardinal);
  var
    Len:integer;
  begin
  Len:=0;
...
procedure TRtcMemDataSet.DataConvert(Field: TField; Source, Dest: Pointer; ToNative: Boolean);
  begin
  {$IF Defined(RTC_WIDECHAR)}
  if TRtcFieldTypes(Field.DataType) in [ft_WideString, ft_FixedWideChar, ft_WideMemo] then
    rtcStrLCopy3(PWideChar(Dest),PWideChar(Source),Field.Size)
  else
    inherited;
  {$ELSEIF Defined(RTC_ANSICHAR)}
  if TRtcFieldTypes(Field.DataType) in [ft_WideString, ft_FixedWideChar, ft_WideMemo] then
    begin
    if ToNative then
      begin
...
function TRtcMemDataSet.GetActiveRecordBuffer: TRtcRecordBuffer;
  ...
  case State of
    ...
    dsCalcFields,dsInternalCalc: Result := CalcBuffer;

    dsFilter: Result:=TempBuffer; // Buffer used in GetRecord when Filtering
...
function TRtcMemDataSet.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
  var
    from_b:PRtcRecordInfo;
    idx:integer;
    myDateTime: TDateTime;
  {$IF Defined(RTC_ANSICHAR) or Defined(RTC_WIDECHAR)}
    MyWideStr:WideString;
  {$IFEND}
  begin
  Result:=False;
    ...
    if assigned(from_b.RowData) and assigned(from_b.RowData.asObject[idx]) then
      ...
      if Buffer<>nil then
        ...
        case TRtcFieldTypes(Field.DataType) of
          ...
          ft_ShortInt,
          ft_Byte:
            LongInt(Buffer^):=from_b.RowData.asInteger[idx];
          ft_Largeint:
            LargeInt(Buffer^):=from_b.RowData.asLargeInt[idx];
          ft_LongWord:
            LongWord(Buffer^):=from_b.RowData.asCardinal[idx];
          ft_Boolean:
            WordBool(Buffer^):=from_b.RowData.asBoolean[idx];
          ft_Currency,
          ft_Float:
            Double(Buffer^):=from_b.RowData.asFloat[idx];
          ft_Extended:
            Extended(Buffer^):=from_b.RowData.asFloat[idx];
          ft_Single:
            Single(Buffer^):=from_b.RowData.asFloat[idx];
          ft_String,
          ft_FixedChar,
          ft_GUID,
          ft_Memo,
          ft_OraClob,
          ft_OraInterval:
            StrLCopy(Buffer, PAnsiChar(from_b.RowData.asString[idx]), Length(from_b.RowData.asString[idx]));
          ft_Date,
          ft_Time,
          ft_DateTime:
            begin
            MyDateTime:=from_b.RowData.asDateTime[idx];
            DataConvert(Field,@MyDateTime,Buffer,True);
            end;
          ft_BCD:
            begin
            if not CurrToBCD(from_b.RowData.asCurrency[idx],TBcd(Buffer^)) then
          ...
          ft_WideMemo:
            begin
            {$IF Defined(RTC_UNICODE)}
              StrPCopy(Buffer, from_b.RowData.asText[idx]);
            {$ELSEIF Defined(RTC_WIDECHAR)}
              MyWideStr:=from_b.RowData.asWideString[idx];
              rtcStrLCopy(PWideChar(Buffer), MyWideStr, Field.Size);
			      {$ELSEIF Defined(RTC_ANSICHAR)}
              MyWideStr:=from_b.RowData.asWideString[idx];
              DataConvert(Field,@MyWideStr,Buffer,True);
            {$ELSE}
              StrPCopy(Buffer, from_b.RowData.asWideString[idx]);
            {$IFEND}
            end;
          ft_Variant:
            Variant(Buffer^):=from_b.RowData.asValue[idx];
          {$IFNDEF FPC}
          ft_TimeStamp,
          ft_OraTimeStamp:
            TSQLTimeStamp(Buffer^):=DateTimeToSQLTimeStamp(from_b.RowData.asDateTime[idx]);
          {$ENDIF}
          {$IFDEF IDE_XEup}
          ft_TimeStampOffset:
            TSQLTimeStampOffset(Buffer^):=DateTimeToSQLTimeStampOffset(from_b.RowData.asDateTime[idx]);
          {$ENDIF}
          ft_FmtBCD:
            begin
            if not CurrToBCD(from_b.RowData.asCurrency[idx],TBcd(Buffer^)) then
...
procedure TRtcMemDataSet.SetFieldData(Field: TField; Buffer: Pointer);
  var
    to_b:PRtcRecordInfo;
    idx:integer;
  {$IF Defined(RTC_WIDECHAR) or Defined(RTC_ANSICHAR)}
    myWideStr:WideString;
  {$IFEND}
    myCurr:Currency;
    myDateTime: TDateTime;
...
*)
        ...
        ft_Byte:
          // TIntegerField
          to_b.RowData.asInteger[idx]:=Integer(Buffer^);
        ft_Largeint:
          // TLargeIntField
          to_b.RowData.asLargeInt[idx]:=LargeInt(Buffer^);
        ft_Boolean:
          // TBooleanField
          to_b.RowData.asBoolean[idx]:=WordBool(Buffer^);
        ft_String,
        ft_FixedChar,
        ft_GUID,
        ft_Memo,
        ...
        ft_OraInterval:
          // TStringField
          to_b.RowData.asString[idx]:=PAnsiChar(Buffer);
        ft_Currency,
        ft_Float:
          // TFloatField
          to_b.RowData.asFloat[idx]:=Double(Buffer^);
        ft_LongWord:
          // TLongWordField
          to_b.RowData.asCardinal[idx]:=LongWord(Buffer^);
        ft_Single:
          // TSingleField
          to_b.RowData.asFloat[idx]:=Single(Buffer^);
        ft_Extended:
          // TExtendedField
          to_b.RowData.asFloat[idx]:=Extended(Buffer^);
        ft_Date,
        ft_Time,
        ft_DateTime:
          // TDataTimeField
          begin
          DataConvert(Field,Buffer,@MyDateTime,False);
          to_b.RowData.asDateTime[idx]:=MyDateTime;
          end;
        ft_BCD:
          // TBCDField
          begin
          if BCDToCurr(TBcd(Buffer^),myCurr) then
        ...
        ft_WideMemo:
          ...
          begin
          {$IF Defined(RTC_UNICODE)}
            to_b.RowData.asText[idx]:=PWideChar(Buffer);
          {$ELSEIF Defined(RTC_WIDECHAR)}
            rtcStrLCopy2(MyWideStr,PWideChar(Buffer),Field.Size);
            to_b.RowData.asWideString[idx]:=MyWideStr;
          {$ELSEIF Defined(RTC_ANSICHAR)}
            DataConvert(Field,Buffer,@MyWideStr,False);
            to_b.RowData.asWideString[idx]:=MyWideStr;
          {$ELSE}
            to_b.RowData.asWideString[idx]:=PWideChar(Buffer);
          {$IFEND}
          end;
        ft_Variant:
          // TVariantField
          to_b.RowData.asValue[idx]:=Variant(Buffer^);
        {$IFNDEF FPC}
        ft_TimeStamp,
        ft_OraTimeStamp:
          // TSQLTimeStampField
          to_b.RowData.asDateTime[idx]:=SQLTimeStampToDateTime(TSQLTimeStamp(Buffer^));
        {$ENDIF}
        {$IFDEF IDE_XEup}
        ft_TimeStampOffset:
          // TSQLTimeStampOffsetField
          to_b.RowData.asDateTime[idx]:=SQLTimeStampOffsetToDateTime(TSQLTimeStampOffset(Buffer^));
        {$ENDIF}
        ft_FMTBcd:
          // TFmtBCDField
          begin
...
destructor TRtcMemDataSet.Destroy;
  begin
  if assigned(FData) and not Active then
    FreeAndNil(FData);
  inherited;
  end;
