...
type
  ...
  protected
    ...
    // @exclude
    function Proxy_PostRequest(Srv:TRtcDataServer; const _data:AnsiString; _notify:boolean):TRtcProxyData;
    ...
    // @exclude
    procedure Debug(const _text:AnsiString; _name:String);
  ...
  public
    ...
    function GetDebugInfo:AnsiString;
  ...
  public
    ...
    proxy:TRtcProxyData;
    uri,head,stext:AnsiString;
    xdata:TRtcHugeString;
    scode:integer;
    xclose:boolean;
    nr:AnsiString;
    ...
    function PostDataFirstEvent(Evnt:TRtcNotifyEvent; const _data,_head,_stext:AnsiString; _scode:integer; _xclose:boolean; _done:boolean; _notify:boolean):boolean;
    function PostDataEvent(Evnt:TRtcNotifyEvent; const _data:AnsiString; _done:boolean; _notify:boolean):boolean;
    function StartNow:boolean;
    function ReStartNow:boolean;
    procedure PostDone(Evnt:TRtcNotifyEvent);

    function GetData2:AnsiString;
    function GetData(var closenow:boolean):AnsiString;

    function Get_Data2(var _notify:boolean):AnsiString;
    function Get_Data(var closenow:boolean; var _notify:boolean):AnsiString;
...
procedure TRtcProxyData.Kill;
  ...
  try
    ...
    on E:Exception do
      begin
      Log('TRtcProxyData.Kill',E,'ERROR');
      raise;
      end;
    end;
  end;

function TRtcProxyData.GetData2: AnsiString;
  begin
  try
    CS.Acquire;
    try
...
function TRtcProxyData.GetData(var closenow:boolean): AnsiString;
  begin
  try
    CS.Acquire;
    try
...
function TRtcProxyData.Get_Data2(var _notify:boolean): AnsiString;
  begin
  try
    CS.Acquire;
    try
...
function TRtcProxyData.Get_Data(var closenow:boolean; var _notify:boolean): AnsiString;
  begin
  try
    CS.Acquire;
    try
...
function TRtcProxyData.PostDataFirstEvent(Evnt:TRtcNotifyEvent; const _data,_head,_stext:AnsiString; _scode:integer; _xclose:boolean; _done:boolean; _notify:boolean):boolean;
  begin
  try
    Result:=False;
    CS.Acquire;
...
function TRtcProxyData.PostDataEvent(Evnt: TRtcNotifyEvent; const _data: AnsiString; _done:boolean; _notify:boolean):boolean;
  begin
  try
    Result:=False;
    CS.Acquire;
...
function TRtcDataRouter.Proxy_PostRequest(Srv:TRtcDataServer; const _data:AnsiString; _notify:boolean):TRtcProxyData;
  var
      ...
      if not Srv.Request.Info.asBoolean['$srv-out'] then // request not posted
        ...
        if not Srv.Request.Info.asBoolean['$man'] then // Content buffering was enabled
          ...
          try
            ...
            Srv.Request.ContentLength:=length(CB.Body);
            Srv.Request.ChunkedTransferEncoding:=False;

            Srv.Request.Info.asBoolean['$srv-out']:=True;

            obj:=Proxy_PostRequest(TRtcDataServer(Sender),CB.Body,False);

            if assigned(OnDebugLog) then
              Debug('<< '+obj.nr+'. SRV : ServerProviderDataReceived ('+Int2Str(length(CB.Body))+') - complete (buffered) '+Srv.Request.Method+' '+Srv.Request.URI,RLOG_ROUTE);
          finally
            CB.Free;
            end;
          end
        ...
        if not Cli.Request.Info.asBoolean['$cli-out'] then
          ...
          if not Cli.Request.Info.asBoolean['$man'] then
            ...
            try
              ...
              Cli.Response.ContentLength:=length(CB.Body);
              Cli.Response.ChunkedTransferEncoding:=False;
              Cli.Request.Info.asBoolean['$cli-out']:=True;

              sent:=obj.PostDataFirstEvent(ServerCanWriteFirst,CB.Body,Cli.Response.HeaderText,Cli.Response.StatusText,Cli.Response.StatusCode,Cli.Request.Close,True,False);
              if assigned(OnDebugLog) then
                Debug('>> '+obj.nr+'. CLI : ClientRequestDataReceived ('+Int2Str(length(CB.Body))+'/'+Cli.Response.ValueCS['CONTENT-LENGTH']+') done (buffered) '+Cli.Request.Method+' '+Cli.Request.URI,RLOG_ROUTE);
            finally
              CB.Free;
              end;
            end
...
function TRtcDataRouter.GetDebugInfo: AnsiString;
  var
    a:integer;
  begin
  try
...
procedure TRtcDataRouter.Debug(const _text: AnsiString; _name: String);
  var
