...
type
  ...
  protected
    // @exclude
    class function Find(const _ID:AnsiString; const _PeerAddr,_ForwardedFor:AnsiString):TRtcServerSession;
    // @exclude
    class function Have(const _ID:AnsiString; const _PeerAddr,_ForwardedFor:AnsiString):boolean;
    // @exclude
    class function Open(_LockType:TRtcSessionLockType; const _PeerAddr,_ForwardedFor:AnsiString):TRtcServerSession;
    // @exclude
    class function CloseID(const _ID:AnsiString; const _PeerAddr,_ForwardedFor:AnsiString; _Event:TRtcSimpleEvent):boolean;
    ...
    // @exclude
    function DenyAccess(const _PeerAddr,_ForwardedFor:AnsiString):boolean;
  ...
  public
    ...
    procedure WriteHeader(const HeaderText: AnsiString; SendNow:boolean=True); overload; virtual; abstract;
    ...
    function FindSession(const ID:AnsiString):boolean;

    { If there is a session with this ID, returns TRUE,
      even if that session is locked. }
    function HaveSession(const ID:AnsiString):boolean;
    ...
    function CloseSession(const ID:AnsiString):boolean;
...
procedure DoneSessions;
  ...
  try
    ...
    if assigned(SessUnLockList) then
      ...
      while (id<>'') and (o<>nil) do
        ...
        {$IFDEF RTC_EXTDEBUG} Log('   - FreeAndNil','DEBUG');{$ENDIF}
        try
          FreeAndNil(o);
        except
          on E:Exception do
            if LOG_AV_ERRORS then
              Log('DoneSessions UnLocked.FreeAndNil("'+id+'")',E,'ERROR');
          end;
    ...
    if assigned(SessLockList) then
      ...
      while (id<>'') and assigned(o) do
        ...
        try
          ...
          on E:Exception do
            if LOG_AV_ERRORS then
              Log('DoneSessions Locked.FreeAndNil("'+id+'")',E,'ERROR');
          end;
        {$IFDEF RTC_EXTDEBUG} Log('   - Search_Min','DEBUG');{$ENDIF}
        try
          id:=SessLockList.search_min(o);
  ...
  try
    ...
    on E:Exception do
      if LOG_AV_ERRORS then
        Log('DoneSessions FreeAndNil(SessCS)',E,'ERROR');
    end;
  {$IFDEF RTC_DEBUG} Log('DoneSessions end.','DEBUG');{$ENDIF}
  end;

{$IFDEF UseGUIDs}
function NewSessionID:AnsiString;
  var
    GUID: TGUID;
  function GuidToStr:AnsiString;
    begin
    SetLength(Result, 32);
    StrLFmt(PAnsiChar(Result), 32,'%.8x%.4x%.4x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x',   // do not localize
...
{$ELSE}
function NewSessionID(const OldID:AnsiString; const PeerAddr:AnsiString):AnsiString;
  var
    tmp:AnsiString;
  function PeerAddrToSessionID:AnsiString;
    const
      code:array[0..15] of AnsiChar =
                ('a','A','b','B','c','C','d','D','e','E','f','F','g','G','h','H');
    var
      ip,loc:integer;
      myIP:byte;
      st:array[1..4] of AnsiString;
    begin
    Result:='';
    for ip:=1 to 4 do
      st[ip]:='';
    ...
    // Convert IP numbers to Hex AnsiString
    for ip:=1 to 4 do
      begin
      if st[ip]<>'' then
        begin
    ...
    for loc:=1 to 10 do
      ...
      ip:=random(10+26+26);
      if ip<10 then
        Result:=Result+AnsiChar(Ord('0')+ip)
      else if ip<36 then
        Result:=Result+AnsiChar(Ord('A')+ip-10)
      else
        Result:=Result+AnsiChar(Ord('a')+ip-36)
      end;
    end;
  procedure IncID(var ID:AnsiString);
    var
      loc:integer;
      ok:boolean;
    begin
...
function SessionTimeToStr(v:TDateTime):AnsiString;
  var
...
{function SessionTimeToStr(DT:TDateTime):AnsiString;
  begin
...
{ Find Session by SessionID and Lock the Session:
   ...
   - add to Locked list }
function FindSession(const ID,PeerAddr,ForwardedFor: AnsiString):TRtcServerSession;
  var
    id2:AnsiString;
    o:TObject;
  begin
...
function HaveSession(const ID,PeerAddr,ForwardedFor: AnsiString):boolean;
  var
    id2:AnsiString;
    o:TObject;
...
function CloseSessionID(const ID,PeerAddr,ForwardedFor: AnsiString; _Event:TRtcSimpleEvent):boolean;
  var
    id2:AnsiString;
    o:TObject;
    sess:TRtcServerSession;
...
destructor TRtcDataServer.Destroy;
  ...
  try
    if FDataServerLinks_Owner and assigned(FDataServerLinks) then
      ...
      FDataServerLinks_Owner:=False;
      FreeAndNil(FDataServerLinks);
      end;
    InitSession;
    FActiveLink:=nil;

    FreeAndNil(FMyRequest); FRequest:=nil;
    FreeAndNil(FMyResponse); FResponse:=nil;
    FreeAndNil(FRequestFixup);
    inherited;
  except
    on E:Exception do
      begin
...
{$ELSEIF Defined(POSIX) or Defined(FPC)}
function TRtcDataServer.DoWaitForStopListen(UserInteractionAllowed: boolean; AllowMessageProcessing:boolean): TRtcWaitForStopListenResult;
  var
    inMain:boolean;
  begin
...
function TRtcDataServer.HaveSession(const ID: AnsiString): boolean;
  begin
  Result:=TRtcServerSession.Have(ID,PeerAddr,Request.ForwardedFor);
  end;

function TRtcDataServer.CloseSession(const ID: AnsiString): boolean;
  begin
  Result:=TRtcServerSession.CloseID(ID,PeerAddr,Request.ForwardedFor,CallSessionClose);
  end;

function TRtcDataServer.FindSession(const ID: AnsiString): boolean;
  begin
  InitSession;
...
class function TRtcServerSession.Find(const _ID,_PeerAddr,_ForwardedFor: AnsiString): TRtcServerSession;
  begin
  Result:=FindSession(_ID,_PeerAddr,_ForwardedFor);
  end;

class function TRtcServerSession.Have(const _ID,_PeerAddr,_ForwardedFor: AnsiString): boolean;
  begin
  Result:=HaveSession(_ID,_PeerAddr,_ForwardedFor);
  end;

class function TRtcServerSession.CloseID(const _ID,_PeerAddr,_ForwardedFor: AnsiString; _Event:TRtcSimpleEvent): boolean;
  begin
...
class function TRtcServerSession.Open(_LockType:TRtcSessionLockType; const _PeerAddr,_ForwardedFor: AnsiString): TRtcServerSession;
  begin
...
function TRtcServerSession.DenyAccess(const _PeerAddr, _ForwardedFor: AnsiString): boolean;
  begin
