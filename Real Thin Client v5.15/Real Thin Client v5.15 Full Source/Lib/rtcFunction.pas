...
type
  ...
  protected
    ...
    // @exclude
    function Function_Exists(const Function_Name:UString):boolean; virtual; abstract;
    end;
  ...
  public
    ...
    function FunctionExists(const FunctionName:UString):boolean;
  ...
  private
    ...
    function GetFuncName: UString;
    procedure SetFuncName(const Value: UString);
  ...
  published
    ...
    property FunctionName:UString read GetFuncName write SetFuncName;
...
function TRtcFunctionGroup.Function_Exists(const Function_Name: UString): boolean;
  var
    idx:integer;
    Func:TRtcAbsFunction;
  begin
...
function TRtcFunctionGroup.ExecuteData(CmdInfo: TRtcCommandInfo; Data: TRtcValueObject;
  ...
  if Data=nil then
    ...
    try
      if assigned(CmdInfo.Command) then
        ...
        if CmdInfo.Command.Function_Exists(TRtcFunctionInfo(Data).FunctionName) then
          ...
          raise Exception.Create('Function or Command "'+UString(TRtcFunctionInfo(Data).FunctionName)+'" not found.');
        end
      else if not CallFunction(CmdInfo, TRtcFunctionInfo(Data), TRtcValue(Result), recursive) then
        raise Exception.Create('Function "'+UString(TRtcFunctionInfo(Data).FunctionName)+'" not found.');
    except
      FreeAndNil(Result);
      raise;
      end;
    ...
    if recursive and not isSimpleValue(Result) then
      ...
      if Data<>Result then FreeAndNil(Data); // Release the old Result
      end;
    end
...
function TRtcFunctionGroup.FunctionExists(const FunctionName: UString): boolean;
  begin
...
function TRtcFunction.Call_Execute(const CmdInfo: TRtcCommandInfo;
  ...
  if (CompareText(Param.FunctionName,FunctionName)=0) then
    ...
    if assigned(FOnExecute) then
      ...
      raise Exception.Create('OnExecute event missing for function "'+UString(FunctionName)+'".');
    end
  else
    Result:=False;
  end;

function TRtcFunction.Function_Exists(const Function_Name: UString): boolean;
  begin
  Result:= CompareText(FunctionName, Function_Name)=0;
  end;

function TRtcFunction.GetFuncName: UString;
  begin
  Result:=FFuncName;
  end;

procedure TRtcFunction.SetFuncName(const Value: UString);
  begin
  FFuncName:=Value;
  end;
