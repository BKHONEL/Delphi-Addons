...
type
  ...
  { @Abstract(Login info or authenticating the user on a proxy and/or a secure server) }
  ...
  private
    ...
    procedure SetCertSubject(const Value: AnsiString);
    procedure SetProxyAddr(const Value: String);
  ...
  public
    ...
    { Assign parameters from another UserLogin object.
      @exclude }
  ...
  published
    ...
    property ProxyAddr:String read FProxyAddr write SetProxyAddr;
    ...
    property CertSubject:AnsiString read FCertSubject write SetCertSubject;
    end;
  ...
  public
    ...
    procedure WriteHeader(const HeaderText: AnsiString; SendNow:boolean=True); overload; override;

    // Use Write to send any Content (document body) out.
    procedure Write(const s:AnsiString=''); override;
...
procedure TRtcHttpClient.SetParams;
  ...
  {$IFDEF RTC_useWinInet}
    if Con is TWinInetProv then
      ...
      TWinInetProv(Con).CertSubject:=AnsiString(FUserLogin.CertSubject);

      TWinInetProv(Con).UserName:=AnsiString(FUserLogin.UserName);
      TWinInetProv(Con).UserPassword:=AnsiString(FUserLogin.UserPassword);

      TWinInetProv(Con).ProxyAddr:=AnsiString(FUserLogin.ProxyAddr);
      TWinInetProv(Con).ProxyBypass:=AnsiString(FUserLogin.ProxyBypass);
      TWinInetProv(Con).ProxyUsername:=AnsiString(FUserLogin.ProxyUserName);
      TWinInetProv(Con).ProxyPassword:=AnsiString(FUserLogin.ProxyPassword);
...
procedure TRtcHttpClient.WriteHeader(const HeaderText: AnsiString; SendNow:boolean=True);
  begin
  if assigned(Con) and (State<>conInactive) then
    begin
    if Request.Active then
...
procedure TRtcHttpClient.Write(const s: AnsiString='');
  begin
  if assigned(Con) and (State<>conInactive) then
    begin
    ...
    if Request.Active then
      ...
      { Data size is known or unimportant.
        We can just write the AnsiString out, without buffering }
...
procedure TRtcHttpUserLogin.AssignTo(Dest: TPersistent);
  begin
  if assigned(Dest) and (Dest<>self) and (Dest is TRtcHttpUserLogin) then
    begin
    TRtcHttpUserLogin(Dest).ProxyAddr:=ProxyAddr;
    TRtcHttpUserLogin(Dest).ProxyBypass:=ProxyBypass;
    TRtcHttpUserLogin(Dest).ProxyUserName:=ProxyUserName;
    TRtcHttpUserLogin(Dest).ProxyPassword:=ProxyPassword;
    TRtcHttpUserLogin(Dest).UserName:=ProxyUserName;
    TRtcHttpUserLogin(Dest).UserPassword:=UserPassword;
    TRtcHttpUserLogin(Dest).CertStoreType:=CertStoreType;
    TRtcHttpUserLogin(Dest).CertSubject:=CertSubject;
    end;
  end;
...
procedure TRtcHttpUserLogin.SetCertSubject(const Value: AnsiString);
  begin
...
procedure TRtcHttpUserLogin.SetProxyAddr(const Value: String);
  begin
...
procedure TRtcHttpUserLogin.SetProxyBypass(const Value: String);
  begin
...
procedure TRtcHttpUserLogin.SetProxyPassword(const Value: String);
  begin
...
procedure TRtcHttpUserLogin.SetProxyUserName(const Value: String);
  begin
