...
procedure TRtcISAPIServer.WriteHeader(const HeaderText: AnsiString; SendNow:boolean=True);
  begin
  if assigned(Con) and (State<>conInactive) then
    begin
    if Response.Sending then
...
procedure TRtcISAPIServer.Write(const s: AnsiString='');
  begin
  if assigned(Con) and (State<>conInactive) then
    begin
