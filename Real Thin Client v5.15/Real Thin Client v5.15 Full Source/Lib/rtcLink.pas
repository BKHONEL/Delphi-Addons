{
  ...
  @html(</b>)
  - Copyright (c) 2011-2012 by Danijel Tkalcec (http://www.realthinclient.com)
  @html(<br><br>)
...
type
  ...
  public
    ...
    procedure RemoteCreate(const xClassName:UString);
    ...
    procedure SetEvent(const xEventName:UString);
    ...
    procedure SetProp(const xPropName:UString; xNow:boolean=False);
    ...
    procedure CallEvent(const xEventName:UString);
    ...
    procedure CallMethod(const xMethodName:UString);
    ...
    procedure Broadcast(const xChannel:UString; const xMethodName:UString);
  ...
  private
    ...
    FParam: TRtcObjectCall;
    FRemoteClass: UString;
    FOnBroadcast: TRtcObjectCallEvent;

    function GetIsServer: boolean;
    function GetIsUpdating: boolean;

    procedure SetRemoteClass(const Value: UString);
  ...
  published
    { Name of the Remote Class (sent as "xClassName" to the remote side).
      If this property is NOT set to the UString known to the remote side,
      the remote Linked Data Module will NOT be created !!! }
    property RemoteClass:UString read FRemoteClass write SetRemoteClass;
...
type
  ...
  private
    ...
    procedure SetBroadcastGroup(const Value: UString);

  protected
    procedure Execute_BadFormat(const xMessage:UString); virtual;
    procedure Execute_ObjectNotFound(xOID:TRtcObjectID; const xMessage:UString); virtual;
    procedure Execute_NoConstructor(const xClassName:UString; const xMessage:UString); virtual;
    procedure Execute_Exception(const E:Exception; const xMessage:UString); virtual;
    ...
    procedure _Subscribe(const xLink:TRtcObjectLink; const xChannel:UString); override;
    procedure _Unsubscribe(const xLink:TRtcObjectLink; const xChannel:UString); override;

    procedure AddBroadcast(const xChannel:UString; xParam:TRtcObjectCall);
  ...
  public
    ...
    procedure _Broadcast(const xChannel:UString; xParam:TRtcObjectCall); override;
    ...
    property BroadcastGroup:UString read FBroadcastGroup write SetBroadcastGroup;
    end;
...
procedure TRtcCustomObjectLink.RemoteCreate(const xClassName: UString);
  begin
...
procedure TRtcCustomObjectLink.SetEvent(const xEventName: UString);
  begin
  if assigned(Manager) and assigned(FParam) then
    begin
...
procedure TRtcCustomObjectLink.CallEvent(const xEventName: UString);
  begin
  if assigned(Manager) and assigned(FParam) then
    begin
    FParam.xName:=xEventName;
...
procedure TRtcCustomObjectLink.CallMethod(const xMethodName: UString);
  begin
  if assigned(Manager) and assigned(FParam) then
    begin
...
procedure TRtcCustomObjectLink.SetProp(const xPropName: UString; xNow: boolean);
  begin
  if assigned(Manager) and assigned(FParam) then
    begin
...
procedure TRtcCustomObjectLink.Broadcast(const xChannel, xMethodName: UString);
  begin
  if assigned(Manager) and assigned(FParam) then
    begin
    FParam.xName:=xMethodName;
...
procedure TRtcLinkedModule.SetRemoteClass(const Value: UString);
  begin
...
procedure TRtcLinkedModule.SetEvent(const xEventName: UString);
  begin
  if assigned(FOLink) then
...
procedure TRtcLinkedModule.CallEvent(const xEventName: UString);
  begin
  if assigned(FOLink) then
    if assigned(FOLink.Manager) then
      begin
...
procedure TRtcLinkedModule.CallMethod(const xMethodName: UString);
  begin
  if assigned(FOLink) then
    if assigned(FOLink.Manager) then
      begin
...
procedure TRtcLinkedModule.SetProp(const xPropName: UString; xNow: boolean);
  begin
  if assigned(FOLink) then
    if assigned(FOLink.Manager) then
      begin
...
procedure TRtcLinkedModule.Broadcast(const xChannel, xMethodName: UString);
  begin
  if assigned(FOLink) then
    if assigned(FOLink.Manager) then
      begin
...
procedure TRtcLinkedModule.Subscribe(const xChannel: UString);
  begin
  if assigned(FOLink) then
    if assigned(FOLink.Manager) then
...
procedure TRtcLinkedModule.Unsubscribe(const xChannel: UString);
  begin
  if assigned(FOLink) then
    if assigned(FOLink.Manager) then
...
procedure TRtcRemoteObjectManager._Subscribe(const xLink: TRtcObjectLink; const xChannel: UString);
  var
    o:TObject;
    objs,objs2:TObjList;
  begin
...
procedure TRtcRemoteObjectManager._Unsubscribe(const xLink: TRtcObjectLink; const xChannel: UString);
  var
    o:TObject;
    objs,objs2:TObjList;
  begin
...
procedure TRtcRemoteObjectManager._Broadcast(const xChannel: UString; xParam: TRtcObjectCall);
  var
    o:TObject;
    objs:TObjList;
...
procedure TRtcRemoteObjectManager.AddBroadcast(const xChannel:UString; xParam: TRtcObjectCall);
  var
    o:TObject;
    objs:TObjList;
    i:RtcIntPtr;
...
procedure TRtcRemoteObjectManager.Execute_ObjectNotFound(xOID: TRtcObjectID; const xMessage: UString);
  begin
...
procedure TRtcRemoteObjectManager.Execute_BadFormat(const xMessage: UString);
  begin
  if not assigned(FExRes) then
    FExRes:=TRtcArray.Create;
...
procedure TRtcRemoteObjectManager.Execute_NoConstructor(const xClassName, xMessage: UString);
  begin
  if not assigned(FExRes) then
    FExRes:=TRtcArray.Create;
...
procedure TRtcRemoteObjectManager.Execute_Exception(const E: Exception; const xMessage: UString);
  begin
  if not assigned(FExRes) then
    FExRes:=TRtcArray.Create;
...
procedure TRtcRemoteObjectManager.SetBroadcastGroup(const Value: UString);
  begin
...
finalization
FreeAndNil(SubscriberCS);
FreeAndNil(SubscribeManagers);
end.
