...
type
  ...
  private
    ...
    FVirtualPath,
    FVirtualPath2,
    FVirtualPathCS: AnsiString;
    FVirtualHost,
    FVirtualHostCS: AnsiString;
    ...
    procedure SetVirtualPath(const Value: AnsiString);
  ...
  public
    ...
    function AddServer(const ServerAddr,ServerPort,RootURI:AnsiString;
    ...
    function HaveSession(SessionID:AnsiString):integer;
    ...
    function UpdateSession(SessionID:AnsiString):integer;
    ...
    function GetBalanceDebugInfo:AnsiString;
    ...
    class function GetVirtualPath(Sender:TRtcDataClient):AnsiString;

    { Return the RootURI associated with the Server for
      which the Sender:TRtcDataClient component was created. }
    class function GetRootURI(Sender:TRtcDataClient):AnsiString;
  ...
  published
    ...
    property VirtualHost:AnsiString read FVirtualHostCS write SetVirtualHost;
    ...
    property VirtualPath:AnsiString read FVirtualPathCS write SetVirtualPath;
...
function TRtcLoadBalancer.AddServer(const ServerAddr, ServerPort, RootURI: AnsiString;
...
function TRtcLoadBalancer.HaveSession(SessionID: AnsiString): integer;
  begin
  ReqCS.Acquire;
  try
...
function TRtcLoadBalancer.UpdateSession(SessionID: AnsiString): integer;
  var
    Con:TRtcClientConPool;
  begin
  ReqCS.Acquire;
...
function TRtcLoadBalancer.GetRequestSessionID(Sender: TRtcDataServer): AnsiString;
  begin
  if Sender<>nil then
...
function TRtcLoadBalancer.FindDataRequestID(Sender:TRtcDataServer; var sid:AnsiString; var AddToQueue:integer):integer;
  var
    a,idx,
    cnt1,cnt2,
    cnt3,cnt4:integer;
...
function TRtcLoadBalancer.GetBalanceDebugInfo: AnsiString;
  var
    a:integer;
    Con:TRtcClientConPool;
  begin
...
procedure TRtcLoadBalancer.SetVirtualHost(const Value: AnsiString);
  begin
  FVirtualHostCS := Value;
  FVirtualHost := Upper_Case(Value);
  end;

procedure TRtcLoadBalancer.SetVirtualPath(const Value: AnsiString);
  begin
  if Value='' then
    begin
    FVirtualPathLen:=0;
...
class function TRtcLoadBalancer.GetRootURI(Sender: TRtcDataClient): AnsiString;
  begin
  Result:=Sender.Info.asString['$root'];
  end;

class function TRtcLoadBalancer.GetVirtualPath(Sender: TRtcDataClient): AnsiString;
  begin
  if assigned(Sender.Info.Obj['$owner']) then
