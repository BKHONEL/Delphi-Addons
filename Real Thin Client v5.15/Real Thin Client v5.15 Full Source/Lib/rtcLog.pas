...
var
  ...
  { Sub-Folder inside AppFileName's directory where all LOG files will be stored.
    ...
    set LOG_FOLDER to an empty AnsiString before calling "StartLog".
...
{ Write exception with a short AnsiString description into the Global App Log file.
  This procedure will have no effect if Log writer not started
  (by calling StartLog) or LOG_EXCEPTIONS is @false }
procedure Log(const s:AnsiString; E:Exception; const name:String=''); overload;

{ Write message into the Global App Log file.
  This procedure will have no effect if Log writer not started. }
procedure Log(const s:AnsiString; const name:String=''); overload;

{ Write message into the Log file for the current date.
  This procedure will have no effect if Log writer not started. }
procedure XLog(const s:AnsiString; const name:String=''); overload;

{ Write exception with a short AnsiString description into the Global App Log file.
  This procedure will have no effect if Log writer not started
  (by calling StartLog) or LOG_EXCEPTIONS is @false }
procedure Log(const s:WideString; E:Exception; const name:String=''); overload;

{ Write message into the Global App Log file.
  This procedure will have no effect if Log writer not started. }
procedure Log(const s:WideString; const name:String=''); overload;

{ Write message into the Log file for the current date.
  This procedure will have no effect if Log writer not started. }
procedure XLog(const s:WideString; const name:String=''); overload;
...
procedure WriteToLog(const ext:String; const text:AnsiString);
  ...
  procedure File_Append(const fname:String; const Data:AnsiString);
    var
      f:TRtcFileHdl;
    begin
...
procedure WriteToBuff(const ext:String; const text:AnsiString);
  var
    obj:TObject;
    data:TRtcHugeString;
  begin
  ...
  if not assigned(obj) then
    begin
    data:=TRtcHugeString.Create;
    LogBuff.insert(ext,data);
    end
  else
    data:=TRtcHugeString(obj);
  data.Add(text);
...
procedure XLog(const s:AnsiString; const name:String='');
  var
    d:TDateTime;
    fname:String;
    s2:AnsiString;
...
procedure XLog(const s:WideString; const name:String='');
  begin
  XLog(Utf8Encode(s),name);
  end;

procedure Log(const s:AnsiString; const name:String='');
  var
    d:TDateTime;
    fname:String;
    s2:AnsiString;
...
procedure Log(const s:AnsiString; E:Exception; const name:String='');
  begin
  if LOG_EXCEPTIONS then
    Log(s+' Exception! '+Utf8Encode(E.ClassName)+': '+Utf8Encode(E.Message), name);
  end;

procedure Log(const s:WideString; const name:String='');
  begin
  Log(Utf8Encode(s),name);
  end;

procedure Log(const s:WideString; E:Exception; const name:String='');
  begin
  if LOG_EXCEPTIONS then
    Log(Utf8Encode(s)+' Exception! '+Utf8Encode(E.ClassName)+': '+Utf8Encode(E.Message), name);
  end;

initialization
ThrCS:=TRtcCritSec.Create;
{$IFDEF RTC_DEBUG}StartLog; Log('rtcLog Initialized. Debug Logging started.','DEBUG');{$ENDIF}
finalization
{$IFDEF RTC_DEBUG}Log('rtcLog Finalized. Debug Logging stopped.','DEBUG');{$ENDIF}
StopLog;
StopLogBuffers;
FreeAndNil(ThrCS);
end.
