...
type
  ...
  public
    ...
    procedure WriteHeader(const Header_Text:AnsiString); overload; virtual;
...
function TRtcMsgClientProvider.Read: AnsiString;
  begin
  if not _Active then
    begin
    Result:='';
...
procedure TRtcMsgClientProvider.WriteHeader(const Header_Text: AnsiString);
  begin
  if not _Active then Exit;
...
procedure TRtcMsgClientProvider.Write(const s: AnsiString; SendNow:boolean=True);
  begin
  if not _Active then Exit;
      ...
      if not FHaveResponse then // Don't have the header yet ...
        ...
        if not FResponseLine then
          ...
          if length(InBuffer)>5 then
            InBuf:=Upper_Case(Copy(InBuffer,1,5))
          else
            InBuf:=Upper_Case(InBuffer);
