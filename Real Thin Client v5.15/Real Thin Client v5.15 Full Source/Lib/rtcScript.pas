...
uses
  ...
  rtcFunction, rtcConn, rtcDataSrv,
  rtcScriptCompile;
...
type
  ...
  TRtcScriptEngine=class(TRtcAbsCommand)
    private
      ...
      FScriptOpen: UString;
      FScriptClose: UString;

      procedure SetScriptClose(const Value: UString);
      procedure SetScriptOpen(const Value: UString);
    ...
    protected
      ...
      // @exclude
      function Function_Exists(const Function_Name:UString):boolean; override;
    ...
    public
      ...
      { Compile a Script UString and return a compiled script object.
      ...
      function Compile(const Script:UString; const FileName:UString=''):TRtcValue;

      { Execute Compiled Script object and return plain data object.

        You can call "asString" on the result object to get the UString,
    ...
    published
      ...
      property ScriptOpen:UString read FScriptOpen write SetScriptOpen;

      { Characters used to close a script block (default = ?> ).
        There always need to be exectly 2 characters for this,
        and only a subset of character combinations is allowed. }
      property ScriptClose:UString read FScriptClose write SetScriptClose;
...
function TRtcScriptEngine.Function_Exists(const Function_Name: UString): boolean;
  begin
...
function TRtcScriptEngine.Call_Execute(const CmdInfo: TRtcCommandInfo;
  ...
  function GetAsString(const obj:TRtcValueObject):AnsiString;
    begin
    if obj is TRtcSimpleValue then
      ...
      raise EConvertError.Create('Can not convert '+obj.ClassName+' to AnsiString.');
    end;

  function GetAsText(const obj:TRtcValueObject):UString;
    begin
    if obj is TRtcSimpleValue then
      ...
      raise EConvertError.Create('Can not convert '+obj.ClassName+' to UString.');
    end;
  ...
  function CheckParam(const parname:UString):TRtcValue;
    var
  ...
  { Check param executes "paramname" and stores the result back into
    the variable, then returns the pointer to that variable.
    You should NOT destroy the object returned. }
  function CheckParamB(const parname:UString):TRtcValue;
    var
      obj1,obj2:TRtcValueObject;
    begin
    // remove old pointer
  ...
  function ExecuteParam(const parname:UString):TRtcValueObject;
    var
      obj1,obj2:TRtcValueObject;
    begin
    obj.asObject:=nil;
  ...
  { Execute Param returns the result, destroying the original.
    You should take care of destroying the object returned. }
  function ExecuteParamB(const parname:UString):TRtcValueObject;
    var
      obj1,obj2:TRtcValueObject;
    begin
    objB.asObject:=nil;
  ...
  { Execute Command returns the result, preserving the original.
    You should take care of destroying the object returned. }
  function ExecuteCommand(const parname:UString):TRtcValueObject;
    var
  ...
  function _Execute(const data:TRtcValueObject):TRtcValueObject;
    ...
    if not assigned(data) then
      ...
      try
        ...
        if Result<>temp then FreeAndNil(temp);
        end;
  ...
  function Get_Variable(vname:UString; var isCopy:boolean; asAddr:boolean):TRtcValueObject; forward;

  function Get_Property(obj:TRtcValueObject; s:UString; var isCopy:boolean; asAddr:boolean):TRtcValueObject;
    var
      dot:integer;
      vname:UString;
    ...
    procedure CheckCodeBlocks;
      ...
      if Result is TRtcFunctionInfo then
            ...
            try
              ...
              try
                ...
                try
                  ...
                  if assigned(objX) and (objX<>temp) then FreeAndNil(objX);
                  end;
              finally
                FreeAndNil(temp);
                end;
        ...
        if not done then
          ...
          FreeAndNil(obj); // free old Result object if it was a copy
    ...
    try
      while s<>'' do
        ...
        if Result=nil then
          raise ERtcScript.Create('Can not get ".'+UString(vname)+'" from NULL');
        ...
        if Result is TRtcArray then
          ...
          raise ERtcScript.Create('Can not get ".'+UString(vname)+'" from a simple data type');
        CheckCodeBlocks;
        end;
    finally
  ...
  procedure Set_Variable(s:UString; var value:TRtcValueObject; asAddr:boolean);
    var
      obj:TRtcValueObject;
      vname:UString;
    ...
    procedure Set_Property(obj:TRtcValueObject);
      ...
      while s<>'' do
        ...
        if obj is TRtcArray then
          begin
          if (vname='COUNT') or (vname='FIELDCOUNT') then
            raise Exception.Create('Can not assign to ".'+UString(vname)+'"')
          ...
          if (vname='FIRST') or (vname='LAST') or (vname='NEXT') or (vname='PRIOR') or
            ...
            raise ERtcScript.Create('Can not assign to ".'+UString(vname)+'"')
          else if vname='ROW' then
            begin
    ...
    procedure Set_HTTPValues(const Vals:TRtcHttpValues);
      ...
      if (vname='') or (vname='TEXT') then
        ...
        begin
        Vals.Value[AnsiString(vname)]:=GetAsString(value);
        FreeAndNil(value);
        end;
      end;
    ...
    procedure Set_HTTPHeaderVar(const Vars:TRtcHttpHeader);
      ...
      if vname='COOKIE' then
        ...
        if vname='COUNT' then
          ...
          begin
          Vars.Value[AnsiString(vname)]:=GetAsString(value);
          FreeAndNil(value);
          end;
        end
    ...
    procedure Set_SessionVar;
      ...
      if vname='ID' then
        ...
        raise ERtcScript.Create('Can not assign to "SESSION.'+UString(vname)+'"')
      else
        begin
        if not assigned(Srv.Session) then
          raise ERtcScript.Create('No session locked, can not set "SESSION.'+UString(vname)+'"');
    ...
    procedure SetVariable;
      ...
      if vname[1]='-' then
        ...
        if not assigned(Comm.Sender) then
          raise ERtcScript.Create('Connection unassigned, can not access "'+UString(vname)+'"')
        else if not (Comm.Sender is TRtcDataServer) then
          raise ERtcScript.Create('Not a RtcDataServer connection, can not access "'+UString(vname)+'"');
        ...
        if vname='-REQUEST' then
          ...
          raise ERtcScript.Create('Unknown variabe "'+UString(vname)+'"');
        end
      else if s='' then
        begin
        if not asAddr then
  ...
  function Get_Variable(vname:UString; var isCopy:boolean; asAddr:boolean):TRtcValueObject;
    var
      s:UString;
    ...
    function Get_HTTPValues(const Vals:TRtcHttpValues):TRtcValueObject;
      ...
      if (vname='') or (vname='TEXT') then
        ...
        Result:=TRtcStringValue.Create(Vals.Value[AnsiString(vname)]);
      end;
    ...
    function Get_HTTPHeaderVar(const Vars:TRtcHttpHeader):TRtcValueObject;
      ...
      if vname='COOKIE' then
        ...
        if vname='COUNT' then
          ...
          begin
          if Vars.Value[AnsiString(vname)]<>'' then
            Result:=TRtcStringValue.Create(Vars.Value[AnsiString(vname)])
          else
            Result:=nil;
          end;
        end
    ...
    function Get_SessionVar:TRtcValueObject;
      ...
      if vname='ID' then
        ...
        if (vname='') or (vname='TOTAL') then
          ...
          raise ERtcScript.Create('Invalid parameter for SESSION.COUNT ... "'+UString(vname)+'"');
        end
        ...
        if (vname='') or (vname='PRIVATE') or (vname='FWDLOCK') then
          ...
          raise ERtcScript.Create('Can not Open a Session with parameter "'+UString(vname)+'"')
        end
        ...
        if vname='' then
          ...
          Result:=TRtcBooleanValue.Create(Srv.FindSession(AnsiString(vname)));
        end
        ...
        if vname='' then
          ...
          begin
          if Srv.FindSession(AnsiString(vname)) then
            ...
            repeat
              ...
              if not Srv.HaveSession(AnsiString(vname)) then Break;
              Sleep(10);
              until Srv.FindSession(AnsiString(vname));
        ...
        if vname='' then
          ...
          Result:=TRtcBooleanValue.Create(Srv.HaveSession(AnsiString(vname)));
        end
      else if vname='CLOSE' then
        begin
        GetNext;
        ...
        if vname='' then
          ...
          Srv.CloseSession(AnsiString(vname));
        end
    ...
    function Get_ResponseVar:TRtcValueObject;
      ...
      if vname='STATUS' then
        ...
        if vname='CODE' then
          ...
          raise ERtcScript.Create('Can not read "RESPONSE.STATUS.'+UString(vname)+'"');
        end
      else if vname='STATUSCODE' then
        Result:=TRtcIntegerValue.Create(Resp.StatusCode)
      else if vname='STATUSTEXT' then
    ...
    function Get_ClientVar:TRtcValueObject;
      ...
      if vname='COUNT' then
        ...
        raise ERtcScript.Create('Unsupported function: "CLIENT.'+UString(vname)+'"');
      end;
    ...
    function Get_ServerVar:TRtcValueObject;
      ...
      if (vname='IP') or (vname='ADDR') or (vname='ADDRESS') then
        ...
        raise ERtcScript.Create('Unsupported function: "SERVER.'+UString(vname)+'"');
      end;
    ...
    if vname[1]='-' then
      begin
      if not assigned(Comm.Sender) then
        raise ERtcScript.Create('No connection object, can not access "'+UString(vname)+'"')
      else if not (Comm.Sender is TRtcDataServer) then
        raise ERtcScript.Create('Not a RtcDataServer connection, can not access "'+UString(vname)+'"');
      ...
      if vname='-REQUEST' then
        ...
        raise ERtcScript.Create('Unable to read variable "'+UString(vname)+'"');
      end
    else
      begin
      if vname='RESULT' then
  ...
  procedure Execute_Variable;
        ...
        try
          ...
          try
            ...
            try
              ...
              Comm.Locals:=tempLocals;
              if assigned(obj) and (obj<>temp) then FreeAndNil(obj);
              end;
          finally
            FreeAndNil(temp);
            end;
    ...
    try
      ...
      if vname='' then
        ...
        begin
        what:=1; // 'Reading variable "'+UString(vname)+'"';
      ...
      on E:Exception do
        ...
        if Pos(UString(func),E.Message)>0 then
          begin
          ...
          raise ERtcForward.Create(UString(func)+#13#10+E.Message);
        end;
      end;
    end;
  ...
  procedure Execute_Property;
    var
      vname,vname2:UString;
      what:byte;
    begin
    what:=0; // 'Checking property';
    try
      vname:=Param.asVarName['$V'];
      what:=1; // 'Reading property "'+UString(vname)+'"';
    ...
    if not SetOneNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not add '+UString(rtcTypeName(obj.isType))+' to '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
    ...
    if obj.isNull then
      ...
      if ctype<>rtc_Null then
        ...
        case ctype of
          ...
          rtc_DataSet:}
            ...
            raise ERtcScript.Create('Can not negate '+UString(rtcTypeName(Res.isType)));
          end;
        end;
      end
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not subtract '+UString(rtcTypeName(objB.isType))+' from '+UString(rtcTypeName(obj.isType)));
        end;
      end;
    end;
    ...
    if obj.isNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not execute '+UString(rtcTypeName(objB.isType))+' SHL '+UString(rtcTypeName(obj.isType)));
        end;
      end;
    end;
    ...
    if obj.isNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not execute '+UString(rtcTypeName(objB.isType))+' SHR '+UString(rtcTypeName(obj.isType)));
        end;
      end;
    end;
    ...
    if SetOneNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not multiply '+UString(rtcTypeName(obj.isType))+' and '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
    ...
    if not SetOneNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not divide '+UString(rtcTypeName(obj.isType))+' by '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
    ...
    if not SetOneNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not execute '+UString(rtcTypeName(obj.isType))+' MOD '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
    ...
    if SetOneNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not execute '+UString(rtcTypeName(obj.isType))+' AND '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
    ...
    if not SetOneNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not execute '+UString(rtcTypeName(obj.isType))+' OR '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
    ...
    if not (obj.isNull or objB.isNull) then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not compare '+UString(rtcTypeName(obj.isType))+' and '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
    ...
    if not SetOneNull then
      ...
      case ctype of
        ...
        rtc_DataSet:}
          ...
          raise ERtcScript.Create('Can not execute '+UString(rtcTypeName(obj.isType))+' XOR '+UString(rtcTypeName(objB.isType)));
        end;
      end;
    end;
  ...
  procedure Do_Chr;
    ...
    case Res.isType of
      ...
      rtc_DataSet:}
        ...
        raise ERtcScript.Create('Can not get Integer from '+UString(rtcTypeName(Res.isType)));
      end;
    ExtractX;
    Res.isNull:=True;
    if assigned(obj) then
      Res.asText:=obj.asText+UChar(ch)
    else
      Res.asText:=UChar(ch);
    end;
      ...
      if length(func)=1 then
        ...
        case func[1] of
          ...
          //'^':
            ...
            raise ERtcScript.Create('Function "'+UString(func)+'" not found');
          end;
        end
  ...
  try
    if Param.FunctionName='!' then // Commands
        ...
        try
          ...
          case func[1] of
            ...
            'X':Execute_NewFunction;
                ...
                raise ERtcScript.Create('Unknown command: "'+UString(func)+'"');
            end;
        except
          on E:Exception do
            begin
            ...
            if E is ERtcForward then
              ...
              raise ERtcForward.Create('['+Param.asText['$R']+':'+Param.asText['$C']+'] '+
                                       E.ClassName+' in '+UString(func)+':'+#13#10+E.Message);
            end;
          end;
      finally
        obj.asObject:=nil;
      ...
      try
        try
          ...
          on E:Exception do
            raise ERtcForward.Create('['+Param.asText['$R']+':'+Param.asText['$C']+'] '+
                                     E.ClassName+' in Function "'+UString(func)+'":'+#13#10+E.Message);
          end;
      finally
        obj.asObject:=nil;
        FreeAndNil(obj);
    ...
    else if Param.FunctionName='$.' then // property UString
      begin
      Result:=True;
      obj:=TRtcValue.Create;
      try
...
function TRtcScriptEngine.Compile(const Script: UString; const FileName:UString=''): TRtcValue;
  var
...
procedure TRtcScriptEngine.SetScriptClose(const Value: UString);
  begin
...
procedure TRtcScriptEngine.SetScriptOpen(const Value: UString);
  begin
  if length(Value)<>2 then
