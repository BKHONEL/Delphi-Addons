...
type
  ...
  TRtcScriptCompiler=class
    private
      ...
      FScriptClose1,FScriptClose2:UChar;
      ...
      function findScriptBegin:UString;
      ...
      function isClosing(const s:UString): boolean;
      function isOpening: boolean;
      function isSeparator: boolean;
      function isDot: boolean;

      function peekName:UString;

      function getName:UString;
      function getProperty:UString;
      function getNumber:UString;
      function getOperator:UString;
      function getOpening:UString;

      procedure skipClosing(const s:UString);
      ...
      procedure SetScript(const Value: UString);

      function GetScriptClose: UString;
      function GetScriptOpen: UString;
    ...
    public
      ...
      property Script:UString read FScript write SetScript;
      property FileName:UString read FFileName write FFileName;
      property FilePath:UString read FFilePath write FFilePath;
      ...
      property ScriptOpen:UString read GetScriptOpen write SetScriptOpen;
      { Characters used to close a script block.
        There always need to be exectly 2 characters for this,
        and only a subset of character combinations is allowed. }
      property ScriptClose:UString read GetScriptClose write SetScriptClose;
...
implementation
  ...
  //RTC_NAMESTART_CHARSET:set of UChar=['a'..'z','A'..'Z','_','$','@'];
  //RTC_NAME_CHARSET:set of UChar=['a'..'z','A'..'Z','0'..'9','_','$','@'];
  //RTC_NAME_CHARSET2:set of UChar=['a'..'z','A'..'Z','0'..'9','_','$','@','.'];

  //RTC_NUMSTART_CHARSET:set of UChar=['0'..'9'];
  //RTC_NUM_CHARSET:set of UChar=['0'..'9','.'];

  //RTC_OPERATOR_CHARSET:set of UChar=['+','-','*','/','%','>','<','=','&','|','!',':','^','#'];
  //RTC_ASSIGN_CHARSET:set of UChar=['+','-','*','/','%','&','|',':','^'];

  //RTC_LIST_SEPARATORS:set of UChar=[',',';'];

  //RTC_WHITESPACE:set of UChar=[#9,#32,#10,#13];
...
procedure TRtcScriptCompiler.SetScript(const Value: UString);
  begin
  FScript:=Value;
  Loc:=1;
  Row:=1;Col:=1;
...
function TRtcScriptCompiler.findScriptBegin: UString;
  var
    i:integer;
    found:boolean;
  begin
...
function TRtcScriptCompiler.getString:UString;
  var
    i,c,r:integer;
    found:boolean;
    terminator:UChar;
  ...
  if (loc<size) and (Script[loc]=FScriptClose1) and (Script[loc+1]=FScriptClose2) then
    ...
    if not found then
      raise ERtcScript.Create('Unterminated short UString (starts with '+terminator+').'#13#10+
...
function TRtcScriptCompiler.getName: UString;
  var
    i:integer;
    found:boolean;
    r,c:integer;
...
function TRtcScriptCompiler.getProperty: UString;
  var
    i:integer;
    found:boolean;
    r,c:integer;
...
function TRtcScriptCompiler.peekName: UString;
  var
    i:integer;
    found:boolean;
  begin
...
function TRtcScriptCompiler.getNumber: UString;
  var
    i:integer;
    found:boolean;
    r,c:integer;
...
function TRtcScriptCompiler.getOperator: UString;
  begin
  case Script[loc] of
    '<','>','=':
      begin
      case Script[loc+1] of
        '<','>','=':
          begin
          Result:=UString('')+Script[loc]+Script[loc+1];
         Inc(loc,2);Inc(col,2);
          end
        else
          begin
          Result:=UChar(Script[loc]);
          Inc(loc);Inc(col);
          end;
        end;
      end;
...
function TRtcScriptCompiler.getOpening: UString;
  begin
  case Script[loc] of
    '(','[','{':
      begin
...
function TRtcScriptCompiler.isClosing(const s:UString): boolean;
  begin
...
procedure TRtcScriptCompiler.skipClosing(const s:UString);
  begin
  if (s='(') and (Script[loc]=')') then
    begin
    Inc(loc);Inc(col);
...
function TRtcScriptCompiler.CompileValue(long_strings:boolean; MidOperators:boolean=True): TRtcValueObject;
  ...
  try
    if isSeparator then
      ...
      func.asString['C']:=AnsiString(fname);
      Result:=func;
      ...
      if Pos('.',fname)=0 then
        ...
        Result:=TRtcFloatValue.Create(Str2Float(AnsiString(fname)));
      end
      ...
      if (parname='CODE') then
        ...
        func.asInteger['$C']:=col;
        func.asString['C']:='NEW';
        func.asString['X']:=AnsiString(fname);
        Result:=func;
        end
        ...
        func.asInteger['$R']:=row;
        func.asInteger['$C']:=col;
        func.asString['C']:=AnsiString(parname);
        Result:=func;
    ...
    while isDot do
      ...
      if not isOpening then
        ...
        func.asString['Y']:=AnsiString(fname);
        Result:=func;
        end
      else
        begin
...
function TRtcScriptCompiler.GetScriptClose: UString;
  begin
  Result:=UString('')+FScriptClose1+FScriptClose2;
  end;

function TRtcScriptCompiler.GetScriptOpen: UString;
  begin
  Result:=UString('')+FScriptOpen1+FScriptOpen2;
  end;

procedure TRtcScriptCompiler.SetScriptClose(const Value: UString);
  begin
...
procedure TRtcScriptCompiler.SetScriptOpen(const Value: UString);
  begin
  if length(Value)<>2 then
