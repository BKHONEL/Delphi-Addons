...
type
  ...
  public
    ...
    procedure Call_Message(Msg:word; Data:Cardinal);
    function Call_ThrMessage(Msg:word; Data:Cardinal):boolean;
    
    procedure DoMessage(Msg:word; Data:Cardinal); virtual; abstract;

    function ReceiveStr(var Str : AnsiString): Integer; virtual; abstract;
    function SendStr(const Str : AnsiString): Integer; virtual; abstract;
    function BuffStr(const Str : AnsiString): Integer; virtual; abstract;
    ...
    property Addr: AnsiString        read FAddr write SetAddr;
    property Port: AnsiString        read FPort write SetPort;

    property UdpMultiCast        : Boolean      read  FMultiCast write SetMultiCast;
    property UdpMultiCastAddr    : AnsiString   read  FMultiCastAddr write SetMultiCastAddr;
...
procedure TRtcSocketBase.Call_Message(Msg: word; Data: Cardinal);
  var
  ...
  if not FEventsOff then
    ...
    if assigned(mt) then
      ...
      if (Data=0) and (Msg<=RTC_SOCKET_MAXMSG) then
        TRtcThread.PostJob(mt,Message_WsSock[Msg])
      else
        begin
...
function TRtcSocketBase.Call_ThrMessage(Msg: word; Data: Cardinal):boolean;
  var
    job:TRtcSocketMesDataJob;
    mt:TRtcThread;
  begin
...
procedure TRtcSocketBase.SetAddr(const Value: AnsiString);
begin
  FAddr := Value;
end;
...
procedure TRtcSocketBase.SetMultiCastAddr(const Value: AnsiString);
begin
...
procedure TRtcSocketBase.SetPort(const Value: AnsiString);
begin
  FPort := Value;
end;
