...
type
  ...
  protected
    ...
    FReadBuff:TRtcHugeString;
    FSendBuff:TRtcHugeString;
...
*)
    ...
    procedure Call_Read(const data:AnsiString);
    ...
    function api_ReceiveStr(var Str : AnsiString): boolean; virtual; abstract;
    ...
    function api_SendStr(var Str : AnsiString; at:integer):Integer; virtual; abstract;

    { Address of connected Peer }
    function api_GetPeerAddr: AnsiString; virtual; abstract;
    { Port number of connected Peer }
    function api_GetPeerPort: AnsiString; virtual; abstract;

    { Local connection address }
    function api_GetLocalAddr: AnsiString; virtual; abstract;
    { Local connection port }
    function api_GetLocalPort: AnsiString; virtual; abstract;
...
constructor TRtcSocketBaseSyn.Create;
  ...
  FReadBuff:=TRtcHugeString.Create;
  FSendBuff:=TRtcHugeString.Create;
  FDataToSend:=0;
  FDataIn:=0;
  FDataOut:=0;
...
destructor TRtcSocketBaseSyn.Destroy;
  ...
  try
    ...
    FreeAndNil(FReadBuff);
    FreeAndNil(FSendBuff);
...
function TRtcSocketBaseSyn.GetLocalAddr: AnsiString;
  begin
  FCS.Acquire;
  try
    Result:=FLocalAddr;
...
function TRtcSocketBaseSyn.GetLocalPort: AnsiString;
  begin
  FCS.Acquire;
  try
    Result:=FLocalPort;
...
function TRtcSocketBaseSyn.GetPeerAddr: AnsiString;
  begin
  FCS.Acquire;
  try
    Result:=FPeerAddr;
...
function TRtcSocketBaseSyn.GetPeerPort: AnsiString;
  begin
  FCS.Acquire;
  try
    Result:=FPeerPort;
...
function TRtcSocketBaseSyn.ReceiveStr(var Str: AnsiString): Integer;
  begin
  FCS.Acquire;
  try
...
function TRtcSocketBaseSyn.BuffStr(const Str: AnsiString): Integer;
  begin
  Result:=length(Str);
  if Result>0 then
    begin
...
function TRtcSocketBaseSyn.SendStr(const Str: AnsiString): Integer;
  begin
  Result:=length(Str);
...
procedure TRtcSocketBaseSyn.Call_Read(const data:AnsiString);
  begin
  FCS.Acquire;
  try
...
function TRtcSocketBaseSyn.GetDataToSend: AnsiString;
  begin
  FCS.Acquire;
  try
    if FSendBuff.Size>0 then
      begin
      Result:=FSendBuff.Get;
      FSendBuff.Clear;
      end
    else
      begin
...
procedure TRtcSocketBaseSyn.Exec_LoopReading;
  ...
  else if api_ReceiveStr(tmp) then
    begin
...
procedure TRtcSocketBaseSyn.Exec_LoopSending;
  ...
  if FSendAt=0 then // all data sent and "FSending" flag set to FALSE
    ...
    begin
    fout:=api_SendStr(FSend,FSendAt);
