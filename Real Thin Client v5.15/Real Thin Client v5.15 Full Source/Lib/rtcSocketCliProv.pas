...
function TRtcSocketClientProvider.Read: AnsiString;
  var
...
procedure TRtcSocketClientProvider.Write(const s: AnsiString; SendNow:boolean=True);
  var
    s_out:AnsiString;
    s_res:TRtcCryptPluginState;
  begin
...
procedure TRtcSocketClientProvider.DirectWrite(const s: AnsiString);
  begin
  if not _Visible then Exit;
...
procedure TRtcSocketClientProvider.BufferWrite(const s: AnsiString);
  begin
  if not _Visible then Exit;
