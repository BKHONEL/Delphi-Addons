...
type
  ...
  protected
    ...
    procedure DirectWrite(const s: AnsiString);
    procedure BufferWrite(const s: AnsiString);
  ...
  public
    ...
    property UdpMultiCast       : Boolean           read  FMultiCast
                                                    write FMultiCast;
    property UdpMultiCastAddr   : AnsiString            read  FMultiCastAddrStr
...
function TRtcSocketServerProvider.Read: AnsiString;
  var
    len:longint;
...
procedure TRtcSocketServerProvider.Write(const s: AnsiString; SendNow:boolean=True);
  var
    s_out:AnsiString;
    s_res:TRtcCryptPluginState;
  begin
...
procedure TRtcSocketServerProvider.DirectWrite(const s: AnsiString);
  begin
...
procedure TRtcSocketServerProvider.BufferWrite(const s: AnsiString);
  begin
  if not _Visible then Exit;
  ...
  try
    ...
    on E:Exception do
      ...
      if LOG_REFUSED_CONNECTIONS then
        Log('Connection refused with Message: '+AnsiString(E.Message),'REFUSED');
...
function TRtcSocketServerProvider.AddClient(Client: TRtcSocketServerProvider):boolean;
  ...
  try
    ...
    if Result then
      FClientList.insert(RtcIntPtr(Client),1);
  finally
    Leave;
    end;
  end;
...
procedure TRtcSocketServerProvider.RemoveClient(Client: TRtcSocketServerProvider);
  ...
  try
    if assigned(FClientList) then
      if FClientList.search(RtcIntPtr(Client))>0 then
...
function TRtcSocketServerProvider.AddThread(Thr: TRtcThread):boolean;
  ...
  try
    ...
    if Result then
      FThrList.insert(RtcIntPtr(Thr),1);
  finally
    Leave;
    end;
  end;
...
procedure TRtcSocketServerProvider.RemoveThread(Thr: TRtcThread);
  ...
  try
    if assigned(FThrList) then
      if FThrList.search(RtcIntPtr(Thr))>0 then
    ...
    if assigned(FThrList) then
      ...
      repeat
        if FThrList.Count>0 then
          begin
          Thr:=TRtcSocketClientThread(FThrList.search_min(i));
          FThrList.Remove(RtcIntPtr(Thr));
          end
        else
          Thr:=nil;
    ...
    if assigned(FClientList) then
      ...
      repeat
        if FClientList.Count>0 then
          begin
          cl:=TRtcSocketServerProvider(FClientList.search_min(i));
          FClientList.Remove(RtcIntPtr(cl));
          end
        else
          cl:=nil;
        if assigned(cl) then
...
destructor TRtcSocketSrvProvUnit.Destroy;
  begin
  try
    FreeAndNil(Message_WSInit);
    FreeAndNil(Message_WSReInit);
    FreeAndNil(Message_WSStop);
    FreeAndNil(Message_WSCloseConn);
    FreeAndNil(Message_WSRelease);
    FreeAndNil(Message_WSRelease_Silent);
    FreeAndNil(Message_WSRelease_Normal);
    inherited;
  except
    on E:Exception do
      begin
