...
type
  ...
  protected
    ...
    { If you need a 100% secure connection, define a Secure Key AnsiString
      ...
      an empty AnsiString (means: no secure key). @html(<br><br>)
...
procedure CryptRead(Crypt:TRtcCryptServer; var Data:AnsiString);
  begin
  if assigned(Crypt) and assigned(Crypt.Read) then
    Crypt.Read.DeCrypt(Data);
  end;

procedure CryptWrite(Crypt:TRtcCryptServer; var Data:AnsiString);
  begin
...
procedure TRtcBaseServerModule.Call_DataReceived(Sender: TRtcConnection);
  ...
  function IsValidControlKey(var Counter:integer; const s:AnsiString):boolean;
  ...
  function ExtractControlKey(var Counter:integer):AnsiString;
    var
      i,at,len:integer;
    begin
    Result:='';
  ...
  function ExtractFinalControlKey(var Counter:integer):AnsiString;
    var
      DecompressTo,
      i,at,len:integer;
    begin
    ...
    if Request.Query.ValueCS['ACTION']='HELLO' then
      ...
      if Request.Query.ValueCS['ID']='NEW' then
        ...
        if Response.StatusCode=410 then
          begin
          // Return empty AnsiString if Session expired or non-existing
          Write;
          Exit;
          end
        else
...
function TRtcBaseServerModule.GetModuleFileName: AnsiString;
  begin
  Result:=FModuleFileName;
  end;

procedure TRtcBaseServerModule.SetModuleFileName(const Value: AnsiString);
  begin
...
function TRtcBaseServerModule.GetModuleHost: AnsiString;
  begin
  Result:=FModuleHost;
  end;

procedure TRtcBaseServerModule.SetModuleHost(const Value: AnsiString);
  begin
...
procedure SetObj(o:TObject;num:integer);
  ...
  CS.Acquire;
  try
    if List.search(RtcIntPtr(o))>0 then
...
function CheckObj(o:TObject):integer;
  ...
  CS.Acquire;
  try
    Result:=List.search(RtcIntPtr(o));
  finally
    CS.Release;
    end;
  end;
...
function TRtcDelayedCallJob.Run(Thr:TRtcThread):boolean;
  ...
  try
    ...
    FreeAndNil(call);
    if not released then FreeAndNil(self);
    end;
  end;
