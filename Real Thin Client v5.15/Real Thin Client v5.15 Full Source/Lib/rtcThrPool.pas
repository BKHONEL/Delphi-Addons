...
procedure PostQuickJob(Event:TRtcQuickJobEvent; Data:TRtcValueObject=nil; AccessGUI:boolean=False);
...
procedure TRtcQuickJob.Post(Data: TRtcValueObject);
  begin
  PostQuickJob(FEvent,Data,AccessGUI);
  end;
...
var
  ...
  WaitList:tXList; // list of all thread objects waiting for execution
...
procedure OpenThreadPool;
  ...
  try
    ...
    while (ThreadPool.Count<RTC_THREAD_POOL_MAX) and // thread limit not reached
      ...
      ThreadPool.insert(RtcIntPtr(NWork),1);
      FreePool.addLast(RtcIntPtr(NWork));
      end;
  finally
    CSThread.Release;
    end;
...
function GetThread:TRtcWorkerThread;
  ...
  if FreePool.Count>0 then // threads available
    ...
    ThreadPool.insert(RtcIntPtr(Result),1);
    end;
...
function ReturnThread(Thr:TRtcWorkerThread):boolean; // executed 1 object, returning for another
  ...
  if Work<>nil then // execution object waiting
    ...
    begin
    FreePool.AddLast(RtcIntPtr(Thr));
    Result:=True;
    end
  else
    begin
...
procedure ClosingThread(var Thr:TRtcWorkerThread);
  ...
  CSThread.Acquire;
  try
    if ThreadPool.search(RtcIntPtr(Thr))>0 then
      ThreadPool.remove(RtcIntPtr(Thr));
    Dec(OpenCnt);
...
{$IFDEF WINDOWS}
  ...
  while CSOpen.WaitFor(10)<>wr_Signaled do
    begin
  ...
  { Wait 0.1 seconds to allow all threads to terminate. }
...
constructor TRtcThread.Create;
  ...
  MsgList:=TXList.Create(32);
  FInfo:=TRtcInfo.Create;

  CSThread.Acquire;
  try
    ThrList.insert(RtcIntPtr(self),1);
  finally
    CSThread.Release;
    end;
  end;
...
procedure TRtcThread.Finalize;
  begin
  if ThrList.search(RtcIntPtr(self))>0 then
...
class function TRtcThread.Lock(me: TObject): boolean;
  ...
  if assigned(CSThread) then
    ...
    CSThread.Acquire;
    try
      if ThrList.search(RtcIntPtr(me))>0 then
        Result:=True;
    finally
      if not Result then
        CSThread.Release;
    ...
    if ThrList.search(RtcIntPtr(me))>0 then
      with TRtcThread(me) do
        ...
        if HighPriority then
          MsgList.addFirst(RtcIntPtr(_Job))
        else
          MsgList.addLast(RtcIntPtr(_Job));

        if not (Working or Waiting) then // not working and not waiting
          begin
          Waiting:=True;
          WaitList.addLast(RtcIntPtr(me)); // add to waiting list
...
procedure TRtcThread.Idle;
  ...
  if Working then
    ...
    if MsgList.Count>0 then
      begin
      Waiting:=True;
      WaitList.addLast(RtcIntPtr(self)); // add to waiting list
      end;
    end;
  end;
