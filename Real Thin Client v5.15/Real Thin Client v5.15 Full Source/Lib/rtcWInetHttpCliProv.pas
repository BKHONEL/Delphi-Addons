...
{ contains the chunked-ext }
  ...
  private
    ...
    FMyLastMethod:AnsiString;
    FMyLastURI:AnsiString;
    ...
    FCertSubject: AnsiString;
    FCertStoreType: TRtcCertStoreType;
    FProxyAddr: AnsiString;
  ...
  protected
    ...
    procedure SendHeaderOut(const s:AnsiString);
  ...
  public
    ...
    procedure WriteHeader(const Header_Text:AnsiString; SendNow:boolean=True); overload; virtual;

    procedure Write(const s:AnsiString; SendNow:boolean=True); override;
    ...
    property ProxyAddr:AnsiString read FProxyAddr write FProxyAddr;
    property ProxyBypass:AnsiString read FProxyBypass write FProxyBypass;
    ...
    property CertSubject:AnsiString read FCertSubject write FCertSubject;
...
type
  ...
  {InternetOpen}
  TInternetOpen =               function(lpszAgent: PAnsiChar; dwAccessType: DWORD;
                                         lpszProxy, lpszProxyBypass:
                                         PAnsiChar; dwFlags: DWORD): HINTERNET; stdcall;

  {InternetConnect}
  TInternetConnect =             function(hInet: HINTERNET; lpszServerName: PAnsiChar;
                                          nServerPort: INTERNET_PORT;
                                          lpszUsername: PAnsiChar; lpszPassword: PAnsiChar;
  ...
  {HttpOpenRequest}
  THttpOpenRequest =             function(hConnect: HINTERNET; lpszVerb: PAnsiChar;
                                          lpszObjectName: PAnsiChar;
                                          lpszVersion: PAnsiChar; lpszReferrer: PAnsiChar;
                                          lplpszAcceptTypes: PLPSTR; dwFlags: DWORD;
                                          dwContext: DWORD): HINTERNET; stdcall;

  {HttpSendRequest}
  THttpSendRequest =             function(hRequest: HINTERNET; lpszHeaders: PAnsiChar;
...
function WinCryptGetProc(const ProcName : AnsiString) : Pointer;
  begin
...
function WinInetGetProc(const ProcName : AnsiString) : Pointer;
  begin
  if Length(ProcName) = 0 then
    Result := nil
  else
    begin
    Result := GetProcAddress(FDllHandle2, PAnsiChar(@ProcName[1]));
    if Result = nil then
  ...
  try
    ...
    try
      if FProxyAddr='' then
        ...
        hSession := InternetOpen(nil, INTERNET_OPEN_TYPE_PROXY, PAnsiChar(FProxyAddr), nil, 0)
      else
        hSession := InternetOpen(nil, INTERNET_OPEN_TYPE_PROXY, PAnsiChar(FProxyAddr), PAnsiChar(FProxyBypass), 0);
    except
      hSession := nil;
      end;
    ...
    try
      hConnect := InternetConnect(hSession, PAnsiChar(GetAddr), myPort,
                                  PAnsiChar(FUserName), PAnsiChar(FUserPassword),
...
function TRtcWInetHttpClientProvider.Read: AnsiString;
  begin
  if not _Active then
    begin
    Result:='';
...
procedure TRtcWInetHttpClientProvider.SendHeaderOut(const s:AnsiString);
  var
    certOK,
    proxyOK:boolean;
    ex:Exception;
  ...
  repeat
    if hRequest=nil then
      ...
      if Request.ContentLength=0 then // No content
        begin
        if sndHeader<>'' then
          FHeaderOut:=HttpSendRequest(hRequest, Addr(sndHeader[1]), length(sndHeader), nil, 0)
        else
        ...
        FTempHeaderBuff:=s;
        if sndHeader<>'' then
          FHeaderOut:=HttpSendRequest(hRequest, Addr(sndHeader[1]), length(sndHeader), Addr(FTempHeaderBuff[1]), length(FTempHeaderBuff))
        else
          FHeaderOut:=HttpSendRequest(hRequest, nil, 0, Addr(FTempHeaderBuff[1]), length(FTempHeaderBuff));
        end;
      end
    else
      begin
      ...
      if length(sndHeader)>0 then
        FBufferIn.lpcszHeader := Addr(sndHeader[1])
      else
  ...
  if not FHeaderOut then
    ...
    if not FHeaderEx then
      ...
      begin
      FDataOut:=FDataOut+length(sndHeader);
...
{$ENDIF RTC_USESETTIMEOUTS}
  ...
  if (hRequest=nil) or
     ...
     (Request.Close<>FMyLastHTTP10) then
    begin
    if hRequest<>nil then
      begin
      try
    ...
    if FMyLastHTTP10 then
      hRequest := HttpOpenRequest(hConnect, PAnsiChar(FMyLastMethod), PAnsiChar(FMyLastURI), 'HTTP/1.0', '', nil,  FMyLastFlags, 0)
    else
      hRequest := HttpOpenRequest(hConnect, PAnsiChar(FMyLastMethod), PAnsiChar(FMyLastURI), 'HTTP/1.1', '', nil,  FMyLastFlags, 0);
...
procedure TRtcWInetHttpClientProvider.WriteHeader(const Header_Text: AnsiString; SendNow:boolean=True);
  begin
  if not _Active then Exit;
...
procedure TRtcWInetHttpClientProvider.Write(const s: AnsiString; SendNow:boolean=True);
  var
    bOK:boolean;
    ex:Exception;
    bWritten:DWORD;
  ...
  if FHeaderEx then
    begin
    FTempWriteBuff:=s;
    bOK := InternetWriteFile(hRequest, Addr(FTempWriteBuff[1]), length(FTempWriteBuff), bWritten);
    if not bOK or (bWritten<>dword(length(FTempWriteBuff))) then
      if _Active then
        begin
        ex:=RtcWInetException.Create('Error Sending the Request [Code #'+IntToStr(GetLastError)+'].');
        try
    ...
    FDataOut:=length(FTempWriteBuff);
...
procedure TRtcWInetHttpClientProvider.AcceptResponse;
  ...
  try
    ...
    if not HttpQueryInfo(hRequest, HTTP_QUERY_RAW_HEADERS_CRLF, Addr(recvHeader[1]), dwBufLen, dwIndex) then
      begin
      if not _Active then Exit;
    ...
    Response.HeaderText:=recvHeader;
...
function TRtcWInetHttpClientProvider.SetupCertificate:boolean;
  ...
  if FCertStoreType=certAny then
    ...
    if hStore<>nil then
      begin
      lpszSubjectName:=PAnsiChar(FCertSubject);
...
destructor TMyWinInet.Destroy;
  ...
  try
    ...
    FreeAndNil(Message_WSOpenConn);
    FreeAndNil(Message_WSReOpenConn);
    FreeAndNil(Message_WSCloseConn);
    FreeAndNil(Message_WSStop);
    FreeAndNil(Message_WSRelease);
    FreeAndNil(LibCS);
    inherited;
  except
    on E:Exception do
