...
// containing the date and time information converted from the header AnsiString
//
...
function WinHttpGetProc(const ProcName : AnsiString) : Pointer;
  begin
  if Length(ProcName) = 0 then
    Result := nil
  else
  ...
  try
    ...
    try
      if FProxyAddr='' then
        ...
        hSession := InternetOpen(nil, WINHTTP_ACCESS_TYPE_NAMED_PROXY, PWChar(myWProxyAddr), nil, 0);
        end
      else
        begin
        myWProxyAddr:=String(FProxyAddr);
        myWProxyBypass:=String(FProxyBypass);
        hSession := InternetOpen(nil, WINHTTP_ACCESS_TYPE_NAMED_PROXY, PWChar(myWProxyAddr), PWChar(myWProxyBypass), 0);
        end;
    except
      hSession := nil;
      end;
...
function TRtcWinHttpClientProvider.Read: AnsiString;
  begin
  if not _Active then
    begin
    Result:='';
...
procedure TRtcWinHttpClientProvider.SendHeaderOut(const s:AnsiString);
  var
    ex:Exception;
  ...
  FTempHeaderBuff:='';
  FTempWriteBuff:='';
  sndHeader:=WideString(Request.HeaderText);
  repeat
    if hRequest=nil then
      Break
    else
      ...
      if Request.ContentLength=0 then // No content
        begin
        if sndHeader<>'' then
          FHeaderOut:=HttpSendRequest(hRequest, PWChar(sndHeader), length(sndHeader), nil, 0,
                                      0, nil)
        else
        ...
        FTempHeaderBuff:=s;
        if sndHeader<>'' then
          FHeaderOut:=HttpSendRequest(hRequest, PWChar(sndHeader), length(sndHeader), Addr(FTempHeaderBuff[1]), length(FTempHeaderBuff),
                                      Request.ContentLength, nil)
        else
          FHeaderOut:=HttpSendRequest(hRequest, nil, 0, Addr(FTempHeaderBuff[1]), length(FTempHeaderBuff),
                                      Request.ContentLength, nil);
        end;
      FHeaderEx:=FHeaderOut and (Request.Contentlength>length(FTempHeaderBuff));
      end;
...
{$ENDIF}
  ...
  if (hRequest=nil) or
    ...
    hRequest := HttpOpenRequest(hConnect, PWChar(myWMethod), PWChar(myWURI), PWChar(myWHTTP), '', nil, myFlags);
...
procedure TRtcWinHttpClientProvider.WriteHeader(const Header_Text: AnsiString; SendNow:boolean=True);
  begin
  if not _Active then Exit;
...
procedure TRtcWinHttpClientProvider.Write(const s: AnsiString; SendNow:boolean=True);
  var
    bOK:boolean;
    ex:Exception;
    bWritten:DWORD;
  ...
  try
    ...
    if not HttpQueryInfo(hRequest, WINHTTP_QUERY_RAW_HEADERS_CRLF, nil, Addr(recvHeader[1]), dwBufLen, dwIndex) then
      ...
      if GetLastError<>ERROR_INSUFFICIENT_BUFFER then
        ...
        begin
        SetLength(recvHeader, dwBufLen div 2);  // DIV 2 because we are receiving the Header as WideString but dwBufLen is in bytes
        if not HttpQueryInfo(hRequest, WINHTTP_QUERY_RAW_HEADERS_CRLF, nil, Addr(recvHeader[1]), dwBufLen, dwIndex) then
          begin
          if _Active then
            begin
    ...
    Response.HeaderText:=AnsiString(recvHeader);
...
function TRtcWinHttpClientProvider.SetupProxy:boolean;
  ...
  if FProxyUsername<>'' then
    ...
    if not InternetSetOption(hRequest, WINHTTP_OPTION_PROXY_USERNAME,
                              PWChar(myWOption), length(FProxyUsername)) then Exit;
    end;
  ...
  if FProxyPassword<>'' then
    ...
    if not InternetSetOption(hRequest, WINHTTP_OPTION_PROXY_PASSWORD,
                              PWChar(myWOption), length(FProxyPassword)) then Exit;
    end;
  ...
  if FUsername<>'' then
    ...
    if not InternetSetOption(hRequest, WINHTTP_OPTION_USERNAME,
                              PWChar(myWOption), length(FUsername)) then Exit;
    end;
  ...
  if FUserPassword<>'' then
    ...
    if not InternetSetOption(hRequest, WINHTTP_OPTION_PASSWORD,
                              PWChar(myWOption), length(FUserPassword)) then Exit;
    end;
