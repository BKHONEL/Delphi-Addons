...
type
    ...
    TWSAAsyncGetHostByAddr = function (HWindow: HWND; wMsg: u_int; addr: PAnsiChar; len, Struct: Integer; buf: PAnsiChar; buflen: Integer): THandle; stdcall;
    ...
    TGetAddrInfo           = function(NodeName: PAnsiChar; ServName: PAnsiChar; Hints: PAddrInfo; var Addrinfo: PAddrInfo): integer; stdcall;
    TFreeAddrInfo          = procedure(ai: PAddrInfo); stdcall;
    TGetNameInfo           = function(addr: PSockAddr; namelen: Integer; host: PAnsiChar; hostlen: DWORD; serv: PAnsiChar; servlen: DWORD; flags: integer): integer; stdcall;
...
function WSocket_ResolveHost(const InAddr : AnsiString) : TInAddr;
function WSocket_ResolvePort(const Port : AnsiString; const Proto : AnsiString) : Word;

function SizeOfSockAddr(const sin: TSockAddr): integer;

function WSocket_GetSinPort(const Sin: TSockAddr): Integer;
function WSocket_GetSinIP(const Sin: TSockAddr): AnsiString;
function WSocket_SetVarSin(var Sin: TSockAddr; const IP, Port: AnsiString; Family, SockProtocol, SockType: integer; PreferIP4: Boolean): integer;
...
function WSocket_SetVarSin(var Sin: TSockAddr; const IP, Port: AnsiString; Family, SockProtocol, SockType: integer; PreferIP4: Boolean): integer;
  ...
  function Get_Addr(const IP, port: AnsiString; var Hints: TAddrInfo; const Sin: PSockAddr): integer;
    var
      Addr: PAddrInfo;
    begin
    Addr := nil;
    try
      ...
      if Hints.ai_socktype = SOCK_RAW then
        ...
        Hints.ai_socktype := 0;
        Hints.ai_protocol := 0;
        Result := _GetAddrInfo(PAnsiChar(IP), nil, @Hints, Addr);
        end
      else
        begin
        if (IP=cAnyHost) or (IP=c6AnyHost) then
          ...
          Result := _GetAddrInfo(nil, PAnsiChar(Port), @Hints, Addr);
          end
          ...
          begin
          Result := _GetAddrInfo(PAnsiChar(IP), PAnsiChar(Port), @Hints, Addr);
          end;
        end;
      if Result = 0 then
        if Addr = nil then
  ...
  if not IsNewApi(family) then
    ...
    if ProtoEnt <> nil then
      ServEnt := _GetServByName(PAnsiChar(Port), ProtoEnt^.p_name);
    if ServEnt = nil then
    ...
    if IP=cBroadcast then
      ...
      begin
      Sin.sin.addr.s_addr := _inet_addr(PAnsiChar(IP));
      if Sin.sin.addr.s_addr = u_long(INADDR_NONE) then
        begin
        HostEnt := _GetHostByName(PAnsiChar(IP));
    ...
    r := Get_Addr(IP, Port, Hints1, @Sin1);
    Result := r;
    if r = 0 then
      sin := sin1
    else if TwoPass then
      begin
      r := Get_Addr(IP, Port, Hints2, @Sin2);
      Result := r;
      if r = 0 then
        sin := sin2
      else
...
function atoi(const value : AnsiString) : Word;
var
    i : Integer;
begin
    Result := 0;
...
function WSocket_GetSinIP(const Sin: TSockAddr): AnsiString;
  ...
  if not IsNewApi(Sin.sin_family) then
    ...
    r := _GetNameInfo(@sin, SizeOfSockAddr(sin), PAnsiChar(host), hostlen,
...
function WSocket_ResolveHost(const InAddr : AnsiString) : TInAddr;
  ...
  begin
    ...
    szData:=PAnsiChar(Trim(InAddr)); { Length already checked above }
...
function WSocket_ResolvePort(const Port : AnsiString; const Proto : AnsiString) : Word;
  var
    szPort   : array [0..31] of AnsiChar;
    szProto  : array [0..31] of AnsiChar;
    Pse      : Pservent;
    lasterr  : longint;
  begin
...
function WinSockGetProc(const ProcName : AnsiString) : Pointer;
  begin
...
function xGetProc(xdll: THandle; const ProcName : AnsiString) : Pointer;
  begin
  if Length(ProcName) = 0 then
    Result := nil
  else
    Result := GetProcAddress(xdll, PAnsiChar(@ProcName[1]));
  end;
