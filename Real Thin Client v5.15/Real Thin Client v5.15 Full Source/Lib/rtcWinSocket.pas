...
type
  ...
  private
    ...
    FMyPort             : AnsiString;
    FMyPortNum          : Integer;
    FMyAddr             : AnsiString;     { IP address for local interface to use }

    FSendNowBuff        : AnsiString;
    FSendNowAt          : integer;
    FSendBuffer         : TRtcHugeString;
    ...
    procedure   SetMyAddr(const Value : AnsiString);
    procedure   SetMyPort(const Value : AnsiString);
    function    GetMyAddr: AnsiString;
    function    GetMyPort: AnsiString;
    procedure   BindSocket;

    function    RealSend(Data:Pointer; Len : Integer; triggerEvent:boolean=True) : Integer;
    function    RealRecv(Data:Pointer; Len : Integer; triggerEvent:boolean=True) : Integer;
  ...
  public
    ...
    procedure   SetAddr(const Value : AnsiString); override;
    procedure   SetPort(const Value : AnsiString); override;

    procedure   Connect; override;
    procedure   Listen; override;
    procedure   Close; override;

    function    GetLocalAddr: AnsiString; override;
    function    GetLocalPort: AnsiString; override;
    ...
    function    ReceiveStr(var Str : AnsiString):integer; override;
    function    SendStr(const Str : AnsiString):integer; override;
    function    BuffStr(const Str : AnsiString):integer; override;
    ...
    property MyPort : AnsiString        read  GetMyPort write SetMyPort;
    property MyAddr : AnsiString        read  GetMyAddr write SetMyAddr;
...
constructor TRtcWinSocket.Create;
  ...
  FSendBuffer := TRtcHugeString.Create;
  FSendNowBuff := '';
  FSendNowAt := 0;
...
function TRtcWinSocket.GetMyAddr: AnsiString;
  begin
  Result:=FMyAddr;
  end;

function TRtcWinSocket.GetMyPort: AnsiString;
  begin
  Result:=FMyPort;
  end;
...
procedure TRtcWinSocket.SetPort(const Value : AnsiString);
  begin
...
procedure TRtcWinSocket.SetMyPort(const Value : AnsiString);
  begin
  if FState <> wsClosed then
    begin
...
procedure TRtcWinSocket.SetMyAddr(const Value : AnsiString);
  begin
  if FState <> wsClosed then
    begin
...
function TRtcWinSocket.GetLocalPort: AnsiString;
  var
    saddr    : TSockAddr;
    saddrlen : integer;
  begin
...
function TRtcWinSocket.GetLocalAddr: AnsiString;
  var
    saddr    : TSockAddr;
    saddrlen : integer;
  begin
...
procedure TRtcWinSocket.SetAddr(const Value : AnsiString);
  begin
  if FState <> wsClosed then
    begin
...
function TRtcWinSocket.GetPeerAddr: AnsiString;
  var
    saddr    : TSockAddr;
    saddrlen : integer;
    Res: integer;
...
function TRtcWinSocket.GetPeerPort: AnsiString;
  var
    saddr    : TSockAddr;
    saddrlen : integer;
    Res : integer;
  ...
  if FProtoType = SOCK_DGRAM then
    ...
    if FMultiCast then
      ...
      if FMyAddr <> '0.0.0.0' then
        ...
        iStatus      := _SetSockOpt(FHSocket, IPPROTO_IP, IP_MULTICAST_IF, PAnsiChar(@laddr), SizeOf(laddr));
        if iStatus <> 0 then
          begin
    ...
    if (FAddr=cBroadcast) or (FAddr=c6Broadcast) then
      begin
      OptVal  := 1;
      iStatus := _setsockopt(FHSocket, SOL_SOCKET, SO_BROADCAST, PAnsiChar(@OptVal), SizeOf(OptVal));
      if iStatus <> 0 then
        begin
...
procedure TRtcWinSocket.Listen;
  ...
  if FProtoType = SOCK_DGRAM then
    ...
    if FMultiCast then
      ...
      StrPCopy(szAddr, FMultiCastAddr);
  ...
  {$ENDIF}
    ...
    if StartingListen then
      begin
      iStatus := _listen(FHSocket, SOMAXCONN);
      if iStatus <> 0 then
        begin
...
procedure TRtcWinSocket.InternalClose(abortiveclose:boolean);
  procedure ReadTheRest;
    ...
    while rcv>0 do
      ...
      temp:='';
      Setlength(temp,rcv);
      if RealRecv(@temp[1],length(temp),False)=rcv then
        begin
        FBuffered:=FBuffered+temp;
        rcv:=GetRcvdCount;
        end
      else
        rcv:=0;
...
function TRtcWinSocket.RealRecv(Data:Pointer; Len:Integer; triggerEvent:boolean=True) : Integer;
  begin
  if Protocol=spTcp then
...
function TRtcWinSocket.ReceiveStr(var Str : AnsiString):integer;
  var
    Res:integer;
  begin
  if bCanRead then
...
function TRtcWinSocket.RealSend(Data:Pointer; Len : Integer; triggerEvent:boolean=True) : Integer;
  begin
  if Protocol=spTcp then
  ...
  try
    repeat
      ...
      if length(FSendNowBuff)=0 then
        ...
        FLastError:=0;
        Count := RealSend(@FSendNowBuff[FSendNowAt+1], Len);
        if Count>0 then
          begin
          FPostNotify:=TRUE;
...
function TRtcWinSocket.SendStr(const Str:AnsiString):integer;
  begin
  if Protocol=spTcp then
    begin
    Result:=length(Str);
...
function TRtcWinSocket.BuffStr(const Str:AnsiString):integer;
  begin
  if Protocol=spTcp then
    begin
    Result:=length(Str);
