...
type
  {$IFNDEF UNICODE}
  TOctetStringStream = TStringStream;
  {$ENDIF}
  ...
  public
    ...
    function BeforeDisconnectEx(var ConnCryptObj:TObject; var OutData:AnsiString): TRtcCryptPluginState; override;
    ...
    function DataReceivedEx(var ConnCryptObj:TObject; const InData:AnsiString; var OutData:AnsiString; var OutPlainText:AnsiString): TRtcCryptPluginState; override;
    ...
    function DataToSendEx(var ConnCryptObj:TObject; const InData:AnsiString; var OutData:AnsiString): TRtcCryptPluginState; override;
  published
  ...
  public
    ...
    function AfterConnectEx(var ConnCryptObj:TObject; var OutData:AnsiString;
                            Protocol: TRtcCryptPluginProtocol; const RemoteAddr, RemoteIP:AnsiString): TRtcCryptPluginState; override;
  ...
  public
    ...
    property IP: AnsiString read fIP write SetIP;
...
function TRtcSSecPlugin.DataReceivedEx(var ConnCryptObj: TObject; 
  const InData: AnsiString; var OutData, OutPlainText: AnsiString): TRtcCryptPluginState;
var
  lObj: TRtcSSecConnCryptObject;
  lIn, lOut, lPT: TOctetStringStream;
  Client: TCustomTLS_ContentLayer;
begin
  if Assigned(ConnCryptObj) then begin
    lObj := ConnCryptObj as TRtcSSecConnCryptObject;
    Client := lObj.fTLSLayer;
    lIn := TOctetStringStream.Create(InData);
    lOut := TOctetStringStream.Create('');
    lPT := TOctetStringStream.Create('');
    try
      ...
      if not Client.Encrypted then
        ...
        Result := cpsReady;
        if (lObj.fBuffer <> '') then begin
          lIn.Size := 0;
          lIn.WriteString(lObj.fBuffer);
          lIn.Position := 0;
          lObj.fBuffer := '';
      ...
      OutData := lOut.DataString;
      OutPlainText := lPT.DataString;
    finally
      lIn.Free;
      lOut.Free;
      lPT.Free;
...
function TRtcSSecPlugin.DataToSendEx(var ConnCryptObj: TObject;
  const InData: AnsiString; var OutData: AnsiString): TRtcCryptPluginState;
var
  lObj: TRtcSSecConnCryptObject;
  lIn, lOut: TOctetStringStream;
  Client: TCustomTLS_ContentLayer;
begin
  if Assigned(ConnCryptObj) then begin
    lObj := ConnCryptObj as TRtcSSecConnCryptObject;
    ...
    if (not Assigned(Client)) or (not Client.Active) then
      ...
      lIn := TOctetStringStream.Create(lObj.fBuffer + InData);
      lOut := TOctetStringStream.Create('');
      try
        lObj.fBuffer := '';
        while lIn.Size > lIn.Position do
      ...
      lObj.fBuffer := lObj.fBuffer + InData;
      OutData := '';
      Result := cpsWaiting;
    end;
  end else
...
function TRtcSSecServerPlugin.AfterConnectEx(var ConnCryptObj: TObject;
  var OutData: AnsiString; Protocol: TRtcCryptPluginProtocol;
  const RemoteAddr, RemoteIP: AnsiString): TRtcCryptPluginState;
var
  lObj: TRtcSSecConnCryptObject;
begin
  Assert(Assigned(FTLSServer),'TLSServer must be assigned');
...
procedure TRtcSSecConnCryptObject.SetAddress(const Value: AnsiString);
begin
  fAddress := Value;
end;

procedure TRtcSSecConnCryptObject.SetIP(const Value: AnsiString);
begin
  fIP := Value;
end;
