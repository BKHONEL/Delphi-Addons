...
resourcestring
  SExpressionError = 'Error at offset %d of the expression'#13#10'%s';
...
begin
  Result := UnaryOpSymbols[Op] + Operand.AsString;
end;
...
procedure TCExpression.Parse(const Expression: String);
  ...
  begin
    ...
    if (UnaryOp <> opNone) and Assigned(Result) then
      Result := TCExprUnaryNode.Create(UnaryOp, Result);
  end;

  function ParseTerm(Level: Integer = 10): TCExprNode;
  var
    OperandLeft, OperandRight: TCExprNode;
    BinaryOp: TCBinaryOp;
  begin
    Result := nil;
    if Level = 1 then
      OperandLeft := ParseFactor
    else
      OperandLeft := ParseTerm(Level - 1);
    if not Assigned(OperandLeft) then
      Exit;
    while IsTokenBinaryOp(BinaryOp) and (BinaryOp in BinaryOpPrecedence[Level]) do
    begin
      NextToken;
      if Level = 1 then
        OperandRight := ParseFactor
      else
        OperandRight := ParseTerm(Level - 1);
      if not Assigned(OperandRight) then
      begin
        OperandLeft.Free;
        Exit;
      end;
      OperandLeft := TCExprBinaryNode.Create(BinaryOp, OperandLeft, OperandRight);
    end;
    Result := OperandLeft;
  end;
  ...
  var
    Condition, OperandTrue, OperandFalse: TCExprNode;
  begin
    Result := nil;
    Condition := ParseTerm;
    ...
    begin
      ...
      if not Assigned(OperandTrue) then
      begin
        Condition.Free;
        Exit;
      end;
      ...
      if not Assigned(OperandFalse) then
      begin
        OperandTrue.Free;
        Condition.Free;
        Exit;
      end;
      Result := TCExprTernaryNode.Create(Condition, OperandTrue, OperandFalse)
    end
    else
      Result := Condition;
  end;
...
begin
  ...
  if not Assigned(Expr) or (TokenID <> ceNone) then
  begin
    Error := ECExpressionError.CreateResFmt(@SExpressionError, [S - PChar(Expression),  Expression]);
